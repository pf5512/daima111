...
public class TiledImageView extends FrameLayout {
	...
	synchronized (mLock) {
	    ...
	    mRenderer.centerY = source != null ? source.getImageHeight() / 2
		    : 0;
	...
	if (renderer == null || renderer.source == null || renderer.scale > 0
		|| getWidth() == 0) {
	    return;
	}
	...
	if (mRenderer.source != null) {
	    ...
	    int xoffset = Math.round((getWidth() - mTempRectF.width()) / 2
		    / scale);
	    int yoffset = Math.round((getHeight() - mTempRectF.height()) / 2
		    / scale);
	...
	public void onDrawFrame(GL10 gl) {
		...
		mRenderer.image.setModel(mRenderer.source, mRenderer.rotation);
		mRenderer.image.setPosition(mRenderer.centerX,
			mRenderer.centerY, mRenderer.scale);
	    }
