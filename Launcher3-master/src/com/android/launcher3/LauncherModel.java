...
public class LauncherModel extends BroadcastReceiver
{
	...
	public void addAndBindAddedApps(
	 ...
	 */
	static void addOrMoveItemInDatabase(
			Context context ,
			ItemInfo item ,
			long container ,
			long screenId ,
	...
	private static TreeMap<Integer , Long> loadWorkspaceScreensDb(
			Context context )
	{
			...
			if( item.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT )
			{
				...
				if( occupied.containsKey( LauncherSettings.Favorites.CONTAINER_HOTSEAT ) )
				{
					if( occupied.get( LauncherSettings.Favorites.CONTAINER_HOTSEAT )[(int)item.screenId][0] != null )
					{
						Log.e(
								TAG ,
								"Error loading shortcut into hotseat " + item + " into position (" + item.screenId + ":" + item.cellX + "," + item.cellY + ") occupied by " + occupied
