...
public class ViewGroup3D extends View3D implements Cullable
{
		...
		for( int i = len ; i >= 0 ; i-- )
		{
			...
			if( child.pointerInParent( point.x , point.y ) )
			{
				if( ( toLocalCoordinates( child , point ) ) && ( child.onTouchDragged( point.x , point.y , pointer ) ) )
				{
					...
					{
						if( !child.equals( lastTouchedChild ) )
						{
							( (View3D)lastTouchedChild ).releaseDark();
			...
			else
			{
				if( child instanceof ViewGroup3D )
				{
					if( ( (ViewGroup3D)child ).lastTouchedChild != null )
					{
						( (View3D)( (ViewGroup3D)child ).lastTouchedChild ).releaseDark();
						( (ViewGroup3D)child ).lastTouchedChild = null;
						lastTouchedChild = null;
					}
				}
				else
	...
	//teapotXu_20130325 add end	
	// added by Hugo.ye 20140207 begin
	public void disposeRecursive()
	{
		...
		for( int i = 0 ; i < getChildCount() ; i++ )
		{
			...
			if( child instanceof ViewGroup3D )
			{
				( (ViewGroup3D)child ).disposeRecursive();
			}
			else
			{
				child.dispose();
