...
public class WidgetHostView extends AppWidgetHostView
{
				...
				switch( state )
				{
					...
					case DRAW_STOP:
						if( widget.state == DRAW_START || widget.state == DRAW_RUN )
						{
							...
							{
								//if(widget.state==DRAW_RUN && widget.hasRun)this.mUpdated++;
								//Log.e("widget", "super dispatchDraw");
								super.dispatchDraw( paramCanvas );
							}
			...
			if( width <= 0 || height <= 0 )
			{
				...
				ReDrawTimer.schedule( new TimerTask() {
					...
					public void run()
					{
						...
						Messenger.sendMsg( Messenger.MSG_HIDE_WORKSPACE , null );
						Messenger.sendMsg( Messenger.MSG_SHOW_WORKSPACE , null );
						update_suc = 0;
						mUpdated++;
						if( Gdx.graphics != null )
							Gdx.graphics.requestRendering();
	...
	//    	//Log.d("widget", "onInterceptTouchEvent");
	//        if (mHasPerformedLongPress) {
	//        	//Log.d("widget", "onInterceptTouchEvent 1");
	...
	//        switch (ev.getAction()) {
	//            case MotionEvent.ACTION_DOWN: {
	//            	//Log.d("widget", "onInterceptTouchEvent 2");
	//                postCheckForLongClick();
	//                break;
	//            } 
	//            
	//            case MotionEvent.ACTION_UP:
	//            case MotionEvent.ACTION_CANCEL:
	//            	//Log.d("widget", "onInterceptTouchEvent 3");
	//                mHasPerformedLongPress = false;
	//                if (mPendingCheckForLongPress != null) {
	//                    removeCallbacks(mPendingCheckForLongPress);
	//                }
