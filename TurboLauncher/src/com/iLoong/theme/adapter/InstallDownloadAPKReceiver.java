package com.iLoong.theme.adapter;// xiatian add whole file //fix bug same as 0001880
...
public class InstallDownloadAPKReceiver extends BroadcastReceiver
{
			...
			Intent intent )
	{
		...
		if( Intent.ACTION_PACKAGE_ADDED.equals( action ) )
		{
			...
			if( packageName != null )
			{
				//examine whether the installed package is Widget of cooee or not 
				...
				for( ResolveInfo resolveInfo : mWidgetResolveInfoList )
				{
					if( packageName.equals( resolveInfo.activityInfo.packageName ) )
					{
						...
						//now this installed package is cooee widget, so send message to this widget to hide the icon itself.
						Intent checkIntent = new Intent( "com.coco.lock.action.CHECK_ICON" );
		...
		else if( action.equals( "com.iLoong.launcher.ToDownloadAPK" ) )
		{
			mIsSelfToDownloadCoCoLauncher = true;
			if( LOG )
				Log.d( TAG , "ToDownloadAPK - mIsSelfToDownload" );
		}
		else if( action.equals( "com.iLoong.launcher.DownloadAPKComplete" ) )
		{
		...
		else if( action.equals( "com.iLoong.launcher.GetToDownloadAPKName" ) )
		{
