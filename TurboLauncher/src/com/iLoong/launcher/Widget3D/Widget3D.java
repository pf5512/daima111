...
public class Widget3D extends ViewGroup3D implements IconBase3D
{
			...
			float parentAlpha )
	{
		...
		else
		{
			if( enableFbo )
			{
				// curFboTime=System.currentTimeMillis();
				// if((curFboTime-lastFboTime)>1000*60*1){
				// fbo.dispose();
				// fbo = new FrameBuffer(Format.RGBA8888, (int) (width), (int)
				// (height),
				// true);
	...
	private void computeViewProject(
			Vector2 vt )
	{
		vTemp.set( width / 2 , height / 2 , 0 );
		vTemp.mul( getWorldTransform() );
	...
	public void onCellMove(int cellX,int cellY){
