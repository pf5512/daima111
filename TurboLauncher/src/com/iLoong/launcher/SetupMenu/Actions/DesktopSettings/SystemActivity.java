...
public class SystemActivity extends Activity implements View.OnClickListener
{
			...
			View v )
	{
		switch( v.getId() )
		{
			...
			case R.id.system_backupdesktop:
				...
				{
					Toast.makeText( this , R3D.getString( R.string.backup_pls_insert_SD ) , Toast.LENGTH_SHORT ).show();
				}
				else
				{
					backdialog( R3D.getString( R.string.backup_title_back ) , R3D.getString( R.string.backup_back_to_SD ) );
				}
				break;
			case R.id.system_restoredesktop:
				restoredialog( R3D.getString( R.string.backup_title_restore ) , R3D.getString( R.string.backup_restore ) );
			...
			String msg )
	{
		...
		builder.setPositiveButton( R3D.getString( R.string.circle_ok_action ) , new OnClickListener() {
		...
		builder.setNegativeButton( R3D.getString( R.string.circle_cancel_action ) , new OnClickListener() {
	...
	public void backupdb()
	{
		...
		try
		{
			...
			Toast.makeText( this , R3D.getString( R.string.backup_success ) , Toast.LENGTH_SHORT ).show();
			setCheckBackup();
		}
		catch( IOException e )
		{
			e.printStackTrace();
			Toast.makeText( this , R3D.getString( R.string.backup_fail ) , Toast.LENGTH_SHORT ).show();
		}
	}
	...
	public boolean restoredb()
	{
		...
		{
			bret = false;
			Toast.makeText( this , R3D.getString( R.string.restore_fail ) , Toast.LENGTH_SHORT ).show();
			e.printStackTrace();
		}
		return bret;
	}
	...
	private boolean checkBackup()
	{
		...
		if( file.exists() )
		{
			...
			strSumm = R3D.getString( R.string.backup_latest ) + "  " + cal.getTime().toLocaleString();
			restoretxt = R3D.getString( R.string.restore_desktoptxt );
			bret = true;
		...
		{
			strSumm = R3D.getString( R.string.backup_no_back );
			restoretxt = R3D.getString( R.string.restore_desktoptxt_nolatest );
