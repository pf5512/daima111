...
public class Desktop3DListener implements ApplicationListener , LauncherModel.Callbacks
{
	...
	public void create()
	{
		...
		{
			ParticleLoader.loadAllParticleEffect();
			int currentParticleTypeNum = SetupMenuActions.getInstance().getStringToIntger( SetupMenu.getKey( RR.string.setting_key_new_particle ) );
	...
	public void create2()
	{
		...
		{
			int currentPage = DefaultLayout.getInstance().loadCurrentScreenNum();
			if( currentPage == -1 )
			{
		...
		{
			root.newsHandle = new NewsHandle( "news_handle" , iLoongLauncher.getInstance() );
...
//		}
	...
	//					if( !DefaultLayout.enable_google_version )
	//					{
	//						if( root.newsHandle != null && DefaultLayout.enable_news && DefaultLayout.show_newspage_with_handle )
	//						{
	...
	//	}
	//Jone add start
	public static void setCoverVisible(
			boolean visible )
	{
		if( root.screenFrontImg == null )
		{
		...
		if( visible )
		{
			timeline.push( root.screenFrontImg.obtainTween( View3DTweenAccessor.OPACITY , Linear.INOUT , 0.5f , 1.0f , 0 , 0 ) );
		}
		else
		{
			timeline.push( root.screenFrontImg.obtainTween( View3DTweenAccessor.OPACITY , Linear.INOUT , 0.5f , 0.0f , 0 , 0 ) );
			//root.screenFrontImg.hide();
		}
	...
	public void resume()
	{
		if( appHost != null && appHost.getVisible() == true && appHost.isNeedReset() )
		{
	...
	public void onTrimMemory()
	{
		...
		{
			if( AppHost3D.appList.iswidgetVisible() || ( DefaultLayout.enable_workspace_miui_edit_mode && ( Workspace3D.WorkspaceStatus == WorkspaceStatusEnum.EditMode ) ) )
			{
				Log.v( "" , " 处于widget可见状态" );
			}
			else
		...
		launcher.postRunnable( new Runnable() {
			...
			public void run()
			{
				...
				for( int i = start ; i < end ; i++ )
				{
					...
					shortcut = CustomShortcutIcon.createCustomShortcut( item , true );
					if( shortcut != null )
					{
						workspace.addInScreen( shortcut , item.screen , item.x , item.y , false );
					}
				}
			}
		} );
	...
	public static void replaceItemInfoIcon(
			ItemInfo item )
	{
		...
		{
			ShortcutInfo sInfo = (ShortcutInfo)item;
			int findIndex = iLoongLauncher.getInstance().equalHotSeatIntent( sInfo.intent );
			if( findIndex != -1 )
			{
				/* 需要从主题中寻找替换的图标 */
				...
				if( findBmp != null )
				{
					...
					sInfo.title = iLoongLauncher.getInstance().getHotSeatString( findIndex );
			...
			final ShortcutInfo info )
	{
		...
		final Bitmap bitmapRemain = info.getIcon( iLoongApplication.mIconCache ).copy( Bitmap.Config.ARGB_8888 , true );
		final Bitmap bitmapTmp = info.getIcon( iLoongApplication.mIconCache ).copy( Bitmap.Config.ARGB_8888 , true );
			...
			final ShortcutInfo info )
	{
		if( addDropShortcutList.size() > 0 )
		{
			...
			{
				if( addSInfo.intent != null && addSInfo.intent.toString().equals( info.intent.toString() ) )
				{
					Log.v( "cooee" , "Desktop3DListner ---- addShortcutFromDrop --- want to add the same shortcut :" + info.intent );
					return;
				}
			}
		}
				...
				if( addDropShortcutList.size() > 0 )
				{
					for( ShortcutInfo sInfo : addDropShortcutList )
					{
						...
						if( sInfo.cellX >= 0 && sInfo.cellY >= 0 )
						{
							...
							Icon3D icon = new Icon3D( sInfo.title.toString() , R3D.findRegion( sInfo , "" ) );
							icon.setItemInfo( sInfo );
							workspace.setTag( null );
							//root.addFlySysShortcut( icon );//带飞入动画添加
							if( workspace.addInScreen( icon , sInfo.screen , sInfo.x , sInfo.y , false ) )
							{
						...
						else
						{
							...
							for( int i = 0 ; i < iLoongLauncher.getInstance().getScreenCount() ; i++ )
							{
								if( i != cur_srcreen && findCellForSpan( i , -1 , -1 , mCell , 1 , 1 ) )
								{
									sInfo.cellX = mCell[0];
									sInfo.cellY = mCell[1];
									sInfo.screen = i;
			...
			public void run()
			{
				...
				while( ite.hasNext() )
				{
					...
					for( int j = 0 ; j < DefaultLayout.hideAppList.size() ; j++ )
					{
						...
						if( ( info.intent.getComponent() != null ) && info.intent.getComponent().toString().contains( compName ) )
						{
							isHideApp = true;
							ite.remove();
				...
				synchronized( Desktop3DListener.this )
				{
					...
					for( int i = 0 ; i < widgets.size() ; i++ )
					{
						...
						if( rawInfo instanceof AppWidgetProviderInfo )
						{
							...
							{
								String name_ex = info.provider.toString() + "_editMode";
								if( !widget2DpkgName.contains( name_ex ) )
								{
									Bitmap widgetHost_bmp = getWidgetPreviewWorkspaceEditMode( info.provider , previewImage , info.icon , cellSpans[0] , cellSpans[1] , -1 , -1 );
									if( widgetHost_bmp != null )
									{
										...
										Log.v( "hjwmiui" , "widgetHostBitmap is new  label is " + widgetInfo.label );
									}
								}
							}
						}
						...
						if( b != null )
						{
							...
							Log.v( "hjwapplist" , "widget2DBitmap is new  label is " + widgetInfo.label );
					...
					launcher.postRunnable( new Runnable() {
						...
						public void run()
						{
							...
							Log.v( "launcher" , "bindAllWidgets done:" + ( System.currentTimeMillis() - time ) );
			...
			public void run()
			{
				...
				for( int i = 0 ; i < N ; i++ )
				{
					...
					for( int j = 0 ; j < Count ; j++ )
					{
						...
						{
							ShortcutInfo sInfo = (ShortcutInfo)child;
							if( child.intent != null && "com.android.contacts.action.QUICK_CONTACT".equals( child.intent.getAction() ) )
							{
								continue;
							}
							else
	...
	public void finishBindApplications()
	{
		...
		checkLoadProgress();
		if( DefaultLayout.enable_release_2Dwidget && DefaultLayout.enable_scroll_to_widget && !AppList3D.hasbind2Dwidget && iLoongApplication.BuiltIn )
		{
			if( appHost != null && appHost.appList != null )
			{
	...
	public void removePageEditItems(
			final ArrayList<ApplicationInfo> apps )
	{
		final Context context = launcher.getApplicationContext();
					...
					if( item.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT || item.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION )
					{
						...
						if( !name.equals( R3D.folder3D_name ) )
						{
							if( R3D.icon_bg_num > 0 )
							{
								...
								if( sInfo.intent != null && sInfo.intent.getComponent() != null && sInfo.intent.getComponent().getPackageName() != null )
								{
									String pkgname = sInfo.intent.getComponent().getPackageName();
									String clsname = sInfo.intent.getComponent().getClassName();
								...
								if( DefaultLayout.app_icon_size > ( sInfo.getIcon( iLoongApplication.mIconCache ).getHeight() ) || needAddBg )
								{
			...
			String packageName )
	{
		...
		if( resolveInfo != null )
		{
			...
			if( !DefaultLayout.isWidgetLoadByInternal( resolveInfo.activityInfo.packageName ) )
			{
				...
				if( DefaultLayout.enable_workspace_miui_edit_mode )
				{
					Widget3DShortcut shortcut2 = new Widget3DShortcut( "Widget3DShortcut" , resolveInfo );
			...
			final String packageName )
	{
		...
		else if( packageName.equals( "com.iLoong.Calendar" ) )
		{
			...
			Widget3DProvider provider = Widget3DManager.getInstance().getWidget3DProvider( "com.iLoong.Calendar" );
			provider.spanX = 4;
			provider.spanY = 4;
			Widget3DManager.getInstance().updateWidgetInfo( "com.iLoong.Calendar" , provider );
				...
				for( int j = 0 ; j < childCount ; j++ )
				{
					...
					if( tag instanceof Widget3DInfo )
					{
						...
						if( info.packageName.equals( packageName ) )
						{
							...
							Root3D.deleteFromDB( info );
							childrenToRemove.add( view );
							//MobclickAgent.onEvent( launcher , "DeleteWidget3DSys" , packageName );
						}
					}
				}
				childCount = childrenToRemove.size();
	...
	public Bitmap getShortcutPreview(
			ResolveInfo info )
	{
	...
	public static Bitmap getWidgetPreviewWorkspaceEditMode(
			Bitmap bitmap ,
			int cellHSpan ,
			int cellVSpan )
	{
	...
	public Bitmap getWidgetPreviewWorkspaceEditMode(
			...
			int maxHeight )
	{
		...
		if( widgetPreviewExists )
		{
			...
			// if (DefaultLayout.enable_workspace_edit_mode)
			{
				maxWidth = Math.min( maxWidth , widgetHost.estimateWidgetCellWidth( 4 ) );
				maxHeight = Math.min( maxHeight , widgetHost.estimateWidgetCellHeight( 4 ) );
			}
		}
		else
		{
			...
			{
				// teapotXu add start
				bitmapWidth = Math.min( maxWidth , widgetHost.estimateWidgetCellWidth( cellHSpan ) );
				bitmapHeight = Math.min( maxHeight , widgetHost.estimateWidgetCellHeight( cellVSpan ) );
				maxWidth = Math.min( maxWidth , widgetHost.estimateWidgetCellWidth( 4 ) );
				maxHeight = Math.min( maxHeight , widgetHost.estimateWidgetCellHeight( 4 ) );
			}
			else
			{
				...
				{
					int app_icon_size = (int)iLoongLauncher.getInstance().getResources().getDimension( android.R.dimen.app_icon_size );
					bitmapWidth = Math.min( app_icon_size , widgetHost.estimateWidgetCellWidth( 4 ) );
					bitmapHeight = Math.min( app_icon_size , widgetHost.estimateWidgetCellHeight( 4 ) );
				}
			...
			View3D v )
	{
		if( v instanceof SetupMenu3D )
		{
			if( setupMenu != null )
			{
				...
				{
					Log.v( "" , "setupMenu3D setSetupMenu d3dname is " + setupMenu.getChildAt( i ).name );
	...
	{
		mWorkspaceEffectPreview = new EffectPreview3D( "EffectPreview3D" , EffectPreview3D.TYPE_WORKSPACE );
		...
		mApplistEffectPreview = new EffectPreview3D( "EffectPreview3D" , EffectPreview3D.TYPE_APPLIST );
		...
		mEffectPreviewTips3D.setWorkspaceEffectPreview3D( mWorkspaceEffectPreview );
			...
			final CooeePluginInfo info )
	{
		...
		// }
		//		SaveScreenSize();
		//		PiScrollView view = new PiScrollView( iLoongLauncher.getInstance() , null );
		//		CooeePluginHostView hostView = new CooeePluginHostView( iLoongLauncher.getInstance() );
		//		hostView.addView( view , new LinearLayout.LayoutParams( UtilsBase.getScreenWidth() , UtilsBase.getScreenHeight() ) );
		//		int screen = iLoongLauncher.getInstance().d3dListener.getScreenCount();
		//		iLoongLauncher.getInstance().xWorkspace.addInScreen( hostView , screen - 2 , 0 , 0 , info.spanX , info.spanY , true , true );
