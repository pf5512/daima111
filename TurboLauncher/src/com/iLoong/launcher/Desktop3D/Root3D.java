...
public class Root3D extends ViewGroup3D implements MenuActionListener
{
	...
	public QSearchGroup qSearchGroup;
	...
	{
		// Log.e("Root3D", "stopMIUIEditEffectNoAnim 0");
		if( Workspace3D.WorkspaceStatus == WorkspaceStatusEnum.NormalMode )
		{
			// Log.e("Root3D", "stopMIUIEditEffect 1");
			return;
		...
		// Log.e("Root3D",
		// "stopMIUIEditEffect 4 workspace.touchable:"+workspace.touchable);
		Workspace3D.WorkspaceStatus = WorkspaceStatusEnum.NormalMode;
		Workspace.WorkspaceStatus = WorkspaceStatusEnum.NormalMode;
		workspace.setFactorAndOrigY( Utils3D.getScreenHeight() / 2 );
		if( workspace.findViewNew( "firstView" ) != null )
					...
					if( DefaultLayout.mainmenu_sort_by_user_fun )
					{
						...
						for( int index = 0 ; index < mDragView3D.getChildCount() ; index++ )
						{
							...
							if( mView3D instanceof Icon3D )
							{
								...
								mIcon3D.clearState();
								appHost.appList.needAddDragViewBackToFolder = true;
								appHost.appList.viewBackToFolder( mIcon3D );
							}
							else if( mView3D instanceof FolderIcon3D )
							{
								FolderIcon3D mFolderIcon3D = (FolderIcon3D)mView3D;
								mFolderIcon3D.clearState();
								appHost.appList.resetAppIconsStatusInFolder( mFolderIcon3D );
							}
						}
						appHost.appList.syncAppsPages();
						appHost.hide();
						startDragS3Effect();
						pageContainer.hide();
						workspace.show();
				...
				case DragLayer3D.MSG_DRAG_END:
							...
							if( dropTarget != null )
							{
								...
								if( dropTarget instanceof FolderIcon3D && ( (View3D)dropTarget ).getParent() instanceof GridView3D )
								{
									...
									if( ( (FolderIcon3D)dropTarget ).getOnDrop() == true )
									{
										...
										// appHost.removeDragViews(dragViewList);//xiatian del
					...
					else
					{
						...
						if( true )
						{
							...
							{
								//for(View3D v:dragLayer.getDragList())
								{
			...
			switch( event_id )
			{
				...
				case Workspace3D.MSG_CHANGE_TO_APPEND_PAGE:
					if( workspace.getTag() instanceof WorkspaceEditView )
					{
						...
						//Log.v( "test123" , "######################### test ###########workspace.getPageNum(): " + workspace.getPageNum() );
						if( workspace.getPageNum() == 2 + DefaultLayout.default_workspace_pagecount_max )
						{
							//don't show the add pages
							//Log.v( "test123" , "######################### test ###########" );
				...
				// if (workspace.getTag() instanceof Integer)
				{
					// int index= ((Integer)(sender.getTag())).intValue();
					...
					if( delView instanceof CellLayout3D )
					{
						...
						}
						{
							...
							// workspace.removeCellLayout(workspace.page_index);
							...
							Log.v( "test123" , "del page begin index=" + workspace.page_index );
							workspace.removeEditModeCellLayout( workspace.page_index );
							// SendMsgToAndroid.changeIconScreenIndex(workspace.page_index);
						}
					}
				}
					break;
		...
		if( sender instanceof TrashIcon3D )
		{
			...
			switch( event_id )
			{
				case TrashIcon3D.MSG_TRASH_DELETE:
					/*List<View3D> view3ds = dragLayer.getDragList();
					for( View3D view : view3ds )
					{
						Log.i( "TrashIcon3D" , "view.name*****" + view.name );
						if( view instanceof IconBase3D && view != null )
						{
							ItemInfo info = ( (IconBase3D)view ).getItemInfo();
							Log.i( "TrashIcon3D" , "info.title*****" + info.title );
							Log.i( "TrashIcon3D" , "getString*****" + iLoongLauncher.getInstance().getString( R.string.mainmenu ) );
							Log.i( "TrashIcon3D" , "shortcut_applist*****countShortcut" + TrashIcon3D.countShortcut( CustomShortcutIcon.CUSTOM_SHORTCUT_TITLE_APPLIST ) );
							if( ( info.title != null && hotSeatApplist == 0 && ( info.title.equals( CustomShortcutIcon.CUSTOM_SHORTCUT_TITLE_APPLIST ) ) && ( TrashIcon3D
									.countShortcut( CustomShortcutIcon.CUSTOM_SHORTCUT_TITLE_APPLIST ) == 1 ) ) || ( info.title != null && info.title.equals( iLoongLauncher.getInstance().getString(
									R.string.mainmenu ) ) && ( TrashIcon3D.countShortcut( CustomShortcutIcon.CUSTOM_SHORTCUT_TITLE_APPLIST ) < 1 ) ) )
							{
								Log.i( "TrashIcon3D" , "*****no**delete*****" );
								dropListBacktoOrig( dragLayer.getDragList() );
								trashIcon.hide();
								dragLayer.hide();
								return true;
					...
					for( int i = 0 ; i < viewList.size() ; i++ )
					{
						if( viewList.get( i ) instanceof Widget3D )
						{
							...
							if( widget3D.packageName.equalsIgnoreCase( "com.iLoong.NumberClock" ) )
							{
								provider.spanX = 4;
								provider.spanY = 2;
							}
	...
	public void showWorkSpaceFromAllApp1()
	{
		...
		//			workspaceAndAppTween.push( appHost.appBar.obtainTween( View3DTweenAccessor.OPACITY , Cubic.IN , workspaceTweenDuration , 0 , 0 , 0 ) );
		//		}
	...
	public void showWidgetListFromWorkspace()
	{
		...
		{
			appHost.appList.setCurrentPage( appHost.appList.appPageCount );
		}
		showAllAppFromWorkspace();
	}
	...
	private void deleteFolder()
	{
		if( ( ThemeManager.getInstance().getBoolean( "miui_v5_folder" ) || DefaultLayout.miui_v5_folder ) )
		{
			...
			if( folder.mInfo.contents.size() == 0 && folder.mFolderMIUI3D.getColseFolderByDragVal() )
			{
				// Log.e("test123", "dealMIUIFolderTweenFinish 2");
				RemoveFolderCellX = folder.mInfo.cellX;
				RemoveFolderCellY = folder.mInfo.cellY;
				System.out.println( "folder = " + folder );
				folder.startTween( View3DTweenAccessor.SCALE_XY , Quint.IN , 0.3f , 0 , 0 , 0 ).setCallback( new TweenCallback() {
							...
							BaseTween arg1 )
					{
						if( arg0 == TweenCallback.COMPLETE )
						{
							...
							{
								removeFolderScreen = workspace.getCurrentScreen();
								Root3D.this.dragLayer.removeDropTarget( folder );
								workspace.removeViewInWorkspace( folder );
							}
							else if( folder.getFromWhere() == FolderIcon3D.FROM_HOTSEAT )
							{
								removeFolderScreen = folder.mInfo.screen;
								RemoveFolderCellX = LauncherSettings.Favorites.CONTAINER_HOTSEAT;
								Root3D.this.dragLayer.removeDropTarget( folder );
								//	hotseatBar.getDockGroup().getShortcutGridview().removeUselessView( folder );
							}
							launcher.removeFolder( folder.mInfo );
							/*
							 * 将安装在T卡和安装在手机的应用放入一个文件夹，然后取下或者更换T卡后，将安装在手机的应用删除，
							 * 此时文件夹会自动删除，
							 * 再插上T卡后，由于文件夹消失，原本安装在T卡的应用的图标也消失，但是管理应用程序中可以找到那些应用
							 * , 现在是在拔出T卡，删除文件夹的同时将数据库中属于T卡的也删除掉
							 */
							//LauncherModel.deleteUserFolderContentsFromDatabase2( launcher , folder.mInfo );// panchong miui‘s solution
							//删除文件夹的时候，同时删除数据库中该文件夹下所有的item(一般是存在T卡上的应用)
							LauncherModel.deleteUserFolderContentsFromDatabase( launcher , folder.mInfo );
							deleteFromDB( folder.mInfo );
							folder.mFolderMIUI3D.resetbDragItemClose();
							folder = null;
						}
					}
				} );
			...
			{
				//				if( folder.mInfo.contents.size() == 1 || folder.mFolderMIUI3D.getColseFolderByDragVal() == false )
				//				{
				//					if( folder.mFolderMIUI3D.isbDragItemClose() && folder.mInfo.contents.size() == 1 )
				//					{
				//						// Log.e("test123", "dealMIUIFolderTweenFinish 5");
				//						final ShortcutInfo tempInfo = folder.mInfo.contents.get( 0 );
				//						Icon3D temp = null;
				//						for( int i = 0 ; i < folder.getChildCount() ; i++ )
				//						{
				//							if( folder.getChildAt( i ) instanceof Icon3D )
				//							{
				//								temp = (Icon3D)folder.getChildAt( i );
				//								break;
				//							}
				//						}
				//						final Icon3D actor = temp;
				//						folder.startTween( View3DTweenAccessor.SCALE_XY , Quint.IN , 0.15f , 0 , 0 , 0 ).setCallback( new TweenCallback() {
				//							
				//							@Override
				//							public void onEvent(
				//									int arg0 ,
				...
				//								}
				//								launcher.removeFolder( folder.mInfo );
				//								if( folder.getFromWhere() != FolderIcon3D.FROM_CELLLAYOUT )//桌面文件夹最后一个图标不删除，只更新记录，否则重启后丢图标
				//								{
				//									//删除文件夹的时候，同时删除数据库中该文件夹下所有的item(一般是存在T卡上的应用)
				//									LauncherModel.deleteUserFolderContentsFromDatabase( launcher , folder.mInfo );
				//								}
				//								deleteFromDB( folder.mInfo );
				//								ItemInfo info = actor.getItemInfo();
				//								info.screen = folder.mInfo.screen;
				//								info.x = folder.mInfo.x;
				//								info.y = folder.mInfo.y;
				//								info.container = -1;
				//								info.cellTempX = folder.mInfo.cellTempX;
				//								info.cellTempY = folder.mInfo.cellTempY;
				//								//need show missed calls or sms
				//								actor.setting();
				//								if( folder.getFromWhere() == FolderIcon3D.FROM_CELLLAYOUT )
				//								{
				//									folder.changeTextureRegion( actor , R3D.workspace_cell_height );
				//									workspace.addBackInScreen( actor , folder.mInfo.x , folder.mInfo.y );
				...
				//										{
				//											Utils3D.changeTextureRegion( actor , Utils3D.getIconBmpHeight() , true );
				//										}
				//									}
				//									hotseatBar.getDockGroup().addSingleItem( actor , info.screen );
				//								}
				//								folder.mFolderMIUI3D.resetbDragItemClose();
				//								if( folder.isHideItemIcon() )
				//								{
				//									actor.show();
				//								}
				//								folder = null;
				//								actor.setScale( 0 , 0 );
				//								actor.startTween( View3DTweenAccessor.SCALE_XY , Quint.IN , 0.15f , 1 , 1 , 0 );
				//							}
				//						} );
				//					}
				//					else
	...
	public void showAllAppFromWorkspace()
	{
		...
		workspaceAndAppTween.push( appHost.appList.obtainTween( View3DTweenAccessor.USER , Cubic.OUT , workspaceTweenDuration , 0 , 0 , 0 ).delay( workspaceTweenDuration ) );
		appHost.appList.color.a = 0;
		workspaceAndAppTween.push( appHost.appList.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , workspaceTweenDuration , 1 , 0 , 0 ).delay( workspaceTweenDuration ) );
		// appHost.appList.setRotationVector(1, 0, 0);
		// appHost.appList.setRotation(-30);
		appHost.appList.setScale( 0.9f , 0.9f );
		workspaceAndAppTween.push( appHost.appList.obtainTween( View3DTweenAccessor.SCALE_XY , Cubic.OUT , workspaceTweenDuration , 1 , 1 , 0 ).delay( workspaceTweenDuration ) );
		if( appHost.tabPluginHost != null )
		{
		...
		if( appHost.appBar != null )
		{
			...
			workspaceAndAppTween.push( appHost.appBar.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , workspaceTweenDuration , 1 , 0 , 0 ).delay( workspaceTweenDuration ) );
		}
		workspace.setUser( 0 );
			...
			BaseTween source )
	{
		...
		if( source == workspaceAndAppTween && type == TweenCallback.COMPLETE && workspaceAndAppTween.getUserData().equals( 0 ) )
		{
			...
			// iLoongLauncher.getInstance().closeOptionsMenu();
			appHost.hide();
		...
		{
			Log.e( "Root3D" , "onEvent  miui_effect_line:" + miui_effect_line );
			if( Workspace3D.WorkspaceStatus == WorkspaceStatusEnum.NormalMode )
			{
				// widgetHost.hide();
				if( dragLayer.isVisible() && !dragLayer.draging )
			...
			else
			{
				...
				// }
				//
				// hotseatBar.setPosition(0, -effectHeight * 1.5f);
				if( hotseatBar != null )
				{
					hotseatBar.hide();
				}
			...
			boolean alreadyOnHome )
	{
		...
		if( appHost != null && appHost.isVisible() )
		{
			...
			{
				appHost.onHomeKey = true; // xiatian add //for mainmenu sort by
											// user
				...
				appHost.appList.force_applist_refesh = true;
				// xiatian add end
			}
			...
			// } else {
			appHost.hide();
			appHost.appList.resetApplistIconState();// xiatian add //for
			...
			if( !folder.blurCompleted )
			{
				...
				if( folder.blurredView == null )
				{
					...
					else
					{
						...
						// Gdx.graphics.getHeight());
						// FolderIcon3D.lwpBackView.setPosition(0, 0);
						// this.addViewBefore(hotseatBar,
						// FolderIcon3D.lwpBackView);
						// }
						folder.addViewAt( folder.mFolderIndex , folder.mFolderMIUI3D );
			...
			Intent intent )
	{
		...
		{
			if( appHost != null && appHost.appBar != null && appHost.appBar.tabIndicator != null )
			{
				appHost.appBar.tabIndicator.tabId = appHost.appBar.TAB_CONTENT;
			}
	...
	private ItemInfo getFolderInfo(
	...
	public void startWholeAnimWithNewspageBySidebar(
			float duration ,
			float x ,
			float a )
	{
		Timeline timeline_with_newspage_amin = null;
		timeline_with_newspage_amin = Timeline.createParallel();
		timeline_with_newspage_amin.push( Tween.to( getWorkspace().getCurrentCellLayout() , View3DTweenAccessor.POS_XY , duration ).target( x , 0 , 0 ).ease( Quad.OUT ) );
		timeline_with_newspage_amin.push( Tween.to( hotseatBar , View3DTweenAccessor.POS_XY , duration ).target( x , 0 , 0 ).ease( Quad.OUT ) );
		timeline_with_newspage_amin.push( Tween.to( getPageIndicator() , View3DTweenAccessor.POS_XY , duration ).target( x , getPageIndicator().y , 0 ).ease( Quad.OUT ) );
		timeline_with_newspage_amin.push( Tween.to( this , View3DTweenAccessor.OPACITY , duration ).target( a , 0 , 0 ).ease( Quad.OUT ) );
		timeline_with_newspage_amin.start( View3DTweenAccessor.manager );
	}
	...
	IOnRemoveCallBack OnRemoveCallBackImpl = new IOnRemoveCallBack() {
		...
		{
			Log.v( "RECYCLE" , "onRemoveCompleted" );
