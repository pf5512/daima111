...
public class Workspace3D extends NPageBase implements DragSource3D , DropTarget3D , MenuActionListener
{
	...
	private CooeePluginHostView virtualNewsView;
	private CellLayout3D newsCellLayout3D;
	private CellLayout newsContainer;
	...
	//	private NinePatch editModeBg;
	public static void setDragMode(
			int mode )
	{
		Log.v( "jbc" , "fef setDragMode:" + mode );
		dragMode = mode;
	}
			...
			int event_id )
	{
		...
		else if( sender instanceof Widget3D )
		{
			...
			//	widget.setScale(Root3D.s3EffectScale, Root3D.s3EffectScale);
			switch( event_id )
			{
