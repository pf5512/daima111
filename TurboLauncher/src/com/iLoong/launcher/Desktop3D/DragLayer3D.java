...
public class DragLayer3D extends ViewGroup3D
{
			...
			float y )
	{
		...
		// view0.setScale(Root3D.s3EffectScale, Root3D.s3EffectScale);
		...
		if( DefaultLayout.rapidly_remove_shortcut && !Workspace3D.isRecentAppVisible() )
		{
			if( !iLoongLauncher.getInstance().d3dListener.pageContainer.isVisible() )
			{
				if( recycle() )
				{
					...
					return iLoongLauncher.getInstance().d3dListener.root.trashIcon;
				}
			}
		}
			...
			float deltaY )
	{
		...
		if( Workspace3D.isRecentAppVisible() )
		{
			if( vecY > Workspace3D.mRecentApplications.mTouchableHeight )
			{
				Workspace3D.mRecentApplications.mLastY = Workspace3D.mRecentApplications.mTouchableHeight;
				dragView.setPosition( Workspace3D.mRecentApplications.mLastX , Workspace3D.mRecentApplications.mLastY );
				return true;
			}
			else if( vecY <= Workspace3D.mRecentApplications.mTouchableLow )
			{
				dragView.setPosition( Workspace3D.mRecentApplications.mLastX , Workspace3D.mRecentApplications.mLastY );
				Workspace3D.mRecentApplications.mOnScrollListener.onLock( getDragList() , dropX , vecY );
				Workspace3D.mRecentApplications.mLastY = Workspace3D.mRecentApplications.mTouchableLow;
				return true;
			}
			if( dragView.getChildAt( 0 ) instanceof RecentApp )
			{
				...
				float distance = Workspace3D.mRecentApplications.mTouchableHeight - orgy;
				if( mDeltaY > 0 )
				{
					if( mDeltaY > distance )
					{
		...
		lastX = x;
		lastY = y;
		// Log.v("launcher", " scroll:" + name + " x:" + curX + " y:" + curY +
		...
		//if(DefaultLayout.rapidly_remove_shortcut&&!recycle()&&!isRecycled||!DefaultLayout.rapidly_remove_shortcut){
		//	isRecycled=false;
		if( Workspace3D.isRecentAppVisible() )
		...
		if( parent instanceof Root3D )
		{
			...
			if( root.folderOpened || root.appHost.folderOpened || Workspace3D.isRecentAppVisible() || root.newsHandle.isDragging )
			{
			...
			else
			{
				...
				if( ( DefaultLayout.mainmenu_sort_by_user_fun ) && ( overTarget instanceof AppHost3D ) )
				{
					...
					if( mAppHost3D.mMoveDrag2Workspace )
					{
						mAppHost3D.mMoveDrag2Workspace = false;
	...
	public boolean recycle()
	{
		...
		//		
		///	if( mScaleY + Math.abs(mVelocityY)/ scroll_velocity > scroll_sensitive  )
			...
			float parentAlpha )
	{
		...
		if( showBorder != 0 )
		{
			...
			if( showBorder == -1 )
				moveToLeft.draw( batch , 0 , R3D.hot_obj_height , borderWidth , borderHeight );
			else if( showBorder == 1 )
				moveToRight.draw( batch , Utils3D.getScreenWidth() - borderWidth , R3D.hot_obj_height , borderWidth , borderHeight );
		}
	}
