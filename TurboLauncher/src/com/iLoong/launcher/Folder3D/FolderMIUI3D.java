...
public class FolderMIUI3D extends ViewGroup3D implements DragSource3D , TextFieldListener
{
			...
			for( int i = 0 ; i < iconsContainer.getChildCount() ; i++ )
			{
				...
				for( int j = 0 ; j < gridPage.getChildCount() ; j++ )
				{
					...
					if( myActor instanceof Icon3D )
					{
						...
						if( gridPage.getFocusView() != myActor )
						{
							tempInfo = ( (Icon3D)myActor ).getItemInfo();
			...
			int iconsNum )
	{
		...
		else if( ( this.getY() + this.getHeight() ) > ( Utils3D.getScreenHeight() - m_posGapMin ) )
		{
			targetPosY = Utils3D.getScreenHeight() - m_posGapMin - this.getHeight();
		}
			...
			ArrayList<View3D> list )
	{
		...
		mFolderIcon.mInfo.contents.clear();
		iconsContainer.getViewList().clear();
		iconsContainer.removeAllViews();
		if( buttonAdd != null )
		{
	...
	class ButtonView3D extends View3D
	{
				...
				float parentAlpha )
		{
			...
			if( FolderIcon3D.applistMode == AppList3D.APPLIST_MODE_UNINSTALL )
			{
		...
		{
			// TODO Auto-generated method stub
			if( FolderIcon3D.applistMode != AppList3D.APPLIST_MODE_UNINSTALL && name.equals( "button_add" ) )
			{
