...
public class GuessPage extends NPageBase
{
	...
	public void load()
	{
		if( allApps == null )
		{
			allApps = QSearchGroup.getAllApp( Color.WHITE );
		}
		setButtons( (ArrayList<? extends View3D>)getGuessList() , 5 , 1 , Utils3D.getScreenWidth() , 190 * UtilsBase.getScreenWidth() / 720f );
	}
			...
			float height )
	{
		...
		if( gridPool == null )
		{
			gridPool = new GridPool( name , pageCapacity , width , height , countX , countY , 0 , 0 , 0 , 0 );
		}
		syncButtonPages();
	}
	...
	private void syncButtonPageItems(
			int page ,
			boolean immediate )
	{
		int numCells = mCellCountX * mCellCountY;
	...
	private List<SearchApp> getGuessList()
	{
		...
		ArrayList<ItemInfo> workspaceItems = application.mModel.mItems;
		ArrayList<ItemInfo> sidebarItems = application.mModel.mSidebarItems;
		ArrayList<Integer> removedIndex = new ArrayList<Integer>();
		for( int i = 0 ; i < allApps.size() ; i++ )
		{
			String name = allApps.get( i ).name;
			for( int j = 0 ; j < workspaceItems.size() ; j++ )
			{
				if( name.equals( workspaceItems.get( j ).title ) )
				{
					removedIndex.add( i );
					break;
				}
			}
			for( int k = 0 ; k < sidebarItems.size() ; k++ )
			{
				if( name.equals( sidebarItems.get( k ).title ) )
				{
					removedIndex.add( i );
					break;
				}
