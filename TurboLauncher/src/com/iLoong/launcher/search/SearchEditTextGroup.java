...
public class SearchEditTextGroup extends ViewGroup3D implements TextFieldListener
{
			...
			String name )
	{
		...
		try
		{
			Bitmap editFrameBmp = Tools.getImageFromInStream( iLoongLauncher.getInstance().getAssets().open( "theme/quick_search/search_circle.9.png" ) );
			NinePatch editFramsNp = new NinePatch( new BitmapTexture( editFrameBmp , true ) , 4 , 4 , 10 , 10 );
			...
			int pointer )
	{
		...
		{
			this.x = 0;
			Root3D.hotseatBar.x = -UtilsBase.getScreenWidth();
		}
		releaseFocus();
	...
	private void startAnim()
	{
		...
		if( mEditText.getDisplayText().equals( "" ) )
		{
			mSearchHint.show();
		}
		else
