...
public class LettersGroup extends ViewGroup3D
{
			...
			String name )
	{
		...
		setPosition( UtilsBase.getScreenWidth() - this.width , QsearchConstants.H_SEARCH_EDIT_GROUP );
		try
		{
			Bitmap lettersBgBmp = Tools.getImageFromInStream( iLoongLauncher.getInstance().getAssets().open( "theme/quick_search/qs_letter_bg.9.png" ) );
			...
			if( y < ( this.height / b.length ) * ( b.length - ( i ) ) && y > ( this.height / b.length ) * ( b.length - ( i + 1 ) ) )
			{
				...
				for( Iterator<String> it = AllAppList.mLetterList.iterator() ; it.hasNext() ; )
				{
					...
					if( firstLetter.equals( b[i] ) )
					{
						...
						pop.setPosition( pop.x , QsearchConstants.Y_LETTER_POP + ( QsearchConstants.H_LETTERS_GROUP / 27 * ( b.length - i - 1 ) ) );
						float totalScroll = 0;
						for( int j = 0 ; j < index ; j++ )
						{
							totalScroll += Desktop3DListener.root.qSearchGroup.mAllAppList.getChildAt( j ).height;
						}
						if( totalScroll > Desktop3DListener.root.qSearchGroup.mAllAppList.getScrollMax() )
							totalScroll = Desktop3DListener.root.qSearchGroup.mAllAppList.getScrollMax();
						Desktop3DListener.root.qSearchGroup.mAllAppList.setUser( totalScroll );
					}
					index++;
				}
			}
				...
				if( letter.equals( name ) )
				{
					for( int j = 0 ; j < b.length ; j++ )
					{
						if( b[j].equals( name ) )
						{
							...
							pop.setPosition( pop.x , QsearchConstants.Y_LETTER_POP + ( QsearchConstants.H_LETTERS_GROUP / 27 * ( b.length - j - 1 ) ) );
							pop.color.a = 1;
						}
					}
					float totalScroll = 0;
					...
					{
						totalScroll += Desktop3DListener.root.qSearchGroup.mAllAppList.getChildAt( i ).height;
					}
					if( totalScroll > Desktop3DListener.root.qSearchGroup.mAllAppList.getScrollMax() )
						totalScroll = Desktop3DListener.root.qSearchGroup.mAllAppList.getScrollMax();
					Desktop3DListener.root.qSearchGroup.mAllAppList.setUser( totalScroll );
