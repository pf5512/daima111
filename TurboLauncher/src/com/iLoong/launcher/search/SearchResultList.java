...
public class SearchResultList extends ListView3D
{
	...
	{
		super( name );
		setSize( QsearchConstants.W_SEARCH_RESULT_LIST , QsearchConstants.H_SEARCH_RESULT_LIST );
	}
	...
	public void load()
	{
		if( allApps == null )
		{
			allApps = QSearchGroup.getAllApp( R3D.qs_all_app_list_icon_text_color );
		}
	}
			...
			float height )
	{
		...
		gridPool = new GridPool(
				...
				countX ,
				countY ,
				R3D.qs_category_grid_padding_left ,
	...
	private void syncButtonRowItems(
			int row )
	{
		int numCells = mCellCountX * mCellCountY;
	...
	class SearchResultItem extends ViewGroup3D
	{
				...
				String name )
		{
			...
			try
			{
				Bitmap dividerBmp = Tools.getImageFromInStream( iLoongLauncher.getInstance().getAssets().open( "theme/quick_search/qs_quess_line.9.png" ) );
				NinePatch dividerNp = new NinePatch( new BitmapTexture( dividerBmp , true ) , 1 , 1 , 1 , 1 );
			...
			String key )
	{
		...
		{
			Desktop3DListener.root.qSearchGroup.mSearchResultGroup.mNoResultTip.hide();
			setButtons( (ArrayList<? extends View3D>)mResultList , 4 , 1 , UtilsBase.getScreenWidth() , R3D.qs_app_list_row_height );
		}
		else
		{
			Desktop3DListener.root.qSearchGroup.mSearchResultGroup.mNoResultTip.show();
		}
	}
	...
	class GridPool
	{
		...
		private GridView3D create()
		{
			...
			grid.setPadding( paddingLeft , paddingRight , paddingTop , paddingBottom );
			return grid;
		}
