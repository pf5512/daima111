 ...
 * @author mzechner */
/** @author jshapcot */
public final class AndroidInput implements Input, OnKeyListener,
		OnTouchListener {
	class KeyEvent {
		static final int KEY_DOWN = 0;
		static final int KEY_UP = 1;
		static final int KEY_TYPED = 2;
		static final int KEY_TYPED_CH = 3;
	...
	public AndroidInput(AndroidApplication activity, View view,
		...
		this.config = config;
		this.onscreenKeyboard = new AndroidOnscreenKeyboard(activity,
				new Handler(), this);
			...
			if (processor != null) {
				...
				for (int i = 0; i < len; i++) {
					...
					case KeyEvent.KEY_TYPED:
						...
						break;
					case KeyEvent.KEY_TYPED_CH:
						if(config.handleKeyTypedChinese)processor.keyTyped(e.keyCharacters);
						break;
					}
				...
				boolean ignoreDrag = false;
				for (int i = 0; i < len; i++) {
					TouchEvent e = touchEvents.get(i);
					currentEventTimeStamp = e.timeStamp;
					//Log.v( "" , "opt event time_diff:"+(System.nanoTime()-currentEventTimeStamp)/1000000 );
					//Log.v( "" , "opt event:"+e.x );
					switch (e.type) {
					...
					case TouchEvent.TOUCH_DRAGGED:
						...
						if(app.optTouch)ignoreDrag = true;
					}
...
//		}
		...
		synchronized (this) {
			...
			case android.view.KeyEvent.ACTION_MULTIPLE:
				if (keyCode == android.view.KeyEvent.KEYCODE_UNKNOWN) {
					...
					event = usedKeyEvents.obtain();
					event.keyChar = character;
					event.keyCharacters = e.getCharacters();
					event.keyCode = android.view.KeyEvent.KEYCODE_UNKNOWN;
					event.type = KeyEvent.KEY_TYPED_CH;
					keyEvents.add(event);
			...
			public void run() {
				...
				if (visible) {
					...
					manager.showSoftInput(
							((AndroidGraphics) app.getGraphics()).getView(), 0);
				} else {
	...
	/**
	 * Returns the rotation matrix describing the devices rotation as per <a
	 * href=
	 * "http://developer.android.com/reference/android/hardware/SensorManager.html#getRotationMatrix(float[], float[], float[], float[])"
	 * >SensorManager#getRotationMatrix(float[], float[], float[], float[])</a>.
	 * Does not manipulate the matrix if the platform does not have an
	 * accelerometer.
	 * 
	 * @param matrix
	 */
	public void getRotationMatrix(float[] matrix) {
		SensorManager.getRotationMatrix(matrix, null, accelerometerValues,
				magneticFieldValues);
	}
		...
		if (config.useAccelerometer) {
			...
			if (manager.getSensorList(Sensor.TYPE_ACCELEROMETER).size() == 0) {
				...
				accelerometerListener = new SensorListener(
						this.nativeOrientation, this.accelerometerValues,
						this.magneticFieldValues);
				accelerometerAvailable = manager.registerListener(
			...
			if (sensor != null) {
				...
				if (compassAvailable) {
					compassListener = new SensorListener(
							this.nativeOrientation, this.accelerometerValues,
							this.magneticFieldValues);
					compassAvailable = manager.registerListener(
							compassListener, sensor,
							SensorManager.SENSOR_DELAY_GAME);
				}
			} else {
	...
	public int lookUpPointerIndex(int pointerId) {
		...
		Gdx.app.log("AndroidInput", "Pointer ID lookup failed: " + pointerId
				+ ", " + buf.toString());
		return -1;
	}
	...
	public int getRotation() {
		int orientation = app.getWindowManager().getDefaultDisplay()
				.getOrientation();
		switch (orientation) {
	...
	/**
	 * Our implementation of SensorEventListener. Because Android doesn't like
	 * it when we register more than one Sensor to a single SensorEventListener,
	 * we add one of these for each Sensor. Could use an anonymous class, but I
	 * don't see any harm in explicitly defining it here. Correct me if I am
	 * wrong.
	 */
	private class SensorListener implements SensorEventListener {
		...
		SensorListener(Orientation nativeOrientation,
