	...
	private static class ComponentSizeChooser extends BaseConfigChooser
	{
				...
				EGLConfig[] configs )
		{
			...
			for( EGLConfig config : configs )
			{
				...
				int distance = Math.abs( r - mRedSize ) + Math.abs( g - mGreenSize ) + Math.abs( b - mBlueSize ) + Math.abs( a - mAlphaSize ) + Math.abs( d - mDepthSize ) + Math
						.abs( s - mStencilSize );
