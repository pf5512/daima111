...
public final class AndroidGraphics implements Graphics , Renderer
{
	...
	private int fps;//当前帧率
	private ArrayList<Integer> framesEverySecond = new ArrayList<Integer>();
	private int FpsAverageValue;//帧率平均值
	private int FpsCrestValue;//帧率峰值
	private int FpsVarianceValue;//帧率方差值
	...
	public Object continue_lock = new Object();
	private boolean isSurfaceCreated = false; //记录surface状态
	private static boolean enable_showFPS = false;
	...
	private void setPreserveContext(
			View view )
	{
	...
	private static boolean isPowerOfTwo(
			int value )
	{
			...
			javax.microedition.khronos.opengles.GL10 gl )
	{
		...
		else
		{
			...
			if( gl instanceof javax.microedition.khronos.opengles.GL11 )
			{
				...
				if( renderer != null )
				{ // silly GT-I7500
					if( !renderer.toLowerCase().contains( "pixelflinger" ) && !( android.os.Build.MODEL.equals( "MB200" ) || android.os.Build.MODEL.equals( "MB220" ) || android.os.Build.MODEL
							.contains( "Behold" ) ) )
					{
	...
	{
		launcherPause = true;
		Log.v( "iLoongLauncher" , "graphics pause0:" + Gdx.graphics.isContinuousRendering() );
		synchronized( synch )
		{
			Log.v( "iLoongLauncher" , "graphics pause1:" + Gdx.graphics.isContinuousRendering() );
			if( !running )
				return;
			running = false;
			pause = true;
			...
			if( pause )
			{
				try
				{
					Log.v( "iLoongLauncher" , "graphics pause2:" + Gdx.graphics.isContinuousRendering() );
					synch.wait( 3500 );
					Log.v( "iLoongLauncher" , "graphics pause3:" + pause );
				}
	...
	void destroy()
	{
		Log.v( "iLoongLauncher" , "graphics destroy0:" + Gdx.graphics.isContinuousRendering() );
		synchronized( synch )
		{
			running = false;
			destroy = true;
			...
			while( destroy )
			{
				try
				{
					Log.v( "iLoongLauncher" , "graphics destroy1:" + Gdx.graphics.isContinuousRendering() );
					synch.wait();
					Log.v( "iLoongLauncher" , "graphics destroy2:" + Gdx.graphics.isContinuousRendering() );
				}
			...
			javax.microedition.khronos.opengles.GL10 gl )
	{
		...
		if( enable_showFPS )
		{
			if( time - frameStart > 1000000000 )
			{
				...
				//				{
				//					Gdx.app.log( "xiatian - framesEverySecond" , "--" + ( i + 1 ) + "--:" + framesEverySecond.get( i ) );
				//				}
	...
	private int calculateFpsAverageValue()
	{
		int sum = 0;
		int num = framesEverySecond.size();
		for( int i = 0 ; i < num - 1 ; i++ )
		{
			sum += framesEverySecond.get( i );
		}
		return( sum / num );
	}
	
	private int calculateFpsCrestValue()
	{
		int ret = 0;
		int num = framesEverySecond.size();
		for( int i = 0 ; i < num - 1 ; i++ )
		{
			int value = framesEverySecond.get( i );
			ret = Math.max( ret , value );
		}
		return ret;
	}
	
	private int calculateFpsVarianceValue()
	{
		int sum = 0;
		int num = framesEverySecond.size();
		for( int i = 0 ; i < num - 1 ; i++ )
		{
			sum += Math.pow( framesEverySecond.get( i ) - FpsAverageValue , 2 );
		}
		return( sum / num );
	}
	...
	public void setShowFPS(
			boolean isShowFPS )
	{
		enable_showFPS = isShowFPS;
	}
	
	public int getFpsAverageValue()
	{
		return FpsAverageValue;
	}
	
	public int getFpsCrestValue()
	{
		return FpsCrestValue;
	}
	
	public int getFpsVarianceValue()
	{
		return FpsVarianceValue;
	}
}
