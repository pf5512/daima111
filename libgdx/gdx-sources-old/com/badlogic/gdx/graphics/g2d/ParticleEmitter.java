...
public class ParticleEmitter {
	...
	public ParticleEmitter (ParticleEmitter emitter) {
		...
		scaleValue.setLow(scaleValue.getLowMin()*scaleFactor, scaleValue.getLowMax()*scaleFactor);
		scaleValue.setHigh(scaleValue.getHighMin()*scaleFactor, scaleValue.getHighMax()*scaleFactor);
		...
		/********* added by zhenNan.ye begin **********/
		int px = 0;
		px = dip2px(xOffsetValue.lowMin);
		xOffsetValue.setLowMin((float)px);
		px = dip2px(xOffsetValue.lowMax);
		xOffsetValue.setLowMax((float)px);
		/********* added by zhenNan.ye begin **********/
		yOffsetValue.load(emitter.yOffsetValue);
		/********* added by zhenNan.ye begin **********/
		px = dip2px(yOffsetValue.lowMin);
		yOffsetValue.setLowMin((float)px);
		px = dip2px(yOffsetValue.lowMax);
		yOffsetValue.setLowMax((float)px);
		/********* added by zhenNan.ye begin **********/
	 ...
	 * added by zhenNan.ye
	 * 
	 * 获取缩放因子,以360dpi(density为2.25)为标准,图片资源都按这个标准出图。
	 * @return
	 */
	private static float getScaleFactor()
	{
		return Gdx.graphics.getDensity()/2.25f;
	}
}
