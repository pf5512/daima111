		...
		public void handleMessage(Message msg) {
			...
			case DOWNLOAD_COMPLETE: {
				...
				Notification finishNotification = new Notification(
						iconResourceId, "", System.currentTimeMillis());
				...
				Intent intentDownloadComplete = new Intent(
						"com.coco.lock2.lockbox.DownloadAPKComplete");
				intentDownloadComplete.putExtra("mNotifyId",
						downloadRunnable.mNotifyId);
	...
	private class DownloadRunnable implements Runnable {
		...
		public DownloadRunnable(String downloadUrl, String downloadFileName) {
			...
			mUpdatePendingIntent = PendingIntent.getActivity(
					DownloadLockBoxService.this, 0, mUpdateIntent, 0);

			String tickerText = getResources().getString(
					R.string.server_begin_download);
			mUpdateNotification = new Notification(iconResourceId, tickerText,
					System.currentTimeMillis());
			mUpdateNotification.setLatestEventInfo(DownloadLockBoxService.this,
					downloadFileName, "0%", mUpdatePendingIntent);

			// xiatian add start //fix bug same as 0001880
			// Log.d("InstallAPKReceiver" , "sendBroadcast - ToDownloadAPK");
			Intent intentToDownload = new Intent(
					"com.coco.lock2.lockbox.ToDownloadAPK");
		...
		public void run() {
			...
			try {
					...
					try {
						if (!mDownloadFile.delete()) {
							mDownloadFile = new File(DOWNLOAD_PATH,
									mDownloadFileName + mNotifyId + ".tmp");
						}
						mDownloadFile.createNewFile();
				...
				if (downloadSize > 0) {
					boolean res = mDownloadFile.renameTo(new File(DOWNLOAD_PATH, mDownloadFileName
							+ ".apk"));
					if(res){
		...
		private long downloadFile(String downloadUrl, File saveFile)
			...
			try {
				...
				if (MainActivity.isCWWAPConnect(getApplicationContext())
						&& MainActivity.getNetWorkType(getApplicationContext()) != NETTYPE_UNICOM) {

					String host = MainActivity
							.getProxyHost(getApplicationContext());
					int port = MainActivity
				...
				while ((readsize = is.read(buffer)) > 0) {
					...
					if (curPercent > percentCount || percentCount >= 100) {
						...
						mUpdateNotification.setLatestEventInfo(
								DownloadLockBoxService.this, loading + " - "
