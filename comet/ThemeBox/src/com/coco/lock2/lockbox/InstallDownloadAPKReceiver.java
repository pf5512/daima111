...
public class InstallDownloadAPKReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {  
		...
		if(Intent.ACTION_PACKAGE_ADDED.equals(action)) {  
			...
			if(
					(mIsSelfToDownloadCoCoLauncher)
					&&(mToDownloadCoCoLauncherNotifyId != -1)
					&&(packageName.equals(mDownloadAPKName))
			)
			{
				mDownloadNotificationManager = 
		...
		//teapotXu_20130305: add start
			if(packageName != null){
				...
				List<ResolveInfo> mWidgetResolveInfoList = p.queryIntentActivities(
						new Intent("com.iLoong.widget", null), 
