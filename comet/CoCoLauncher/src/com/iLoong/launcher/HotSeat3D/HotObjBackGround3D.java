...
public class HotObjBackGround3D extends Object3DBase {
	...
	public HotObjBackGround3D(String name) {
		super(name);
	...
	public HotObjBackGround3D(String name, Context context, float width,
			float height, float parent_pos_y) {
		super(name);
		...
		try {
			this.region.setRegion(loadTexture(mContext,
					"theme/dock3dbar/3ddockUV.png"));
			stream = ThemeManager.getInstance().getInputStream(
					"launcher/dock3dobj/dock.obj");
	...
	public void setParentPosY(float pos_y) {
		offset_y = pos_y;
	}
	...
	private TextureRegion loadTexture(Context context, String imageFile) {
		...
		Texture texture = new BitmapTexture(ThemeManager.getInstance()
				.getBitmap(imageFile));
