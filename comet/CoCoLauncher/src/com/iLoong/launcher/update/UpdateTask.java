...
public class UpdateTask {
	...
	private void judgeUpdate() {
		...
		if (cur_year != record_year || cur_month != record_month
				|| cur_day != record_day) {
			// 下载version.xml判断更新
			...
			if (updateType == 1) {
				progressDialog = ProgressDialog.show(
						...
						mContext.getResources().getString(
								RR.string.soft_update_checking), true, true);
			}
		}
		...
		protected void onPostExecute(Boolean result) {
			if (updateType == 0) {
				...
				if (result) {
					...
					Toast.makeText(mContext, RR.string.noneed_update,
	...
	private void addNotificaction() {
		...
		notification.tickerText = mContext.getResources().getString(
				RR.string.soft_need_update);
		// 添加声音提示
		notification.defaults = Notification.DEFAULT_SOUND;
		// audioStreamType的值必须AudioManager中的值，代表着响铃的模式
		notification.audioStreamType = android.media.AudioManager.ADJUST_LOWER;
		Intent intent = new Intent(mContext, UpdateActivity.class);
		intent.putExtra(UpdateManager.EXTRA_PARAM,
				UpdateManager.EXTRA_VALUE_SERVICE);
		...
		notification.setLatestEventInfo(mContext,
				...
				mContext.getString(R.string.soft_check_update), pendingIntent);
