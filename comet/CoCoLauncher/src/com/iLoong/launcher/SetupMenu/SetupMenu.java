...
public class SetupMenu {
	...
	public SetupMenuItem getMenuItem(int action) {
	...
	public SetupMenu(Context context) {
		...
		mTabCellWidth = (int) (((float) mWidth / (mTabMenus.size() > 0 ? mTabMenus
				.size() : 1)) + 0.999);
		...
		if (DefaultLayout.popmenu_style == POPMENU_STYLE_ANDROID4) {
			...
			mSetupMenuLanyout
					.setPadding((int) horMargin, 0, (int) horMargin, 0);
		}
		FrameLayout.LayoutParams popwinlp = new FrameLayout.LayoutParams(
	...
	public void LoadSetupMenuXml() {
		...
		try {
			mrows = iLoongLauncher.getInstance().getResources()
					.getInteger(RR.integer.setup_menu_row);
			mcolumns = iLoongLauncher.getInstance().getResources()
					.getInteger(RR.integer.setup_menu_columns);

			String[] menus;
			menus = iLoongLauncher.getInstance().getResources()
					.getStringArray(RR.array.setupMenuAll);
			...
			if (FeatureConfig.enable_DefaultScene) {
				...
				{
					menus = iLoongLauncher.getInstance().getResources()
							.getStringArray(RR.array.setupMenuScene);
			...
			for (int i = 1; i < menulistall.size(); i++) {
				...
				if (menuitems != null) {
					...
					Field tmpF = (Field) RR.getStringClass().getDeclaredField(
							tmp[1]);
		...
		public void startElement(String namespaceURI, String localName,
			...
			if (localName.equals(TAG_SETUPMENU)) {
				...
				if (menuitems != null) {
					...
					try {
						mi.iconbmp = ThemeManager.getInstance().getBitmap(
								mi.icon);
						// mi.iconbmp =
					...
					if (atts.getValue("icon2") != null) {
						mi.icon2 = SETUPMENU_FOLDERNAME
								+ atts.getValue("icon2");
						try {
							mi.icon2bmp = ThemeManager.getInstance().getBitmap(
									mi.icon2);
							// mi.icon2bmp =
	...
	public static String phoneinfo() {
		TelephonyManager phoneMgr = (TelephonyManager) iLoongLauncher
	...
	public static void DialogMessage(String title, String msg) {
		...
		builder.setPositiveButton(R3D.getString(RR.string.circle_ok_action),
				new OnClickListener() {
					@Override
	...
	// wanghongjian add start //enable_DefaultScene
	public int getSceneMenu(String pkg) {
		// TODO Auto-generated method stub
		int size = 0;
