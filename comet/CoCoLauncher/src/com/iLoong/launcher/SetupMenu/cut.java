...
public class cut {
     ...
     * sortkey： 返回排序的索引
    */
    public static native int sort(int type, String[] key, int[] sortkey);
    public static native int sort(int type, int[] key, int[] sortkey);
    /*
     ...
     * return：bmp文件大小
    */
    public static native int bmp(int w, int h, int[] pixel, byte[] bmp);
    
    public static void sortByAlpha(int type, String[] key, int[] sortKey){
    ...
    public static void sortByDefault(int type, String[] key1, boolean[] key2, int[] sortKey){
    	Utils3D.showTimeFromStart("start sort");
    ...
    private static int partition(Record [] arr,int low,int high,int type,int[] result) 
    { 
    ...
    private static void swap(Record arr[],int a,int b,int[] result){
    	if(a == b)return;
    	Record s;
    	s = arr[a];
    	arr[a] = arr[b];
    ...
    private static void quickSort(Record [] records,int low,int high,int type,int[] result) 
    { 
        if (low <high ) 
        { 
    ...
    static class Record{
    	...
    	public static int compare(Record first,Record second){
