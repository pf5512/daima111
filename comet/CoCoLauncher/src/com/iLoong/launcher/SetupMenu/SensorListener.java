...
public class SensorListener{
	...
	public void sentSensorAngleToAllWidget3D(float x,float y,float z) {
		if(DefaultLayout.show_sensor){
			/*
	...
	//		Log.v("xiatian","transformSensorEventToAngle -- after:"+"xAngle=" + xAngle + ",yAngle=" + yAngle);		
			...
			//是否超过有效数据最小偏移角度
			if(
					!isDataValid(xAngle,xAngleLast)
					&&!isDataValid(yAngle,yAngleLast)
			)
			{
				return;
			}
	...
	public boolean isDataValid(float mDataCur,float mDataLast) {
		if(DefaultLayout.show_sensor){
			boolean mDataCurIsPositiveNumber = (mDataCur>0);
			boolean xDataLastIsPositiveNumber = (mDataLast>0);
		
			if(
					((mDataCurIsPositiveNumber)&&(xDataLastIsPositiveNumber)&&((Math.abs(mDataLast-mDataCur)>SENSOR_ANGLE_OFFSET_MIN)))
					||((!mDataCurIsPositiveNumber)&&(!xDataLastIsPositiveNumber)&&((Math.abs(mDataLast-mDataCur)>SENSOR_ANGLE_OFFSET_MIN)))
					||(Math.abs(mDataLast-mDataCur)>SENSOR_ANGLE_OFFSET_MIN)
			)
			{
				return true;
			}
