...
public class SetupMenuActions {
	...
	public synchronized void RegisterAction(int actionid, Action action) {
	...
	public synchronized void RegisterListener(int actionid,
	...
	public synchronized void UnRegisterListener(int actionid,
			MenuActionListener ml) {
		Integer key = Integer.valueOf(actionid);
	...
	public void Handle(int actionid) {
		Integer key = Integer.valueOf(actionid);
	...
	private void NotifyListeners(int actionid, Action act) {
		Integer key = Integer.valueOf(actionid);
		ArrayList<MenuActionListener> mls = mActionListeners.get(key);
		if (mls != null) {
	...
	public void ActivityFinish(int requestCode) {
	...
	public int getStringToIntger(String key) {
		int val = 0;
		...
		else if (key.equals(SetupMenu.getKey(RR.string.setting_key_sensor))
		...
		else if (key.equals(SetupMenu.getKey(RR.string.setting_key_particle))
				&& DefaultLayout.enable_particle) {
			val = PreferenceManager.getDefaultSharedPreferences(
					SetupMenu.getContext()).getBoolean(key, true) ? 1 : 0;
		}
		/************** added by zhenNan.ye end *******************/
			...
			val = Integer.valueOf(
					PreferenceManager.getDefaultSharedPreferences(
							iLoongLauncher.getInstance()).getString(key, "-1"))
					.intValue();
			if (val == -1) {
				if (key.equals(SetupMenu
