...
public class SystemAction extends Action {
	...
	public SystemAction(int actionid, String action) {
		super(actionid, action);
	}

	public static void Init() {
		SetupMenuActions.getInstance().RegisterAction(ActionSetting.ACTION_SYSTEM_SETTINGS,
				getAction(ActionSetting.ACTION_SYSTEM_SETTINGS));
		SetupMenuActions.getInstance().RegisterAction(ActionSetting.ACTION_SOFTWARE_MANAGEMENT,
				getAction(ActionSetting.ACTION_SOFTWARE_MANAGEMENT));
		SetupMenuActions.getInstance().RegisterAction(ActionSetting.ACTION_WALLPAPER,
				getAction(ActionSetting.ACTION_WALLPAPER));
		SetupMenuActions.getInstance().RegisterAction(ActionSetting.ACTION_LOCK_SCREEN,
				getAction(ActionSetting.ACTION_LOCK_SCREEN));
		SetupMenuActions.getInstance().RegisterAction(ActionSetting.ACTION_RESTART,
				getAction(ActionSetting.ACTION_RESTART));
	}

	private static Action getAction(int actionid) {
		final String action;
	...
	private void LockScreen() {
		DevicePolicyManager mDPM = (DevicePolicyManager) SetupMenu.getContext().getSystemService(
	...
	public static class ResestActivity extends Activity {
		...
		public void onStart() {
			if(iLoongLauncher.getInstance() != null)iLoongLauncher.getInstance().finish();
