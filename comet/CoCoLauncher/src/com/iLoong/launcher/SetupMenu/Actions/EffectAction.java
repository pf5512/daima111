...
public class EffectAction extends Action {
	...
	public EffectAction(int actionid, String action) {
		super(actionid, action);
		putIntentAction(SetupMenu.getContext(), EffectSettingActivity.class);
	}

	public static void Init() {
		SetupMenuActions.getInstance().RegisterAction(ActionSetting.ACTION_EFFECTS_SETTINGS,
				new EffectAction(ActionSetting.ACTION_EFFECTS_SETTINGS, EffectAction.class.getName()));
	...
	protected void OnPutValue(String key) {
		if (key.equals(SetupMenu.getContext().getResources().getString(RR.string.setting_key_desktopeffects))
				|| key.equals(SetupMenu.getContext().getResources().getString(RR.string.setting_key_appeffects))) {
			mBundle.putInt(
					key,
					Integer.valueOf(PreferenceManager.getDefaultSharedPreferences(SetupMenu.getContext()).getString(
							key, "0")));
		}
	}
...
//			pf.setSummary(summarys[Integer.valueOf(PreferenceManager.getDefaultSharedPreferences(SetupMenu.getContext()).getString(key, "0"))]);
			...
			pf.setSummary(summarys[Integer.valueOf(PreferenceManager
					.getDefaultSharedPreferences(SetupMenu.getContext()).getString(key, "2"))]);
		}
