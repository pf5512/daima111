...
public class Widget3DHost {
	 ...
	 */
	public void initialize(List<ResolveInfo> resolveInfoList) {
		...
		for (ResolveInfo resolveInfo : resolveInfoList) {
			...
			if (!mProviderHaspMap
				...
				mProviderHaspMap.put(resolveInfo.activityInfo.packageName,
						provider);
			}
		}
	}
	...
	public void addWidget3DProvider(String packageName,
	...
	public boolean containsWidget3DProvider(String packageName) {
	...
	public void removeWidget3D(String packageName) {
		mProviderHaspMap.remove(packageName);
	}

	public void uninstallWidget3D(String packageName) {
		mProviderHaspMap.remove(packageName);
	 ...
	 * @return
	 */
	public int generateWidgetID(String packageName) {
		if (mProviderHaspMap.containsKey(packageName)) {
			Widget3DProvider provider = mProviderHaspMap.get(packageName);
	...
	public void addWidget3D(Widget3D widget) {
		if (!this.mWidget3DList.contains(widget)) {
			this.mWidget3DList.add(widget);
		}
	}

	public Widget3D getWidget3D(String packageName) {
	...
	public boolean containsWidget(Widget3D widget) {
	...
	public void deleteWidget3DInstance(Widget3D widget) {
		if (this.mWidget3DList.contains(widget)) {
	...
	public class Widget3DProvider {
		...
		public void addWidgetId(int widgetId) {
			instanceCount++;
			currentMaxWidgetId = currentMaxWidgetId > widgetId ? currentMaxWidgetId
					: widgetId;
