...
public class PhotoBucket implements MediaDataObserver {
	...
	public class PhotoBucketView extends ViewGroup3D implements MediaView {
		...
		public PhotoBucketView(String name) {
			super(name);
		}
		...
		public void draw(SpriteBatch batch, float parentAlpha) {
			...
			if (AppHost3D.selectState) {
				if (selected)
					batch.draw(R3D.selectedRegion, x + (width + bgWidth) / 2
							- 1 * R3D.selectedRegion.getRegionWidth(), y
							+ titleHeight + (height - titleHeight - bgHeight)
							/ 2 + 8);
				else
					batch.draw(R3D.unselectRegion, x + (width + bgWidth) / 2
							- 1 * R3D.unselectRegion.getRegionWidth(), y
							+ titleHeight + (height - titleHeight - bgHeight)
							/ 2 + 8);
			}
		}
		...
		public void setSize(float w, float h) {
			...
			if (region.getTexture() == null) {
				...
				region.setRegion(new BitmapTexture((AppBar3D
						.titleToPixmapWidthLimit(title + "(" + photos.size()
								+ ")", (int) (w - numberWidth - 4),
	...
	public void update(int type) {
		// TODO Auto-generated method stub
		PhotoBucket photoBucket = MediaCache.getInstance().photoBuckets
				.get(this.bucketId);
		if (photoBucket != null)
