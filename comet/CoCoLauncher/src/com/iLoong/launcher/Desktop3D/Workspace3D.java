...
public class Workspace3D extends NPageBase implements DragSource3D,
		DropTarget3D, MenuActionListener {
	...
	public void setCurIcon(Icon3D icon) {
		cur_icon = icon;
	}

	public Workspace3D(String name) {
		super(name);
		...
		circleAble = SetupMenuActions.getInstance()
				.getStringToIntger("circled");
	...
	public boolean addInCurrenScreen(View3D child, int x, int y) {

		return addInScreen(child, getCurrentPage(), x, y, false);

	}

	public View3D getViewByItemInfo(ItemInfo info) {
		if (info == null)
			return null;
	...
	public void addBackInScreen(View3D child, int x, int y) {
		...
		if (screen < 0 || screen >= view_list.size()) {
			...
			Log.e(TAG, "The screen must be >= 0 and < " + getChildCount()
					+ " (was " + screen + "); skipping child:" + child);
	...
	public boolean removeViewInWorkspace(View3D view) {
		boolean res = true;
	...
	public boolean addInScreen(View3D child, int screen, int x, int y,
			boolean insert) {
		boolean res = false;
		...
		if (res) {
			...
			if (item instanceof UserFolderInfo) {
				iLoongLauncher.getInstance().addFolderInfoToSFolders(
						(UserFolderInfo) item);
			}
		} else {
			ItemInfo itemInfo;
	...
	public View3D createShortcut(ShortcutInfo info) {
	...
	public void bindItem(ItemInfo item) {
		...
		case LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER:
			...
			FolderIcon3D rootFolder = iLoongLauncher.getInstance()
					.getOpenFolder();
			/*
	...
	public boolean onDrop(ArrayList<View3D> list, float x, float y) {
		...
		if (iconGroupInput.size() > 1) {
			if (Workspace3D.NEW_CELLLAYOU) {
				...
				return group.addView(list, scaleX(viewParent.x
						+ viewParent.width / 2), scaleY(viewParent.y
						+ viewParent.height / 2));
			} else {
				if (!group.checkIconNums(iconGroupInput.size()))
					return true;
				IconGroup3D iconGroup = new IconGroup3D("IconGroupOnDropView",
						iconGroupInput);
			...
			if (Workspace3D.NEW_CELLLAYOU) {
				...
				dropRes = group.addView(list, scaleX(viewParent.x
						+ viewParent.width / 2), scaleY(viewParent.y
						+ viewParent.height / 2));
				...
				if (dropRes && view instanceof Widget) {
					...
					SendMsgToAndroid.sendMoveWidgetMsg(view,
							widget.getItemInfo().screen);
				}
	...
	void forceSetCellLayoutDropType(int index) {
	...
	public void DealCircleSomethingResult(float x, float y) {
	...
	public static void checkBoundary(View3D target) {
		...
		if (parent != null) {
			...
			Log.e("checkBoundary",
					"Can not check boundary whose parent is null :" + target);
		}
	}
	...
	public void onDropCompleted(View3D target, boolean success) {
		// TODO Auto-generated method stub
		if (Workspace3D.NEW_CELLLAYOU) {
			CellLayout3D group = (CellLayout3D) getChildAt(this
					.getCurrentScreen());
...
//zqh add , set workspace fling effect
	public  void setDesktopEffectType(int type){
	    setEffectType(type);
	}
	@Override
	...
	public void draw(SpriteBatch batch, float parentAlpha) {
		...
		if (iLoongLauncher.getInstance().popResult > CIRCLE_POP_NONE_ACTION) {
			...
			delCircleSomehing
					.Process_delALL(iLoongLauncher.getInstance().popResult);
	...
	public boolean multiTouch2(Vector2 initialFirstPointer,
		...
		if (root.isPageContainerVisible()) {
			return true;
		}
		...
		if (circleAble == 0 && workspaceOnLong == false) {
			float zoomDst = initialFirstPointer.dst(initialSecondPointer)
					- firstPointer.dst(secondPointer);
			Log.d("testdrag", "zoomDst=" + zoomDst + initialFirstPointer
		...
		if (circleAble == 1 && workspaceOnLong == false) {
			...
			if (zoomDst >= ZOOM_DISTANCE) {
				...
				if (getCurrentCellLayout().multiTouch2(initialFirstPointer,
						initialSecondPointer, firstPointer, secondPointer) == true) {
				...
				if (getChildAt(this.getChildCount() - 1).name == "CircleSomething3D") {
					return super.multiTouch2(initialFirstPointer,
							initialSecondPointer, firstPointer, secondPointer);
				}
			}
		}
