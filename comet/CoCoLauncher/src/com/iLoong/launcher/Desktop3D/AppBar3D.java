...
public class AppBar3D extends ViewGroup3D implements
		PageScrollListener, TweenCallback {
	public AppTab tab;
...
//			t.setFilter(TextureFilter.Linear, TextureFilter.Linear);
	...
	public void setAppHost(AppHost3D ah){
		appHost =ah;
	}
	...
	public  void onContentTableChange(int content){
	...
	public void setAppList(AppList3D appList) {
	...
	public void setAppPopMenu2(AppPopMenu2 popMenu2) {
		this.popMenu2 = popMenu2;
	}
	public static Bitmap titleToPixmap(String title, int titleWidth,
			int titleHeight, int alignH, int alignV, int padding,
			boolean bigWord, int color) {
		...
		if (titleWidth == -1) {
			...
			while (paint.measureText(title) > titleWidth
					- paint.measureText("..") - 2) {
		...
		if (x < 0)
			x = 0;
		float titleY = (float) (titleHeight - Math.ceil(fontMetrics.ascent
...
//					canvas.drawText(title, x, titleY, paint);				
	...
	public static Bitmap titleToPixmap(String title,int titleHeight,boolean bigWord, int color, boolean shadow) {
		Paint paint = new Paint();
		paint.setColor(color);
		paint.setAntiAlias(true);
		if (DefaultLayout.title_style_bold)
		...
		FontMetrics fontMetrics = paint.getFontMetrics();
		titleHeight = (int) Math.ceil(-fontMetrics.ascent
				+ fontMetrics.descent);
		Bitmap bmp = Bitmap.createBitmap(titleWidth, titleHeight,
				Config.ARGB_8888);
		Canvas canvas = new Canvas(bmp);
...
//					canvas.drawText(title, x, titleY, paint);				
	...
	public static Bitmap titleToPixmapWidthLimit(String title,int widthLimit,int size, int color, boolean shadow) {
		Paint paint = new Paint();
		paint.setColor(color);
		paint.setAntiAlias(true);
		paint.setTextSize(size);
		if (paint.measureText(title) > widthLimit - 2) {
			while (paint.measureText(title) > widthLimit
					- paint.measureText("..") - 2) {
				title = title.substring(0, title.length() - 1);
			}
			title += "..";
		...
		int titleWidth = (int) (paint.measureText(title));
		FontMetrics fontMetrics = paint.getFontMetrics();
		int titleHeight = (int) Math.ceil(-fontMetrics.ascent
				+ fontMetrics.descent);
		Bitmap bmp = Bitmap.createBitmap(titleWidth, titleHeight,
				Config.ARGB_8888);
		Canvas canvas = new Canvas(bmp);
	...
	public static Bitmap titleToPixmap(String title, int titleWidth,
			int titleHeight, int alignH, int alignV, int padding, int color) {
		return titleToPixmap(title, titleWidth, titleHeight, alignH, alignV,
				padding, false, color);
	}

	public static Bitmap titleToPixmap(String title, int titleWidth,
			int titleHeight, int alignH, int alignV, int color) {
		return titleToPixmap(title, titleWidth, titleHeight, alignH, alignV, 0,
				false, color);
	}

	public static Bitmap titleToPixmap(String title, int titleWidth,
			int titleHeight, int alignH, int alignV, boolean bigWord, int color) {
		return titleToPixmap(title, titleWidth, titleHeight, alignH, alignV, 0,
				bigWord, color);
	}
	...
	public class AppTab extends View3D {
		...
		public AppTab(String name) {
			...
			widgetTitle = new TextureRegion(new BitmapTexture(AppBar3D.titleToPixmap(
					R3D.appbar_tab_widget, (int) R3D.appbar_height, false,
					R3D.appbar_tab_color, false)));//R3D.findRegion("appbar-tab-widget");	
			contentTitle = new TextureRegion(new BitmapTexture(AppBar3D.titleToPixmap(R3D.appbar_tab_app,
					(int) R3D.appbar_height, false, R3D.appbar_tab_color, false)));//R3D.findRegion("appbar-tab-app");
			appTitle = contentTitle;
			tabArrow = new TextureRegion(new BitmapTexture(ThemeManager.getInstance().getBitmap(
					"theme/pack_source/appbar-tab-arrow.png")));//R3D.findRegion("appbar-tab-arrow");
			if(DefaultLayout.enable_explorer){
				tabContentWidth = 5*R3D.appbar_menuitem_paddingleft
						+ musicTexture.getRegionWidth() * Utils3D.getDensity()
						/ 1.5f+getTabContentWidth();
			}
			else tabContentWidth = contentTitle.getRegionWidth()+2*R3D.appbar_tab_padding;
			...
			{
				uninstallTitle = new TextureRegion(new BitmapTexture(AppBar3D
						.titleToPixmap(R3D.appbar_tab_edit_mode,
			...
			else
			{
				...
				.titleToPixmap(R3D.appbar_tab_uninstall,
						(int) R3D.appbar_height, false, R3D.appbar_tab_color,
						false)));//R3D.findRegion("appbar-"+R3D.getString(RR.string.uninstall_app));
			}
		//teapotXu add end for Folder in Mainmenu
			
//			downloadAppTitle= R3D.findRegion("appbar-"+R3D.getString(RR.string.download_apps));
			hideTitle = new TextureRegion(new BitmapTexture(AppBar3D
					.titleToPixmap(R3D.appbar_tab_hide,
					(int) R3D.appbar_height, false, R3D.appbar_tab_color,
					false)));//R3D.findRegion("appbar-"+R3D.getString(RR.string.hide_icon));
...
//			}
			...
			if (mode == AppList3D.APPLIST_MODE_NORMAL) {
				if(DefaultLayout.enable_explorer){
					batch.draw(tabArrow, R3D.appbar_menuitem_paddingleft+(musicTexture.getRegionWidth() * Utils3D.getDensity()
							/ 1.5f-tabArrow.getRegionWidth())/2, y + (height-tabArrow.getRegionHeight())/2);
					batch.draw(contentTitle, R3D.appbar_menuitem_paddingleft+musicTexture.getRegionWidth() * Utils3D.getDensity()
							/ 1.5f, y + (height-contentTitle.getRegionHeight())/2);
				}
				else{
				...
				if (!appList.mHideMainmenuWidget)
					batch.draw(widgetTitle, tabContentWidth + R3D.appbar_padding_left
							+ R3D.findRegion("appbar-divider").getRegionWidth() + R3D.appbar_tab_padding, y + (height-widgetTitle.getRegionHeight())/2);
...
//						/ 2, naviBackWidth,
//						naviBackHeight);
				batch.draw(hideTitle, R3D.appbar_tab_padding
						, y+(height-hideTitle.getRegionHeight())/2);
...
//						/ 2, naviBackWidth,
//						naviBackHeight);
				batch.draw(uninstallTitle, R3D.appbar_tab_padding
						, y+(height-uninstallTitle.getRegionHeight())/2);
			}
...
//						/ 2, naviBackWidth,
//						naviBackHeight);
				batch.draw(downloadAppTitle, R3D.appbar_tab_padding
						, y+(height-downloadAppTitle.getRegionHeight())/2);
			}
//			if(DefaultLayout.blend_func_dst_gl_one){
...
//			}
		...
		public boolean onClick(float x, float y) {
			if (mode != AppList3D.APPLIST_MODE_NORMAL){
				...
				else if (appList.mHideMainmenuWidget)return false; 
				else if (x >= tabContentWidth + R3D.appbar_padding_left
						&& x < tabContentWidth + R3D.appbar_padding_left
								+ tabWidgetWidth) {
	...
	class AppTabIndicator extends View3D {
		...
		public AppTabIndicator(String name) {
			super(name);
			x = 0;
			y = 0;
			width = Utils3D.getScreenWidth();
		...
		public void draw(SpriteBatch batch, float parentAlpha) {
			// super.draw(batch, parentAlpha);
			float w = this.getUser()
					/ (tab.tabContentWidth + R3D.findRegion("appbar-divider")
							.getRegionWidth());
			batch.setColor(color.r, color.g, color.b, color.a * parentAlpha);
...
//			 0,width,height/4);
			...
			if (!appList.mHideMainmenuWidget)
				indicatorBg.draw(batch, this.getUser(), 0, (1 - w)
						* tab.tabContentWidth + w * tab.tabWidgetWidth, height * 3 / 4);
...
//					content.hide();
	...
	class AppHomeButton extends View3D {
		...
		public AppHomeButton(String name) {
			super(name);
			...
			if (menu == null || !menu.isVisible()) {
				...
				x = Utils3D.getScreenWidth() - 2 * width - R3D.appbar_menu_right
							- R3D.appbar_home_right;
			}
...
//			batch.draw(region, x + width / 2 - regionWidth / 2, y + height / 2
//					- regionHeight / 2,regionWidth,regionHeight);
			batch.draw(region, x, y + height / 2
					- region.getRegionHeight() / 2, width, region.getRegionHeight());
...
//						R3D.getString(RR.string.uninstall_app));
			...
			if(DefaultLayout.enable_explorer){
				width = 5*R3D.appbar_menuitem_paddingleft
						+ musicTexture.getRegionWidth() * Utils3D.getDensity()
						/ 1.5f+getTabContentWidth();
				itemWidth = width;
		...
		private void addItem(TextureRegion region, String title) {
			
			ViewGroup3D item = new ViewGroup3D("popitem");
			item.y = (this.getChildCount()) * itemHeight;
			item.x = 0;
			View3D itemIcon = new View3D("itemicon", region);
			itemIcon.setPosition(R3D.appbar_menuitem_paddingleft, itemHeight/2
					- (region.getRegionHeight() * Utils3D.getDensity() / 1.5f)
					/ 2);
			itemIcon.setSize(region.getRegionWidth() * Utils3D.getDensity()
					/ 1.5f, region.getRegionHeight() * Utils3D.getDensity()
					/ 1.5f);
			item.addView(itemIcon);
			TextureRegion titleRegion = R3D.findRegion(title);
			View3D itemTitle = new View3D(
					"itemtitle",titleRegion);
			itemTitle.setPosition(2*R3D.appbar_menuitem_paddingleft
					+ itemIcon.width, itemHeight/ 2 - itemTitle.height/ 2);
			//itemTitle.setSize(itemWidth - itemTitle.x, itemHeight);
			item.addView(itemTitle);
			item.setSize(2*R3D.appbar_menuitem_paddingleft
					+ itemIcon.width+titleRegion.getRegionWidth(), itemHeight);
			View3D itemDivider = new View3D(
					"itemtitle",R3D.findRegion("appbar-popmenu-divider"));
...
//		batch.draw(R3D.findRegion("appbar-bg"), 0, y, width, height);
	...
	public void pageScroll(float degree, int index, int count) {
		...
		} else if (index == AppList3D.appPageCount
				+ AppList3D.widgetPageCount - 1
				&& degree < 0) {
			// tabIndicator.setUser((tab.tabAppWidth+R3D.findRegion("appbar-divider").getRegionWidth())*(1-Math.abs(degree)));
