...
public class CircleSomething3D extends View3D {
	...
	public CircleSomething3D(String name) {
		super(name);
	}
		...
		for (int i = 0; i < Count; i++)
		{
			...
			if (childView instanceof ViewCircled3D) {
				...
				if (tmpViewCircled.getCircled() == false) {
					bSelected = Utils3D.lineInActor(childView,
							downPath, dragPath);
					//Log.d("launcher123", " circle_somethingSelected downPath.x="+downPath.x+" downPath.y="+downPath.y);	
					//Log.d("launcher123", " circle_somethingSelected drag.x="+dragPath.x+" drag.y="+dragPath.y);
					if (bSelected == true) {
						tmpViewCircled.setCircled(true);
						//Log.v("launcher123", " circle_somethingSelected tmpViewCircled="+tmpViewCircled);	
						//Log.v("launcher123", " circle_somethingSelected selectColor.r="+selectColor.r +" selectColor.g="+selectColor.g);
	...
	//	Log.v("1234", "CircleSomething3D onTouchUp pointer=" + pointer);
		if (pointer == 0)
	...
	private void circle_sendMoveMsg(float x,float y)
	{
...
//				y, null);
	...
	private void circle_sendUpMsg(float x,float y)
	{
	...
	private void circle_sendToastMsg(float x, float y)
	{
...
private void circle_operate(float x,float y)
{
	...
	//	Log.v("1234", "CircleSomething3D onTouchDragged pointer=" + pointer+"touchDownPointer1="+touchDownPointer1);
	...
	{
		//Log.v("1234", "CircleSomething3D multiTouch2");
