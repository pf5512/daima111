...
public class PageSelect3D extends ViewGroup3D{
	...
	public PageSelect3D(String name) {
		//super(name);
	...
	public void setPageContainer(PageContainer3D pageContainer){
	...
	//for show PageEdit
	public void setupPageEdit(ArrayList<View3D> pageList){
		...
		while(i < count){
			...
			else page.setBackgroud(PageContainer3D.pageSelectedBg);
	...
	public void setupPage(ArrayList<View3D> pageList){
		cellLayoutList = pageList;
		for(int i=0;i < cellLayoutList.size();i++){
			View3D view = null;
	...
	private void calculateParams(boolean anim){
		width = Utils3D.getScreenWidth();
		x = 0;
		y = 0;
		...
		if(count >0){
			...
			if(width <= Utils3D.getScreenWidth()){
				...
				if(currentIndex != enterIndex && anim){
					...
					if(targetX < Utils3D.getScreenWidth()-width)targetX = Utils3D.getScreenWidth()-width;
					...
					if(x > 0){
						targetX = 0;
						if(anim)showTimeline.push(obtainTween(View3DTweenAccessor.POS_XY, Quad.OUT, 0.3f, targetX, y, 0));
						else x = targetX;
					}
	...
	public View3D addPage(View3D view,int index,boolean fromUser){
		ViewGroup3D page = new ViewGroup3D("page_select");
		page.transform = true;
		page.setSize(pageWidth,pageHeight);
		page.setOrigin(pageWidth/2, pageHeight/2);
		...
		if(index != enterIndex){
			...
			else page.setRotation(ROTATE_DEGREE);
		}
		else {
			if(index != currentIndex){
				page.setBackgroud(PageContainer3D.pageBg);
		...
		if(index == currentIndex && !fromUser){
			...
			showTimeline.push(view.obtainTween(View3DTweenAccessor.POS_XY, (enterIndex != currentIndex)?Quad.IN:Cubic.OUT, (enterIndex != currentIndex)?0.3f:0.4f, (pageWidth-scale*view.width)/2, (pageHeight-scale*view.height)/2, 0));
			showTimeline.push(view.obtainTween(View3DTweenAccessor.SCALE_XY, (enterIndex != currentIndex)?Quad.IN:Cubic.OUT, (enterIndex != currentIndex)?0.3f:0.4f, scale, scale, 0));
		}
		else {
			view.x = (pageWidth-scale*view.width)/2;
			view.y = (pageHeight-scale*view.height)/2;
	...
	public boolean scroll(float x, float y, float deltaX, float deltaY) {
		...
		if(deltaX != 0 && width > Utils3D.getScreenWidth()){
			...
			if(deltaX > 0)targetX -= deltaX*(width+targetX-Utils3D.getScreenWidth())/(Utils3D.getScreenWidth()-screenX);
			else {
				if(targetX != 0)targetX += deltaX*targetX/screenX;
			}
			if(-targetX+Utils3D.getScreenWidth() > width)targetX = Utils3D.getScreenWidth()-width;
			if(targetX > 0)targetX = 0;
			stopTween();
		...
		if(deltaX > 0){
			...
			if(click == true){
				click = false;
				pointer = pointerNextPage(x);
				if(pointer == -1)pointer = pointerCurrentPage(x);
				if(pointer == -1)pointer = pointerPreviousPage(x);
				if(pointer == currentIndex)pointer = getChildCount()-1;
				...
				pointer = pointerCurrentPage(x);
				if(pointer == -1)pointer = pointerPreviousPage(x);
				if(pointer == -1)pointer = pointerNextPage(x);
			}
		...
		if(checkTimer){
			...
			else if(lastX-x>MOVE_DISTANCE 
					|| lastX-x<-MOVE_DISTANCE 
		...
		if(pointer == -1 || pointer == getChildCount()-1)return true;
		...
		if(pointer >= currentIndex)pointer++;
		startSwitch(pointer);
	...
	public void switchToPage(int selected){
		if(animatingHide || animatingShow){
			return;
		}
	...
	public void startSwitch(int selected){
		View3D previousTarget = getChildAt(this.getChildCount()-1);
		View3D nextTarget = getChildAt(selected);
		if(selected > currentIndex)nextTarget = getChildAt(selected-1);
	...
	public int pointerCurrentPage(float x){
	...
	public int pointerPreviousPage(float x){
		View3D view;
	...
	public int pointerNextPage(float x){
		View3D view;
	...
	public boolean pointerXInPage(View3D page,float x){
		point.x = x - page.x;
		return ((point.x > pageWidth/3 && point.x <  page.width+pageWidth/3));
	}
	
	public boolean pointerYInPage(View3D page,float y){
		point.y = y - page.y;
		return ((point.y > 0 && point.y <  page.height));
	}
	
	private void resetToParent(View3D view){
...
//		}
	...
	public boolean hide(boolean anim){
		if(animatingHide || animatingShow){
			return false;
		}
...
//			}
		...
		for(int i = 0;i < getChildCount();i++){
			...
			if(i == getChildCount()-1){
				...
				if(homeIcon != null)page.removeView(homeIcon);
				View3D v = null;
	...
	public boolean keyUp(int keycode) {
		if(keycode == KeyEvent.KEYCODE_BACK)pageContainer.hide(true);
		return true;
	}
...
//		targetOffset = (float)(index)/(cellLayoutList.size());
	...
	private void calculateWallpaperOffset(){
		...
		Tween.to(animView, View3DTweenAccessor.USER, 0.5f).ease(Cubic.OUT)
			.target(targetOffset).start(View3DTweenAccessor.manager);
	...
	private void setWallpaperOffset(){
		if(animView.getUser() == targetOffset || targetOffset == -1)return;
	...
	public List<View3D> getViewList(){
		...
		for(int i=0;i < size;i++){
			...
			if(i == size-1)viewList.add(currentIndex, view);
			else viewList.add(view);
		}
		return viewList;
	}
	...
	class ViewInfoHolder{
		...
		public void getInfo(View3D view){
			this.x = view.x;
			this.y = view.y;
		...
		public void applyInfo(View3D v){
			v.x = this.x;
			v.y = this.y;
			v.width = this.width;
