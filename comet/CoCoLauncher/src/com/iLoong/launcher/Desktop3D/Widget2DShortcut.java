...
public class Widget2DShortcut extends View3D{
	...
	private NinePatch widget_holo = null;
	
	public static final int MSG_WIDGET_SHORTCUT_LONGCLICK = 0;
	
	public Widget2DShortcut(String name) {
		super(name);
		// TODO Auto-generated constructor stub
	}

	public Widget2DShortcut(String name, TextureRegion region) {
		super(name,region);
		resumeImage = new ImageView3D("resumeicon",
				Icon3D.resumeTexture,
				R3D.workspace_multicon_width, R3D.workspace_multicon_height);
		hideImage = new ImageView3D("hideicon",
				Icon3D.hideTexture, R3D.workspace_multicon_width,
		...
		if (widget_holo == null) {
			...
			if(!iLoongLauncher.releaseTexture)bm.recycle();
		}
	}
	
	public void setInfo(WidgetShortcutInfo widgetInfo){
			...
			if(!isDragView){
				if (DefaultLayout.show_widget_shortcut_bg) {
					if (bg != null) {
						if (DefaultLayout.widget_shortcut_title_top) {
							batch.draw(bg, x, y,
									width, height * (1 - R3D.widget_preview_title_weight));
						} else {
							batch.draw(bg, x, y + height
									* R3D.widget_preview_title_weight, width, height
									* (1 - R3D.widget_preview_title_weight));
						}
					}
				}
				batch.draw(region, previewX+x, previewY+y, originX, originY, 
...
//			else
			if (DefaultLayout.widget_shortcut_title_top) {
				batch.draw(titleRegion, x, y + height*(1-R3D.widget_preview_title_weight), originX, originY, 
						titleRegion.getRegionWidth(), titleRegion.getRegionHeight(), scaleX,scaleY, rotation);
			} else {
		...
		if (cellRegion[0] != null) {
			...
			if (titleX < 0)
				titleX = 0;
			if (DefaultLayout.widget_shortcut_title_top) {
				batch.draw(cellRegion[0], width * 3 / 4 + x + titleX, y + ((R3D.widget_preview_title_weight * height)-cellRegion[0].getRegionHeight())/2 + height*(1-R3D.widget_preview_title_weight), originX, originY, 
						cellRegion[0].getRegionWidth(), cellRegion[0].getRegionHeight(), scaleX,scaleY, rotation);
				batch.draw(xRegion, width * 3 / 4 + x + titleX + cellRegion[0].getRegionWidth(), y + ((R3D.widget_preview_title_weight * height)-xRegion.getRegionHeight())/2 + height*(1-R3D.widget_preview_title_weight), originX, originY, 
						xRegion.getRegionWidth(), xRegion.getRegionHeight(), scaleX,scaleY, rotation);
				batch.draw(cellRegion[1], width * 3 / 4 + x + titleX + cellRegion[0].getRegionWidth() + xRegion.getRegionWidth(), y + ((R3D.widget_preview_title_weight * height)-cellRegion[1].getRegionHeight())/2 + height*(1-R3D.widget_preview_title_weight), originX, originY, 
						cellRegion[1].getRegionWidth(), cellRegion[1].getRegionHeight(), scaleX,scaleY, rotation);
			} else {
				batch.draw(cellRegion[0], width * 3 / 4 + x + titleX, y + ((R3D.widget_preview_title_weight * height)-cellRegion[0].getRegionHeight())/2, originX, originY, 
						cellRegion[0].getRegionWidth(), cellRegion[0].getRegionHeight(), scaleX,scaleY, rotation);
				batch.draw(xRegion, width * 3 / 4 + x + titleX + cellRegion[0].getRegionWidth(), y + ((R3D.widget_preview_title_weight * height)-xRegion.getRegionHeight())/2, originX, originY, 
						xRegion.getRegionWidth(), xRegion.getRegionHeight(), scaleX,scaleY, rotation);
				batch.draw(cellRegion[1], width * 3 / 4 + x + titleX + cellRegion[0].getRegionWidth() + xRegion.getRegionWidth(), y + ((R3D.widget_preview_title_weight * height)-cellRegion[1].getRegionHeight())/2, originX, originY, 
						cellRegion[1].getRegionWidth(), cellRegion[1].getRegionHeight(), scaleX,scaleY, rotation);
			}
		}
		if (hide && widgetInfo.isHide && resumeImage != null) {
		...
		if (hide && !widgetInfo.isHide && hideImage != null) {
		...
		if (hide) {
			if (!widgetInfo.textureName.equals("")) {
				...
				PreferenceManager
						...
						.putBoolean("HIDE:" + widgetInfo.textureName, widgetInfo.isHide)
						.commit();
			}
			return true;
		}
