...
public class IconGroup3D extends ViewGroup3D
{
	 ...
	 * @param stage
	 */
	public IconGroup3D(List<View3D> circleGroup)
	{
		this(null,circleGroup);
	}
	public IconGroup3D(String name ,List<View3D> circleGroup)
	{
		super(name);
	...
	private void IconContainInit(List<View3D> circleGroup)
	{
		iconContain = new GridView3D("iconGroupGrid", gridRight - gridLeft,
...
//		}
	...
	private void buildElements(List<View3D> circleGroup)
	{
        ...
        buttonResize = new ResizeButton3D("button_resize", 
        		          R3D.getTextureRegion("shell-interactive-grid-scale-button"));
		TextureRegion gridbgTexture = R3D.getTextureRegion("shell-interactive-grid-bg");
		
		gridbackground = new NinePatch(gridbgTexture,R3D.icongroup_round_radius,R3D.icongroup_round_radius,
				                                     R3D.icongroup_round_radius,R3D.icongroup_round_radius);
...
//		  buttonResize.y=gridBottom-buttonOK.height/2;
	...
	private void outRegionCalcGrid()
	{
		...
		{
			if (gridBottom-(int)ScaleY>this.getHeight() - 2 * iconH
					- buttonResize.getHeight() / 4)
			{
				bNeedDealVer=false;
			}
		}
...
//		buttonResize.y = gridBottom - buttonOK.height / 2;
	...
	private void translateIconGroup(float offsetX,float offsetY)
	{
		gridLeft+=offsetX;
		gridBottom-=offsetY;
...
//	}
	...
	public boolean onTouchUp(float x, float y, int pointer)
	{
		...
		if (x>buttonOK.x-buttonOK.width/2 && x<buttonOK.x+buttonOK.width+buttonOK.width/2 && 
				y>buttonOK.y-buttonOK.height/2 && y<buttonOK.y+buttonOK.height+buttonOK.height/2)
			{
				DealButtonOKDown();
				return true;
			}
	...
	public boolean onTouchDown (float x, float y, int pointer) {
		...
		if (x>buttonResize.x-buttonResize.width/2  && x<(buttonResize.x+buttonResize.width*1.5) 
                &&  y<(buttonResize.y+buttonResize.height*1.5) 
                && y>(buttonResize.y-buttonResize.height*0.5))
		{
			super.onTouchDown(x, y, pointer);
			return true;
		}
		...
		if ( (hitView.name ==iconContain.name) || (x>gridLeft && x<gridRight && 
			 y>gridBottom && y<gridTop))
		{
			bPermitTrans=true;
			transLateX = x;
			translateY = y;
	...
	/*重叠的情况下的图标排列*/
	protected void onLayout_Get_RowAndColNum(int count,int max_row_num, int max_col_num,float width, float height)
	{
		/* 确定每行和列显示的实际数目 */
...
/*内部内，让拖放Button有获取焦点功能*/	
	 public class ResizeButton3D extends ImageView3D
	 {
		float startX = 0;
		float startY = 0; 
		public ResizeButton3D(String name) {
			super(name);
			// TODO Auto-generated constructor stub
		}
		 
		public ResizeButton3D(String name,TextureRegion textureRegion) {
			super(name,textureRegion);
			// TODO Auto-generated constructor stub
		}
		
		public ResizeButton3D (String name, Texture texture) {
			super(name,texture);	
		}
		@Override
		public boolean onTouchDown (float x, float y, int pointer) {
		...
		public boolean scroll(float x, float y, float deltaX, float deltaY) 
		{
			...
			if (Math.sqrt((x - startX) * (x - startX)
					+ (y - startY) * (y - startY)) > MIN_FLING_DISTANCE) {
				/* 重新得到放大比例因子 */
				ScaleX = (x - startX);
				ScaleY = (startY - y);
