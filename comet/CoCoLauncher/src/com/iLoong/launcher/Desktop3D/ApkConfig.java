...
public class ApkConfig {
	...
	private static final String APKCONFIG_PATH = "theme/apk/";
	...
	public boolean apkConfigRight() {
		...
		for (int i = 0; i < configApkList.size(); i++) {
			...
			try {
				// InputStream
				// inputStream=iLoongLauncher.getInstance().getAssets().open(apkname);
				InputStream inputStream = ThemeManager.getInstance()
						.getInputStream(apkname);
				if (inputStream != null) {
					inputStream.close();
				}
			} catch (IOException e) {
	...
	public void LoadXml(String Filename, DefaultHandler handler) {
		...
		try {
			...
			InputSource xmlin = new InputSource(ThemeManager.getInstance()
