...
public class PageContainer3D extends ViewGroup3D implements MenuActionListener{
	...
	public PageContainer3D(String name) {
		super(name);
	...
	//show PageEdit
	public void setupPageEdit(ArrayList<View3D> workspaceList,int selectedPage,int home){
		...
		if(!pageSelect.hideForEdit())return;
	...
	//show PageSelect
	public void setupPageSelect(ArrayList<View3D> workspaceList,int selectedPage,int home){
		width = Utils3D.getScreenWidth();
		height = Utils3D.getScreenHeight();
		x = 0;
		y = 0;
	...
	public void setEnterPage(int enterPage){
	...
	public void setHomePage(int home){
	...
	public void removeCellLayout(int i,boolean temp){
	...
	public void addCellLayout(int i,CellLayout3D celllayout){
...
//		celllayoutList.add(tmpCell);
	...
	public void switchToPage(int selected){
		if(pageSelect.isVisible())pageSelect.switchToPage(selected);
	}
...
//		}
	...
	public boolean onTouchUp(float x, float y, int pointer){
		if (pageModeIcon.pointerInAbs(x, y)) return false;
	...
	public void setActionListener() {
		SetupMenuActions.getInstance().RegisterListener(
				ActionSetting.ACTION_SCREEN_EDITING, this);
	}
