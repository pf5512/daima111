...
public class ApplistVirtualIconManager {
	...
	public ApplistVirtualIconManager(AppList3D appList) {
		mAppList = appList;
	...
	private static List<ResolveInfo> queryAllApps() {
		...
		final PackageManager packageManager = iLoongLauncher.getInstance()
				.getPackageManager();
				...
				for (int j = 0; j < currentVirtualIcons.size(); j++) {
					...
					for (int i = 0; i < apps.size(); i++) {
						if (apps.get(i).activityInfo.packageName
								.equals(currentVirtualIcons.get(j).packageName)) {
							find = true;
							break;
						}
					}
	...
	public static void onAppsAdd(ArrayList<ApplicationInfo> apps) {
		mAppList.removeVirtualApps(apps);
	}

	public static void onAppsRemoved(ArrayList<ApplicationInfo> apps) {
		ArrayList<ApplicationInfo> needToAdd = new ArrayList<ApplicationInfo>();
		for (int j = 0; j < apps.size(); j++) {
			for (int i = 0; i < currentVirtualIcons.size(); i++) {
				if (currentVirtualIcons.get(i).packageName
	...
	public static void insertVirtualIcons(
		...
		for (int j = 0; j < virtualIcons.size(); j++) {
			boolean find = false;
			for (int i = 0; i < apps.size(); i++) {
				if (apps.get(i).activityInfo.packageName.equals(virtualIcons
						.get(j).packageName)) {
					find = true;
					break;
				}
...
//			if(!ThemeManager.getInstance().loadFromTheme("theme/dynamic_menu/" + imageName)){	//xiatian del			
			if(!ThemeManager.getInstance().loadFromTheme("theme/icon/80/" + imageName)){	//xiatian add
			//xiatian end
				
				w *= DefaultLayout.thirdapk_icon_scaleFactor;
				h *= DefaultLayout.thirdapk_icon_scaleFactor;
			}
			appsItem.iconBitmap = Tools.resizeBitmap(appsItem.iconBitmap,
					w, h);
			Resources res = iLoongLauncher.getInstance().getResources();
			int identifier = res.getIdentifier(title, "string", iLoongLauncher
			...
			appsItem.intent.setComponent(
					new ComponentName(
							"com.coco.lock2.lockbox",
							"com.coco.lock2.lockbox.MainActivity"
							)
					);
			//xiatian add end
