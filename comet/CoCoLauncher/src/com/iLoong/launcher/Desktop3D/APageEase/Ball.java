...
public class Ball {
	private static void build_ball(View3D icon ,float rotaDegX,float rotaDeg,float ratio,float this_width,float colora,float this_height)
	{
		Color color;
		Vector2 old_pos;
	...
	private static void destroy_ball(View3D icon ,float rotaDegX,float rotaDeg,float ratio,float this_width,float colora,float this_height)
	{
		Color color;
		Vector2 old_pos;
		old_pos = (Vector2)icon.getTag();
		icon.setPosition(old_pos.x+(this_width/2 - old_pos.x-icon.originX)*ratio , old_pos.y+(this_height/2-old_pos.y-icon.originY)*ratio);
		icon.setPosition(this_width/2-icon.originX+(old_pos.x-(this_width/2-icon.originX))*ratio , 
	...
	private static void rotate_ball(View3D icon ,float degree,float rotaDegX,float rotaDeg,float des_rotaDeg,float ratio,float this_width,float this_height)
	{		
		Color color;
		color = icon.getColor();
	...
	public static void updateEffect(GridView3D cur_view,GridView3D next_view,float degree, float yScale, float this_width)
	{
		...
		else if(degree<=(-1/8f) && degree>(-7/8f)) 
		{
			...
			//teapotXu add start: 解决圆柱等特效边缘前后两个view交叉重叠的问题
			{
