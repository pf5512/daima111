			...
			for(int column = 0; column < next_view_colomn_num; column++)
			{
				for(int row = 0; row < next_view_row_num; row++)
				{
					...
					if(item_index < next_view.getChildCount()){
						...
						if(Math.abs(degree)*2 < 1f + (float)(next_view_colomn_num - column)/(next_view_colomn_num)
							&& Math.abs(degree)*2 > 1f +(next_view_colomn_num - column - 1f)/next_view_colomn_num) 
						{
							//确保它的后一列显示
...
//			Log.v("cooee", "Erase ---- 0~~-1/2f --- enter");
			...
			for(int column = cur_view_colomn_num -1; column >= 0; column--)
			{
				for(int row = 0; row < cur_view_row_num; row++)
				{
					...
					if(item_index < cur_view.getChildCount()){
						...
						if(Math.abs(degree) > (float)(cur_view_colomn_num -column -1)/(2*cur_view_colomn_num)
							&& Math.abs(degree) < (float)(cur_view_colomn_num - column)/(2*cur_view_colomn_num)) 
						{
	...
	private static void ExaminePreColumnsStatusAndImplement(ViewGroup3D cur_view, int cur_column, int pre_column, boolean is_shown_or_disappeard)
	{
