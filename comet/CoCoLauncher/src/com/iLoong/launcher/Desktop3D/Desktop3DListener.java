...
public class Desktop3DListener implements ApplicationListener,
	...
	public void create() {
		...
		if (DefConfig.DEF_NEW_SIDEBAR == true) {
			...
			root.getSidebar().setIconCache(iconCache);
	...
	public void create2() {
		...
		int homePage = PreferenceManager.getDefaultSharedPreferences(
				SetupMenu.getContext()).getInt("home_page",
		...
		if (DefConfig.DEF_NEW_SIDEBAR == true) {
			...
			dragLayer.addDropTarget(root.getSidebar());
		}
	...
	private void replaceItemInfoIcon(ItemInfo item) {
	...
	public Icon3D addShortcut(final ShortcutInfo info) {
		...
		final Bitmap bitmapRemain = Bitmap.createBitmap(info
				.getIcon(iLoongApplication.mIconCache));
		final Bitmap bitmapTmp = Bitmap.createBitmap(info
				.getIcon(iLoongApplication.mIconCache));
	...
	public void addShortcutFromDrop(final ShortcutInfo info) {
		...
		iLoongLauncher.getInstance().postRunnable(new Runnable() {
			...
			public void run() {
				R3D.packer.updateTextureAtlas(R3D.packerAtlas,
						TextureFilter.Linear, TextureFilter.Linear);
				Icon3D icon = new Icon3D(info.title.toString(), R3D.findRegion(
						info, info.title.toString()));
				icon.setItemInfo(info);

				workspace.setTag(null);
				if (workspace.addInScreen(icon, info.screen, info.x, info.y,
						false)) {
				...
				synchronized (Desktop3DListener.this) {
					...
					for (int i = 0; i < widgets.size(); i++) {
						...
						if (rawInfo instanceof AppWidgetProviderInfo) {
							...
							b = getWidgetPreview(info.provider, previewImage,
									info.icon, cellSpans[0], cellSpans[1], -1,
									-1);
	...
	public void bindAppsRemoved(final ArrayList<ApplicationInfo> apps,
		...
		launcher.postRunnable(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				if (permanent) {
					if (root.isPageContainerVisible() == false) {
		...
		launcher.postRunnable(new Runnable() {
			...
			public void run() {
				...
				for (int i = 0; i < count; i++) {
					...
					if (DefConfig.DEF_NEW_SIDEBAR == true) {
						if (item.itemType == LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER) {
							...
							String name = folderInfo.title.toString();
							if (!name.equals(R3D.folder3D_name)) {
								R3D.pack(name, FolderIcon3D.titleToTexture(
										name, Color.WHITE));
							}
				...
				if (DefConfig.DEF_NEW_SIDEBAR == false) {
					SideBar sidebar = root.getSidebar();
	...
	public void finishBindWorkspace() {
		...
		if (FeatureConfig.enable_DefaultScene && DefaultLayout.scene_old) {
			...
			String pkg = preferences.getString("scenepkg",
					FeatureConfig.scene_pkg);
			String cls = preferences.getString("scenecls",
					FeatureConfig.scene_cls);
	...
	public void sortApp(final int checkId) {
		launcher.postRunnable(new Runnable() {
	...
	private void addWidget3DInUpdated(ResolveInfo resolveInfo) {
		if (mIsWidget3DUpdated) {
			...
			if(mUpdatedWidget3DInfo != null)wdgInfo = mUpdatedWidget3DInfo;
