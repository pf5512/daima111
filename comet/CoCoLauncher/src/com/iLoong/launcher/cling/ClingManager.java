...
//    public boolean widgetClingFired = false;
    ...
    public static ClingManager getInstance(){
    	if(clingManager == null){
    		synchronized(ClingManager.class){
    			if(clingManager == null)clingManager = new ClingManager();
    		}
    	}
    	return clingManager;
    }
    ...
    public void setLauncher(iLoongLauncher launcher){
    	...
    	layoutParams = new FrameLayout.LayoutParams(android.view.ViewGroup.LayoutParams.FILL_PARENT,
    			android.view.ViewGroup.LayoutParams.FILL_PARENT);
    	okButton = new ImageView(context);
    	okButton.setTag("ok");
		okButton.setBackgroundResource(RR.drawable.cling_ok);
		FrameLayout.LayoutParams buttonParams = new FrameLayout.LayoutParams(android.view.ViewGroup.LayoutParams.WRAP_CONTENT,
    	...
    	pointParams = new FrameLayout.LayoutParams(android.view.ViewGroup.LayoutParams.WRAP_CONTENT,
    			android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
...
//    	}
    ...
    public void removeTarget(int priority){
    	if(!showCling)return;
    	for(int i=0;i<targets.size();i++){
    ...
    public ClingTarget getTarget(int priority){
    	if(!showCling)return null;
    	for(int i=0;i<targets.size();i++){
    		ClingTarget target = targets.get(i);
    		int _priority = target.getClingPriority();
    		if(priority==_priority){
    ...
    public void fireAllAppCling(ClingTarget target){
    	if(!showCling)return;
    ...
    public int firePageIndicatorCling(ClingTarget target){
    	if(!showCling)return -1;
    	if(pageIndicatorClingFired)return CLING_STATE_DISMISSED;
    ...
    public int fireFolderCling(ClingTarget target){
    	if(!showCling)return -1;
    	if(folderClingFired)return CLING_STATE_DISMISSED;
    ...
    public void resetFolderCling(ClingTarget target){
    	if(!showCling)return;
...
//    }
    ...
    public int fireSelectCling(ClingTarget target){
    	if(!showCling)return -1;
...
//    }
    ...
    public void showAllAppCling(int x,int y,int r){
    	if(!showCling)return;
    	SharedPreferences.Editor editor = prefs.edit();
        editor.putInt(WORKSPACE_DISMISSED_KEY, PAGEINDICATOR_CLING);
        editor.commit();
    ...
    public void showPageIndicatorCling(int x,int y,int r){
    	if(!showCling)return;
    	SharedPreferences.Editor editor = prefs.edit();
        editor.putInt(WORKSPACE_DISMISSED_KEY, FOLDER_CLING);
        editor.commit();
    ...
    public void showFolderCling(int x,int y,int r){
    	if(!showCling)return;
    	SharedPreferences.Editor editor = prefs.edit();
		editor.putInt(WORKSPACE_DISMISSED_KEY, END_CLING);
		editor.commit();
...
//    }
    ...
    public void showSelectCling(int x,int y,int r,int gap){
    	if(!showCling)return;
    	SharedPreferences.Editor editor = prefs.edit();
		editor.putInt(SELECT_DISMISSED_KEY, END_CLING);
		editor.commit();
...
//		}
	...
	public void setActionListener() {
		...
		SetupMenuActions.getInstance().RegisterListener(
				ActionSetting.ACTION_RESET_WIZARD, this);
	}
