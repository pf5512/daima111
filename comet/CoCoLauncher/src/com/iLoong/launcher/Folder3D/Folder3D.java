   ...
   private static boolean displayButton=false;
	...
	{  
		...
		if (findView("button_ok") == null) {
			...
			buttonOK.setSize(R3D.icongroup_button_width,
					R3D.icongroup_button_height);
			
		    buttonOK.setPosition(this.width+R3D.folder_group_left_margin - R3D.icongroup_button_width,
						this.height-R3D.icongroup_button_height-R3D.icongroup_button_height/4);
...
//				myActor.show();
	...
	private void  buildIconGroup( )
	{
		...
		NinePatch gridbackground = new NinePatch(gridbgTexture,
				R3D.folder_group_left_round+2, R3D.folder_group_right_round,
				R3D.folder_group_top_round+2, R3D.folder_group_bottom_round);
		
		iconContain = new GridView3D("iconGroupGrid", this.width,
				this.height , countX, countY);
		...
		inputTextView.setPosition(R3D.folder_group_left_round,
						(float) (iconContain.height-R3D.folder_group_text_height-R3D.folder_group_top_round) );

		if (Utils3D.getScreenHeight()<400 && R3D.icon_bg_num>0)
		{
			iconContain.setPadding(2 * childMargin, 2 * childMargin, 
					(int)(R3D.folder_group_text_height+R3D.folder_group_top_round),2*childMargin);	
		}
		else
		{
			iconContain.setPadding(2 * childMargin, 2 * childMargin, 
					(int)(R3D.folder_group_text_height+R3D.folder_group_top_round+R3D.icongroup_margin_top),2*childMargin);	
		}
//		else
//		{
...
//		}
		...
		NinePatch gridTitlebackground = new NinePatch(gridTitleTexture,
				R3D.folder_group_text_round, R3D.folder_group_text_round,
				R3D.folder_group_text_round, R3D.folder_group_text_round);
		...
		titleTexture = new BitmapTexture(titleToTexture(
				mFolderIcon.mInfo.title.toString(), titleWidth, titleHeight));
		addView(iconContain);
	...
	public void onInputNameChanged( )
	{
			...
			titleWidth = (int) (Utils3D.getScreenWidth()
			        -R3D.folder_group_left_margin
			        -R3D.folder_group_right_margin- R3D.icongroup_button_width * 2);
			titleHeight = R3D.folder_group_text_height;;
			texture = new BitmapTexture(titleToTexture(inputNameString, titleWidth,
					titleHeight));
			t = this.titleTexture;
	...
	private Bitmap titleToTexture(String title,int titleWidth,int titleHeight)
	{
	...
	private void startAnimation_rotate(ArrayList<View3D> templist)
	{
		...
		for (int i = 0; i < Count; i++) {
			...
			if (i == Count-2) {
				...
				animation_line.push(Tween
						.to(myActor, View3DTweenAccessor.ROTATION, 0.2f)
						.target(-R3D.folder_icon_rotation_degree, 0, 0)
						.ease(Linear.INOUT).delay(0.1f+delayFactor * (Count - i)));
			} else if (i == Count-3) {
				...
				animation_line.push(Tween
						.to(myActor, View3DTweenAccessor.ROTATION, 0.2f)
						.target(0, 0, 0).ease(Linear.INOUT).delay(0.1f+delayFactor * (Count - i)));
			}
			animation_line.push(Tween.to(myActor, View3DTweenAccessor.SCALE_XY, 0.2f)
					.target(mFolderIcon.getScaleFactor(0), mFolderIcon.getScaleFactor(0), 0)
					.delay(0.1f+delayFactor * (Count - i))
					.ease(Linear.INOUT));
			animation_line.push(Tween.to(myActor, View3DTweenAccessor.POS_XY, 0.3f)
					.target(mFolderIcon.getPosx(), mFolderIcon.getPosy(), 0).ease(Cubic.OUT)
					.delay(delayFactor * (Count - i)));
			//animation_line.push(viewTween);
		}
		//this.x = this.x - mFolderIcon.x;
		//this.y = this.y - mFolderIcon.y;
		animation_line.push(Tween.to(this, View3DTweenAccessor.POS_XY, 0.4f)
				.target(-mFolderIcon.x, -Utils3D.getScreenHeight())
				.ease(Linear.INOUT));
	...
	private void startAnimation_scale(ArrayList<View3D> templist)
	{
		...
		for (int i = 0; i < Count; i++) {
			...
			.target(mFolderIcon.getPosx(),
					mFolderIcon.getPosy(), 0).ease(Linear.INOUT)
					.delay(delayFactor * i)
					);
			animation_line.push(Tween.to(view, View3DTweenAccessor.SCALE_XY,duration)
					.target(mFolderIcon.getScaleFactor(i), mFolderIcon.getScaleFactor(i), 0)
					.delay(delayFactor * i)
					.ease(Cubic.OUT));
		}
		//this.x = this.x - mFolderIcon.x;
		//this.y = this.y - mFolderIcon.y;
			...
			duration= 0.5f;
		animation_line.push(Tween.to(this, View3DTweenAccessor.POS_XY, duration)
				.target(-mFolderIcon.x, -Utils3D.getScreenHeight())
				.ease(Linear.INOUT));
...
//				R3D.folder_group_bottom_margin, 0, 0);
	...
	public boolean onTouchUp(float x, float y, int pointer) {
		...
		if ( buttonOK.isVisible() && x>buttonOK.x-buttonOK.width/2 && y>buttonOK.y-buttonOK.height/2 &&
			x<Utils3D.getScreenWidth() && y<Utils3D.getScreenHeight()
			)
		{
