...
public class FolderIconPath extends ViewGroup3D {
	...
	public FolderIconPath(String name) {
		super(name);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.iLoong.launcher.UI3DEngine.ViewGroup3D#onTouchDown(float, float,
	 * int)
	 */
	@Override
	public boolean onTouchDown(float x, float y, int pointer) {
		// TODO Auto-generated method stub
		if (buttonOK.isVisible() == true && pointer == 0
				&& bCanDealButtonDown == true) {
			if (x > buttonOK.x - buttonOK.width / 2
					&& x < buttonOK.x + buttonOK.width + buttonOK.width / 2
					&& y > buttonOK.y - buttonOK.height / 2
					&& y < buttonOK.y + buttonOK.height + buttonOK.height / 2) {
				bTouchDown = true;
			}
		}
	...
	public boolean onTouchUp(float x, float y, int pointer) {
		if (buttonOK.isVisible() == false && pointer == 0) {
			...
			buttonOK.startTween(View3DTweenAccessor.SCALE_XY, Quad.IN, 0.5f,
					0f, 0, 0f);
			buttonShowTween = Tween
					.to(buttonOK, View3DTweenAccessor.ROTATION, 0.5f)
					.target(360, 0, 0).ease(Quad.OUT)
	...
	private void animation_rotate() {
		...
		for (int i = 0; i < Count; i++) {
			...
			view.y = view.y - mFolderIcon.y + R3D.workspace_cell_height
			...
			viewTween.push(Tween.to(view, View3DTweenAccessor.ROTATION, 0.2f)
					.target(mFolderIcon.getRotateDegree(), 0, 0)
					.ease(Linear.INOUT).delay(0.3f + delayFactor * i));
			viewTween.push(Tween
					.to(view, View3DTweenAccessor.SCALE_XY, 0.2f)
					.target(mFolderIcon.getScaleFactor(0),
							mFolderIcon.getScaleFactor(0), 0)
					.delay(0.3f + delayFactor * i).ease(Linear.INOUT));
			viewTween.push(Tween.to(view, View3DTweenAccessor.POS_XY, 0.5f)
					.target(mFolderIcon.getPosx(), mFolderIcon.getPosy())
					.ease(Cubic.OUT).delay(delayFactor * (i)));
	...
	private void animation_iphone() {
		...
		for (int i = 0; i < Count; i++) {
			...
			viewTween.push(Tween.to(view, View3DTweenAccessor.POS_XY, duration)
					.target(mFolderIcon.getPosx(), mFolderIcon.getPosy(), 0)
					.delay(delayFactor * i).ease(Linear.INOUT));
			viewTween.push(Tween
					.to(view, View3DTweenAccessor.SCALE_XY, duration)
					.target(mFolderIcon.getScaleFactor(i),
							mFolderIcon.getScaleFactor(i), 0)
					.delay(delayFactor * i).ease(Cubic.OUT));
			buttonHideTween.push(viewTween);
	...
	public void DealButtonOKDown() {
		...
		if (buttonOK.isVisible() && bCanDealButtonDown == true) {
			...
			TotalPath.clear();
			stopAnimation();
			mFolderIcon.changeTextureRegion(controlIcon,
			...
			if (mFolderIcon.folder_style == FolderIcon3D.folder_rotate_style) {
				buttonHideTween.push(Tween
						.to(buttonOK, View3DTweenAccessor.ROTATION, 0.5f)
						.target(-360, 0, 0).ease(Quad.OUT));
				buttonHideTween.push(Tween
						.to(buttonOK, View3DTweenAccessor.SCALE_XY, 0.5f)
						.target(0, 0, 0).ease(Quad.OUT));
			}
			buttonHideTween.start(View3DTweenAccessor.manager)
					.setCallback(this);
		}
	}
			...
			for (int i = 0; i < Count; i++) {
				...
				if (mFolderIcon.folder_style == FolderIcon3D.folder_rotate_style) {
					animation_line.push(Tween
							.to(view, View3DTweenAccessor.CPOS_XY, 0.1f)
							.target(TotalPath.get(targetPosOffset * i
									+ targetPosOffset / 2).x,
									TotalPath.get(targetPosOffset * i
											+ targetPosOffset / 2).y)
							.ease(Linear.INOUT));
				} else {
					animation_line.push(Tween
							.to(view, View3DTweenAccessor.CPOS_XY, 0.1f)
							.target(TotalPath.get(targetPosOffset
									* (Count - 1 - i) + targetPosOffset / 2).x,
									TotalPath.get(targetPosOffset
											* (Count - 1 - i) + targetPosOffset
											/ 2).y).ease(Linear.INOUT));
				}
			}
	...
	private void circle_sendMoveMsg(float x, float y, float moveDst) {
	...
	private void circle_operate(float x, float y) {
	...
	void setFolderIcon(FolderIcon3D icon) {
		...
		buttonOK = new ImageView3D("button_ok",
				R3D.getTextureRegion("public-button-return"));

		buttonScale = 1.0f;
		buttonOK.setSize(R3D.icongroup_button_width * buttonScale,
				R3D.icongroup_button_height * buttonScale);

		buttonOK.setOrigin(buttonOK.width / 2, buttonOK.height / 2);
		if ((mFolderIcon.mInfo.x + mFolderIcon.folder_front.width > getWidth()
				- buttonOK.width)) {

			if (mFolderIcon.folder_style == FolderIcon3D.folder_rotate_style) {
				buttonOK.setPosition(mFolderIcon.mInfo.x - buttonOK.width / 2,
						mFolderIcon.mInfo.y - buttonOK.height / 2);
			} else {
				buttonOK.setPosition(mFolderIcon.mInfo.x - buttonOK.width / 2,
						mFolderIcon.mInfo.y);
			}
			...
			if (mFolderIcon.mInfo.x < buttonOK.height / 2) {
				pos_x = mFolderIcon.mInfo.x + mFolderIcon.folder_front.width
						- buttonOK.width / 2;
			}

			if (mFolderIcon.folder_style == FolderIcon3D.folder_rotate_style) {
				buttonOK.setPosition(pos_x, mFolderIcon.mInfo.y
						+ (mFolderIcon.folder_front.height - buttonOK.height)
						/ 2);
			} else {
				buttonOK.setPosition(pos_x, mFolderIcon.mInfo.y);
			}
			...
			if (mFolderIcon.folder_style == FolderIcon3D.folder_rotate_style) {
				buttonOK.setPosition(mFolderIcon.mInfo.x
						+ mFolderIcon.folder_front.width - buttonOK.width / 2,
						mFolderIcon.mInfo.y - buttonOK.height / 2);
			} else {
				buttonOK.setPosition(mFolderIcon.mInfo.x
						+ mFolderIcon.folder_front.width - buttonOK.width / 2,
		...
		temp.y = downPath.y = mFolderIcon.mInfo.y
				+ mFolderIcon.folder_front.height / 2;
		TotalPath.add(temp);
		...
		for (int i = 0; i < Count; i++) {
			...
			if (view instanceof Icon3D) {
				...
				view.setRotation(0);
				mFolderIcon
						.changeTextureRegion(view, R3D.workspace_cell_height);
	...
	public void changeIcon(View3D oldIcon, View3D newIcon) {
