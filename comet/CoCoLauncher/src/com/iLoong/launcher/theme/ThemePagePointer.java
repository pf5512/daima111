...
public class ThemePagePointer extends LinearLayout {
	public static final String PAGE_NV_NORMAL = "setupmenu/page_nv_normal.png";
	public static final String PAGE_NV_FOCUSED = "setupmenu/page_nv_focused.png";
	...
	public void Release(){
		if(mvnf != null) mvnf.recycle();
		if(mvnn != null) mvnn.recycle();
		if(minf != null) minf.recycle();
		if(minn != null) minn.recycle();
	}	
	...
	public final void Init(int count) {
		...
		if (mUseInfo) {
			...
			if(minn != null) imageview.setImageBitmap(minn);
			addView(imageview);
		}
		for (int j = 0; j < count; j++) {
			ImageView imageview = new ImageView(mContext);
			if(mvnn != null) imageview.setImageBitmap(mvnn);
			addView(imageview);
		}
		SelectPage(page);
	}
	...
	public final void SelectPage(int nextpage) {
		...
		if (!mUseInfo) {
			...
			if(mvnn != null) child.setImageBitmap(mvnn);
			page = nextpage;
			child = (ImageView) getChildAt(page);
			if(mvnf != null) child.setImageBitmap(mvnf);
		} else {

			ImageView child = (ImageView) getChildAt(page);
			if (page == 0) {
				if(minn != null) child.setImageBitmap(minn);
			} else {
				if(mvnn != null) child.setImageBitmap(mvnn);
			}
			page = nextpage;
			...
			child = (ImageView) getChildAt(page);
			if (page == 0) {
				if(minf != null) child.setImageBitmap(minf);
			} else {
				if(mvnf != null) child.setImageBitmap(mvnf);
			}
		}
