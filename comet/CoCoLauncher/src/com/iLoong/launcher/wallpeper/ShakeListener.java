...
//	}
	...
	public boolean checkSensorSupported() {
		try {
			SensorManager tmpSensorMgr = (SensorManager) mContext
					.getSystemService(Context.SENSOR_SERVICE);
			ShakeListener tempListener = new ShakeListener(mContext);
			boolean supported = tmpSensorMgr.registerListener(tempListener,
					tmpSensorMgr.getDefaultSensor(Sensor.TYPE_ACCELEROMETER),
	...
	public void initSounds() {
		...
		soundPool.setOnLoadCompleteListener(new OnLoadCompleteListener() {
			...
			public void onLoadComplete(SoundPool soundPool, int sampleId,
					int status) {
				if (status != 0) {
					Log.e("SOUND LOAD", " Sound ID: " + sampleId
							+ " Failed to load.");
				} else {
