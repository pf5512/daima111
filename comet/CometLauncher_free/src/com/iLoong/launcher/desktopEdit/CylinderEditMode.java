...
public class CylinderEditMode extends ViewGroup3D
{
	...
	private boolean bAppendAnim1;
	private boolean bAppendAnim2;
			...
			float parentAlpha )
	{
		...
		if( !bEnterAnim && !bExitAnim && !bAppendAnim1 && !bAppendAnim2 )
		{
			updateAllPagesDrawOrder();
		}
		if( bAppendAnim1 || bDeleteAnim )
		{
		...
		if( bAppendAnim2 )
		{
			...
			Gdx.gl.glScissor( 0 , (int)( page3DPosY - grooveView.getHeight() / 3 ) , (int)Utils3D.getScreenWidth() , (int)( Utils3D.getScreenHeight() - page3DPosY ) );
		}
			...
			int pointer )
	{
		...
		if( bEnterAnim || bExitAnim || bAppendAnim1 || bAppendAnim2 || bDeleteAnim || bAutoMoveAnim || DesktopEdit.bDragActionForIcon || DesktopEdit.bDragActionForPage || bDropTween )
		{
			return true;
		}
		requestFocus();
			...
			int keycode )
	{
		...
		{
			if( !bEnterAnim && !bExitAnim && !bExitAnimExtra && !bFlingAnim && !bAutoEffectAnim && !bAutoMoveAnim && !bAppendAnim1 && !bAppendAnim2 && !bDeleteAnim )
			{
				exitCylinderEditMode();
				return true;
			}
		...
		if( source == timeline && type == TweenCallback.COMPLETE )
		{
			...
			if( bAutoMoveAnim )
			{
				...
				else if( DesktopEdit.bDragActionForPage )
				{
					...
					{
						desktopEdit.setCurrentPageIndex( desktopEdit.prePageIndex() );
					}
					else if( DesktopEdit.autoMoveStatus == DesktopEdit.AUTO_MOVE_RIGHT )
					{
						desktopEdit.setCurrentPageIndex( desktopEdit.nextPageIndex() );
					}
				}
			...
			if( bAppendAnim1 )
			{
				...
				timeline.push( appendPage.obtainTween( View3DTweenAccessor.POS_XY , Elastic.INOUT , 1f , appendPage.getX() , pagePosY , 0 ) );
				timeline.push( grooveView.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , 1f , 0 , 0 , 0 ) );
				timeline.start( View3DTweenAccessor.manager ).setCallback( this );
			}
			if( bDeleteAnim )
			{
			...
			PageContainer dropPage )
	{
		...
		dropPage.setRotationX( 0 );
		dropPage.x = dropPage.getParent().x;
		dropPage.y = dropPage.getParent().y;
		dropTween = dropPage.startTween( View3DTweenAccessor.POS_XY , Linear.INOUT , 0.3f , pagePosX , pagePosY , 0 ).setCallback( this );
		bDropTween = true;
		bNotDispatchClick = true;
			...
			View3D newPage )
	{
		...
		{
			deletePage = pageList.set( pageList.size() - 1 , appendPage );
			timeline.push( deletePage.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , duration , 0 , 0 , 0 ) );
			pageNum = pageList.size();
			radius = calcCylinderRadius( pageNum );
		}
		else
		{
			pageList.add( ( (PageContainer)appendPage ).pageIndex , appendPage );
			PageContainer addPage = (PageContainer)pageList.get( pageList.size() - 1 );
			...
			for( int i = 0 ; i < pageNum ; i++ )
			{
				...
				if( i == pageNum - 1 )
				{
					targetAngle = page.getRotation() + polygonAngle;
				}
				else if( i == pageNum - 2 )
				{
					continue;
				...
				else
				{
					...
					{
						targetAngle = page.getRotation() + ( j * polygonAngle - ( page.getRotation() + 360 ) );
					}
					else
					{
						targetAngle = page.getRotation() + ( j * polygonAngle - page.getRotation() );
					}
					j++;
				}
				timeline.push( page.obtainTween( View3DTweenAccessor.ROTATION , Cubic.OUT , duration , targetAngle , 0 , 0 ) );
				page.setUser( page.getOriginZ() );
				timeline.push( page.obtainTween( View3DTweenAccessor.USER , Cubic.OUT , duration , radius , 0 , 0 ) );
			}
		}
		appendPage.setPosition( pagePosX , page3DPosY - appendPage.getHeight() - grooveView.getHeight() / 2 );
		appendPage.setOriginZ( radius );
		appendPage.setRotationY( 0 );
		float groovePosX = appendPage.getX() - ( grooveView.getWidth() - appendPage.getWidth() ) / 2;
		grooveView.setPosition( groovePosX , pagePosY - grooveView.getHeight() / 2 );
		grooveView.show();
		grooveView.color.a = 0;
		timeline.push( grooveView.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , duration , 1 , 0 , 0 ) );
		timeline.start( View3DTweenAccessor.manager ).setCallback( this );
		bAppendAnim1 = true;
		bNotDispatchClick = true;
	}
			...
			View3D deletePage )
	{
		...
		{
			PageContainer addPage = new PageContainer( "addPage" , desktopEdit.getPageWidth() , desktopEdit.getPageHeight() );
			addPage.buildAddPage( ( (PageContainer)deletePage ).pageIndex );
			pageList.add( addPage );
			addPage.setPosition( deletePage.getX() , deletePage.getY() );
			addPage.setRotationY( deletePage.getRotation() );
			addPage.setOriginZ( deletePage.getOriginZ() );
		...
		else
		{
			...
			if( pageGroup.findView( "addPage" ) == null )
			{
				...
				addPage.setPosition( pagePosX , pagePosY );
				addPage.setRotationY( ( addPage.pageIndex - ( (PageContainer)deletePage ).pageIndex ) * polygonAngle );
				addPage.setOriginZ( radius );
				pageGroup.addViewAt( 0 , addPage );
				for( int i = ( (PageContainer)deletePage ).pageIndex ; i < pageNum - 1 ; i++ )
				{
					View3D page = pageList.get( i );
					temTimeline2.push( page.obtainTween( View3DTweenAccessor.ROTATION , Cubic.OUT , 1f , page.getRotation() - polygonAngle , 0 , 0 ) );
				}
				addPage.color.a = 0;
				temTimeline2.push( addPage.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , 1f , 1 , 0 , 0 ) );
			}
	...
	{
		return( bEnterAnim || bExitAnim || bAutoMoveAnim || bAutoEffectAnim || bAppendAnim2 || bDeleteAnim || bFlingAnim );
	}
