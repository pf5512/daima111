...
public class AudioItem extends ReusableTextureHolder implements ThumbnailClient
{
	...
	public void setAlbumDisplayName(
			String name )
	{
		if( name == null )
			name = "";
	...
	public class AudioView extends View3D implements MediaView
	{
				...
				float parentAlpha )
		{
			...
			if( AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_AUDIO )
			{
				if( titleRegion == null )
				{
					titleRegion = new TextureRegion( new BitmapTexture( ( AppBar3D.titleToPixmapWidthLimit(
							title ,
							(int)( width - R3D.selectedRegion.getRegionWidth() - defaultAudioWidth - 25 ) ,
				...
				{
					artistRegion = new TextureRegion( new BitmapTexture( ( AppBar3D.titleToPixmapWidthLimit(
							artist ,
							(int)( width - R3D.selectedRegion.getRegionWidth() - defaultAudioWidth - 25 ) ,
							R3D.photo_title_size ,
							Color.WHITE ,
			...
			if( is3dRotation() )
			{
				...
				if( audio.texture != null )
					batch.draw( region , x + _thumbnailX , y + _thumbnailY , region.getRegionWidth() , region.getRegionHeight() );
				else
					batch.draw( defaultRegion , x + _xDefault , y + _yDefault , defaultAudioWidth , defaultAudioHeight );
				if( AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_AUDIO )
				{
					...
					if( AppHost3D.selectState )
					{
						if( selected )
							batch.draw(
									R3D.selectedRegion ,
									x + width - R3D.selectedRegion.getRegionWidth() - 10 ,
									y + ( height - R3D.selectedRegion.getRegionHeight() ) / 2 ,
									R3D.selectedRegion.getRegionWidth() ,
									R3D.selectedRegion.getRegionHeight() );
						else
							batch.draw(
									R3D.unselectRegion ,
									x + width - R3D.unselectRegion.getRegionWidth() - 10 ,
									y + ( height - R3D.unselectRegion.getRegionHeight() ) / 2 ,
			...
			else
			{
				if( AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_AUDIO )
				{
					if( AudioAlbum.bgRegion != null )
					{
						...
						batch.draw(
								AudioAlbum.bgRegion ,
								x + _xDefault - R3D.audio_left_padding ,
								y + _yDefault - R3D.audio_bottom_padding ,
								originX - _thumbnailX ,
								originY - _thumbnailY ,
								bgWidth ,
								bgHeight ,
				...
				if( audio.texture != null )
					...
					batch.draw( defaultRegion , x + _xDefault , y + _yDefault , originX - _xDefault , originY - _yDefault , defaultAudioWidth , defaultAudioHeight , scaleX , scaleY , rotation );
				if( AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_AUDIO )
				{
					batch.draw( titleRegion , x + _titleX , y + _titleY , titleRegion.getRegionWidth() , titleRegion.getRegionHeight() );
					batch.draw( artistRegion , x + _artistX , y + _artistY , artistRegion.getRegionWidth() , artistRegion.getRegionHeight() );
