...
public class GridLocalAdapter extends BaseAdapter
{
			...
			DownModule down )
	{
		...
		new Thread() {
			...
			public void run()
			{
				...
				freshUpdate();// new图标更新
				queryPackage();
				Log.v( "time" , "searchlock = " + ( System.currentTimeMillis() - preTime ) + "" );
			}
		}.start();
	}
	...
	private synchronized void queryPackage()
	{
		...
		Log.v( "time" , "getCurrentlock = " + ( System.currentTimeMillis() - preTime ) + "" );
		preTime = System.currentTimeMillis();
		List<LockInformation> installList = mgr.queryInstallList();
		Log.v( "time" , "getallInstalledLock = " + ( System.currentTimeMillis() - preTime ) + "" );
		preTime = System.currentTimeMillis();
			...
			ViewGroup parent )
	{
		...
		if( lockInfo.isNeedLoadDetail() )
		{
			...
			if( imgThumb == null )
			{
				...
				{
					StaticClass.saveMyBitmap( context , lockInfo.getPackageName() , lockInfo.getClassName() , lockInfo.getThumbImage() );
				}
			}
			if( lockInfo.getThumbImage() == null )
			{
		...
		ImageView update = (ImageView)retView.findViewById( R.id.imageNeedUpdate );
		// 安装的解锁才有判断有没有更新
		...
		if( lockInfo.isInstalled() || lockInfo.getDownloadStatus() == DownloadStatus.StatusFinish )
		{
