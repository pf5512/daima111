...
public class ThemeDownModule
{
	...
	{
		Log.v( LOG_TAG , "downloadListFinish:" + StaticClass.ACTION_HOTLIST_CHANGED );
		...
		uas.batchInsertAddress( ApplicationType.AppTheme , AddressType.AddressThumb , parserXml.getPictureAddress() );
		uas.deleteAddress( ApplicationType.AppTheme , AddressType.AddressPreview );
		uas.batchInsertAddress( ApplicationType.AppTheme , AddressType.AddressPreview , parserXml.getPictureAddress() );
		uas.deleteAddress( ApplicationType.AppTheme , AddressType.AddressApp );
		uas.batchInsertAddress( ApplicationType.AppTheme , AddressType.AddressApp , parserXml.getApplicationAddress() );
		saveListTime();
	...
	private class DownloadApkThread extends Thread
	{
		...
		private String getUrlAddress()
		{
			...
			List<String> urlList = urlService.queryAddress( ApplicationType.AppTheme , aType );
			if( urlList.size() == 0 )
			{
		...
		public void run()
		{
			while( true )
			{
				...
				try
				{
					...
					URL url = new URL( getUrlAddress() + "?" + curDownApk.getParams() + "&" + getPhoneParams() );
		...
		public void run()
		{
			while( true )
			{
				...
				try
				{
					...
					URL url = new URL( getUrlAddress( curDownImage.downType ) + "?" + curDownImage.getParams() + "&" + getPhoneParams() );
	...
	private class DownloadListThread extends Thread
	{
		...
		public void run()
		{
			...
			try
			{
				...
				String listUrl = StaticClass.DEFAULT_LIST_URL + "?p07=2" + "&" + getPhoneParams();
				...
				// 缓存
				byte buf[] = new byte[1024];
				Log.v( "********************" , "4444444444444444444444444444444444" );
				// 写入到文件
				while( true )
				{
