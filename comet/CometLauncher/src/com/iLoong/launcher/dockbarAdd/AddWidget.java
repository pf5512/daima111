...
public class AddWidget extends ViewGroup3D
{
	...
	private List<View3D> getAllWidget()
	{
		...
		float width = ( Utils3D.getScreenWidth() - R3D.applist_padding_left - R3D.applist_padding_right ) / AppList3D.mWidgetCountX - R3D.app_widget3d_gap;
		float height = ( Utils3D.getScreenHeight() - R3D.appbar_height - R3D.applist_padding_top - R3D.applist_padding_bottom ) / AppList3D.mWidgetCountY - R3D.app_widget3d_gap;
		for( int j = 0 ; j < allwidgets.size() ; j++ )
		{
			...
			{
				Widget3DVirtual widgetIcon = (Widget3DVirtual)allwidgets.get( j );
				widgetIcon.setSize( width , height );
				widgetIcon.makeShortcut();
				widgetIcon.oldX = widgetIcon.x;
				widgetIcon.oldY = widgetIcon.y;
				allApps.add( widgetIcon );
			}
