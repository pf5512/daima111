			...
			while( !destroy )
			{
				...
				if( type == AppHost3D.CONTENT_TYPE_PHOTO || type == AppHost3D.CONTENT_TYPE_VIDEO )
				{
					synchronized( timeoutLock )
					{
						...
						{
							if( curClient != null )
								timeoutLock.wait( DECODE_TIMEOUT );
							while( !decoding && curClient != null )
								timeoutLock.wait( DECODE_TIMEOUT );
							if( curClient != null )
							{
			...
			while( !destroy )
			{
				synchronized( decodeLock )
				{
					...
					if( curClient.getResType() == AppHost3D.CONTENT_TYPE_PHOTO )
					{
						...
						try
						{
							thumbnail = MediaStore.Images.Thumbnails.getThumbnail( iLoongLauncher.getInstance().getContentResolver() , id , kind , options );
						}
						catch( Exception e )
						{
							if( kind == MediaStore.Images.Thumbnails.MINI_KIND )
							{
								thumbnail = MediaStore.Images.Thumbnails.getThumbnail( iLoongLauncher.getInstance().getContentResolver() , id , MediaStore.Images.Thumbnails.MICRO_KIND , options );
							}
							e.printStackTrace();
						}
						if( thumbnail != null )
					...
					else if( curClient.getResType() == AppHost3D.CONTENT_TYPE_VIDEO )
					{
						...
						try
						{
							thumbnail = MediaStore.Video.Thumbnails.getThumbnail( iLoongLauncher.getInstance().getContentResolver() , id , kind , options );
						}
						catch( Exception e )
						{
							if( kind == MediaStore.Video.Thumbnails.MINI_KIND )
							{
								thumbnail = MediaStore.Video.Thumbnails.getThumbnail( iLoongLauncher.getInstance().getContentResolver() , id , MediaStore.Video.Thumbnails.MICRO_KIND , options );
							}
							e.printStackTrace();
						}
						if( thumbnail != null )
