...
public class Widget3DManager
{
			...
			public void run()
			{
				...
				for( ResolveInfo info : mWidgetResolveInfoList )
				{
					...
					if( metaData != null && metaData.containsKey( "preInitialize" ) && metaData.containsKey( "preInitialize" ) && metaData.getBoolean( "preInitialize" ) == true )
					{
						try
						{
							Context widgetPluginContext = iLoongApplication.ctx.createPackageContext( info.activityInfo.packageName , Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY );
	...
	public void processDefaultWidgetView()
	{
		...
		for( int i = 0 ; i < DefaultLayout.GetTotalDefaultWidgetNumber() ; i++ )
		{
			...
			if( loadByInternal )
			{
				...
				String widgetShortName = pkgName.substring( pkgName.lastIndexOf( "." ) + 1 ).toLowerCase( Locale.ENGLISH );
				...
				if( provider.instanceCount >= provider.maxInstanceCount )
				{
					iLoongLauncher.getInstance().mMainHandler.post( new Runnable() {
						...
						public void run()
						{
							...
							// TODO Auto-generated method stub
							Toast.makeText( iLoongApplication.ctx , alertMessage , Toast.LENGTH_SHORT ).show();
						}
					} );
					return false;
				}
						...
						if( showAlert )
						{
							iLoongLauncher.getInstance().mMainHandler.post( new Runnable() {
								...
								public void run()
								{
									...
									try
									{
										Context widgetPluginContext = iLoongApplication.ctx.createPackageContext(
			...
			int widgetId )
	{
		...
		try
		{
			Context widgetPluginContext = iLoongApplication.ctx.createPackageContext( resolveInfo.activityInfo.packageName , Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY );
			// 设置同launcher语言一�?
			widgetPluginContext.getResources().updateConfiguration( iLoongApplication.ctx.getResources().getConfiguration() , iLoongApplication.ctx.getResources().getDisplayMetrics() );
			...
			final String className )
	{
		...
		if( mWidget3DHost.containsWidget3DProvider( packageName ) )
		{
			...
			if( provider != null && provider.maxInstanceCount != -1 && provider.instanceCount >= provider.maxInstanceCount )
			{
			...
			ResolveInfo resolveInfo )
	{
		...
		WidgetClassLoader loader = new WidgetClassLoader( dexPath , dexOutputDir , libPath , iLoongApplication.ctx.getClassLoader() );
			...
			boolean isInit )
	{
		if( DefaultLayout.show_sensor )
		{
			...
			if( widget3DList != null && widget3DList.size() > 0 )
			{
				for( int i = 0 ; i < widget3DList.size() ; i++ )
				{
					widget3DList.get( i ).onSensorAngleChange( xAngle , yAngle , isInit );
				}
			}
		}
	}
