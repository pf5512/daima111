...
public class TextField3D extends View3D
{
	...
	public NinePatch cursorPatch = null;
			...
			float parentAlpha )
	{
		...
		if( focused && editable )
		{
			...
			{
				cursorPatch.draw( batch , x + visibleTextEnd , y + ( this.height - cursorHeight ) / 2 , cursorPatch.getTotalWidth() , cursorHeight );
			}
	...
	private void updateDisplayText()
	{
		if( passwordMode )
		{
			...
			displayText = text;
		displayText = textFormater.formatDisplayTitle( displayText , (int)width , (int)height );
		calculateOffsets();
	...
	public boolean keyTyped(
			char character )
	{
		// if (text.length() >= R3D.folder_name_length_max) {
		// SendMsgToAndroid.sendToastMsg(iLoongLauncher.getInstance()
		...
		if( stage != null && stage.getKeyboardFocus() == this && editable )
		{
			...
			if( text.length() > 0 )
			{
				if( character == ' ' )
				{
					if( text.length() > R3D.folder_name_length_max - 1 )
					{
						SendMsgToAndroid.sendToastMsg( iLoongLauncher.getInstance().getString( RR.string.fold_name_too_long ) );
						return true;
					}
					else
					{
			...
			String characters )
	{
		if( stage != null && stage.getKeyboardFocus() == this && editable )
		{
			...
			if( !hasSelection )
			{
				if( text.length() + characters.length() > R3D.folder_name_length_max )
				{
					SendMsgToAndroid.sendToastMsg( iLoongLauncher.getInstance().getString( RR.string.fold_name_too_long ) );
					return true;
				}
	...
	static public class DefaultTextureDefaultFormatter implements DisplayTextFormatter
	{
