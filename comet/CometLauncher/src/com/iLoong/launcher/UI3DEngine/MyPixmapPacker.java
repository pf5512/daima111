...
public class MyPixmapPacker implements Disposable
{
			...
			boolean isDefault )
	{
		...
		if( duplicateBorder )
		{
			...
			page.image.drawPixmap( image , (int)rect.x + (int)rect.width , (int)rect.y - 1 , (int)rect.x + (int)rect.width + 1 , (int)rect.y , image.getWidth() - 1 , 0 , image.getWidth() , 1 );
			page.image.drawPixmap( image , (int)rect.x - 1 , (int)rect.y + (int)rect.height , (int)rect.x , (int)rect.y + (int)rect.height + 1 , 0 , image.getHeight() - 1 , 1 , image.getHeight() );
			page.image.drawPixmap(
					image ,
					(int)rect.x + (int)rect.width ,
					(int)rect.y + (int)rect.height ,
	 ...
	 */
	public synchronized PixmapRect getRect(
			String name )
	{
		for( Page page : pages )
		{
