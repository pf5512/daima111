...
public class FolderIcon3D extends ViewGroupCircled3D implements FolderListener , DropTarget3D , IconBase3D , ClingManager.ClingTarget
{
	...
	ImageView3D folder_cover;
	FolderCoverContainer folder_cover_container;
	...
	public static Bitmap folder_cover_bmp = null;
	public static TextureRegion folderCoverRegion = null;
	public float folderCoverHeight = R3D.folder_front_height;
	public static int folderCoverMarginTop = 6;
	public static float folderCoverRotion = -120;
	private static float frontBmpLeftPadding = 0;
	public static float folderMarginTop;
	...
	public static boolean bIconPosChanged;
			...
			UserFolderInfo folderInfo )
	{
		...
		mMaxNumItems = 9;//R3D.folder_max_num;
			...
			Bitmap titleBg )
	{
		...
		{
			return Utils3D.titleToBitmap( folderFront , null , null , null , textureWidth , textureHeight );
	...
	public static void createFolderCoverPlatePicTexture()
	{
		if( folderCoverPlateTexture == null )
		{
			...
			{
				float density = iLoongApplication.ctx.getResources().getDisplayMetrics().density;
				if( R3D.folder_transform_num == 3 && R3D.getInteger( "folder_style" ) != 1 )
				{
				...
				else
				{
					if( R3D.icon_bg_num > 0 )
					{
						folder_cover_plate_bmp = Bitmap.createScaledBitmap( original_icon_cover_bmp , DefaultLayout.app_icon_size , DefaultLayout.app_icon_size , false );
					}
					else
					{
			...
			{
				folderCoverPlateTexture = new TextureRegion( new BitmapTexture( MergeBmpToPixmap( folder_cover_plate_bmp , null ) , true ) );
			}
		}
	}
	...
	public static void createFrontBmp()
	{
		...
		{
			Bitmap tmp = ThemeManager.getInstance().getBitmap( "theme/folder/widget-folder-bg.png" );
			if( tmp != null )
			{
				float density = iLoongApplication.ctx.getResources().getDisplayMetrics().density;
				if( R3D.folder_transform_num == 3 && R3D.getInteger( "folder_style" ) != 1 )
				...
				else
				{
					...
					else
					{
						...
						drawable.setTargetDensity( iLoongLauncher.getInstance().getResources().getDisplayMetrics() );
						folder_front_bmp = Utilities.createIconBitmap( drawable , iLoongLauncher.getInstance() );
					}
					if( folder_front_bmp != tmp )
						tmp.recycle();
				}
			}
		}
	...
	public static void createFolderCoverBmp()
	{
		if( folder_cover_bmp == null )
		{
			Bitmap tmp = ThemeManager.getInstance().getBitmap( "theme/folder/widget-folder-cover.png" );
			if( tmp != null )
			{
				float density = iLoongApplication.ctx.getResources().getDisplayMetrics().density;
				if( R3D.folder_transform_num == 3 && R3D.getInteger( "folder_style" ) != 1 )
				{
					folder_cover_bmp = Tools.resizeBitmap( tmp , density / 1.5f );
				}
				else
				{
					if( R3D.icon_bg_num > 0 )
					{
						if( tmp.getWidth() == DefaultLayout.app_icon_size && tmp.getHeight() == DefaultLayout.app_icon_size )
						{
							folder_cover_bmp = tmp;
						}
						else
						{
							float scaleFactor = (float)DefaultLayout.app_icon_size / (float)tmp.getWidth();
							folder_cover_bmp = Bitmap.createScaledBitmap( tmp , DefaultLayout.app_icon_size , (int)( tmp.getHeight() * scaleFactor ) , false );
							tmp.recycle();
						}
					}
					else
					{
						BitmapDrawable drawable = new BitmapDrawable( (Bitmap)tmp );
						drawable.setTargetDensity( iLoongLauncher.getInstance().getResources().getDisplayMetrics() );
						folder_cover_bmp = Utilities.createIconBitmap( drawable , iLoongLauncher.getInstance() );
					}
					if( folder_cover_bmp != tmp )
						tmp.recycle();
				}
			}
		}
	...
	public void folderCoverToPixmap3D()
	{
		createFolderCoverBmp();
		...
		{
			folderMarginTop = ( R3D.workspace_cell_height - folder_front_bmp.getHeight() - 2 * singleLineHeight ) / 2;
		}
		else
		{
			folderMarginTop = ( R3D.workspace_cell_height - folder_front_bmp.getHeight() - singleLineHeight ) / 2;
		}
		if( folderMarginTop < 0 )
		{
			folderMarginTop = 0;
		}
		folderCoverMarginTop = folder_front_bmp.getHeight() - folder_cover_bmp.getHeight() + (int)( folderMarginTop + 0.5f );
		folderCoverHeight -= folderCoverMarginTop;
		if( folderCoverRegion == null )
		{
			int textureWidth = R3D.workspace_cell_width;
			int textureHeight = (int)folderCoverHeight;
			float paddingLeft = ( textureWidth - folder_cover_bmp.getWidth() ) / 2;
			Bitmap frontPixmap = Bitmap.createBitmap( textureWidth , textureHeight , Config.ARGB_8888 );
			Canvas canvas = new Canvas( frontPixmap );
			canvas.drawBitmap( folder_cover_bmp , paddingLeft , 0 , null );
			folder_cover_bmp.recycle();
			folderCoverRegion = new TextureRegion( new BitmapTexture( frontPixmap ) );
			folderCoverRegion.getTexture().setFilter( TextureFilter.Linear , TextureFilter.Linear );
		}
	}
			...
			int count )
	{
		...
		else
		{
			...
			{
				this.foldernumTexture = R3D.findRegion( "folder-maxnumber-" + count );
			}
			else
			{
				this.foldernumTexture = R3D.findRegion( "folder-number-" + count );
			}
		}
	}
			...
			ArrayList<View3D> mCircleSomething )
	{
		...
		for( int i = 0 ; i < Count ; i++ )
		{
			...
			animation_line.push( Tween.to( myActor , View3DTweenAccessor.POS_XY , 0.3f ).target( pos_x , pos_y ).ease( Quad.INOUT ).delay( delayFactor * i ) );
		}
			...
			ArrayList<View3D> mCircleSomething )
	{
		int Count = mCircleSomething.size();
		View3D myActor;
		//		if (Count > mMaxNumItems) {
		//			for (int i = Count-1; i > mMaxNumItems-1; i--) {
		//				mCircleSomething.remove(i);
		//			}
		//			SendMsgToAndroid.sendOurToastMsg(R3D.folder3D_full);
		//		}
		Count = mCircleSomething.size();
		for( int i = 0 ; i < Count ; i++ )
		{
			myActor = mCircleSomething.get( i );
			if( myActor instanceof Icon3D )
			...
			boolean bCompress )
	{
		...
		else
		{
			...
			scale2 = ( getIconBmpHeight() - folderCoverMarginTop ) / ( R3D.folder_front_height - folderCoverMarginTop );
		}
		...
		if( bCompress )
		{
			...
			V2 = folder_cover.region.getV() + ( folder_cover.region.getV2() - folder_cover.region.getV() ) * scale2;
			folder_cover.region.setV2( V2 );
			if( folder_style == folder_rotate_style )
			{
				folder_front.setSize( R3D.workspace_cell_width , R3D.folder_front_height * scale );
				folder_cover_container.setSize( R3D.workspace_cell_width , folderCoverHeight * scale2 );
				folder_cover.setSize( R3D.workspace_cell_width , folderCoverHeight * scale2 );
			}
			else
			{
				folder_front.setSize( R3D.workspace_cell_width , getIconBmpHeight() );
				folder_cover_container.setSize( R3D.workspace_cell_width , getIconBmpHeight() - folderCoverMarginTop );
				folder_cover.setSize( R3D.workspace_cell_width , getIconBmpHeight() - folderCoverMarginTop );
			}
			...
			folderCoverHeight = getIconBmpHeight() - folderCoverMarginTop;//folderCoverHeight * scale;			
		...
		else
		{
			...
			V2 = folder_cover.region.getV() + ( folder_cover.region.getV2() - folder_cover.region.getV() ) / scale2;
			folder_cover.region.setV2( V2 );
			folder_cover.setSize( R3D.workspace_cell_width , R3D.folder_front_height - folderCoverMarginTop );
			folder_cover_container.setSize( R3D.workspace_cell_width , R3D.folder_front_height - folderCoverMarginTop );
			folderCoverHeight = R3D.folder_front_height - folderCoverMarginTop;
			bDisplayFolderName = true;
			// icon_pos_y=R3D.folder_icon_posy;
		}
		folder_front.setOrigin( folder_front.width / 2 , folder_front.height / 2 );
		folder_cover_container.setOrigin( folder_cover_container.width / 2 , folder_cover_container.height );
		folder_cover.setOrigin( folder_cover.width / 2 , folder_cover.height / 2 );
		if( folder_style == folder_rotate_style )
		{
			folder_back.setPosition( folder_front.x + frontBmpLeftPadding , folder_front.y + folder_front_height - R3D.folder_gap_height );
		}
	...
	private void buildElements()
	{
		...
		if( folderCoverRegion != null )
		{
			folder_cover = new ImageView3D( "folder_cover" , folderCoverRegion );
			folder_cover.setSize( R3D.workspace_cell_width , folderCoverHeight );
			folder_cover.setOrigin( folder_cover.getWidth() / 2 , folder_cover.getHeight() / 2 );
			folder_cover_container = new FolderCoverContainer( "folder_cover_container" , this );
			folder_cover_container.transform = true;
			folder_cover_container.setSize( R3D.workspace_cell_width , folderCoverHeight );
			folder_cover_container.setRotationVector( 1 , 0 , 0 );
			folder_cover_container.setOrigin( folder_cover_container.getWidth() / 2 , folder_cover_container.getHeight() );
			folder_cover_container.addView( folder_cover );
		}
		if( folder_style == folder_iphone_style )
		{
			folder_front.hide();
		...
		addView( folder_cover_container );
		//zhujieping add
		if( ThemeManager.getInstance().getBoolean( "miui_v5_folder" ) || DefaultLayout.miui_v5_folder )
		{
	...
	private void openV5Folder()
	{
		...
		mFolderMIUI3D.setOrigin( mInfo.x + R3D.workspace_cell_width / 2 , mInfo.y + R3D.workspace_cell_height / 2 );
		if( miuiV5Folder && DefaultLayout.blur_enable )
		{
			//			mFolderMIUI3D.openV5FolderIconsAnim();
			folderTween = mFolderMIUI3D.getBgView().startTween( View3DTweenAccessor.OPACITY , Quint.IN , DefaultLayout.blurDuration , DefaultLayout.maskOpacity , 0 , 0 )
		...
		else
		{
			...
			{
				folderTween = mFolderMIUI3D.getBgView().startTween( View3DTweenAccessor.OPACITY , Quint.IN , DefaultLayout.blurDuration , 0.77f , 0 , 0 ).setUserData( tween_anim_mfolder )
						.setCallback( this );
			}
			...
			float deltaY )
	{
		...
		if( mInfo.opened == false )
		{
			...
			{
				if( !DefaultLayout.folder_no_dragon && canDragIcon( deltaX , deltaY ) && this.getItemInfo().container != LauncherSettings.Favorites.CONTAINER_HOTSEAT )
				{
			...
			float y )
	{
		...
		if( mCurNumItems == 0 )
		{
			...
			if( FolderMIUI3D.useOrignalDialog )
			{
				Root3D.getInstance().showScreenBg();
				SendMsgToAndroid.SendCometFolderRename( this );
			}
			else
			{
				viewParent.onCtrlEvent( this , MSG_FOLDERICON3D_CLICK );
			...
			BaseTween source )
	{
		...
		if( type == TweenCallback.COMPLETE && source == animation_line )
		{
			...
			{
				// bringToFront
				// removed by Hugo.ye 20131114 begin
				//				int index = 0;
				//				float posx = 0;
				//				float posy = 0;
				//				if (folder_style == folder_rotate_style) {
				...
				//				}
				// removed by Hugo.ye 20131114 end
				// added by Hugo.ye 20131114 begin
				animation_line = Timeline.createParallel();
				animation_line.push( folder_cover_container.obtainTween( View3DTweenAccessor.ROTATION , Linear.INOUT , 0.3f , 0 , 0 , 0 ) );
				// added by Hugo.ye 20131114 end
				bAnimate = true;
		...
		if( type == TweenCallback.COMPLETE && source == iconTimeline )
		{
			if( bIconPosChanged )
			{
			...
			CharSequence title )
	{
		...
		this.setTag( mFolderName );
		if( mInfo != null && mInfo.folderFrom != null && mInfo.folderFrom.equals( FolderInfo.FOLDER_FROM_NEW ) )
		{
			if( viewParent != null )
			{
				viewParent.onCtrlEvent( this , MSG_UPDATE_VIEW );
		...
		for( int i = 0 ; i < Count ; i++ )
		{
			...
			if( myActor instanceof Icon3D )
			{
				...
				if( bAnim )
				{
					...
					//					myActor.setOrigin(myActor.width/2, myActor.height/2);
					viewTween.push( Tween.to( myActor , View3DTweenAccessor.SCALE_XY , duration ).target( getScaleFactor( iconIndex ) , getScaleFactor( iconIndex ) , 0 ).ease( Linear.INOUT )
							.delay( delayFactor * iconIndex ) );
					getPos( iconIndex );
					viewTween.push( Tween.to( myActor , View3DTweenAccessor.POS_XY , duration ).target( mPosx , mPosy , 0 ).ease( Linear.INOUT ).delay( delayFactor * iconIndex ) );
					if( iconIndex >= R3D.folder_transform_num )
					{
						myActor.hide();
					}
	...
	public void folder_layout_after_close(
			boolean anim )
	{
		int iconIndex = 0;
		int Count = getChildCount();
		...
		if( anim )
		{
			iconTimeline = Timeline.createParallel();
		}
		for( int i = 0 ; i < Count ; i++ )
		{
			myActor = getChildAt( i );
			if( myActor instanceof Icon3D )
			{
				...
				{
					myActor.color.a = 0;
					iconTimeline.push( myActor.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , 5f , 1 , 0 , 0 ) );
				}
				iconIndex++;
			}
		}
		if( anim )
		{
			iconTimeline.start( View3DTweenAccessor.manager ).setCallback( this );
		}
		else
		{
			if( bIconPosChanged )
			...
			float y )
	{
		...
		{
			Root3D.getInstance().hotseatBar.getDockGroup().deleteFromHotseatItemsList( view );
		}
		Root3D.getInstance().hotseatBar.getDockGroup().childPositonUpdate();
			...
			float y )
	{
		...
		for( View3D view : list )
		{
			if( view instanceof Icon3D )
			{
				if( !mInfo.opened )
				{
					if( folder_cover_container != null )
					{
						if( Math.abs( folder_cover_container.getRotation() ) < 0.00001f )
						{
							folder_cover_container.startTween( View3DTweenAccessor.ROTATION , Back.OUT , 0.3f , folderCoverRotion , 0 , 0 );
						}
					}
				}
				return true;
	...
	public void onDragOverLeave()
	{
		if( folder_cover_container != null )
		{
			folder_cover_container.startTween( View3DTweenAccessor.ROTATION , Cubic.INOUT , 0.3f , 0 , 0 , 0 );
		}
	}
			...
			float parentAlpha )
	{
		...
		else
		{
			if( transform )
			{
				...
				if( !mInfo.opened )
				{
					if( folder_front.region != null && folder_style == folder_iphone_style )
					{
							...
							if( ThemeManager.getInstance().getBoolean( "miui_v5_folder" ) || DefaultLayout.miui_v5_folder && DefaultLayout.blur_enable )
							{
								if( !ishide )
								{
									batch.draw(
											...
											offsetY + folder_front.y + folder_front_height - folder_front.getHeight() ,
											folder_front.originX ,
											folder_front.originY ,
											folder_front.width ,
											folder_front.height ,
											scaleX ,
											scaleY ,
			...
			float parentAlpha )
	{
		...
		if( !mInfo.opened )
		{
			...
			{
				batch.draw( titleTexture , (int)( offsetX + folder_front.x ) , (int)( offsetY + folder_front.y ) , titleTexture.getRegionWidth() , titleTexture.getRegionHeight() );
			}
		}
		transform = true;
		...
		if( folderCoverPlateTexture != null )
		{
			batch.draw( folderCoverPlateTexture , ( offsetX + folder_front.x ) , ( offsetY + folder_front.y ) , folderCoverPlateTexture.getRegionWidth() , folderCoverPlateTexture.getRegionHeight() );
		}
		//teapotXu_20130411 add end
		//		if (titleTexture != null && bDisplayFolderName == true) {
		...
		{
			num_pos_x = offsetX + folder_front.x + R3D.workspace_cell_width - frontBmpLeftPadding - R3D.folder_num_circle_width + ( R3D.folder_num_circle_width - R3D.folder_num_width ) / 2;
			num_pos_y = offsetY + folder_front.y + folder_front_height - R3D.folder_gap_height - R3D.folder_num_circle_width - R3D.folder_num_offset_y + ( R3D.folder_num_circle_width - R3D.folder_num_height ) / 2;
			batch.draw( foldernumTexture , num_pos_x , num_pos_y , R3D.folder_num_width , R3D.folder_num_height );
		}
		if( mInfo.contents.size() > 0 && clingState == ClingManager.CLING_STATE_WAIT && this.getItemInfo().cellX < 2 && ( this.getItemInfo().cellY == 0 || this.getItemInfo().cellY == 2 ) )
		{
			clingState = ClingManager.getInstance().fireFolderCling( this );
		}
		if( clingState == ClingManager.CLING_STATE_SHOW )
		{
			if( this.getItemInfo().cellX >= 2 || this.getItemInfo().cellY == 1 || this.getItemInfo().cellY == 3 )
			{
	...
	public boolean visible()
	{
		...
		if( !viewParent.isVisible() )
			return false;
		return this.isVisible() && viewParent.onCtrlEvent( this , MSG_FOLDER_CLING_VISIBLE );
	}
	...
	//	private  static boolean  verifyDevice(String curDevice) {
	//	     String IGNORE_DEVICE_CONFIG_NAME ="blurdevices.ini";
	//        JSONObject config = ThemeDownModule.getConfig(iLoongLauncher.getInstance(), IGNORE_DEVICE_CONFIG_NAME);
	//        if (config == null) {
	//            return true;
	//        }
	//        try {
	//            
	//            JSONObject tmp = config.getJSONObject("devices");
	//            for(int i=0;i<tmp.length();i++){
	//                Log.v("Root3D", "device"+i+":"+tmp.getString("device"+i));
	//                if( curDevice.contains(tmp.getString("device"+i))){
	//                    Log.v("Root3D", "CONAINS");
	//                    return false;
	//                }
	//            }
	//          
	...
	private void MiuiV5FolderBlurFinish()
	{
		...
		else
		{
			if( blurredView == null )
			{
				...
				//teapotXu add start for Folder in Mainmenu 
				if( DefaultLayout.mainmenu_folder_function == true && this.mInfo.opened == true && this.from_Where == FolderIcon3D.FROM_GRIDVIEW3D && this.exit_to_where == FolderIcon3D.TO_APPLIST )
				{
				...
				if( getFromWhere() == FROM_CELLLAYOUT )
				{
					...
					if( liveWallpaperActive )
					{
						lwpBackView = new ImageView3D( "lwpBackView" , ThemeManager.getInstance().getBitmap( "theme/pack_source/translucent-black.png" ) );
	...
	//zhujieping add end
	class FolderCoverContainer extends ViewGroup3D
	{
		
		private FolderIcon3D mFolderIcon;
		
		public FolderCoverContainer()
		{
				...
				float parentAlpha )
		{
			...
			if( !mInfo.opened )
			{
				if( folder_cover.region != null && folder_style == folder_iphone_style )
				{
					if( ThemeManager.getInstance().getBoolean( "miui_v5_folder" ) || DefaultLayout.miui_v5_folder && DefaultLayout.blur_enable )
					{
						if( !ishide )
						{
							batch.draw(
									folder_cover.region ,
									folder_cover.x ,
									folder_cover.y + folderCoverHeight - folder_cover.getHeight() ,
									folder_cover.originX ,
									folder_cover.originY ,
									folder_cover.width ,
									folder_cover.height ,
									folder_cover.scaleX ,
									folder_cover.scaleY ,
