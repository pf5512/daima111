...
public class HotDockGroup extends NPageBase implements ClingManager.ClingTarget
{
	...
	private static int dockPageNum = 3;
	private float rotateAngleRec;
	private boolean bCompleteBuildModel;
	private boolean bFingerLeave;
			...
			int height )
	{
		...
		dockPageNum = SetupMenuActions.getInstance().getStringToIntger( SetupMenu.getKey( RR.string.dock_row_key ) ) + 1;
		for( int i = 0 ; i < dockPageNum ; i++ )
		{
			gridPage = new HotGridView3D( "hotGridView3D" + i , width , height , R3D.hot_dock_icon_number , 1 );
			gridPage.setPageIndex( i );
			gridPage.transform = true;
			gridPage.setPadding( 0 , 0 , 0 , R3D.hot_grid_bottom_margin );
			gridPage.setOrigin( this.width / 2 , this.height / 2 );
			gridPage.setPosition( 0 , 0 );
			addPage( gridPage );
		}
			...
			float deltaY )
	{
		...
		if( !isTurning )
		{
			...
			{
				if( Root3D.getInstance() != null )
				{
					if( Root3D.getInstance().getDesktopEdit() != null )
					{
						if( Root3D.getInstance().getDesktopEdit().isAnimation() )
						{
							return false;
						}
					}
		...
		isTurning = true;
		if( !bCompleteBuildModel )
		{
			buildModel();
			bCompleteBuildModel = true;
		}
	...
	protected void updateEffect()
	{
		...
		if( view_list.size() == 1 )
		{
			View3D cur_view = view_list.get( 0 );
			cur_view.setRotationY( xScale * 30 );
			if( !moving )
			{
				moving = true;
			}
		...
		float rotateAngle = calcRotationAngle( view_list.size() );
		float gapAngle = rotateAngle - rotateAngleRec;
		rotateAngleRec = rotateAngle;
		for( int i = 0 ; i < view_list.size() ; i++ )
		{
			View3D page = view_list.get( i );
			...
			int index )
	{
		...
		addToHotseatItemsList( (View3D)folderIcon3D );
		mFocusGridView.updateItemInfoDB();
		...
		if( findView != null )
		{
			...
			else
			{
				if( findView instanceof FolderIcon3D )
				{
					...
					for( View3D child : child_list )
					{
						deleteFromHotseatItemsList( child );
					}
					childPositonUpdate();
					mFocusGridView.updateItemInfoDB();
				}
				else
				{
					...
					else
					{
						...
						deleteFromHotseatItemsList( findView );
			...
			int pageIndex )
	{
		...
		HotGridView3D gridPage = (HotGridView3D)getChildAt( pageIndex );
		gridPage.addItem( view );
	}
	...
	{
		float radius = calcCylinderRadius( view_list.size() );
		float polygonAngle = calcPolygonAngle( view_list.size() );
		int currentPageIndex = getCurrentPage();
		int j = 0;
		View3D gridPage = null;
		for( int i = currentPageIndex ; i < view_list.size() ; i++ )
		{
			gridPage = view_list.get( i );
			gridPage.show();
			if( i == currentPageIndex )
			{
				gridPage.bringToFront();
			}
			gridPage.setOriginZ( radius );
			gridPage.setRotationY( j * polygonAngle );
			j++;
		}
	...
	private float calcPolygonAngle(
			int pageNum )
	{
		return (float)( 360f / pageNum );
	}
	
	private float calcCylinderRadius(
			int pageNum )
	{
		if( pageNum == 2 )
		{
			return -Utils3D.getScreenWidth() / 5f;
		}
		else if( pageNum == 3 )
		{
			return -Utils3D.getScreenWidth() / 3.58f;
		}
		else if( pageNum == 4 )
		{
			return -Utils3D.getScreenWidth() / 2.068f;
		}
		else if( pageNum == 5 )
		{
			return -Utils3D.getScreenWidth() / 1.5f;
		}
		return 0;
	}
	...
	private void updateAllPagesAlpha()
	{
		View3D gridPage = null;
		for( int i = 0 ; i < view_list.size() ; i++ )
		{
			...
			if( bFingerLeave )
			{
				if( view_list.size() == 2 )
				{
					if( Math.abs( gridPage.getRotation() ) <= 180 )
					{
						gridPage.color.a = 1 - Math.abs( gridPage.getRotation() ) / 180;
					}
					else
					{
						gridPage.color.a = ( gridPage.getRotation() % 180 ) / 180;
						if( Math.abs( gridPage.getRotation() ) >= 360 )
						{
							gridPage.color.a = 1;
						}
					}
				}
				else if( view_list.size() == 3 )
				{
					if( Math.abs( gridPage.getRotation() ) < 120 || ( Math.abs( gridPage.getRotation() ) > 180 && Math.abs( gridPage.getRotation() ) < 240 ) )
					{
						gridPage.color.a = 1 - Math.abs( gridPage.getRotation() % 120 ) / 120;
					}
					else if( ( Math.abs( gridPage.getRotation() ) >= 120 && Math.abs( gridPage.getRotation() ) <= 180 ) || ( Math.abs( gridPage.getRotation() ) >= 240 && Math.abs( gridPage
							.getRotation() ) < 360 ) )
					{
						gridPage.color.a = gridPage.getRotation() % 120 / 120;
					}
					else if( Math.abs( gridPage.getRotation() ) % 360 < ESPINON )
					{
						gridPage.color.a = 1;
					}
				}
				else if( view_list.size() == 4 )
				{
					if( Math.abs( gridPage.getRotation() ) < 90 || ( Math.abs( gridPage.getRotation() ) >= 135 && Math.abs( gridPage.getRotation() ) < 180 ) || ( Math.abs( gridPage.getRotation() ) >= 225 && Math
							.abs( gridPage.getRotation() ) < 270 ) )
					{
						gridPage.color.a = 1 - Math.abs( gridPage.getRotation() ) % 90 / 90;
					}
					else if( ( Math.abs( gridPage.getRotation() ) >= 270 && Math.abs( gridPage.getRotation() ) < 360 ) || ( Math.abs( gridPage.getRotation() ) >= 90 && Math.abs( gridPage
							.getRotation() ) < 135 ) || ( Math.abs( gridPage.getRotation() ) >= 180 && Math.abs( gridPage.getRotation() ) < 225 ) )
					{
						gridPage.color.a = Math.abs( gridPage.getRotation() ) % 90 / 90;
					}
					else if( Math.abs( gridPage.getRotation() ) % 360 < ESPINON )
					{
						gridPage.color.a = 1;
				...
				else if( view_list.size() == 5 )
				{
					if( Math.abs( gridPage.getRotation() ) < 72 || ( ( Math.abs( gridPage.getRotation() ) >= 108 && Math.abs( gridPage.getRotation() ) < 144 ) ) || ( Math.abs( gridPage.getRotation() ) >= 180 && Math
							.abs( gridPage.getRotation() ) < 216 ) || ( Math.abs( gridPage.getRotation() ) >= 252 && Math.abs( gridPage.getRotation() ) < 288 ) )
					{
						gridPage.color.a = 1 - Math.abs( gridPage.getRotation() ) % 72 / 72;
					}
					else if( ( Math.abs( gridPage.getRotation() ) >= 72 && Math.abs( gridPage.getRotation() ) < 108 ) || ( Math.abs( gridPage.getRotation() ) >= 144 && Math.abs( gridPage
							.getRotation() ) < 180 ) || ( Math.abs( gridPage.getRotation() ) >= 216 && Math.abs( gridPage.getRotation() ) < 252 ) || ( Math.abs( gridPage.getRotation() ) >= 288 && Math
							.abs( gridPage.getRotation() ) < 360 ) )
					{
						gridPage.color.a = Math.abs( gridPage.getRotation() ) % 72 / 72;
					}
					else if( Math.abs( gridPage.getRotation() ) % 360 < ESPINON )
					{
						gridPage.color.a = 1;
			...
			else
			{
				if( Math.abs( gridPage.getRotation() ) <= 180 )
				{
					gridPage.color.a = 1 - Math.abs( gridPage.getRotation() ) / 260;
				}
				else
				{
					gridPage.color.a = ( gridPage.getRotation() - 180 ) / 260 + 0.3f;
				}
			}
		}
	}
	
	public void dockbarRowChange(
			int pageNum )
	{
		HotGridView3D gridPage = null;
		if( pageNum < dockPageNum )
		{
			View3D childView3d = null;
			ArrayList<View3D> listToRemove = new ArrayList<View3D>();
			if( page_index >= pageNum )
			{
				setCurrentPage( 0 );
			}
			for( int i = dockPageNum - 1 ; i >= pageNum ; i-- )
			{
				gridPage = (HotGridView3D)view_list.remove( i );
				listToRemove.clear();
				for( int j = 0 ; j < gridPage.getChildCount() ; j++ )
				{
					childView3d = gridPage.getChildAt( j );
					listToRemove.add( childView3d );
				}
				for( View3D view : listToRemove )
				{
					view.remove();
				}
				gridPage.remove();
		...
		else if( pageNum > dockPageNum )
		{
			ItemInfo info = null;
			for( int i = 0 ; i < pageNum - dockPageNum ; i++ )
			{
				gridPage = new HotGridView3D( "hotGridView3D" , width , height , R3D.hot_dock_icon_number , 1 );
				gridPage.setPageIndex( view_list.size() );
				gridPage.transform = true;
				gridPage.setPadding( 0 , 0 , 0 , R3D.hot_grid_bottom_margin );
				gridPage.setOrigin( this.width / 2 , this.height / 2 );
				gridPage.setPosition( 0 , 0 );
			...
			for( View3D item : mHotseatItems )
			{
				info = ( (IconBase3D)item ).getItemInfo();
				if( info.cellX < getPageNum() )
				{
					bindItem( item , info.cellX );
				}
			}
		}
		dockPageNum = pageNum;
