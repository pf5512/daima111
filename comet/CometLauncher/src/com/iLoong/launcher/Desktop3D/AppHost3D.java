...
public class AppHost3D extends ViewGroup3D implements AppBar3D.OnTabChangeListener , DropTarget3D
{
	...
	public boolean forbidTouch = false;
			...
			int keycode )
	{
		...
		else if( keycode == KeyEvent.KEYCODE_MENU && appList.xScale == 0 )
		{
			if( DefaultLayout.popmenu_style == SetupMenu.POPMENU_STYLE_ANDROID4 || DefaultLayout.setup_menu_support_scroll_page )
			{
				...
				else if( popMenu2 != null && !popMenu2.isVisible() && appList.canPopMenu() && !forbidTouch )
				{
					popMenu2.show();
					return true;
				}
			...
			float parentAlpha )
	{
		...
		if( ( ThemeManager.getInstance().getBoolean( "miui_v5_folder" ) || DefaultLayout.miui_v5_folder ) && DefaultLayout.blur_enable && ( folderOpened && appHost_folder != null ) )
		{
		...
		else if( sender instanceof BottomBar )
		{
			switch( event_id )
			{
				...
				case BottomBar.SHARE:
					...
					if( data.size() == 0 )
						return true;
					if( currentContentType == CONTENT_TYPE_AUDIO || currentContentType == CONTENT_TYPE_PHOTO )
						bottomBar.curState = BottomBar.STATE_DISPLAY;
					else
						bottomBar.hide();
					selectState = false;
