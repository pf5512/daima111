			...
			case SORT_DEFAULT:
				...
				else
				{
					...
					for( int i = 0 ; i < mApps.size() ; i++ )
					{
						...
						{
							titleKey[i] = mApps.get( i ).title.toString().replaceAll( " " , "" ).replaceAll( " " , "" );// 注意！！两个空格不一样
					...
					for( int i = 0 ; i < mItemInfos.size() ; i++ )
					{
						...
						if( itemInfo instanceof ApplicationInfo )
						{
							if( ( (ApplicationInfo)itemInfo ).intent.getBooleanExtra( "isApplistVirtualIcon" , false ) == true )
								...
								nameKey[i] = itemInfo.title.toString().replaceAll( " " , "" ).replaceAll( " " , "" );// 注意！！两个空格不一样
						}
						else
						{
				...
				// teapotXu add end for Folder in Mainmenu
				{
					...
					for( int i = 0 ; i < mApps.size() ; i++ )
					{
						if( ( mApps.get( i ).intent.getBooleanExtra( "isApplistVirtualIcon" , false ) == true ) )
							...
							nameKey[i] = mApps.get( i ).title.toString().replaceAll( " " , "" ).replaceAll( " " , "" );// 注意！！两个空格不一样
					}
					cut.sortByAlpha( 1 , nameKey , sortArray );
				}
				break;
	...
	protected void updateEffect()
	{
		...
		if( xScale > 0 )
		{
			...
			if( mScrollToWidget )
			{
				...
				{
					APageEase.updateEffect( pre_view , cur_view , xScale - 1 , tempYScale , mTypelist.get( 1 ) );
				}
				else
