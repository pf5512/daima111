...
public class DownloadThemeService {
	...
	public DownloadThemeItem queryByPackageName(String packageName) {
		DownloadThemeItem result = null;

		SQLiteDatabase db = dbHelper.getReadableDatabase();
		Cursor cursor = null;
		try {
			cursor = db.query(TABLE_NAME, null, FIELD_PACKAGE_NAME + "=?",
	...
	public boolean updateDownloadSizeAndStatus(String packageName, int downSize, int totalSize,
			DownloadStatus status) {
	...
	public boolean updateDownloadStatus(String packageName, DownloadStatus status) {
	...
	private DownloadThemeItem readLockInfo(Cursor cursor) {
		try {
			...
			item.setDownloadStatus(DownloadStatus.fromValue(cursor.getInt(cursor
					.getColumnIndexOrThrow(FIELD_DOWNLOAD_STATUS))));
