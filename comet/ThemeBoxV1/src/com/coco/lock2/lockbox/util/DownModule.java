...
public class DownModule {
	...
	private void downloadListFinish() {
		...
		uas.batchInsertAddress(ApplicationType.AppLock,
		...
		uas.batchInsertAddress(ApplicationType.AppLock, AddressType.AddressApp,
	...
	public void downloadApkStatusUpdate(String pkgName, DownloadStatus status) {
		synchronized (downApkDb) {
	...
	public List<LockInformation> queryInstallList() {
		...
		List<ResolveInfo> infoList = mContext.getPackageManager()
				.queryIntentActivities(intentLockView, 0);
	...
	private void downloadApkFinish(String pkgName) {
		synchronized (downApkDb) {
			downApkDb
					.updateDownloadStatus(pkgName, DownloadStatus.StatusFinish);
		}
	 ...
	 * 获取手机的其他信�?
	 */
	private static String getPhoneParams(Context context) {
	...
	public static String getApkUrl(Context context, String srcPackageName,
			String destPackageName) {
		String apkUrl = StaticClass.DEFAULT_APK_URL;
		Log.d(LOG_TAG, "apkUrl=" + apkUrl);
		String result = String.format("%s?p01=%s&p06=1&p07=%s&%s", apkUrl,
				destPackageName, srcPackageName, getPhoneParams(context));
			...
			while (true) {
				...
				try {
					...
					URL url = new URL(getUrlAddress() + "?"
							+ curDownApk.getParams() + "&" + "p07="
							+ "com.coco.lock2.lockbox" + "&"
					...
					DownloadLockItem item = threadDb
							.queryByPackageName(curDownApk.packname);
	...
	private class DownloadListThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				if(ver != ""){
					url = new URL(
							"http://yu01.coomoe.com/ui02/getapplist.ashx?p07=com.coco.lock2.lockbox"
									+"&p08="+ver+ "&" + getPhoneParams(mContext));					
				}
				else{
					url = new URL(
							"http://yu01.coomoe.com/ui02/getapplist.ashx?p07=com.coco.lock2.lockbox"
