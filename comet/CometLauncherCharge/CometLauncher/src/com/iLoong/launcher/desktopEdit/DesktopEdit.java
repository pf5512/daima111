...
public class DesktopEdit extends ViewGroup3D implements DragSource3D , DropTarget3D
{
	...
	private void setupPageList()
	{
		...
		//			PageContainer addPage = new PageContainer("addPage", pageWidth,
		//					pageHeight);
	...
	private void onAppendPage()
	{
		...
		View3D cell = cellLayoutList.get( getCurrentPageIndex() );
		ViewInfoHolder holder = new ViewInfoHolder();
		holder.getInfo( cell );
		holder.parent = cell.getParent();
		cell.setTag( holder );
		...
		pageList.add( getCurrentPageIndex() , newPage );
		for( int i = getCurrentPageIndex() + 1 ; i < pageList.size() ; i++ )
		{
		...
		newPage.buildEditPage( cell , getCurrentPageIndex() );
	...
	public void exeDeletePage()
	{
		...
		{
			if( homePageIndex == pageList.size() )
			{
				homePageIndex--;
				if( homePageIndex < 0 )
				{
	...
	{
		iLoongLauncher.getInstance().getD3dListener().getTrashIcon().set( false );
		iLoongLauncher.getInstance().getD3dListener().getTrashIcon().hide();
	...
	{
		return( this.curPageIndex == getPageNum() - 1 ? 0 : this.curPageIndex + 1 );
	}
	
	public int prePageIndex()
	{
		return( this.curPageIndex == 0 ? getPageNum() - 1 : this.curPageIndex - 1 );
	}
			...
			float y )
	{
		...
		if( list.get( 0 ) instanceof PageContainer )
		{
			...
			{
				if( toChangePageIndex == getPageNum() && dropPage.pageIndex == 0 )
				{
					View3D temView3d = pageList.remove( 0 );
					pageList.add( dropPage.pageIndex , dropPage );
					pageList.add( temView3d );
				}
				else if( toChangePageIndex == 0 && dropPage.pageIndex == getPageNum() )
				{
					View3D temView3d = pageList.remove( getPageNum() - 1 );
					pageList.add( 0 , temView3d );
					pageList.add( dropPage.pageIndex , dropPage );
				}
				else
				{
