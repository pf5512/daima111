...
public class PhotoItem extends ReusableTextureHolder
{
	...
	public class PhotoView extends View3D implements MediaView
	{
				...
				float parentAlpha )
		{
			...
			if( is3dRotation() )
			{
				if( AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_PHOTO )
					batch.draw( defaultRegion , x + _xDefault , y + _yDefault , defaultPhotoWidth , defaultPhotoHeight );
				if( photo.texture != null )
					batch.draw( region , x + _x , y + _y , region.getRegionWidth() , region.getRegionHeight() );
				if( AppHost3D.selectState && AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_PHOTO )
				{
					if( selected )
						batch.draw(
								R3D.selectedRegion ,
								x + _xDefault + defaultPhotoWidth - R3D.selectedRegion.getRegionWidth() ,
								y + _yDefault + 6 ,
						...
						batch.draw(
								R3D.unselectRegion ,
								x + _xDefault + defaultPhotoWidth - R3D.unselectRegion.getRegionWidth() ,
								y + _yDefault + 6 ,
			...
			{
				if( AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_PHOTO )
					batch.draw( defaultRegion , x + _xDefault , y + _yDefault , originX - _xDefault , originY - _yDefault , defaultPhotoWidth , defaultPhotoHeight , scaleX , scaleY , rotation );
				if( photo.texture != null )
					batch.draw( region , x + _x , y + _y , originX - _x , originY - _y , region.getRegionWidth() , region.getRegionHeight() , scaleX , scaleY , rotation );
				if( AppHost3D.selectState && AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_PHOTO )
				{
					if( selected )
						batch.draw(
