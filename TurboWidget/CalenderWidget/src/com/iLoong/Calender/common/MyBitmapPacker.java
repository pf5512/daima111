...
public class MyBitmapPacker implements Disposable
{
	...
	public class Page
	{
		...
		public Bitmap image;
		public MyBitmapTexture texture;
	...
	public final Array<Page> pages = new Array<Page>();
	//Page currPage;
	boolean disposed;
	
	public MyBitmapPacker(
...
//				Log.e( "pack" , " file read error:" + e.getMessage() );
	...
	private void newPage()
	{
		...
		if( enablePack )
		{
			...
			if( vNum < minVNum )
				vNum = minVNum;
//			Log.e( "pack" , " pack size:" + pageWidth + "*" + pageHeight );
		}
		...
		if( !enablePack )
		{
			...
			if( page.addedRects.size > 0 && ( threadId != mainThreadId ) )
			{
				...
				while( ite.hasNext() )
				{
					...
					{
						MyBitmapTexture texture = new MyBitmapTexture( rect.bitmap );
...
//						Log.v( "pack" , "read time=" + ( System.currentTimeMillis() - time ) );
					...
					if( !page.disposed )
						page.texture = new MyBitmapTexture( page.image );
					page.texture.setFilter( defaultFilter , defaultMagFilter );
...
//						Log.v( "pack" , "read time=" + ( System.currentTimeMillis() - time ) );
					...
					( (MyBitmapTextureData)page.texture.getTextureData() ).setBitmap( page.image );
					page.texture.reload();
					page.texture.setFilter( defaultFilter , defaultMagFilter );
					for( String name : page.addedRects )
					{
