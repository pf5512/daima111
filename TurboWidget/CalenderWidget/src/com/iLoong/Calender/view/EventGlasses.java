...
public class EventGlasses extends PluginViewObject3D
{
	...
	protected void initObjScale()
	{
		...
		{
			this.mScaleY = WidgetCalender.scale;
		}else{
			...
			float parentAlpha )
	{
		...
		float nowglscissorHeight = Parameter.Calender_Week_Glass_Height * WidgetCalender.scale + Parameter.Calender_Day_Glass_Height * WidgetCalender.height_scale * 6 - glscissordownmoveHeight - glscissorupmoveHeight;
		if( WidgetCalender.ifstartTween || WidgetCalender.ifstartclickbacktimeline )
		{
			Gdx.gl.glEnable( GL10.GL_SCISSOR_TEST );
			Gdx.gl.glScissor(
					0 ,
					(int)( WidgetCalender.getIntance().calenderalldown.getY() + firstHeight + Utils3D.getScreenHeight() - R3D.Workspace_cell_each_height * 4 ) ,
