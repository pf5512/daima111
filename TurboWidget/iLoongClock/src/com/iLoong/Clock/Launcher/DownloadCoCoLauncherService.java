...
public class DownloadCoCoLauncherService extends Service
{
	...
	private final String DOWNLOAD_PATH = Environment.getExternalStorageDirectory() + "/Coco/download/";
				...
				Message msg )
		{
			...
			switch( result )
			{
				case DOWNLOAD_COMPLETE:
				{
					...
					Notification finishNotification = new Notification( R.drawable.ic_launcher , "" , System.currentTimeMillis() );
					finishNotification.setLatestEventInfo(
							DownloadCoCoLauncherService.this ,
					...
					//Log.d("InstallCoCoLauncherReceiver" , "sendBroadcast - downloadRunnable.mNotifyId:" + downloadRunnable.mNotifyId);
					Intent intentDownloadComplete = new Intent( "com.coco.themes.DownloadCoCoLauncherComplete" );
				...
				{
					mUpdateNotificationManager.cancel( downloadRunnable.mNotifyId );
					Notification failNotification = new Notification( R.drawable.ic_launcher , "" , System.currentTimeMillis() );
	...
	private class DownloadRunnable implements Runnable
	{
				...
				String downloadFileName )
		{
			...
			mUpdatePendingIntent = PendingIntent.getActivity( DownloadCoCoLauncherService.this , 0 , mUpdateIntent , 0 );
			String tickerText = getResources().getString( R.string.server_begin_download );
			mUpdateNotification = new Notification( R.drawable.ic_launcher , tickerText , System.currentTimeMillis() );
			mUpdateNotification.setLatestEventInfo( DownloadCoCoLauncherService.this , downloadFileName , "0%" , mUpdatePendingIntent );
			//xiatian add start	//fix bug same as 0001880
			Intent intentToDownload = new Intent( "com.coco.themes.ToDownloadCoCoLauncher" );
				...
				File saveFile ) throws Exception
		{
			...
			try
			{
				...
				while( ( readsize = is.read( buffer ) ) > 0 )
				{
					...
					{
						percentCount += 5;
						mUpdateNotification.setLatestEventInfo( DownloadCoCoLauncherService.this , loading + " - " + mDownloadFileName , curPercent + "%" , mUpdatePendingIntent );
						mUpdateNotificationManager.notify( mNotifyId , mUpdateNotification );
					}
				}
			}
			...
			Context context )
	{
		...
		if( simOperator != null )
		{
			...
			{
				netType = DownloadCoCoLauncherService.NETTYPE_MOBILE;
			}
			else if( simOperator.startsWith( "46001" ) )
			{
				netType = DownloadCoCoLauncherService.NETTYPE_UNICOM;
			}
			else if( simOperator.startsWith( "46003" ) )
			{
				netType = DownloadCoCoLauncherService.NETTYPE_TELECOM;
			}
		}
		return netType;
	}
