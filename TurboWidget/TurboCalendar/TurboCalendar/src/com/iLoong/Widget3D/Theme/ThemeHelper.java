...
public class ThemeHelper {
	...
	public static String getLauncherThemeImagePath(String packageName,
		...
		return "theme/widget/" + packageName + "/" + themeName + "/image/"
				+ imageName;
	}
	...
	public static String getLauncherThemeObjPath(String packageName,
		...
		if (original) {
			return "theme/widget/" + packageName + "/" + themeName
					+ "/original_obj/" + objName;
		} else {
			return "theme/widget/" + packageName + "/" + themeName + "/obj/"
					+ objName;
		}
	}
	...
	public static InputStream getThemeObjStream(MainAppContext appContext,
			String objName, boolean original) {
		InputStream is = null;
		String objPath = getLauncherThemeObjPath(
				appContext.mWidgetContext.getPackageName(),
				appContext.mThemeName, objName, original);
		...
		if (is == null) {
			if (appContext.mWidgetContext.equals(appContext.mContainerContext)) {
				is = WidgetThemeManager.getInstance().getSysteThemeInputStream(
						objPath);
				if (is == null) {
					objPath = getLauncherThemeObjPath(
							appContext.mWidgetContext.getPackageName(),
							"iLoong", objName, original);
				...
				objPath = getThemeObjPath(appContext.mThemeName, objName,
						original);
				try {
					is = getInputStream(appContext.mWidgetContext, true,
							objPath);
