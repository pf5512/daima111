...
public class MyThemeDescription {
	...
	public static final String CONFIG_FILENAME = HOME_DIR + "config.xml";
	public static final String WIDGET_CLOCK_CONFIG_FILENAME = HOME_DIR + "widget/clock/config.xml";
	...
	public MyThemeDescription(Context context) {
		mContext = context;
	...
	public MyThemeDescription(Context context, String defaultFileName)
	{
		mContext = context;
		mGdxContext = new AndroidFiles(mContext.getAssets());
		ConfigHandler cfghandler = new ConfigHandler();
	...
	public MyThemeDescription(Context context, String defaultFileName1,String defaultFileName2)
	{
		mContext = context;
		mGdxContext = new AndroidFiles(mContext.getAssets());
		ConfigHandler cfghandler = new ConfigHandler();
		LoadXml(defaultFileName1, cfghandler);
		ConfigHandler wccfghandler = new ConfigHandler();
		LoadXml(defaultFileName2, wccfghandler);
	}
	...
	public Bitmap getDefaultBitmap() {
		...
		try {
			if (themeimgs.size() > 0)
				defaultbitmap = Tools.getImageFromInStream(mContext.getAssets()
						.open(PREVIEW_DIR + themeimgs.get(0)));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public Bitmap getBitmap(String filename) {
		Bitmap bmp = null;
		try {
			bmp = Tools.getImageFromInStream(mContext.getAssets()
					.open(filename));
		} catch (IOException e) {
			e.printStackTrace();
		}
...
/*
	public Bitmap getIcon(String icon) {
		...
		path += icon;
		try {
			iconbmp = Tools.getImageFromInStream(mContext.getAssets()
					.open(path));
		} catch (IOException e) {
		}
		return iconbmp;
...
*/
	...
	public void LoadXml(String Filename, DefaultHandler handler) {
		...
		try {
			...
			InputSource xmlin = new InputSource(mContext.getAssets().open(
					Filename));
...
/*
	public void getInfo(TextView text) {
