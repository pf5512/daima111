...
View.OnTouchListener {
    ...
    public final static String DATA_SERVICE_ACTION = "com.cooee.app.cooeejewelweather3D.dataprovider.weatherDataService";
    ...
    public final static String SETTING_URI = "content://com.cooee.app.cooeejewelweather3D.dataprovider/setting";

    public final static String BROATCAST_URI = "com.cooee.weather.data.action.UPDATE_RESULT";
    public final static String UPDATA_TO_CONSOLVEWEATHER_CHANGECITY = "Consolve_Weather_ChangeCity";
	...
	//		Log.v("aq", "send broadcast: userId = " + mUserId
...
/*		LinearLayout addlayout = (LinearLayout)findViewById(R.id.add_button);		
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case WeatherObserver.MSG_REFRESH:
                        Log.v("weijie", "WeatherObserver.MSG_REFRESH");
                        readData();
                        updateViews();
                        break;
                    case WeatherReceiver.MSG_REFRESH:
                        Log.v("weijie", "WeatherReceiver.MSG_REFRESH");
                        requesting = false;
                        readData();
                        updateViews();
                        break;
                    case WeatherReceiver.MSG_AVAILABLE:
                        Log.v("weijie", "WeatherReceiver.MSG_AVAILABLE");
                        requesting = false;
                        readData();
                        updateViews();
                        break;
                    case WeatherReceiver.MSG_FAILED:
                        // ����ʧ��
                        Log.v("weijie", "WeatherReceiver.MSG_FAILED");
                        requesting = false;
                        ...
                        if (mDataEntity == null) {
                            requesting = false;
                            requestingFailed = true;
                            Log.e("weijie", "WeatherReceiver.MSG_FAILED");
                            updateViews();
                        }
                        requestingFailed = false;
                        break;
                    case WeatherReceiver.MSG_INVILIDE:
                    	Log.v("weijie", "WeatherReceiver.MSG_INVILIDE");
                        requesting = false;
                        requestingFailed = true;
                        readData();
                        updateViews();
        ...
        if(city!= null && !city.equals(""))
        {
	        for (int i = 0; i < mPoscalCodList.size(); i++)
	            if (city.equals((String) mPoscalCodList.get(i))) {
	                mCurrentIndex = i;
	                break;
	            }
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {    	
    	 ...
    	 MenuItem setting=menu.add(Menu.NONE,1,1, null/*getResources().getString(R.string.setting)*/);
    	 ...
    	 if(isChinese())
    		 setting.setIcon(R.drawable.setting_icon_c);
    	 else
         setting.setIcon(R.drawable.setting_icon);
         //setting.setHeaderIcon(R.drawable.setting_icon);
         
         MenuItem edit=menu.add(Menu.NONE,2,2, null/* getResources().getString(R.string.exit)*/);
         ...
         if(isChinese())
        	 edit.setIcon(R.drawable.exit_icon_c);
    	 else
    ...
    public void updateViews() {
        ...
        ImageView iv;
        try {
        	Log.e("weijie", "updateViews mPoscalCodList = "+mPoscalCodList);
            if (mPoscalCodList == null)
                mPoscalCodList = new ArrayList<String>();
            Log.e("weijie", "updateViews mPoscalCodList size= "+mPoscalCodList.size());
            if (mPoscalCodList.size() == 0) {
            ...
            if (!mCurrentPostalCode.equals("none")) {
            	...
            	mDataEntity = null;
            Log.e("weijie", "updateViews mPoscalCodList mDataEntity= "+mDataEntity);
            ...
            if (mDataEntity == null) {
                ...
                if(!uselastdata)
                {
                findViewById(R.id.feature_weather_layout).setVisibility(View.INVISIBLE);
                findViewById(R.id.today_tmp_layout).setVisibility(View.INVISIBLE);
             //   findViewById(R.id.weather_image_shoot).setVisibility(View.INVISIBLE);
                findViewById(R.id.weather_image_shoot).setBackgroundColor(0xff000000);
                
                ImageView iv1 = (ImageView) findViewById(R.id.weather_image_shoot);
                ...
                {
                	findViewById(R.id.feature_weather_layout).setVisibility(View.VISIBLE);
                    findViewById(R.id.today_tmp_layout).setVisibility(View.VISIBLE);
                 //   findViewById(R.id.weather_image_shoot).setVisibility(View.VISIBLE);
                    uselastdata = false;
                }
                    ...
                    if (mCurrentPostalCode.equals("none"))
                    {
                        ...
                        if(true)
                        {
                            ...
                            intent.setClassName(this,
                                    "com.cooee.app.cooeejewelweather3D.view.WeatherAddPost");
		...
		if(true)
		{
			...
			condition = mDataEntity.getCondition();
    ...
    {  
        ...
        {  
        	...
        	android.util.Log.e("weijie", "onActivityResult resultCode="+resultCode+",resultCode="+resultCode);
        	
        	if(0!=resultCode){
        		Bundle bunde = data.getExtras();    
	            String cityname = bunde.getString("citys");  
	            Log.e("weijie", "onActivityResult mCurrentPostalCode = "+mCurrentPostalCode);
	            Log.e("weijie", "onActivityResult cityname = "+cityname);
	            updateEntity();
	            Log.e("weijie", "onActivityResult 1 mDataEntity = "+mDataEntity);
	        	...
	        	Log.e("weijie", "onActivityResult 2 mDataEntity = "+mDataEntity);
	        	readData();
	        	Log.e("weijie", "onActivityResult 3 mDataEntity = "+mDataEntity);
...
//					if(a.equals(getResources().getString(R.string.defaultcity)));
     ...
     */
    private boolean uselastdata = false;
    public void changePostalCode() {
    	
    	if(true)
    	{
    		Log.e("weijie", "changePostalCode");
    ...
    public void refreshData() {
    	readData();
    	if (true || mSettingEntity.getUpdateWhenOpen() == 1) 
    	{
			if(mDataEntity == null)
			{
    			 uselastdata = false; 
