...
public class ThemeHelper
{
			...
			boolean original )
	{
		if( original )
		{
			return "theme/widget/" + packageName + "/" + themeName + "/original_obj/" + objName;
		}
		else
		{
			return "theme/widget/" + packageName + "/" + themeName + "/obj/" + objName;
		}
	}
	
	public static String getLauncherThemeSubImagePath(
			String packageName ,
			String subPath ,
			String imageName )
	{
	...
	public static String getThemeSubImagePath(
			String subPath ,
			String imageName )
	{
	...
	public static BitmapTexture getThemeSubTexture(
			MainAppContext appContext ,
			...
			String fileName )
	{
		...
		String packagename = (String)appContext.paramsMap.get( "widgetpackagename" );
		if( packagename == null )
		{
			packagename = appContext.mWidgetContext.getPackageName();
		}
		String bitmapPath = getLauncherThemeSubImagePath( packagename , subImageFolder , fileName );
		...
		else
		{
			if( appContext.mWidgetContext.equals( appContext.mContainerContext ) )
			{
				...
				if( stream == null )
				{
					bitmapPath = getLauncherThemeImagePath( packagename , "iLoong" , fileName );
					stream = WidgetThemeManager.getInstance().getCurrentThemeInputStream( bitmapPath );
					if( stream == null )
					{
						stream = WidgetThemeManager.getInstance().getSysteThemeInputStream( bitmapPath );
			...
			{
				AndroidFiles widgetAsset = new AndroidFiles( appContext.mWidgetContext.getAssets() );
				String filePath = getThemeSubImagePath( subImageFolder , fileName );
				FileHandle file = widgetAsset.internal( filePath );
				if( !file.exists() )
				{
					filePath = getLauncherThemeImagePath( packagename , "iLoong" , fileName );
					stream = WidgetThemeManager.getInstance().getCurrentThemeInputStream( filePath );
					if( stream == null )
					{
						...
						if( file != null )
						{
							texture = new BitmapTexture( WidgetThemeManager.getInstance().getBitmap( file.read() ) );
						}
					}
					else
					{
