...
import com.iLoong.Widget3D.Theme.WidgetAssetFile.AssetFilePrefix;
...
public class WidgetThemeManager
{
	...
	public Bitmap getImageFromInStream(
			InputStream is )
	{
		Bitmap image = null;
		try
	...
	public Texture getThemeTexture(
			String fileName )
	{
			...
			boolean recycle )
	{
		...
		if( bitmap != null )
		{
			return new BitmapTexture( bitmap , recycle );
		}
		return null;
	}
			...
			String fileName )
	{
		...
		String bitmapPath = ThemeHelper.getLauncherThemeImagePath( packagename , appContext.mThemeName , fileName );
		stream = getCurrentThemeInputStream( bitmapPath );
		if( stream == null )
		{
			// Widget当前主题包
			bitmapPath = ThemeHelper.getThemeImagePath( appContext.mThemeName , fileName );
			stream = ThemeHelper.getInputStream( appContext.mWidgetContext , true , bitmapPath );
			if( stream == null )
			{
				// Launcher 默认主题包
				bitmapPath = ThemeHelper.getLauncherThemeImagePath( packagename , "iLoong" , fileName );
				stream = WidgetThemeManager.getInstance().getSysteThemeInputStream( bitmapPath );
				if( stream == null )
				{
					// Launcher Widget默认主题包
					bitmapPath = ThemeHelper.getThemeImagePath( "iLoong" , fileName );
					stream = ThemeHelper.getInputStream( appContext.mWidgetContext , true , bitmapPath );
				}
			}
		}
	...
	public InputStream getSysteThemeInputStream(
			String fileName )
	{
		return ThemeHelper.getInputStream( mSystemThemeDescription.getThemeContext() , true , fileName );
	}
	
	public InputStream getCurrentThemeInputStream(
			String fileName )
	{
		return ThemeHelper.getInputStream( mThemeDescription.getThemeContext() , true , fileName );
	...
	public int[] measureBitmap(String fileName){
		InputStream stream = null;
		// 读取优先级 当前主题->widget->launcher默认->widget默认
		// Launcher当前主题包
		String packagename = null;
		 ...
		 )
	{
		...
		try
		{
			...
			size[0] = opts.outWidth;// 获取图片的原始宽度
			size[1] = opts.outHeight;// 获取图片原始高度
			is.close();
			Log.d( "WidgetClock" , " initConfig  srcWidth:  "+size[0] +" , "+size[1] );
			return size;
