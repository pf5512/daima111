...
public class YahooClient
{
			...
			final Context mContext )
	{
		...
		if( WeatherClockHelper.isHaveInternet( mContext ) )
		{
			...
			try
			{
					...
					while( event != XmlPullParser.END_DOCUMENT )
					{
						...
						if( event == XmlPullParser.START_TAG )
						{
							if( tagName.equalsIgnoreCase( "html" ) || tagName.equalsIgnoreCase( "h1" ) )
							{
								...
								{
									CityFinderActivity.mHandler.obtainMessage( CityFinderActivity.MSG_TOASTCONNECTEDERROR ).sendToTarget();
								}
								break;
							}
				...
				else
				{
					if( CityFinderActivity.mHandler != null )
					{
						CityFinderActivity.mHandler.obtainMessage( CityFinderActivity.MSG_TOASTREQUESTFAILED ).sendToTarget();
					}
				}
			}
			catch( Exception e )
		...
		else
		{
			if( CityFinderActivity.mHandler != null )
			{
				CityFinderActivity.mHandler.obtainMessage( CityFinderActivity.MSG_TOASTNOTCONNECTED ).sendToTarget();
			}
		}
		return result;
	}
				...
				if( statue == yahooHttpConn.HTTP_OK )
				{
					...
					if( what == 1 )
					{
						if( weather != null && weather.getList() != null && weather.getList().size() == 5 )
						{
							...
							bundle.putSerializable( "myweatherinfo" , weather );
							bundle.putSerializable( "myresult" , result );
							if( CityFinderActivity.mHandler != null )
							{
								CityFinderActivity.mHandler.obtainMessage( CityFinderActivity.MSG_SUCCESS , bundle ).sendToTarget();
							}
						}
						else
						{
							if( CityFinderActivity.mHandler != null )
							{
								CityFinderActivity.mHandler.obtainMessage( CityFinderActivity.MSG_FAILURE ).sendToTarget();
							}
						}
					}
					else if( what == 2 )
					{
						...
						{
							Bundle bundle = new Bundle();
							bundle.putSerializable( "myupdateweatherinfo" , weather );
							if( WidgetWeatherClock.updatehandle != null )
							{
								WidgetWeatherClock.updatehandle.obtainMessage( WidgetWeatherClock.MSG_UpdateForSUCCESS , bundle ).sendToTarget();
							}
						}
						else
						{
							if( WidgetWeatherClock.updatehandle != null )
							{
								WidgetWeatherClock.updatehandle.obtainMessage( WidgetWeatherClock.MSG_UpdateForFAILURE ).sendToTarget();
							}
						}
					}
				}
