...
public class Tools
{
	...
	public static TextureRegion getTextureRegionByPicName(
			String picname ,
			int width ,
			int height )
	{
	...
	/**
	 * 将指定分辨率的资源缩放为想要的分辨率的资源
	 * @param specifiedScreenWidth	指定的分辨率宽度
	 * @param dstScreenWidth		目标分辨率宽度
	 * @param specifiedResName		指定资源路径
	 * @return
	 */
	public static Bitmap getScaledBitmapBySpecifiedRes(
			int specifiedScreenWidth ,
			int dstScreenWidth ,
			String specifiedResName )
	{
		Bitmap bitmap = null;
		Rect outRect = new Rect();
		BitmapFactory.Options op = new BitmapFactory.Options();
		op.inScaled = true;
		op.inDensity = specifiedScreenWidth;
		op.inTargetDensity = dstScreenWidth;
		op.inScreenDensity = dstScreenWidth;
		try
		{
			bitmap = BitmapFactory.decodeStream( ThemeManager.getInstance().getInputStream( specifiedResName ) , outRect , op );
			bitmap.setDensity( iLoongLauncher.getInstance().getResources().getDisplayMetrics().densityDpi );
		}
		catch( Exception e )
		{
