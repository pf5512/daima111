...
public class AudioAlbum
{
	...
	private int getVisibleAudio()
	{
		...
		{
			if( audios.get( i ).thumbnailPath == null || audios.get( i ).thumbnailPath.equals( "" ) )
				continue;
			return i;
		}
		return -1;
	...
	public void free()
	{
		if( visibleAudioIndex != -1 && audios.size() > 0 && visibleAudioIndex < audios.size() )
	...
	public class AudioAlbumView extends ViewGroup3D implements MediaView
	{
				...
				float parentAlpha )
		{
			...
			if( defaultRegion != null && visibleAudioIndex == -1 )
			{
				...
				batch.draw(
						defaultRegion ,
						x + ( width - bgWidth ) / 2 + R3D.audio_left_padding ,
						y + titleHeight + ( height - titleHeight - bgHeight ) / 2 + R3D.audio_bottom_padding ,
						R3D.audio_width ,
						R3D.audio_height );
			}
			...
			{
				if( selected )
					batch.draw( R3D.selectedRegion , x + ( width + bgWidth ) / 2 - R3D.selectedRegion.getRegionWidth() , y + titleHeight + ( height - titleHeight - bgHeight ) / 2 );
				else
					batch.draw( R3D.unselectRegion , x + ( width + bgHeight ) / 2 - R3D.unselectRegion.getRegionWidth() , y + titleHeight + ( height - titleHeight - bgHeight ) / 2 );
			}
		}
				...
				float h )
		{
			...
			if( visibleAudioIndex != -1 )
			{
				...
				view.y = titleHeight + ( height - titleHeight - bgHeight ) / 2 + R3D.audio_bottom_padding;
			...
			if( defaultRegion == null )
				defaultRegion = AudioItem.defaultRegions[Math.abs( getLabel().hashCode() ) % 6];
		}
