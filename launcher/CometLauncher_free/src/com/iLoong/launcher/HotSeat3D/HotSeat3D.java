...
import com.iLoong.launcher.dockbarAdd.AddIcon3D;
...
public class HotSeat3D extends ViewGroupOBJ3D implements DropTarget3D , DragSource3D
{
			...
			ArrayList<ItemInfo> items )
	{
		...
		HotGridView3D viewDock1 = dockGroup.getShortcutGridview1();
		viewDock1.enableAnimation( false );
		HotGridView3D viewDock2 = dockGroup.getShortcutGridview2();
		viewDock2.enableAnimation( false );
		for( ItemInfo item : items )
		{
			if( item.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT || item.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION )
			{
				...
				if( item.angle == TYPE_ICON )
				{
					...
					String name1[] = { "add app" , "widget" , "folder" , "shortcut" };
					for( int i = 0 ; i < 4 ; i++ )
					{
						AddIcon3D addicon = null;
						try
						{
							Bitmap bitmap = BitmapFactory.decodeStream( iLoongLauncher.getInstance().getAssets().open( "theme/hotseatbar/" + name[i] ) );
							if( bitmap != null )
							{
								addicon = new AddIcon3D( "add app" , bitmap , name1[i] , null );
								addicon.setItemInfo( info );
								addicon.setSize( icon.getWidth() , icon.getWidth() );
								Log.v( "mainGroup11" , "width:" + bitmap.getWidth() + "height:" + bitmap.getHeight() );
								Log.v( "mainGroup11" , "width:" + icon.getWidth() + "height:" + icon.getHeight() );
								mainGroup.bindItem( addicon );
								Log.v( "mainGroup11" , "mainGroup1111111" );
							}
						}
						catch( IOException e )
						{
							Log.v( "mainGroup11" , "mainGroup33333" );
							e.printStackTrace();
						}
					}
				}
				else
				{
					...
					else if( info.cellX == 1 )
					{
						dockGroup.bindItem1( icon );
					}
					else if( info.cellX == 2 )
					{
						dockGroup.bindItem2( icon );
					}
				}
			}
			else if( item.itemType == LauncherSettings.Favorites.ITEM_TYPE_VIRTURE_VIEW )
			{
				...
				if( view != null )
				{
					...
					else
					{
						...
						else if( item.cellX == 1 )
						{
							dockGroup.bindItem1( view );
						}
						else if( item.cellX == 2 )
						{
							dockGroup.bindItem2( view );
						}
						//						dockGroup.bindItem(view);
						android.util.Log.i( "bindItem" , Integer.toString( item.cellX ) + "ccc" );
					}
				}
				else
			...
			else if( item.itemType == LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER )
			{
				...
				if( folderInfo.contents.size() > 0 )
				{
					...
					{
						dockGroup.bindItem1( (View3D)newFolder );
					}
					else if( item.cellX == 2 )
					{
						dockGroup.bindItem2( (View3D)newFolder );
					}
					//					dockGroup.bindItem((View3D) newFolder);
		...
		viewDock1.enableAnimation( true );
		viewDock2.enableAnimation( true );
		finishBind = true;
		viewDock.setCellCount( viewDock.getChildCount() , 1 );
		viewDock1.setCellCount( viewDock1.getChildCount() , 1 );
		viewDock2.setCellCount( viewDock2.getChildCount() , 1 );
	}
		...
		for( ItemInfo item : items )
		{
			if( item.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT || item.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION )
			{
				...
				else
				{
					if( info.cellX == 0 )
					{
						dockGroup.bindBackItem( icon );
					}
					else if( info.cellX == 1 )
					{
						dockGroup.bindBackItem1( icon );
					}
					else if( info.cellX == 2 )
					{
						dockGroup.bindBackItem2( icon );
					}
				}
			}
			else if( item.itemType == LauncherSettings.Favorites.ITEM_TYPE_VIRTURE_VIEW )
			{
				...
				if( view != null )
				{
					...
					else
					{
						if( item.cellX == 0 )
						{
							dockGroup.bindBackItem( view );
						}
						else if( item.cellX == 1 )
						{
							dockGroup.bindBackItem1( view );
						}
						else if( item.cellX == 2 )
						{
							dockGroup.bindBackItem2( view );
						}
						//						dockGroup.bindItem(view);
						android.util.Log.i( "bindItem" , Integer.toString( item.cellX ) + "ccc" );
					}
			...
			else if( item.itemType == LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER )
			{
				...
				if( folderInfo.contents.size() > 0 )
				{
					...
					if( item.cellX == 0 )
					{
						dockGroup.bindBackItem( (View3D)newFolder );
					}
					else if( item.cellX == 1 )
					{
						dockGroup.bindBackItem1( (View3D)newFolder );
					}
					else if( item.cellX == 2 )
					{
						dockGroup.bindBackItem2( (View3D)newFolder );
					}
					//					dockGroup.bindItem((View3D) newFolder);
					newFolder.createAndAddShortcut( iconCache , folderInfo );
				}
	...
	{
		HotGridView3D hotGrid;
		if( dockGroup.getCurrentPage() == 1 )
		{
			hotGrid = dockGroup.getShortcutGridview1();
		}
		else if( dockGroup.getCurrentPage() == 2 )
		{
			hotGrid = dockGroup.getShortcutGridview2();
		}
		else
		{
			hotGrid = dockGroup.getShortcutGridview();
		}
	...
	public void startMyTween()
	{
		...
		else if( STATE_BACK == getHot3DState() )
		{
			...
			animation_line.push( Tween.to( dockGroup , View3DTweenAccessor.ROTATION , anim_duration ).target( R3D.hot_obj_rot_deg , 0 , 0 ).ease( Cubic.OUT ) );
			animation_line.push( Tween.to( dockGroup , View3DTweenAccessor.POS_XY , anim_duration ).target( 0 , 0 , 0 ).ease( Cubic.OUT ) );
			if( dockGroup.getCurrentPage() == 0 )
			{
				dockGroup.mShortcutView1.hide();
				dockGroup.mShortcutView2.hide();
			}
			else if( dockGroup.getCurrentPage() == 1 )
			{
				dockGroup.mShortcutView.hide();
				dockGroup.mShortcutView2.hide();
			}
			else if( dockGroup.getCurrentPage() == 2 )
			{
