...
public class AddFeatures extends ViewGroup3D
{
	...
	public static final int MSG_SHOW_ADD_APP_VIEW = 6; //应用添加
	public static final int MSG_SHOW_FOLDER_VIEW = 7;//song 文件夹添加
	public static final int MSG_SHOW_ADD_SHORTCUT_VIEW = 8; //快捷方式添加
	public static final int MSG_SHOW_ADD_WIDGET_VIEW = 9;//WIDGET添加
			...
			int index )
	{
		...
		View3D cur_view = children.get( index );
		View3D view0 = children.get( 0 );
		if( view0 != null && cur_view != null )
		{
			//mPosx += (view0.getWidth()- cur_view.getWidth())/2;
			//mPosy += (view0.getHeight()- cur_view.getHeight())/2;
		}
	}
			...
			int y )
	{
		...
		if( x < mPaddingLeft || y < mPaddingBottom || x > this.width - mPaddingRight || y > this.height - mPaddingTop )
			return -1;
		return( ( ( (int)this.height - y - mPaddingTop ) / mCellHeight ) * mCountX + ( x - mPaddingLeft ) / mCellWidth );
	}
			...
			float y )
	{
		if( auto_focus )
		{
			...
			//				mFocus = children.get(getIndex((int)x,(int)y));//(View3D) hit(x,y);
			//			}		
			//			if(mFocus==this)
			//				mFocus = null;
			//			if(mFocus!=null)
			...
			//				//mFocus.startTween(View3DTweenAccessor.SCALE_XY,Elastic.OUT, 0.8f, 1f, 1f, 0f);
	...
	/* (non-Javadoc)
	 * @see com.iLoong.launcher.UI3DEngine.ViewGroup3D#scroll(float, float, float, float)
	 */
	@Override
	...
	{ // first or last in screen
		...
		//		Log.v("test", "mFocusViewWidth = " + mFocusViewWidth + "mFocusViewX = " + mFocusViewX);
		switch( temp )
		{
			case MOVE_LEFT:
				if( -mFocusViewX > 30 )
	...
	public boolean onClick(
			...
			float y )
	{
		if( x >= getChildAt( 0 ).x && x <= getChildAt( 0 ).x + getChildAt( 0 ).width )
		{
			this.viewParent.onCtrlEvent( this , AddFeatures.MSG_SHOW_ADD_APP_VIEW );
		}
		else if( x >= getChildAt( 1 ).x && x <= getChildAt( 1 ).x + getChildAt( 1 ).width )
		{//song 文件夹添加
			this.viewParent.onCtrlEvent( this , AddFeatures.MSG_SHOW_ADD_WIDGET_VIEW );
		}
		else if( x >= getChildAt( 2 ).x && x <= getChildAt( 2 ).x + getChildAt( 2 ).width )
		{//song 文件夹添加
			this.viewParent.onCtrlEvent( this , AddFeatures.MSG_SHOW_FOLDER_VIEW );
		}
		else if( x >= getChildAt( 3 ).x && x <= getChildAt( 3 ).x + getChildAt( 3 ).width )
		{
			this.viewParent.onCtrlEvent( this , AddFeatures.MSG_SHOW_ADD_SHORTCUT_VIEW );
		}
		return true;
	}
}
