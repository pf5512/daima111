...
public class HotSeatMainGroup extends ViewGroupOBJ3D
{
	...
	private AddFeatures mShortcutView;
	private AddFeatures mFocusGridView;
			...
			int height )
	{
		...
		mShortcutView = new AddFeatures( "shortcutview" , width , height , 4 , 1 );
		...
		mShortcutView.setPadding( R3D.hot_grid_right_margin , R3D.hot_grid_right_margin , 0 , R3D.hot_grid_bottom_margin );
		mShortcutView.setPosition( 0 , R3D.hot_grid_bottom_margin );
			...
			float deltaY )
	{
		...
		bPermitAnim = true;
		float mFocusViewWidth = mFocusGridView.getEffectiveWidth();
		if( deltaY < 0 && bStartScrollDown == false )
		{
		...
		else
		{
			if( ( deltaX == 0 || Math.abs( deltaY ) / Math.abs( deltaX ) > 1 ) )
			{
				...
				return viewParent.onCtrlEvent( this , HotSeat3D.MSG_MAINGROUP_SCROLL_UP );
			}
		}
			...
			int event_id )
	{
		...
		if( sender instanceof AddFeatures )
		{
			switch( event_id )
			{
				case AddFeatures.MSG_SHOW_ADD_APP_VIEW:
					return viewParent.onCtrlEvent( this , HotSeat3D.MSG_SHOW_ADD_APP_VIEW );
				case AddFeatures.MSG_SHOW_FOLDER_VIEW:
					return viewParent.onCtrlEvent( this , HotSeat3D.MSG_SHOW_FOLDER_VIEW );
				case AddFeatures.MSG_SHOW_ADD_SHORTCUT_VIEW:
					return viewParent.onCtrlEvent( this , HotSeat3D.MSG_SHOW_ADD_SHORTCUT_VIEW );
				case AddFeatures.MSG_SHOW_ADD_WIDGET_VIEW:
					return viewParent.onCtrlEvent( this , HotSeat3D.MSG_SHOW_ADD_WIDGET_VIEW );
			}
		}
	...
	public AddFeatures getShortcutGridview()
	{
