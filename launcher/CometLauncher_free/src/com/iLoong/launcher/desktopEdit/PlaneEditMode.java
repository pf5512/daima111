...
public class PlaneEditMode extends ViewGroup3D
{
			...
			PageContainer dropPage )
	{
		...
		dropPage.startTween( View3DTweenAccessor.POS_XY , Linear.INOUT , 0.3f , posX , 0 , 0 );
			...
			float velocityY )
	{
		...
		{
			targetPageIndex = curPageIndex + pageNum;
			targetPageIndex = ( targetPageIndex >= desktopEdit.getPageNumIncludeAddPage() - 1 ? desktopEdit.getPageNumIncludeAddPage() - 1 : targetPageIndex );
		}
			...
			float parentAlpha )
	{
		...
		if( bAppendAnimtion || bDeleteAnimtion )
		{
			...
			Gdx.gl.glScissor( 0 , (int)( pageGroupPosY - grooveView.getHeight() / 4 ) , (int)Utils3D.getScreenWidth() , (int)( Utils3D.getScreenHeight() - pageGroupPosY ) );
		}
	...
	private void buildPlaneFromDesktop()
	{
		...
		for( int i = 0 ; i < pageList.size() ; i++ )
		{
			...
			if( i == curPageIndex )
			{
				...
				timeline.push( page.obtainTween( View3DTweenAccessor.SCALE_XY , Cubic.OUT , duration , 1f , 1f , 0 ) );
				View3D homeView = ( (ViewGroup3D)page ).findView( "pageHomeView" );
				if( homeView != null )
				{
					homeView.color.a = 0;
					timeline.push( homeView.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , duration , 1 , 0 , 0 ) );
				}
				View3D bgView = ( (ViewGroup3D)page ).findView( "pageBg" );
				if( bgView != null )
				{
					bgView.color.a = 0;
					timeline.push( bgView.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , duration , 1 , 0 , 0 ) );
				}
				View3D deleteView = ( (ViewGroup3D)page ).findView( "pageDeleteView" );
				if( deleteView != null )
				{
					deleteView.color.a = 0;
					timeline.push( deleteView.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , duration , 1 , 0 , 0 ) );
				}
			...
			View3D newPage )
	{
		...
		if( pageList.size() == DesktopEdit.PAGE_COUNT_MAX )
		{
			...
			temTimeline.push( deletePage.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , 1f , 0 , 0 , 0 ) );
		}
		else
		{
			...
			temTimeline.push( addPage.obtainTween( View3DTweenAccessor.POS_XY , Cubic.OUT , 0.5f , targetPosX , 0 , 0 ).delay( 0.5f ) );
		}
		...
		temTimeline.push( grooveView.obtainTween( View3DTweenAccessor.OPACITY , Cubic.OUT , 1f , 1 , 0 , 0 ) );
		timeline.push( temTimeline );
		timeline.push( newPage.obtainTween( View3DTweenAccessor.POS_XY , Elastic.INOUT , 1f , newPage.getX() , 0 , 0 ) );
			...
			float deltaY )
	{
		...
		if( desktopEdit.getCurrentPageIndex() == ( desktopEdit.getPageNumIncludeAddPage() - 1 ) )
		{
			if( offsetX <= -pageGroupOffsetX / 2 )
			{
				offsetX = -( pageGroupOffsetX / 2 );
	...
	private void startAutoEffect()
	{
		...
		{
			if( currentPage == ( desktopEdit.getPageNumIncludeAddPage() - 1 ) )
			{
	...
	private void autoMoveForDragAction()
	{
		...
		if( DesktopEdit.bDragActionForIcon )
		{
			...
			else if( DesktopEdit.autoMoveStatus == DesktopEdit.AUTO_MOVE_RIGHT )
			{
				...
				{
					if( targetPageIndex >= desktopEdit.getPageNumIncludeAddPage() - 1 )
					{
						return;
					}
				}
		...
		else if( DesktopEdit.bDragActionForPage )
		{
			...
			else if( DesktopEdit.autoMoveStatus == DesktopEdit.AUTO_MOVE_RIGHT )
			{
				...
				if( curPageIndex < desktopEdit.getPageNumIncludeAddPage() )
				{
				...
				else if( targetPageIndex > desktopEdit.getPageNumIncludeAddPage() )
				{
					return;
				}
			}
