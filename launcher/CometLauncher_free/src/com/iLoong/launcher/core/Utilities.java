...
public final class Utilities
{
			...
			Context context )
	{
		...
		{
			int color = context.getResources().getColor( RR.color.window_background );
	...
	public static Bitmap createIconBitmap(
			...
			Context context )
	{
		return createIconBitmap( icon , context , DefaultLayout.app_icon_size );
	}
			...
			int iconSize )
	{
		...
		{ // we share the statics :-(
			...
			//Log.e("icon", "scale:"+width+","+height+","+sourceWidth+","+sourceHeight);
			icon.draw( canvas );
			icon.setBounds( sOldBounds );
			//Log.d("title", "createIconBitmap:width,height="+bitmap.getWidth()+","+bitmap.getHeight());
			return bitmap;
		}
	}
	 ...
	 * Draw the view into a bitmap.
	 */
	public static Bitmap getViewScaledBitmap(
			View v ,
			int scaleWidth ,
			int scaleHeight )
	{
		float scaleW = (float)scaleWidth / (float)v.getWidth();
		float scaleH = (float)scaleHeight / (float)v.getHeight();
		if( scaleW > scaleH )
			scaleW = scaleH;
		final Bitmap bitmap = Bitmap.createBitmap( (int)scaleWidth , (int)scaleHeight , Bitmap.Config.ARGB_8888 );
		final Canvas c = new Canvas( bitmap );
		c.scale( scaleW , scaleW );
		//        if(v instanceof CellLayout){
		//        	CellLayout cellLayout = (CellLayout)v;
		//        	cellLayout.dispatchDraw(c);
		//        }
		//        else if(v instanceof MainMenu){
		//        	MainMenu mainMenu = (MainMenu)v;
		//        	mainMenu.dispatchDraw(c);
		//        }
		return bitmap;
	}
	 ...
	 * android.RR.dimen.launcher_application_icon_size.
	 *
			...
			Context context )
	{
		...
		{ // we share the statics :-(
			...
			{
				Bitmap res = createIconBitmap( new BitmapDrawable( bitmap ) , context );
				bitmap.recycle();
				return res;
			}
		}
	...
	public static void initStatics(
			Context context )
	{
		sIconWidth = sIconHeight = (int)DefaultLayout.app_icon_size;
		sIconTextureWidth = sIconTextureHeight = sIconWidth;
	}
	
	public static Matrix getMarixRotate(
			Camera c ,
			Matrix outMatrix ,
			int centerX ,
			int centerY ,
			float rotate )
	{
		final Camera camera = c;
		//        final Matrix matrix = new Matrix();
		camera.save();
		camera.rotateY( rotate );
		camera.getMatrix( outMatrix );
		camera.restore();
		outMatrix.preTranslate( -centerX , -centerY );
		outMatrix.postTranslate( centerX , centerY );
		return outMatrix;
	}
	...
	public static class BubbleText
	{
				...
				Context context )
		{
			...
			final float cellWidth = resources.getDimension( RR.dimen.title_texture_width );
			...
			if( false )
			{
				Log.d(
						TAG ,
						"mBitmapWidth=" + mBitmapWidth + " mBitmapHeight=" + mBitmapHeight + " w=" + ( (int)( mBubbleRect.width() + 0.5f ) ) + " h=" + ( (int)( ( MAX_LINES * mLineHeight ) + leading + 0.5f ) ) );
			}
		}
