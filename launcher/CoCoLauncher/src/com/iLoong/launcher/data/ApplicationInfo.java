...
public class ApplicationInfo extends ItemInfo {
	...
	public ApplicationInfo(ResolveInfo info) {
		...
		if (sysVersion < 9) {
			...
			PackageManager pm = iLoongLauncher.getInstance()
					.getPackageManager();
			try {
		...
		isHideIcon = PreferenceManager.getDefaultSharedPreferences(
	...
	public int getUseFrequency() {
		...
		if (intent != null && intent.getComponent() != null) {
			useFrequency = PreferenceManager.getDefaultSharedPreferences(
					iLoongLauncher.getInstance()).getInt(
					"FREQUENCY:" + intent.getComponent().toString(), 0);
		}
		// Log.d("launcher",
	...
	public void removeUseFrequency() {
		if (intent != null && intent.getComponent() != null) {
			PreferenceManager
					...
					.edit()
					.remove("FREQUENCY:" + intent.getComponent().toString())
					.commit();
		}
	}
	...
	public void removeHide() {
		if (intent != null && intent.getComponent() != null) {
			PreferenceManager
					.getDefaultSharedPreferences(iLoongLauncher.getInstance())
					.edit().remove("HIDE:" + intent.getComponent().toString())
					.commit();
		}
	}
	...
	public ApplicationInfo(String packageName, boolean isIconToDownload) {
