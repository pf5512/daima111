...
public abstract class PagedView extends LinearLayout {
	...
	public void setSwitchListener(ViewSwitchListener vsl) {
		mSwitchListener = vsl;
	}
	...
	public void InitToScreen(int screen, int pagewidth) {
	...
	public void OnClicksnapToScreen(int whichScreen) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if (mDirection == MOVE_LEFT && movex > screenWidth * mMoveThreshold) {
					whichScreen = Math.max((mLoop ? -1 : 0),
							Math.min(mCurrentScreen + 1, getChildCount() - (mLoop ? 0 : 1)));
	...
	public static void setAlphaAnimation(boolean visible){
	...
	public static void setIndicatorMargin(int deltaX){
	...
	public void dispatchDraw(Canvas canvas) {
		...
		canvas.save();
		canvas.clipRect(getScrollX(), getScrollY(), getScrollX() + this.getRight() - this.getLeft(), getScrollY()
