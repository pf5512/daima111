...
public class UpdateManager implements DLManager.DownLoadListener {
	...
	class CheckClient implements Runnable {
		...
		public CheckClient(List<NameValuePair> formparams) {
			mParams = formparams;
		}
	...
	private void CheckUpdate(String xml) {
		if (xml == null) {
	...
	private void noupdatedialog(String title, String msg) {
	...
	public static int getApkVersionCode(String apk) {
	...
	public void downloadhome() {
		DLManager.getInstance().DownloadLauncher(DownLoadTitle, APKNAME, iLoongApplication.getInstance().getPackageName(),
				murl, iLoongApplication.getInstance().getVersionCode(), this);
	}
