...
public class DLManager {
	...
	public static final String httpUrl = iLoongApplication.getInstance().getResources()
			.getString(RR.string.setting_apkdownload);
	
	public static final String serverAPPUrl = iLoongApplication.getInstance().getResources()
			.getString(RR.string.server_app_url);
	
	public static final String otherAPPUrl = iLoongApplication.getInstance().getResources()
			.getString(RR.string.other_app_url);
	
	public static final String DLResponseUrl = iLoongApplication.getInstance().getResources()
			.getString(RR.string.download_response_url);
	...
	private NotificationManager mNM = (NotificationManager) SetupMenu.getContext().getSystemService(
	...
	public interface DownLoadListener {
		...
		public void OnError(String ApkName, String packname, String errmsg);

		public void OnDownLoadComplete(String ApkName, String packname);
		
		public void OnCheckedComplete(String ApkName, String packname, int flag, String VersionCode, String VersionName);
	}
	...
	public downloadinfo GetDownLoadInfo(long downid) {
	...
	public String getAPKid(String DLurl) {
		String res = null;
		String sub = DLurl;
		int index = -1;
	...
	public void responseToServe(downloadinfo dli) {
		String apkid = null;
		String url = null;
	...
	public void DownloadComplete(downloadinfo dli) {
		if (dli.dll != null) {
			responseToServe(dli);
	...
	public boolean isAPKExist(String apkname) {
		if(apkname == null)
			return false;
		try{
	...
	public void CheckWidgetApk(String title, String name, String packname,DownLoadListener dll) {
		checkserverapk(title, name, packname, TYPE_WIDGET, "0", dll);
	}	
	
	public void DownloadWidget(String title, String name, String packname, String customerid, DownLoadListener dll) {
		download(title, name, packname, customerid, TYPE_WIDGET, "0", dll);
	}

	public void DownloadLauncher(String title, String name, String packname, String url, String versioncode,
	...
	private void checkserverapk(String title, String name, String packname, String type, String versioncode,
			DownLoadListener dll) {
	...
	private void onError(downloadinfo dli, String errormsg, boolean flag){
		dli.dll.OnError(dli.name, dli.packname, errormsg);
		if(flag)
			mDownloads.remove(dli);
	}

	private void download(String title, String name, String packname, String customerid, String type, String versioncode,
			DownLoadListener dll) {
...
//			SendMsgToAndroid.startActivity(intent1);
			File file = new File(DownloadLockBoxService.DOWNLOAD_PATH, title+ ".apk");
			if(file.exists()){
			...
			else TabLockFactory.downloadAPK(iLoongLauncher.getInstance(),RR.getPackageName(),packname,title);
		}
	...
	private boolean FindDownLoad(String packname) {
		for (int i = 0; i < mDownloads.size(); i++) {
	...
	public boolean HaveDownLoad(String packname) {
		if(packname == null)
			return false;
	...
	class downloadClient implements Runnable {
		...
		public downloadClient(downloadinfo dli) {
			mDoanloadInfo = dli;
	...
	class downloadGetClient implements Runnable {
		...
		public downloadGetClient(downloadinfo dli) {
			mDoanloadInfo = dli;
...
//				mParams.add(para3);
	...
	private void CheckDownload(String xml, downloadinfo dli) {
		if (xml == null) {
	...
	private void CheckGetDownload(String jsonObj, downloadinfo dli) {
		if (jsonObj == null) {
			onError(dli, R3D.getString(RR.string.dlmng_check_network), true);
			return;
		}
	...
	public void OnCheckedComplete(int flag, downloadinfo dli){
		dli.dll.OnCheckedComplete(dli.name, dli.packname, flag, dli.VersionCode, dli.VersionName);
		mDownloads.remove(dli);
	}
	
	public void ParseXml(String xml, downloadinfo dli) {
	...
	class DownloadXMLHandler extends DefaultHandler {
		...
		DownloadXMLHandler(downloadinfo dli) {
			mdli = dli;
		}
	...
	private void DownLoadApk(downloadinfo dli) {
	...
	public void CreateNotify(downloadinfo dli) {
	...
	private void notify(downloadinfo dli, int progress) {
		if (progress == 100) {
