...
public class BottomBar extends View3D{
	...
	public BottomBar(String name) {
		super(name);
		height = R3D.bottom_bar_height;
		//y = Utils3D.getScreenHeight()-height;
		bg = new NinePatch(new BitmapTexture(ThemeManager.getInstance().getBitmap(
				"theme/pack_source/bottom-bar-bg.png")),
				5, 5, 5, 5);
		...
		selectallTitle = new TextureRegion(new BitmapTexture((
				AppBar3D.titleToPixmapWidthLimit(iLoongLauncher.getInstance().getResources().getString(RR.string.bottombar_selectall), 
						(int)width/4,18, Color.WHITE, true))));
		shareTitle = new TextureRegion(new BitmapTexture((
				AppBar3D.titleToPixmapWidthLimit(iLoongLauncher.getInstance().getResources().getString(RR.string.bottombar_share), 
						(int)width/4,18, Color.WHITE, true))));
		settingTitle = new TextureRegion(new BitmapTexture((
				AppBar3D.titleToPixmapWidthLimit(iLoongLauncher.getInstance().getResources().getString(RR.string.bottombar_setting), 
						(int)width/4,18, Color.WHITE, true))));
		deleteTitle = new TextureRegion(new BitmapTexture((
				AppBar3D.titleToPixmapWidthLimit(iLoongLauncher.getInstance().getResources().getString(RR.string.bottombar_delete), 
						(int)width/4,18, Color.WHITE, true))));
		albumTitle = new TextureRegion(new BitmapTexture((
				AppBar3D.titleToPixmapWidthLimit(iLoongLauncher.getInstance().getResources().getString(RR.string.bottombar_album), 
						(int)width/4,18, Color.WHITE, true))));
		artistTitle = new TextureRegion(new BitmapTexture((
				AppBar3D.titleToPixmapWidthLimit(iLoongLauncher.getInstance().getResources().getString(RR.string.bottombar_artist), 
						(int)width/4,18, Color.WHITE, true))));
		folderTitle = new TextureRegion(new BitmapTexture((
				AppBar3D.titleToPixmapWidthLimit(iLoongLauncher.getInstance().getResources().getString(RR.string.bottombar_folder), 
						(int)width/4,18, Color.WHITE, true))));
		
		selectallImage = new TextureRegion(new BitmapTexture(ThemeManager.getInstance().getBitmap(
				"theme/pack_source/ic_menu_mark.png")));
		shareImage = new TextureRegion(new BitmapTexture(ThemeManager.getInstance().getBitmap(
				"theme/pack_source/ic_menu_share.png")));
		settingImage = new TextureRegion(new BitmapTexture(ThemeManager.getInstance().getBitmap(
				"theme/pack_source/ic_menu_set_as.png")));
		deleteImage = new TextureRegion(new BitmapTexture(ThemeManager.getInstance().getBitmap(
				"theme/pack_source/ic_menu_close_clear_cancel.png")));
		albumImage = new TextureRegion(new BitmapTexture(ThemeManager.getInstance().getBitmap(
				"theme/pack_source/ic_menu_audioalbum.png")));
		artistImage = new TextureRegion(new BitmapTexture(ThemeManager.getInstance().getBitmap(
				"theme/pack_source/ic_menu_artist.png")));
		folderImage = new TextureRegion(new BitmapTexture(ThemeManager.getInstance().getBitmap(
				"theme/pack_source/ic_menu_folder.png")));
	...
	public void draw(SpriteBatch batch, float parentAlpha) {
			...
			if (is3dRotation())
				...
				while(ite.hasNext()){
					...
					if(entry.getValue()){
						...
						batch.draw(title, titleX, titleY, originX, originY, title.getRegionWidth(), title.getRegionHeight(), scaleX,
								scaleY, rotation);
						int imageX = (int) (x + itemWidth*n + (itemWidth-image.getRegionWidth())/2);
						int imageY = 2*titleY+title.getRegionHeight();
						batch.draw(image, imageX, imageY, originX, originY, image.getRegionWidth(), image.getRegionHeight(), scaleX,
								scaleY, rotation);
						n++;
					}
				}
		...
		case STATE_DISPLAY:
			...
			batch.draw(naviBack, naviBackX +naviBackOffsetX
					...
					/ 2, naviBackWidth,
					naviBackHeight);
			batch.draw(displayRegion, R3D.appbar_tab_padding
					+ R3D.appbar_menu_width/2, y+height/3);
