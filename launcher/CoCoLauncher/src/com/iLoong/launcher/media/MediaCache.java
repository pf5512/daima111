...
public class MediaCache {
	...
	public static final String[] PROJECTION_IMAGES = new String[] {
			...
			Images.ImageColumns.DATA, Images.ImageColumns.BUCKET_ID,
	...
	public static final String[] PROJECTION_AUDIOS = new String[] {
	...
	private MediaCache() {
		...
		context.getContentResolver().registerContentObserver(
				...
				videoObserver);
		context.getContentResolver().registerContentObserver(
				Images.Media.EXTERNAL_CONTENT_URI, true, photoObserver);
		context.getContentResolver().registerContentObserver(
				MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, true,
				audioObserver);
	}

	public synchronized void attach(MediaDataObserver observer) {
		observers.add(observer);
	}

	public synchronized void attach(int index, MediaDataObserver observer) {
		observers.add(index, observer);
	}

	public synchronized void detach(MediaDataObserver observer) {

	}

	public PhotoBucket getPhotoBucket(Integer bucketId) {
		return photoBucketsCache.get(bucketId);
	}

	public synchronized void notifyObservers(int type) {
	...
	class MediaObserver extends ContentObserver {
		...
		public MediaObserver(int syncType) {
			super(null);
			type = syncType;
		}
	...
	class Task extends Thread {
		...
		private void syncVideoData() {
			...
			where.append(" AND " + MediaStore.Video.Media.MIME_TYPE
					+ " NOT LIKE 'audio%'");
			cursor = context.getContentResolver().query(
					MediaStore.Video.Media.EXTERNAL_CONTENT_URI,
					PROJECTION_VIDEOS, where.toString(), null, null);
			if (cursor == null)
				return;
			int idIndex = cursor
		...
		private void syncPhotoData() {
			...
			cursor = context.getContentResolver().query(
					Images.Media.EXTERNAL_CONTENT_URI, PROJECTION_IMAGES, null,
					null, null);
			if (cursor == null)
				return;
			...
			int titleIndex = cursor
					.getColumnIndexOrThrow(Images.ImageColumns.TITLE);
			int bucketDisplayIndex = cursor
					.getColumnIndexOrThrow(Images.ImageColumns.BUCKET_DISPLAY_NAME);
		...
		private void syncAudioData() {
			...
			cursor = context.getContentResolver().query(
					...
					PROJECTION_AUDIOS, where.toString(), null, mAudioSortOrder);
			if (cursor != null) {
				if (!cursor.moveToFirst())
					return;
				AudioAlbum defaultAlbum = null;
				int audioIdIdx = cursor
				...
				int audioSizeIdx = cursor
						.getColumnIndexOrThrow(MediaStore.Audio.Media.SIZE);
				int audioDataIdx = cursor
						.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA);
				int audioArtistIdx = cursor
						.getColumnIndexOrThrow(MediaStore.Audio.Media.ARTIST);
				int albumIdIdx = cursor
						.getColumnIndexOrThrow(MediaStore.Audio.Media.ALBUM_ID);
				int albumNameIdx = cursor
						.getColumnIndexOrThrow(MediaStore.Audio.Media.ALBUM);
				int audioTitleIdx = cursor
				...
				do {
					...
					if (audio.albumId >= 0) {
						...
						if (defaultAlbum == null) {
							...
							defaultAlbum.title = context.getResources()
									.getString(RR.string.default_album_name);
							audioAlbumsCache.put(defaultAlbum.albumId,
									defaultAlbum);
				...
				if (defaultAlbum != null) {
					if (!audioAlbumsCache.containsKey(defaultAlbum.albumId))
						audioAlbumsCache
								.put(defaultAlbum.albumId, defaultAlbum);
				} else
	...
	public void exeDelete(final ArrayList<File> deleteFiles) {
		new Thread() {
			...
			public void run() {
				for (int i = 0; i < deleteFiles.size(); i++) {
					if (deleteFiles.get(i).delete())
						SendMsgToAndroid.sendToastMsg(deleteFiles.get(i)
								.getAbsolutePath()
								+ " "
								+ iLoongLauncher.getInstance().getResources()
										.getString(RR.string.media_delete_ok)
								+ "!");
				}
				super.run();
				deleteTaskNum--;
	...
	public boolean deleteVideo(VideoItem video) {
		videos.remove(video);
		// ignoreMediaChange = true;
		int num = context.getContentResolver().delete(
				MediaStore.Video.Media.EXTERNAL_CONTENT_URI,
				MediaStore.Video.Media._ID + "=" + Long.toString(video.id),
				null);
	...
	public boolean deletePhoto(PhotoItem photo) {
		...
		int num = context.getContentResolver().delete(
				...
				Images.ImageColumns._ID + "=" + Long.toString(photo.id), null);
		//
		// if(num <= 0)ignoreMediaChange = false;
	...
	public boolean deletePhotoBucket(PhotoBucket bucket) {
		photoBuckets.remove(bucket.bucketId);
		photoBucketIds.remove((Integer) bucket.bucketId);
		return true;
	}

	public boolean deleteAudio(AudioItem audio) {
		audios.remove(audio);
		if (audio.album != null)
	...
	public boolean deleteAudioAlbum(AudioAlbum album) {
		switch (album.type) {
