...
public class VideoItem  extends ReusableTextureHolder implements ThumbnailClient{
	...
	public class VideoView extends View3D implements MediaView{
		...
		public void draw(SpriteBatch batch, float parentAlpha) {
			...
			if (is3dRotation()){
				if(video.texture != null)batch.draw(video.texture, x+_x, y+_y, video.texture.getWidth(), video.texture.getHeight());
				...
				if(AppHost3D.selectState && AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_VIDEO){
					if(selected)batch.draw(R3D.selectedRegion, x+_xBg+bgWidth-R3D.selectedRegion.getRegionWidth(), y+_yBg+6,
							R3D.selectedRegion.getRegionWidth(), R3D.selectedRegion.getRegionHeight());
					else batch.draw(R3D.unselectRegion, x+_xBg+bgWidth-R3D.unselectRegion.getRegionWidth(), y+_yBg+6,
							R3D.unselectRegion.getRegionWidth(), R3D.unselectRegion.getRegionHeight());
				}
			}
			else{
				if(video.texture != null)batch.draw(video.texture, x+_x, y+_y, video.texture.getWidth(), video.texture.getHeight());
				batch.draw(bgRegion, x+_xBg, y+_yBg, originX-_xBg, originY-_yBg, bgWidth, bgHeight, scaleX,
						scaleY, rotation);
				if(AppHost3D.selectState && AppHost3D.currentContentType == AppHost3D.CONTENT_TYPE_VIDEO){
					if(selected)batch.draw(R3D.selectedRegion, x+_xBg+bgWidth-R3D.selectedRegion.getRegionWidth(), y+_yBg+6,
		...
		public void setSize(float w, float h) {
			...
			if(region.getTexture()==null){
				region.setRegion(new BitmapTexture((
						Utils3D.titleToPixmapWidthLimit(title, (int)w-4,R3D.photo_title_size, Color.WHITE,2))));
			}
		}
