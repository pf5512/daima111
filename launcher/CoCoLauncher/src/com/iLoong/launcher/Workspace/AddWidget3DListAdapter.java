...
public class AddWidget3DListAdapter extends BaseAdapter {
	...
	public class ListItem {
		...
		public ListItem(Resources res, int textResourceId, Bitmap bitmap,
				int actionTag, ResolveInfo resolveInfo, boolean installed) {
		...
		public ListItem(String name, Drawable image, String packageName,
				String apkName, boolean installed) {
			this.text = name;
			this.image = image;
			this.installed = installed;
	...
	public AddWidget3DListAdapter(iLoongLauncher launcher) {
		super();
		...
		// ArrayList<WidgetIcon> defaultIconList =
		// Widget3DManager.defIcon3DList;
		// ArrayList<WidgetIcon> internalWidgetList = new
		// ArrayList<WidgetIcon>();
		// for (int i = 0; i < defaultIconList.size(); i++) {
		// ShortcutInfo itemInfo = (ShortcutInfo) defaultIconList.get(i)
		// .getItemInfo();
		// String packageName = itemInfo.intent.getComponent()
		// .getPackageName();
		// if (DefaultLayout.isWidgetLoadByInternal(packageName)) {
		// internalWidgetList.add(defaultIconList.get(i));
		// for (int j = 0; j < mWidgetResolveInfoList.size(); j++) {
		...
		for (ResolveInfo resolve : mWidgetResolveInfoList) {
			try {
				String themeName = "";
				themeName = ThemeManager.getInstance()
				...
				Context widgetPluginContext = iLoongApplication.ctx
						.createPackageContext(resolve.activityInfo.packageName,
				...
				if (!themeName.equals("iLoong")) {
					...
					themeName = Widget3DManager.getInstance()
							.checkThemeExist(
									ThemeManager.getInstance()
											.getCurrentThemeContext(),
									widgetName, themeName);
				}
				widgetPluginContext.getResources()
						.updateConfiguration(
								iLoongApplication.ctx.getResources()
										.getConfiguration(),
								iLoongApplication.ctx.getResources()
										.getDisplayMetrics());
				int appNameIdentifier = widgetPluginContext.getResources()
						.getIdentifier("app_name", "string",
								resolve.activityInfo.packageName);
				// int iconIdentifier = widgetPluginContext.getResources()
				// .getIdentifier("widget_ico", "drawable",
				// resolve.activityInfo.packageName);
				InputStream is = null;
					...
					if (useTheme) {
						...
						if (!themeName.equals("iLoong")) {
							...
							String widgetName = packageName.substring(
									...
									.toLowerCase();
							themeName = Widget3DManager.getInstance()
									.checkThemeExist(widgetPluginContext,
											widgetName, themeName);
						}
				...
				String shortName = widgetPluginContext
						.getPackageName()
						.substring(
								widgetPluginContext.getPackageName()
										.lastIndexOf(".") + 1).toLowerCase();
				String iconPathInThemeMgr = "theme/widget/" + shortName + "/"
						+ iconPath;
				is = ThemeManager.getInstance().getCurrentThemeInputStream(
						iconPathInThemeMgr);
				if (is == null) {
				...
				ListItem item = new ListItem(
						widgetPluginContext.getResources(), appNameIdentifier,
						bitmap, ITEM_THEME_DEFAULT_ICON, resolve, true);
				if (DefaultLayout.isWidgetLoadByInternal(widgetPluginContext
						.getPackageName())) {
					item.isInternal = true;
				}
				mItems.add(item);
				// if (themeName.equals("iLoong")) {
				// mItems.add(new ListItem(widgetPluginContext.getResources(),
				// appNameIdentifier, bitmap, ITEM_THEME_DEFAULT_ICON,
				// resolve, true));
				//
				// } else {
				// mItems.add(new ListItem(widgetPluginContext.getResources(),
				// appNameIdentifier, bitmap,
				// ITEM_THEME_DEFAULT_OTHER, resolve, true));
				//
				// }
				DefaultLayout
						.RemoveDefWidgetWithPkgname(resolve.activityInfo.packageName);
			} catch (Exception e) {
		...
		// int defaultWidgetCount = defaultIconList.size();
		// for (int i = 0; i < defaultWidgetCount; i++) {
		// Widget3DVirtual icon = (Widget3DVirtual) defaultIconList.get(i);
		...
		// String apkName = null;
		// Bitmap image = icon.getPreviewBitmap();
		// Drawable drawable = new BitmapDrawable(image);
		// ListItem item = new ListItem(name, drawable, packageName,
		// apkName, false);
		// item.isInternal = icon.isInternal;
		// item.iconResourceId = icon.iconResourceId;
		// mItems.add(item);
		// } catch (Exception e) {
		// // TODO Auto-generated catch block
		// e.printStackTrace();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = mInflater.inflate(RR.layout.add_list_item, parent,
					false);
		}
		...
		item.image = Utils3D.createIconThumbnail(item.image, mIconWidth,
				mIconHeight);
	...
	public ResolveInfo getItemResolveInfo(int position) {
		ListItem item = (ListItem) getItem(position);
