...
public class ThemeDescription {
	...
	private String PREVIEW_FILENAME = "/preview/preview.xml";
	...
	public ThemeDescription(Context context) {
		...
		specificThemeDir = ThemeManager.getInstance().getSpecificThemeDir(
				context, defaultThemeDir);
	...
	public ThemeDescription(Context context, String fileName) {
		mContext = context;
		mAndroidFiles = new AndroidFiles(mContext.getAssets());
		defaultThemeDir = ThemeManager.getInstance().getDefaultThemDir();
		autoAdaptThemeDir = getAutoAdaptDir(context, defaultThemeDir);
		...
		try {
			if (themeimgs.size() > 0)
				defaultbitmap = Tools.getImageFromInStream(mContext.getAssets()
						.open(autoAdaptThemeDir + File.separator + "preview"
								+ File.separator + themeimgs.get(0)));
		} catch (IOException e) {
			e.printStackTrace();
		}
