...
//		.show();
	...
	public static boolean checkToDownload(ShortcutInfo info, boolean needed) {
		...
		if (DLManager.getInstance().isAPKExist(apkname)) {
			if (needed && customID != null && customID.equals("widget"))
				DLManager.getInstance().CheckWidgetApk(dlTitle,
					apkname, pkgname,
					getInstance());
			else
	...
	public static boolean downloadWithoutCheckVersion(String title, String apkName, String pkgName, String customID) {
	...
	public static boolean inAssertDir(String apkName){
		String[] apks = null;
		try {
		...
		for(int i = 0;i < apks.length;i++){
			if(apks[i].equals(apkName))return true;
		}
		return false;
	}
	
	public static String getAssertAPKPath(String apkName){
	...
	public static boolean CheckAppIsDownloaded(String path) {
		boolean res = false;
		Intent intent = new Intent(Intent.ACTION_VIEW);
		intent.setDataAndType(Uri.fromFile(new File(path)), "application/vnd.android.package-archive");
		iLoongLauncher.getInstance().startActivity(intent);
	...
	public void OnError(String ApkName, String packname, String errmsg) {
		...
		.setTitle(title)
		.setMessage(errmsg)
		.setPositiveButton(iLoongLauncher.getInstance().getString(RR.string.circle_ok_action), null)
		.show();
	}

	@Override
	public void OnDownLoadComplete(String ApkName, String packnam) {
		// TODO Auto-generated method stub
		...
		String str = MimeTypeMap.getSingleton().getMimeTypeFromExtension(
				MimeTypeMap.getFileExtensionFromUrl(paramString));
	...
	@Override
	public void OnCheckedComplete(String ApkName, String packname, int flag,
