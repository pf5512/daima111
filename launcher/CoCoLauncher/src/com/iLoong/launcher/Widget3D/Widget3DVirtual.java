...
public class Widget3DVirtual extends WidgetIcon {
	...
	public Widget3DVirtual(String name, Bitmap bmp, String title) {
		super(name);
		preview = bmp;
	}
	...
	public boolean onLongClick(float x, float y) {
		...
		boolean isLoadByInternal = DefaultLayout
				.isWidgetLoadByInternal(info.intent.getComponent()
						.getPackageName());
		if (isLoadByInternal) {
			...
			return viewParent
					.onCtrlEvent(this, MSG_WIDGET3D_SHORTCUT_LONGCLICK);
		} else {
			if (info.intent != null
					&& info.intent.getAction().equals(
							Intent.ACTION_PACKAGE_INSTALL)) {
				SendMsgToAndroid.sendOurToastMsg(R3D
						.getString(RR.string.download_to_install));
				return true;
			}
	...
	public void makeShortcut() {
		if (titleRegion == null || cellTitleRegion == null
			...
			if (title.equals("clock")) {
				title = iLoongLauncher.getInstance().getResources()
						.getString(RR.string.clock);
			}
			if (title.equals("memo")) {
				title = iLoongLauncher.getInstance().getResources()
						.getString(RR.string.memo);
			}
			if (title.equals("robot")) {
				title = iLoongLauncher.getInstance().getResources()
						.getString(RR.string.robot);
			}
			...
			if (preview != null) {
				...
				Bitmap bitmap = Bitmap.createScaledBitmap(preview, bitmapWidth,
						bitmapHeight, false);
				if (!bitmap.equals(preview))
					preview.recycle();
				previewRegion = new TextureRegion(new BitmapTexture((bitmap)));
	...
	public void draw(SpriteBatch batch, float parentAlpha) {
		...
		if (DefaultLayout.blend_func_dst_gl_one) {
			...
			if (srcBlendFunc != GL11.GL_SRC_ALPHA
					|| dstBlendFunc != GL11.GL_ONE_MINUS_SRC_ALPHA)
...
//					drawHeight);
			...
			if (is3dRotation()) {
				batch.draw(previewRegion, previewX + x, previewY + y,
						drawWidth, drawHeight);
			}

			else {
				batch.draw(previewRegion, previewX + x, previewY + y, originX
						- previewX, originY - previewY, drawWidth, drawHeight,
						scaleX, scaleY, rotation);
			}
		}
		...
		if (titleRegion != null) {
			if (DefaultLayout.widget_shortcut_title_top) {
				if (is3dRotation())
					...
					batch.draw(titleRegion, x, y + height
							* (1 - R3D.widget_preview_title_weight), originX,
							originY, titleRegion.getRegionWidth(),
							titleRegion.getRegionHeight(), scaleX, scaleY,
							rotation);
			} else {
				if (is3dRotation())
		...
		if (cellTitleRegion != null) {
			if (is3dRotation()) {
				...
				float titleX = 10;
				titleX = (float) (width / 4 - 2
						* cellTitleRegion[0].getRegionWidth() - xRegion
				...
				if (DefaultLayout.widget_shortcut_title_top) {
					batch.draw(
							...
							width * 3 / 4 + x + titleX,
							y
									+ ((R3D.widget_preview_title_weight * height) - cellTitleRegion[0]
					...
					batch.draw(
							...
							y
									+ ((R3D.widget_preview_title_weight * height) - cellTitleRegion[1]
											.getRegionHeight()) / 2 + height
									* (1 - R3D.widget_preview_title_weight));
				} else {
					batch.draw(
				...
				if (DefaultLayout.widget_shortcut_title_top) {
					batch.draw(
							...
							y
									+ ((R3D.widget_preview_title_weight * height) - cellTitleRegion[0]
											.getRegionHeight()) / 2 + height
									* (1 - R3D.widget_preview_title_weight),
							originX - width * 3 / 4, originY,
							cellTitleRegion[0].getRegionWidth(),
							cellTitleRegion[0].getRegionHeight(), scaleX,
							scaleY, rotation);
					batch.draw(
							xRegion,
							width * 3 / 4 + x + titleX
							...
							y
									+ ((R3D.widget_preview_title_weight * height) - xRegion
											.getRegionHeight()) / 2 + height
									* (1 - R3D.widget_preview_title_weight),
							originX - width * 3 / 4, originY, xRegion
									.getRegionWidth(), xRegion
									.getRegionHeight(), scaleX, scaleY,
							rotation);
					batch.draw(
							cellTitleRegion[1],
							width * 3 / 4 + x + titleX
							...
							cellTitleRegion[1].getRegionHeight(), scaleX,
							scaleY, rotation);
				} else {
					batch.draw(
							cellTitleRegion[0],
					...
					batch.draw(
							...
							y
									...
									- width * 3 / 4, originY,
							xRegion.getRegionWidth(),
							xRegion.getRegionHeight(), scaleX, scaleY, rotation);
					batch.draw(
							cellTitleRegion[1],
							width * 3 / 4 + x + titleX
									+ cellTitleRegion[0].getRegionWidth()
		...
		if (is3dRotation()) {
			...
			if (hide && isHide) {
				batch.draw(Icon3D.resumeTexture, stateX, stateY,
						stateIconWidth, stateIconHeight);
			}
			if (hide && !isHide) {
				batch.draw(Icon3D.hideTexture, stateX, stateY, stateIconWidth,
						stateIconHeight);
			}
		} else {
			...
			if (hide && isHide) {
				batch.draw(Icon3D.resumeTexture, stateX, stateY, stateOriginX,
						stateOriginY, stateIconWidth, stateIconHeight, scaleX,
						scaleY, rotation);
			}
			if (hide && !isHide) {
				batch.draw(Icon3D.hideTexture, stateX, stateY, stateOriginX,
						stateOriginY, stateIconWidth, stateIconHeight, scaleX,
						scaleY, rotation);
			}
		}
	...
	public void removeHide() {
		...
		if (name != null) {
			PreferenceManager
					...
					.edit().remove("HIDE:" + name.toString()).commit();
		}
