...
public class Utils3D {
	...
	static public int showPidMemoryInfo(String title) {
		if (DEBUG) {
	...
	static public int showPidMemoryInfo(Context context, String title) {
		if (DEBUG) {
			int myProcessID = Process.myPid();
			int pids[] = new int[1];
			pids[0] = myProcessID;
	...
	static public boolean isLowMemory() {
		...
		Log.i("MemoryInfo", "availMem:" + memInfo.availMem / 1024 / 1024
				+ ",threshold" + memInfo.threshold / 1024 / 1024);
		return memInfo.lowMemory;
	}

	public static void showTimeFromStart(String s) {
		 //Log.i("Launcher.Model",s+":"+(System.currentTimeMillis()-iLoongLauncher._time));
	}

	static public String sync_do_exec(String cmd) {
		String s = "\n";
		try {
	...
	static public boolean do_exec(final String cmd, String packageName) {
	...
	public static int getStatusBarHeight() {
		...
		try {
			...
			e1.printStackTrace();
			Rect rect = new Rect();
			View v = iLoongLauncher.getInstance().getWindow()
	...
	public static byte[] bmp2Array(Bitmap bmp) {
	...
	public synchronized static Pixmap bmp2Pixmap(Bitmap bmp, boolean recycle) {
		if (defaultPixels == null) {
			defaultPixels = new int[R3D.workspace_cell_width
					* R3D.workspace_cell_height + 64];
			defaultBmpbuff = new byte[R3D.workspace_cell_width
					* R3D.workspace_cell_height * 4 + 256];
		}
		int w = bmp.getWidth();
		int h = bmp.getHeight();
		int[] pixels;
	...
	/* 判断特定Actor是否被框选中 */
	public static boolean overlapRectangles(View3D actor, Rectangle r2) {

		if (actor.x < r2.x + r2.width && actor.x + actor.width > r2.x
				&& actor.y < r2.y + r2.height && actor.y + actor.height > r2.y)
			return true;
		else
			return false;
	}

	public static Bitmap resizeBmp(Bitmap bmp, float maxWidth, float maxHeight) {
		float scale = 1f;
		...
		if (scale != 1f) {
			...
			Bitmap res = Bitmap.createScaledBitmap(bmp, bitmapWidth,
					bitmapHeight, false);
			bmp.recycle();
			return res;
		} else
	...
	public static boolean lineInActor(View3D actor, Vector2 startPoint,
			Vector2 endPoint) {
	...
	public static float getRotDegrees(float moveX, float moveY, float centerX,
			float centerY) {
		float retDeg;
	...
	private static boolean ClipT(float p, float q) {
		boolean flag = true;
		float r;
		if (p < 0.0) {
			r = q / p;
	...
	public static Bitmap GetBmpFromImageName(String imageName) {
		Bitmap image;
		Bitmap retImage;
		if (imageName == null) {
			return null;
		...
		// else
		// {
		retImage = Utilities.createIconBitmap(
				new BitmapDrawable((Bitmap) image), iLoongApplication.ctx);
		// }
		if (image != null) {
			image.recycle();
		}
	...
	public static void changeTextureRegionHeight(TextureRegion region,
			float scale) {
		float V2 = region.getV() + (region.getV2() - region.getV()) * scale;
		region.setV2(V2);
	}

	public static void changeTextureRegion(View3D myActor, float iconHeight,
			boolean bNeedScale) {
	...
	public static void changeTextureRegion(ArrayList<View3D> view3DArray,
			float iconHeight, boolean bNeedScale) {
		View3D myActor;
	...
	public static synchronized Bitmap folderTitleToBitmap(String title,
			Bitmap title_bg, int textureWidth) {
		Paint paint = new Paint();
		paint.setColor(Color.WHITE);
		paint.setAntiAlias(true);
		paint.setTextSize(R3D.icon_title_font);
		...
		if (DefaultLayout.font_double_line) {
			...
			if (title != null) {
				splitTitle = splitTitle(title, R3D.icon_title_font,
						textureWidth);
			} else {
				splitTitle = null;
			}
		...
		if (title_bg != null) {
			titleHeight = Math.round(cellHeight - bmpHeight - singleLineHeight
					/ 2 + title_bg.getHeight() / 2);
		}
		Bitmap bmp = Bitmap.createBitmap(textureWidth, titleHeight,
				Config.ARGB_8888);
		Canvas canvas = new Canvas(bmp);
		Log.d("title", "folder:width,height=" + textureWidth + ","
				+ titleHeight);

		if (title_bg != null) {
			folderTitleFontTopping = singleLineHeight / 2f
					- title_bg.getHeight() / 2f;
			if (folderTitleFontTopping < 0) {
				canvas.drawBitmap(Icon3D.titleBg, 0, 0, null);
			} else {
				canvas.drawBitmap(Icon3D.titleBg, 0, folderTitleFontTopping,
						null);
			}
		...
		for (int i = 0; i < splitTitle.length; i++) {
			...
			if (textWidth > R3D.workspace_cell_width) {
				if (title_bg != null) {
					...
					if (textWidth > textureWidth - 2 * paddintText) {
						...
						int length = paint.breakText(splitTitle[i], 0,
								splitTitle[i].length(), true, textureWidth - 2
										* paddintText, null);
			...
			paint.setColor(Color.WHITE);
			if (title_bg == null) {
				paint.setShadowLayer(SHADOW_LARGE_RADIUS, 0.0f,
						SHADOW_Y_OFFSET, SHADOW_LARGE_COLOUR);
				canvas.drawText(splitTitle[i], paddintText, titleY, paint);
				paint.setShadowLayer(SHADOW_SMALL_RADIUS, 0.0f, 0.0f,
						SHADOW_SMALL_COLOUR);
				canvas.drawText(splitTitle[i], paddintText, titleY, paint);
			} else {
				paint.clearShadowLayer();
			...
			if (textWidth > textureWidth && title_bg == null) {
				...
				for (int j = 0; j < alphaW; j++) {
					...
					canvas.drawLine(
							textureWidth - j - 1,
							(float) (titleY - Math.ceil(fontMetrics.descent
									- fontMetrics.ascent)), textureWidth - j,
							titleHeight, mErasePaint);
				}
			}
		}
		return bmp;
	...
	public static synchronized Bitmap titleToBitmap(Bitmap b, String title,
			Bitmap icn_bg, Bitmap title_bg, int textureWidth, int textureHeight) {
		return titleToBitmap(b, title, icn_bg, title_bg, textureWidth,
				textureHeight, true);
	}
	//zhujieping add
	public static float measureText(Paint paint,String s){
		Rect rect = new Rect();
		paint.getTextBounds(s,0,s.length(),rect);
		return rect.width();
	}
	
	public static Bitmap titleToPixmapWidthLimit(String title, int widthLimit,
			int size, int color, int line) {
		Paint paint = new Paint();
		...
		if (splitTitle != null && splitTitle.length > 0) {
			int titleWidth = Math.min((int) (paint.measureText(title)),
					widthLimit);
			FontMetrics fontMetrics = paint.getFontMetrics();
			int singleLineHeight = (int) Math.ceil(-fontMetrics.ascent
					+ fontMetrics.descent);
			bmp = Bitmap.createBitmap(titleWidth, singleLineHeight
					* splitTitle.length, Config.ARGB_8888);
			Canvas canvas = new Canvas(bmp);
			float x = 0;
				...
				if (textWidth > widthLimit) {
					...
					for (int j = 0; j < alphaW; j++) {
						...
						canvas.drawLine(
								widthLimit - j - 1,
								(float) (titleY - Math.ceil(fontMetrics.descent
										- fontMetrics.ascent)), widthLimit - j,
								titleY, mErasePaint);
					}
				}
			}
	...
	public static synchronized Bitmap titleToBitmap(Bitmap b, String title,
			Bitmap icn_bg, Bitmap title_bg, int textureWidth,
			int textureHeight, boolean recycle) {
	...
	//teapotXu_20130128: add icon_cover_plate end		
		...
		if (title_bg != null)
			canvas.drawBitmap(title_bg, 0, bmpHeight + singleLineHeight / 2
					- title_bg.getHeight() / 2, null);
		...
		if (icn_bg != null) {
			...
			if (DefaultLayout.app_icon_size != R3D.workspace_cell_width
					&& DefaultLayout.app_icon_size != R3D.workspace_cell_height) {
				if (b != null) {
					iconbgPaddingTop = paddingTop
							- (icn_bg.getHeight() - Utilities.sIconTextureHeight)
							/ 2;
					if (iconbgPaddingTop < 0) {
						iconbgPaddingTop = 0;
					}
				}
				canvas.drawBitmap(icn_bg,
						(textureWidth - icn_bg.getWidth()) / 2,
						iconbgPaddingTop, null);
		...
		if(b != null && !b.isRecycled() && Icon3D.getMask() != null && icn_bg != null){
			saveLayer = canvas.saveLayer(0, 0, textureWidth, textureHeight, null,
		...
		if (b != null && !b.isRecycled()) 
		{
			...
			//xiatian add start	//add icon shadow
			if(
					(DefaultLayout.icon_shadow_radius != 0f)
					&&(icn_bg == null)
			)
			{
			...
			if (b.getHeight() != Utilities.sIconTextureHeight) {
				if (icn_bg != null) {
					paddingTop = iconbgPaddingTop
							- (b.getHeight() - Utilities.sIconTextureHeight)
							/ 2;
				} else {
			...
			if (b.getWidth() != Utilities.sIconTextureWidth) {
				paddingLeft = paddingLeft
						- (b.getWidth() - Utilities.sIconTextureWidth) / 2;
			...
			//xiatian add start	//adjust third apk icon offset when have iconbg
			if(icn_bg != null)
				drawThirdAPKIconByOffsetInTheme(
						b,
						iconbgRect,
						iconbgPaddingTop,
						paddingTop,
	...
	//teapotXu_20130128: add icon_cover_plate start
		if ((icn_bg != null) && icon_cover_plate_bmp != null) {
			if (DefaultLayout.app_icon_size != R3D.workspace_cell_width
					&& DefaultLayout.app_icon_size != R3D.workspace_cell_height) {
				if (b != null) {
					iconcoverplatePaddingTop = iconcoverplatePaddingTop
							- (icon_cover_plate_bmp.getHeight() - Utilities.sIconTextureHeight)
							/ 2;
					if (iconcoverplatePaddingTop < 0) {
						iconcoverplatePaddingTop = 0;
					}
				}
				canvas.drawBitmap(icon_cover_plate_bmp,
						(textureWidth - icon_cover_plate_bmp.getWidth()) / 2,
						iconcoverplatePaddingTop, null);
			} else {
				canvas.drawBitmap(icon_cover_plate_bmp, 0, 0, null);
			}
		}
	...
	//teapotXu_20130128: add icon_cover_plate end
		if (splitTitle != null && splitTitle.length > 0) {
			for (int i = 0; i < splitTitle.length; i++) {
				...
				if (textWidth > R3D.workspace_cell_width) {
					if (title_bg != null) {
						...
						if (textWidth > textureWidth - 2 * paddintText) {
							...
							int length = paint.breakText(splitTitle[i], 0,
									splitTitle[i].length(), true, textureWidth
											- 2 * paddintText, null);
							splitTitle[i] = splitTitle[i].substring(0,
									length + 1);
						}
					} else {
						paddintText = 0;
				...
				float titleY = bmpHeight - fontMetrics.ascent + i
						* singleLineHeight;
				paint.setColor(Color.WHITE);
				if (title_bg == null) {
					paint.setShadowLayer(SHADOW_LARGE_RADIUS, 0.0f,
				...
				if (textWidth > textureWidth && title_bg == null) {
					...
					for (int j = 0; j < alphaW; j++) {
						...
						canvas.drawLine(
								textureWidth - j - 1,
								(float) (titleY - Math.ceil(fontMetrics.descent
										- fontMetrics.ascent)), textureWidth
										- j, textureHeight, mErasePaint);
					}
				}
			}
		}
	...
	public static float getNumberWidth(int number) {
		String s = number + "";
		char c;
	...
	public static float getTitleHeight(float size, int line) {
		Paint paint = new Paint();
		paint.setTextSize(size);
		FontMetrics fontMetrics = paint.getFontMetrics();
		float singleLineHeight = (float) Math.ceil(fontMetrics.bottom
	...
	public static float getTitleWidth(String s, float size) {
		Paint paint = new Paint();
		paint.setTextSize(size);
	...
	public static float getIconBmpHeight() {
		...
		if (DefaultLayout.show_font_bg == true && Icon3D.titleBg != null) {
			return bmpHeight + paddingTop + space_height
					- (Icon3D.titleBg.getHeight() - singleLineHeight) / 2f;
		} else {
			return bmpHeight + paddingTop + space_height;
		}
	}

	public static String[] splitTitle(String title, float textSize,
			float maxWidth) {
		Paint paint = new Paint();
		paint.setAntiAlias(true);
	...
	public static Drawable createIconThumbnail(Drawable icon, int width,
			int height) {
	...
	//xiatian add start	//add icon shadow
	private static Bitmap addShadowToBitmap(Bitmap originalBitmap,boolean recycle)
	{
		...
		Bitmap ret = Bitmap.createBitmap(
				shadowBitmap32.getWidth(), 
				shadowBitmap32.getHeight() - 2*offsetXY[1],
				Config.ARGB_8888
				);
		
		Canvas c = new Canvas(ret);
		
		Rect srcShadow = new Rect(
				-offsetXY[0],
				-offsetXY[1],
				-offsetXY[0] + originalBitmap.getWidth(),
				shadowBitmap32.getHeight()
				);
		Rect dstShadow = new Rect(
				-offsetXY[0],
				-2*offsetXY[1],
				-offsetXY[0] + originalBitmap.getWidth(),
				-offsetXY[1] + shadowBitmap32.getHeight()
				); 
		c.drawBitmap(shadowBitmap32, srcShadow, dstShadow, shadowPaint);
		
		Rect srcOriginal = new Rect(
				0,
				0,
				originalBitmap.getWidth(),
				originalBitmap.getHeight()
				);
		Rect dstOriginal = new Rect(
				-offsetXY[0],
				-offsetXY[1],
				-offsetXY[0] + originalBitmap.getWidth(),
				-offsetXY[1] + originalBitmap.getHeight()
				); 
    ...
    public static void RGBA8888toARGB8888(byte []src,int []dest){
        int j=0;
    ...
    public static Bitmap setScreenshotBackground(Bitmap fore,Bitmap bg){
        Bitmap bmp = Bitmap.createBitmap(Utils3D.getScreenWidth(), Utils3D.getScreenHeight(),
                Config.ARGB_8888);
        Canvas canvas = new Canvas(bmp);
    ...
    public static String getScreenShot(Bitmap bg){
	...
	public static synchronized Bitmap TitleToBitmap(String title,Bitmap title_bg,
			int textureWidth, int fontSize,int color)
	{
		Paint paint = new Paint();
		paint.setAntiAlias(true);
		paint.setColor(color);
		...
		FontMetrics fontMetrics = paint.getFontMetrics();
		float  fontLineHeight = (float) Math.ceil(fontMetrics.descent
				- fontMetrics.ascent);
