...
public class AppList3D extends NPageBase implements MenuActionListener,
		DragSource3D, ClingManager.ClingTarget {
	...
	private NinePatch iconFocus = new NinePatch(R3D.findRegion("icon_focus"),
			20, 20, 20, 20);
	...
	public AppList3D(String name) {
		...
		sortId = PreferenceManager.getDefaultSharedPreferences(
				iLoongLauncher.getInstance()).getInt("sort_app", -1);
		...
		appGridPool = new GridPool(5, Utils3D.getScreenWidth(), (int) height,
				mCellCountX, mCellCountY);
		widgetGridPool = new GridPool(5, Utils3D.getScreenWidth(),
				(int) height, mWidgetCountX, mWidgetCountY);
	...
	public void syncAppsPageItems(int page, boolean immediate) {
		...
		for (int i = startIndex; i < endIndex; ++i) {
			...
			// teapotXu add start for Folder in Mainmenu
			if ((DefaultLayout.mainmenu_folder_function == true && getItemInfo(i) instanceof ApplicationInfo)
			...
			// teapotXu add end for Folder in Mainmenu
			{
				...
				if (icon != null) {
					...
					icon = new AppIcon3D(info.title.toString(),
			...
			// teapotXu add start for Folder in Mainmenu
				...
				Log.v("cooee",
						"---syncAppsPageItems--- cur icon is a folder, info is : "
								+ folderMap_key);
		...
		for (int i = startIndex; i < endIndex; i++) {
			if (i < getWidget3DCount()) {
				...
				if (widget instanceof Widget3DShortcut) {
					...
					widgetShortcut.setSize(layout.getCellWidth()
							- R3D.app_widget3d_gap, layout.getCellHeight()
					...
					widgetIcon.setSize(layout.getCellWidth()
							- R3D.app_widget3d_gap, layout.getCellHeight()
							- R3D.app_widget3d_gap);
				...
				if (widgetInfo == null)
					continue;
				Widget2DShortcut widgetShortcut = widget2DMap
						.get(widgetInfo.textureName);
	...
	public void syncWidget3DPageItems(int page, boolean immediate) {
		int numCells = mWidgetCountX * mWidgetCountY;
		int startIndex = page * numCells;
		...
		for (int i = startIndex; i < endIndex; i++) {
			...
			if (widget instanceof Widget3DShortcut) {
				...
				widgetIcon.setSize(
						layout.getCellWidth() - R3D.app_widget3d_gap,
						layout.getCellHeight() - R3D.app_widget3d_gap);
				widgetIcon.makeShortcut();
				if (mode == APPLIST_MODE_UNINSTALL)
					widgetIcon.showUninstall();
				else if (mode == APPLIST_MODE_HIDE)
	...
	public void syncWidget2DPageItems(int page, boolean immediate) {
		int numCells = mWidgetCountX * mWidgetCountY;
		int startIndex = page * numCells;
	...
	public void syncWidgetPageCount() {
		int numCells = mWidgetCountX * mWidgetCountY;
		widgetPageCount = (getWidget3DCount() + getWidget2DCount() + numCells - 1)
				/ numCells;
		 if (page_index >= appPageCount + widgetPageCount)
			 page_index = appPageCount + widgetPageCount - 1;
	}
	...
	public void syncWidget2DPageCount() {
		...
		if (page_index >= appPageCount + widget3DPageCount + widget2DPageCount)
			page_index = appPageCount + widget3DPageCount + widget2DPageCount
					- 1;
	}
	...
	public void startAnimation() {
		...
		while (iconIter.hasNext()) {
			...
			if (icon.getParent() != null) {
				if (!icon.oldVisible) {
					icon.setScale(0, 0);
					tween = icon.startTween(View3DTweenAccessor.SCALE_XY,
							Cubic.OUT, 0.8f, 1, 1, 0);
					icon.oldVisible = true;
				} else if (icon.oldAppGridIndex != icon.newAppGridIndex
						|| ((icon.oldX != icon.x || icon.oldY != icon.y) && icon.newAppGridIndex == page_index)) {
					// Log.d("launcher",
					// "anim:"+val.name+","+val.oldAppGridIndex+","+val.newAppGridIndex);
					float oldX = (icon.oldAppGridIndex - page_index)
							* Utils3D.getScreenWidth() + icon.oldX;
					float oldY = icon.oldY;
					float newX = (icon.newAppGridIndex - page_index)
							* Utils3D.getScreenWidth() + icon.x;
					float newY = icon.y;
					icon.setPosition(oldX, oldY);
					tween = icon.startTween(View3DTweenAccessor.POS_XY,
							Cubic.OUT, 0.8f, newX, newY, 0);
					icon.oldX = newX;
					icon.oldY = newY;
		...
		while (widgetIter.hasNext()) {
			...
			if (val.getParent() != null) {
				if (!val.oldVisible) {
					val.setScale(0, 0);
					val.startTween(View3DTweenAccessor.SCALE_XY, Cubic.OUT,
							0.8f, 1, 1, 0);
					val.oldVisible = true;
				} else if (val.oldAppGridIndex != val.newAppGridIndex
						|| ((val.oldX != val.x || val.oldY != val.y) && val.newAppGridIndex == page_index)) {
					// Log.d("launcher",
					// "anim:"+val.name+","+val.oldAppGridIndex+","+val.newAppGridIndex);
					float oldX = (val.oldAppGridIndex - page_index)
							* Utils3D.getScreenWidth() + val.oldX;
					float oldY = val.oldY;
					float newX = (val.newAppGridIndex - page_index)
							* Utils3D.getScreenWidth() + val.x;
					float newY = val.y;
					val.setPosition(oldX, oldY);
					val.startTween(View3DTweenAccessor.POS_XY, Cubic.OUT, 0.8f,
							newX, newY, 0);
					val.oldX = newX;
					val.oldY = newY;
			...
			if (view instanceof Widget3DShortcut) {
				...
				if (widget.getParent() != null) {
					...
					if (!widget.oldVisible) {
						widget.setScale(0, 0);
						widget.startTween(View3DTweenAccessor.SCALE_XY,
								Cubic.OUT, 0.5f, 1, 1, 0);
						widget.oldVisible = true;
					} else if (widget.oldAppGridIndex != widget.newAppGridIndex
							|| ((widget.oldX != widget.x || widget.oldY != widget.y) && widget.newAppGridIndex == page_index)) {
						float oldX = (widget.oldAppGridIndex - page_index)
								* Utils3D.getScreenWidth() + widget.oldX;
						float oldY = widget.oldY;
						float newX = (widget.newAppGridIndex - page_index)
								* Utils3D.getScreenWidth() + widget.x;
						float newY = widget.y;
						widget.setPosition(oldX, oldY);
						widget.startTween(View3DTweenAccessor.POS_XY,
								Cubic.OUT, 0.5f, newX, newY, 0);
						widget.oldX = newX;
						widget.oldY = newY;
						widget.oldAppGridIndex = widget.newAppGridIndex;
					}
	...
	public void onEvent(int type, BaseTween source) {
		...
		// teapotXu add start for Folder in Mainmenu
			...
			if (userData != null && userData instanceof Icon3D) {
				//
				ViewGroup3D cur_gridview = (ViewGroup3D) view_list
						.get(page_index);
	...
	public void setApps(ArrayList<ApplicationInfo> list) {
		...
		ApplistVirtualIconManager virtualIconConfig = new ApplistVirtualIconManager(
				this);
		virtualIconConfig.insertVirtualIcon();

	}

	public void setFolders(ArrayList<FolderInfo> list) {
		if (inited) {
	...
	public void setWidget3D() {
		...
		List<Widget3DShortcut> mWidgetList = Widget3DManager.getInstance()
				.getWidgetList();
	...
	private ApplicationInfo getApp(int position) {
		if (mode == APPLIST_MODE_HIDE) {
			if (sortArray == null)
		...
		if (mode == APPLIST_MODE_HIDE)
			...
			for (int i = 0; i < mWidget3DList.size(); i++) {
				if (mWidget3DList.get(i) instanceof Widget3DShortcut) {
					Widget3DShortcut shortcut = (Widget3DShortcut) mWidget3DList
							.get(i);
					if (!shortcut.isHideWidget)
						widgetCount++;
				} else if (mWidget3DList.get(i) instanceof Widget3DVirtual) {
					Widget3DVirtual shortcut = (Widget3DVirtual) mWidget3DList
							.get(i);
					if (!shortcut.isHide)
						widgetCount++;
				}
	...
	private View3D getWidget3D(int position) {
		if (mode == APPLIST_MODE_HIDE) {
	...
	private WidgetShortcutInfo getWidget2D(int position) {
		if (mode == APPLIST_MODE_HIDE) {
	...
	public void setMode(int mode) {
		if (this.mode != mode) {
			...
			// syncWidget3DPages();
			// }
			if (this.mode == APPLIST_MODE_HIDE
					&& mode == APPLIST_MODE_UNINSTALL) {
				this.mode = mode;
				refresh();
			} else {
				this.mode = mode;
				...
				if (DefaultLayout.mainmenu_folder_function == true) {
					...
					if (this.is_maimenufolder_open
							&& this.mOpenFolderIcon != null) {
						String folderMap_key = ICON_MAP_FOLDER_LABEL
								+ mOpenFolderIcon.getItemInfo().id;
						FolderIcon3D folderIcon = folderInfoMap
								.get(folderMap_key);
				...
				if (iLoongApplication.BuiltIn
						&& !DefaultLayout.hide_mainmenu_widget)
					syncWidgetPages();
				// syncWidget3DPages();
				// syncWidget2DPages();
				startAnimation();
	...
	public void draw(SpriteBatch batch, float parentAlpha) {
		...
		if (DefaultLayout.blend_func_dst_gl_one) {
			...
			if (srcBlendFunc != GL11.GL_SRC_ALPHA
					|| dstBlendFunc != GL11.GL_ONE)
		...
		if (translucentBg != null) {
			batch.setColor(
					...
					color.a
							* parentAlpha
							* (height + y - (DefaultLayout.applist_style_classic ? R3D.hot_obj_height
									: 0)) / height);
		...
		if (!firstlyCome && page_index != appPageCount) {
			...
			iconFocus
					.draw(batch,
							locationX
									- (R3D.Workspace_cell_each_width - R3D.workspace_cell_width)
									/ 2,
							locationY
									+ (R3D.workspace_cell_height - R3D.Workspace_cell_each_height)
									/ 2, R3D.Workspace_cell_each_width,
	...
	public boolean onCtrlEvent(View3D sender, int event_id) {
		if (sender instanceof Widget3DVirtual) {
			...
			case Widget3DVirtual.MSG_WIDGET3D_SHORTCUT_LONGCLICK: {
				String className = DefaultLayout
						.getWidgetItemClassName(info.intent.getComponent()
								.getPackageName());
				Widget3D dragObj = Widget3DManager.getInstance().getWidget3D(
						info.intent.getComponent().getPackageName(), className);
				if (dragObj == null)
					return true;
				dragObj.setPosition(DragLayer3D.dragStartX - dragObj.width / 2,
						DragLayer3D.dragStartY - dragObj.height / 2);
				clearDragObjs();
				dragObjects.clear();
				dragObjects.add(dragObj);
				this.setTag(new Vector2(dragObj.x, dragObj.y));
				Vector2 v = (Vector2) widget.getTag();
				dragObj.setPosition(v.x - dragObj.width / 2, v.y
						- dragObj.height / 2);
				releaseFocus();
				return viewParent
			...
			case Icon3D.MSG_ICON_LONGCLICK:
				...
				for (View3D view : selectedObjects) {
					if (view instanceof Icon3D) {
						...
						// teapotXu add start for Folder in Mainmenu
						if (DefaultLayout.mainmenu_folder_function == true
								&& this.mode == APPLIST_MODE_UNINSTALL) {
			...
			case Widget2DShortcut.MSG_WIDGET_SHORTCUT_LONGCLICK:
				...
				Vector2 v = (Vector2) widget.getTag();
				widgetClone.setPosition(v.x - widgetClone.width / 2, v.y
						- widgetClone.height / 2);
				releaseFocus();
				return viewParent
						.onCtrlEvent(this, DragSource3D.MSG_START_DRAG);
			}
	...
	public void reomveApps(ArrayList<ApplicationInfo> list, boolean permanent) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (DefaultLayout.mainmenu_folder_function == true) {
				...
				if (mItemInfos.contains(info)) {
					...
					if (!is_removed) {
						Log.e("cooee",
								" error -- AppList3D---reomveApps --- removeAppWhenInFolder --- return false---");
					}
				}

				// if mainmenu folder is open, close the folder first
				if (this.is_maimenufolder_open == true
						&& mOpenFolderIcon != null) {
					String folderMap_key = ICON_MAP_FOLDER_LABEL
							+ mOpenFolderIcon.getItemInfo().id;
					FolderIcon3D folderIcon = folderInfoMap.get(folderMap_key);
	...
	private boolean removeAppWhenInFolder(ApplicationInfo info) {
		...
		for (int index = 0; index < mItemInfos.size(); index++) {
			...
			if (itemInfo instanceof UserFolderInfo) {
				...
				for (ShortcutInfo folder_sInfo : folderInfo.contents) {
					...
					if (appInfo != null
							&& (appInfo.packageName != null && folder_sInfo.appInfo.packageName
									.equals(info.packageName))
							&& (appInfo.componentName != null && folder_sInfo.appInfo.componentName
						...
						// then remove the info in UserFolderInfo
						{
							...
							if (folderIcon != null) {
								...
								Log.e("cooee",
										" error --- AppList3D--- removeAppWhenInFolder --- cannot find FolderIcon ---");
							}
		...
		for (int i = 0; i < mWidget3DList.size(); i++) {
			...
			if (view instanceof WidgetIcon) {
				...
				if (info.intent.getComponent().getPackageName()
						.equals(widget.resolve_info.activityInfo.packageName)) {
	...
	public void removeWidget(String packageName) {
		...
		for (int i = 0; i < mWidget3DList.size(); i++) {
			...
			if (view instanceof Widget3DVirtual) {
				...
				Widget3DShortcut widget = (Widget3DShortcut) view;
				if (widget.resolve_info != null
						&& widget.resolve_info.activityInfo.packageName
								.equals(packageName)) {
					widget.removeHide();
					mWidget3DList.remove(view);
					widgetIconPosition = i;
		...
		if (widgetIconPosition != -1) {
			if (DefaultLayout.GetDefaultWidgetImage(packageName) != null) {
				String imageName = DefaultLayout.THEME_WIDGET_APPLIST
						+ DefaultLayout.GetDefaultWidgetImage(packageName);
				...
				info.intent.setComponent(new ComponentName(packageName,
						packageName));
				info.spanX = spanX;
				info.spanY = spanY;
		...
		if (!ret) {
			...
			if (DefaultLayout.enable_particle) {
				if (ParticleManager.particleManagerEnable) {
					startParticle(
							ParticleManager.PARTICLE_TYPE_NAME_CLICK_WORKSPACE,
							x, y);
				}
			}
	...
	public void show() {
		...
		if (DefaultLayout.mainmenu_folder_function == true) {
			if ((mItemInfos == null || mItemInfos.size() == 0) && toastLoad) {
				toastLoad = false;
				SendMsgToAndroid.sendCircleToastMsg(iLoongLauncher
						.getInstance().getResources()
						.getString(RR.string.applist_load_toast));
			}
		} else {
	...
	public void hide() {
		// teapotXu add start for Folder in Mainmenu
		if (DefaultLayout.mainmenu_folder_function == true
				&& this.applist_hide_reason == this.APP_HIDE_REASON_FOR_FOLDER_OPEN) {
			Log.v("cooee",
					"AppList3d ----- hide() -----app list hide because folder in Mainmenu wants to open ");
		} else
	...
	public void hideFocus(int page) {
	 ...
	 * @param :
	 * **/
	public void getFocusLocation(int focus) {
		// teapotXu add start for Folder in Mainmenu
		View3D icon = null;
		if (DefaultLayout.mainmenu_folder_function == true) {
	...
	public void onKeySelect() {
		if (hideFocus == false) {
			...
			if (DefaultLayout.mainmenu_folder_function == true) {
				ItemInfo info = getItemInfo(currentCell);
				if (info instanceof ApplicationInfo) {
					ShortcutInfo sInfo = ((ApplicationInfo) info)
							.makeShortcut();
					String infoName = R3D.getInfoName(sInfo);
					icon = iconMap.get(infoName);
				} else if (info instanceof FolderInfo) {
	...
	public void onKeyEven(int page) {
	...
	protected void updateFocus(int pageIndex, int direction) {
		int numCells = mCellCountX * mCellCountY;
	...
	public int estimateWidgetCellWidth(int cellHSpan) {
		if (cellHSpan > 4)
			cellHSpan = 4;
		float widgetWidth = (float) (width - R3D.applist_padding_left - R3D.applist_padding_right)
				/ ((float) mWidgetCountX) - R3D.app_widget3d_gap;
	...
	public int estimateWidgetCellHeight(int cellVSpan) {
		if (cellVSpan > 4)
			cellVSpan = 4;
		float widgetHeight = ((float) (height - R3D.applist_padding_top - R3D.applist_padding_bottom)
				/ ((float) mWidgetCountY) - R3D.app_widget3d_gap)
				* (1 - R3D.widget_preview_title_weight);
		float widgetCellHeight = widgetHeight / ((float) 4);
		return (int) (widgetCellHeight * ((float) cellVSpan));
	}

	public void sortApp(int checkId, boolean refresh) {
		if (!Desktop3DListener.bAppDone)
			return;
		if (checkId != sortId) {
			sortId = checkId;
			PreferenceManager
					...
					.edit().putInt("sort_app", sortId).commit();
		}
		// teapotXu add start for Folder in Mainmenu
		if (DefaultLayout.mainmenu_folder_function == true) {
			...
			if (DefaultLayout.mainmenu_folder_function == true) {
				...
				for (int i = 0; i < mItemInfos.size(); i++) {
					...
					if (mItemInfos.get(i) instanceof ApplicationInfo) {
						...
						priorityKey[i] = false;
						if ((appInfo.intent.getBooleanExtra(
										"isApplistVirtualIcon", false) == true)) {
							titleKey[i] = "0000000";
							priorityKey[i] = true;
						} else {

							titleKey[i] = appInfo.title.toString()
									.replaceAll(" ", "").replaceAll(" ", "");// 注意！！两个空格不一�?
						...
						if (((FolderInfo) itemInfo).sort_folder_name != null) {
							titleKey[i] = ((FolderInfo) itemInfo).sort_folder_name
									.toString().replaceAll(" ", "")
									.replaceAll(" ", "");// 注意！！两个空格不一�?

							if (((FolderInfo) itemInfo).sort_folder_name
									.toString().contains("_aaaa")) {
								priorityKey[i] = true;
							} else {
								priorityKey[i] = false;
							}
							...
							titleKey[i] = ((FolderInfo) itemInfo).title
									.toString().replaceAll(" ", "")
									.replaceAll(" ", "");// 注意！！两个空格不一�?
						}
					}
				...
				for (int i = 0; i < mApps.size(); i++) {
					...
					priorityKey[i] = false;
					if ((mApps.get(i).intent.getBooleanExtra(
									"isApplistVirtualIcon", false) == true)) {
						titleKey[i] = "0000000";
						priorityKey[i] = true;
					} else {

						titleKey[i] = mApps.get(i).title.toString()
								.replaceAll(" ", "").replaceAll(" ", "");// 注意！！两个空格不一�?
						if (DefaultLayout.getInstance().hasReplaceIcon(
								mApps.get(i).componentName.getPackageName(),
								mApps.get(i).componentName.getClassName())) {
							priorityKey[i] = true;
						}
					}
				}
		...
		case SORT_NAME:
			...
			if (DefaultLayout.mainmenu_folder_function == true) {
				...
				for (int i = 0; i < mItemInfos.size(); i++) {
					...
					if (itemInfo instanceof ApplicationInfo) {
						if (((ApplicationInfo) itemInfo).intent
										.getBooleanExtra(
												"isApplistVirtualIcon", false) == true)
							nameKey[i] = "0000000";
						else

							nameKey[i] = itemInfo.title.toString()
									.replaceAll(" ", "").replaceAll(" ", "");// 注意！！两个空格不一�?
					} else {
						// when select item is a folder
						nameKey[i] = ((FolderInfo) itemInfo).title.toString()
			...
			// teapotXu add end for Folder in Mainmenu
			{
				...
				for (int i = 0; i < mApps.size(); i++) {
					...
					if ((mApps.get(i).intent.getBooleanExtra(
						...
						nameKey[i] = mApps.get(i).title.toString()
								.replaceAll(" ", "").replaceAll(" ", "");// 注意！！两个空格不一�?
				}
				cut.sortByAlpha(1, nameKey, sortArray);
			}
				...
				for (int i = 0; i < mItemInfos.size(); i++) {
					...
					if (itemInfo instanceof ApplicationInfo) {
						if ((((ApplicationInfo) itemInfo).intent
							...
							useKey[i] = (int) (((ApplicationInfo) itemInfo)
									.getUseFrequency() / 1000);
					} else {
						// when select item is a folder
						if (itemInfo instanceof FolderInfo) {
			...
			if (DefaultLayout.mainmenu_folder_function == true) {
				...
				for (i = 0; i < mItemInfos.size(); i++) {
					...
					for (j = 0; j < facAppNum; j++) {
						for (int k = 0; k < DefaultLayout.facApp.get(j).pkgNameArray
								.size(); k++) {

							ItemInfo appInfo = (ItemInfo) mItemInfos.get(i);
							if (appInfo instanceof ApplicationInfo) {
								if (((ApplicationInfo) appInfo).packageName
										.equals(DefaultLayout.facApp.get(j).pkgNameArray
												.get(k))) {
									if (DefaultLayout.facApp.get(j).className == null
											|| ((ApplicationInfo) appInfo).componentName
													.getClassName()
													.equals(DefaultLayout.facApp
															.get(j).className)) {
					...
					if (j == facAppNum) {
						...
						if (mItemInfos.get(i) instanceof ApplicationInfo) {
							ApplicationInfo appInfo = (ApplicationInfo) mItemInfos
									.get(i);
							if ((appInfo.intent.getBooleanExtra(
											"isApplistVirtualIcon", false) == true))
								facKey[i] = 0;
				...
				for (i = 0; i < mApps.size(); i++) {
					...
					for (j = 0; j < facAppNum; j++) {
						for (int k = 0; k < DefaultLayout.facApp.get(j).pkgNameArray
								.size(); k++) {
							if (mApps.get(i).packageName
									.equals(DefaultLayout.facApp.get(j).pkgNameArray
											.get(k))) {
								if (DefaultLayout.facApp.get(j).className == null
										|| mApps.get(i).componentName
												.getClassName()
												.equals(DefaultLayout.facApp
														.get(j).className)) {
									facKey[i] = j - facAppNum;
		...
		if (xScale > 0) {
			...
			if (mScrollToWidget) {
				if (page_index == 0 || page_index >= appPageCount) {
					APageEase.updateEffect(pre_view, cur_view, xScale - 1,
							tempYScale, mTypelist.get(0));

					} else
						APageEase.updateEffect(pre_view, cur_view, xScale - 1,
								tempYScale, mTypelist.get(mType));
				} else {
					if (page_index >= appPageCount) {
						APageEase.updateEffect(pre_view, cur_view, xScale - 1,
								tempYScale, mTypelist.get(0));
			...
			if (mScrollToWidget) {
				if (page_index >= appPageCount - 1
						&& page_index <= appPageCount + widgetPageCount - 1) {
					APageEase.updateEffect(cur_view, next_view, xScale,
							tempYScale, mTypelist.get(0));
				} else
					APageEase.updateEffect(cur_view, next_view, xScale,
							tempYScale, mTypelist.get(mType));
	...
	public boolean onTouchDown(float x, float y, int pointer) {
		...
		if (DefaultLayout.mainmenu_folder_function == true) {
			if (syncAppAnimFinished == false) {
				Log.v("cooee",
						"AppList3D---- onTouchDown--- invalid ---because syncApp's animation is running");
				return true;
			}
		}
		// teapotXu add end for Folder in Mainmenu
	...
	class GridPool {
		...
		public GridPool(int initCapacity, float width, float height,
		...
		private GridView3D create() {
			GridView3D grid = new GridView3D("allapplist", width, height,
					countX, countY);
			grid.setPadding(R3D.applist_padding_left,
					R3D.applist_padding_right, R3D.applist_padding_top,
					R3D.applist_padding_bottom);
			if (mCellCountX == 3) {
				grid.setPadding(R3D.applist_padding_left_ex,
						R3D.applist_padding_right_ex, R3D.applist_padding_top,
						R3D.applist_padding_bottom);
			}
			return grid;
		}
	...
	class AppIcon3D extends Icon3D {
		...
		public AppIcon3D(String name, TextureRegion region) {
			super(name, region);
	...
	public void resume() {
		...
		if (DefaultLayout.mainmenu_folder_function == true) {
			...
			if (this.is_maimenufolder_open && this.mOpenFolderIcon != null) {
				Log.d("cooee",
						"AppList3D ----- resume() ----- donot sysncAppsPages  because folder in Mainmenu is open !----");
				return;
			}
		}
		// teapotXu add end for Folder in Mainmenu
	...
	public void addAppNoRepeat(ArrayList<ApplicationInfo> apps,
	...
	// teapotXu add start for Folder in Mainmenu
	public void addAppIntoItemInfoNoRepeat(ArrayList<ItemInfo> itemInfos,
			ArrayList<ApplicationInfo> adds) {
		for (int i = 0; i < adds.size(); i++) {
	...
	public void addItemIntoItemInfoNoRepeat(ArrayList<ItemInfo> itemInfos,
	...
	public void addApps(ArrayList<ApplicationInfo> list, boolean isSortApps,
	...
	public void removeVirtualApps(ApplicationInfo app, boolean permanent,
			boolean isSortApps, boolean isSynchronization) {
		// teapotXu add start for Folder in Mainmenu
		if (DefaultLayout.mainmenu_folder_function == true) {
			for (int i = 0; i < mItemInfos.size(); i++) {
				ItemInfo itemInfo = mItemInfos.get(i);
				...
				if (itemInfo instanceof ApplicationInfo) {
					if (((ApplicationInfo) itemInfo).packageName
							.equals(app.packageName)
							&& ((ApplicationInfo) itemInfo).intent
									.getBooleanExtra("isApplistVirtualIcon",
											false)) {
						mItemInfos.remove(i);
						break;
					}
			...
			for (int i = 0; i < mApps.size(); i++) {
				if (mApps.get(i).packageName.equals(app.packageName)
						&& mApps.get(i).intent.getBooleanExtra(
								"isApplistVirtualIcon", false)) {
					mApps.remove(i);
					break;
				}
			}
	...
	public void addVirtualApps(ArrayList<ApplicationInfo> list) {
		// teapotXu add start for Folder in Mainmenu
		if (DefaultLayout.mainmenu_folder_function == true) {
			addAppIntoItemInfoNoRepeat(mItemInfos, list);
		} else
	...
	public void removeVirtualApps(ArrayList<ApplicationInfo> apps) {
	...
	private void refreshBg() {
		...
		if (mainmenu_bg_value.equals("-1")) {
			PreferenceManager
					...
					.edit()
					.putString(mainmenu_bg_key,
							DefaultLayout.defaultMainmenuBgIndex + "").commit();
		...
		} else if ((mainmenu_bg_value
				.equals(DefaultLayout.lastAppListMainmenuBgIndex + ""))
				&& (translucentBg != null)) {
			return;
		} else {
			DefaultLayout.lastAppListMainmenuBgIndex = Integer.valueOf(
					mainmenu_bg_value).intValue();
		}
	...
	public boolean addDrageViewIntoAppList(ArrayList<View3D> list, int x, int y) {
		// TODO Auto-generated method stub
		boolean res = true;
		ItemInfo itemInfo;
			...
			for (View3D i : list) {
															...
															// == true)
				{
					...
					if (icon.info != null && icon.info instanceof ShortcutInfo) {
						...
						ApplicationInfo view_applicationInfo = ((ShortcutInfo) ((Icon3D) i)
								.getItemInfo()).appInfo;
					...
					for (int pageIndex = 0; pageIndex < appPageCount; pageIndex++) {
						GridView3D layout = (GridView3D) view_list
								.get(pageIndex);
						...
						for (int iconIdx = 0; iconIdx < layout.getChildCount(); iconIdx++) {
							...
							if (icon instanceof Icon3D
								...
								icon.setPosition(icon_cur_pos_x < 0 ? 0f
										: icon_cur_pos_x,
										icon_cur_pos_y < 0 ? 0f
												: icon_cur_pos_y);
								...
								if (pageIndex == page_index) {
									// 如果icon只是在本页中移动
									dragIconBackTween = icon.startTween(
											View3DTweenAccessor.POS_XY,
											Linear.INOUT, 0.3f,
											icon_original_posX,
											icon_original_posY, 0).setCallback(
											this);
								} else {
									// icon已经移动其他页面上，
									float target_x = 0f;
									...
									ViewGroup3D cur_grid_view = (ViewGroup3D) view_list
											.get(page_index);

									icon_anim.setPosition(icon.getX(),
											icon.getY());
									cur_grid_view.addView(icon_anim);

									dragIconBackTween = icon_anim
											.startTween(
													View3DTweenAccessor.POS_XY,
													Linear.INOUT, 0.3f,
													target_x, target_y, 0)
		...
		case CELL_DROPTYPE_SINGLE_DROP_FOLDER:
			...
			if (mTargetCellIndex >= cur_view.getChildCount()) {
				Log.v("cooee",
						"------cooee------applist3D--- addDragView --- CELL_DROPTYPE_SINGLE_DROP_FOLDER --- mTargetCellIndex = "
								+ mTargetCellIndex);
				break;
			}
			...
			if (quiet_appInfo != null) {
				...
				if (DefaultLayout.getInstance().hasReplaceIcon(
						quiet_appInfo.componentName.getPackageName(),
						quiet_appInfo.componentName.getClassName())) {
			...
			if (icon.info != null && icon.info instanceof ShortcutInfo) {
				AppListDB.getInstance().addOrMoveItem(folderInfo,
						LauncherSettings.Favorites.CONTAINER_APPLIST);
				iLoongLauncher.getInstance()
						.addFolderInfoToSFolders(folderInfo);
				...
				// 获取到quiet view 对应的ApplicationInfo
				ApplicationInfo quiet_applicationInfo = ((ShortcutInfo) icon
						.getItemInfo()).appInfo;
				ApplicationInfo view_applicationInfo = ((ShortcutInfo) ((Icon3D) view)
						.getItemInfo()).appInfo;
				...
				AddFolderAndRemoveAppInApps(folderInfo, remove_appList,
						quiet_apps_idx);
		...
		if (iconGroupInput.size() > 1) {
			...
			if (dropRes) {
				...
				if (x != tx || y != ty) {
					...
					view.startTween(View3DTweenAccessor.POS_XY, Cubic.OUT,
							0.5f, tx, ty, 0).setCallback(this);
				}
	...
	public boolean onDropOver(ArrayList<View3D> list, float x, float y) {
		...
		if (list.size() > 0) {
			...
			if (list.size() > 1) {
				...
				if (view instanceof ViewCircled3D) {
					ViewGroup3D cur_view = (ViewGroup3D) view_list
							.get(page_index);
	...
	public void setAppListHideReason(int hide_reason) {
	...
	private int findNearestCellIndex(int pixelX, int pixelY) {
		...
		if (cur_view_group instanceof GridView3D) {
			nearestCellIndex = ((GridView3D) cur_view_group).getIndex(pixelX,
					pixelY);
			...
			if (DefaultLayout.enhance_generate_mainmenu_folder_condition == true) {
				...
				if (nearestCellIndex < cur_view_size && nearestCellIndex >= 0) {
					View3D nearestCellView = cur_view_group
							.getChildAt(nearestCellIndex);
					float avail_range_radius = nearestCellView.getWidth() / 4;
					float cell_view_center_x = nearestCellView.getX()
							+ nearestCellView.getWidth() / 2;
					float cell_view_center_y = nearestCellView.getY()
							+ nearestCellView.getHeight() / 2;

					if (pixelX < (cell_view_center_x - avail_range_radius)
							|| pixelX > (cell_view_center_x + avail_range_radius)
							|| pixelY < (cell_view_center_y - avail_range_radius)
							|| pixelY > (cell_view_center_y + avail_range_radius)) {
						nearestCellIndex = -1;
						Log.v("cooee",
								" AppList3D ---- findNearestCellIndex ----- current position is not in the availible area of "
										+ nearestCellView.name);
					}
				}
			}
		}
	...
	private int GetViewIsOccupy(View3D view, int item_index) {
		...
		if (item_index < cur_view.getChildCount()) {
			...
			if (cur_view_item instanceof Icon3D) {
				ShortcutInfo cur_view_item_sInfo = (ShortcutInfo) ((Icon3D) cur_view_item)
						.getItemInfo();
				ShortcutInfo view_sInfo = (ShortcutInfo) ((Icon3D) view)
						.getItemInfo();

				Log.v("cooee",
						"------Applist3D ---GetViewIsOccupy---- source view = "
								+ view.name);
				Log.v("cooee",
						"------Applist3D ---GetViewIsOccupy---- cur_view_item = "
								+ cur_view_item.name);

				if (cur_view_item.name.equals(view.name)
						&& (view_sInfo.intent.getAction().equals(
								cur_view_item_sInfo.intent.getAction()) && view_sInfo.intent
								.getComponent().equals(
										cur_view_item_sInfo.intent
												.getComponent()))) {
					// the same icon, do nothing
					Log.v("cooee",
							"------Applist3D ---GetViewIsOccupy---- this two view is the same ---- ");
					drop_status = ICON_DROP_OVER_NONE;
				} else {
					Log.v("cooee",
							"------Applist3D ---GetViewIsOccupy---- they are different--- cause a folder ---- ");
				...
				Log.v("cooee",
						"------Applist3D ---GetViewIsOccupy---- cur_view_item is a folder ---- error case ---- ");

				drop_status = ICON_DROP_OVER_NONE;
			}
		} else {
			Log.v("cooee",
					"------Applist3D ---GetViewIsOccupy---- index is over flow ---- error case ---- ");

			drop_status = ICON_DROP_OVER_NONE;
		}

		Log.v("cooee",
				"------Applist3D ---GetViewIsOccupy---- exit ---- drop_status ---- "
						+ drop_status);
	...
	private View3D cellMakeFolder(int item_index, boolean preview,
			boolean compress) {
		...
		folder.x = cur_view.getChildAt(item_index).getX()
				+ (app_icon_width - folder.width) / 2;
		folder.y = cur_view.getChildAt(item_index).getY()
				+ (app_icon_height - folder.height) / 2;
		// Log.d("launcher",
	...
	private View3D createVirturFolder(boolean compress) {
		UserFolderInfo folderInfo = new UserFolderInfo();
	...
	private void AddFolderAndRemoveAppInApps(ItemInfo folderInfo,
			ArrayList<ApplicationInfo> removedAppList, int insert_idx) {
	...
	private void resetAppIconsStatusInFolder(FolderIcon3D folderIcon) {
		// 需要重置Folder中的ICon的状态
		...
		for (View3D dragview : removedViewList) {
			if (dragview != null && dragview instanceof Icon3D) {
				...
				if (dragItemInfo != null
						&& dragItemInfo instanceof ShortcutInfo) {
	...
	public void addBackInScreen(View3D child, int x, int y) {
		...
		if (child instanceof DropTarget3D) {
			...
			if (child instanceof FolderIcon3D
					&& ((FolderIcon3D) child).getFolderIconNum() == 0) {
		...
		if (child instanceof Icon3D && ((Icon3D) child).getItemInfo() != null) {
			ShortcutInfo sItemInfo = (ShortcutInfo) ((Icon3D) child)
					.getItemInfo();

			if (sItemInfo.appInfo != null
					&& !mItemInfos.contains(sItemInfo.appInfo)) {
