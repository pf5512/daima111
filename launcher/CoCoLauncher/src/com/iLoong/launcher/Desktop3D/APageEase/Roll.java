...
public class Roll {
	...
	public static void updateEffect(ViewGroup3D cur_view,ViewGroup3D next_view,float degree, float yScale, float this_width,boolean scroll_right_to_left)
	{
...
//    	}
	...
	//
	private static void drawRollEffect(ViewGroup3D c_view,ViewGroup3D n_view, float degree, boolean roll_from_right_to_left)
	{
		...
		if(grid_items_padding_right < 0)
		{
			...
			{
				Vector2 icon_0_old_pos = (Vector2)c_view.getChildAt(0).getTag();
				Vector2 icon_1_old_pos = (Vector2)c_view.getChildAt(1).getTag();
		...
		for(int index=0;index < roll_stage_index;index++)
		{
			for(int c_row=0;c_row<CUR_VIEW_ROWS;c_row++)
			{
				...
				if(false == roll_from_right_to_left)
				{
					...
					{
						//此为翻转的最后一列 || 第一列最后一次翻转
						view_left_padding = ((Vector2)(c_view.getChildAt(0).getTag())).x + 2.0f;
					}					
				}
				else
				{
		...
		for(int keepIdx = roll_stage_index;keepIdx<CUR_VIEW_COLUMNS;keepIdx++)
		{
			for(int c_row=0;c_row<CUR_VIEW_ROWS;c_row++)
			{
				...
				{
					item = c_view.getChildAt(item_index);
					Vector2 old_pos = (Vector2)item.getTag();
	...
	private static void rollActionStage(View3D item, int stage_index, float cur_ratio,float view_left_padding,float item_padding_right,boolean roll_from_right_to_left)
	{
		Vector2 old_pos;
