...
public class Snake {
	...
	public static void updateEffect(ViewGroup3D cur_view,ViewGroup3D next_view,float degree, float yScale, float this_width)
	{
		...
		float item_begin_pos_x = ((Vector2)(cur_view.getChildAt(0).getTag())).x;
		float item_begin_pos_y = ((Vector2)(cur_view.getChildAt(0).getTag())).y;
		float item_end_pos_x = 0f;
		float item_end_pos_y = 0f;
		float item_width = cur_view.getChildAt(0).getWidth();
		float item_height = cur_view.getChildAt(0).getHeight();
		float item_padding_x = 0.0f;
		float item_padding_y = 0.0f;
		float CUR_VIEW_GROUP_WIDTH = cur_view.getWidth();
...
//		Log.v("cooee","-----degree = " + degree);
		...
		{
			item_padding_x = ((Vector2)(cur_view.getChildAt(1).getTag())).x - item_begin_pos_x - item_width;
		}
		else if(1 < next_view.getChildCount())
		{
			item_padding_x = ((Vector2)(next_view.getChildAt(1).getTag())).x - item_begin_pos_x - item_width;
		}
...
//			item_padding_x = 0;
		...
		{
			item_padding_y = item_begin_pos_y - ((Vector2)(cur_view.getChildAt(cur_countX).getTag())).y  - item_height;
		}
		else if(next_countX < next_view.getChildCount())
		{
			item_padding_y = item_begin_pos_y - ((Vector2)(next_view.getChildAt(next_countX).getTag())).y - item_height;
		}
...
//			item_padding_y = 0;		
		...
		{
			item_end_pos_x = ((Vector2)(cur_view.getChildAt(cur_countX-1).getTag())).x;
		}
		else if(next_countX -1 < next_view.getChildCount())
		{
			item_end_pos_x = ((Vector2)(next_view.getChildAt(cur_countX-1).getTag())).x;
		}
		else
		{
		...
		if((cur_countY-1)*cur_countX < cur_view.getChildCount())
		{
			...
			item_end_pos_y = ((Vector2)(cur_view.getChildAt(cur_countX*(cur_countY-1)).getTag())).y;
		}
		else if((next_countY-1)*next_countX < next_view.getChildCount())
		{
			item_end_pos_y = ((Vector2)(next_view.getChildAt((next_countY-1)*next_countX).getTag())).y;
		}
		else
		{
...
//    	}
	...
	private static void SnakeEffectMovingRight2Left(ViewGroup3D cur_view, float degree,
			                                        float item_begin_pos_x, float item_begin_pos_y,
			                                        float item_end_pos_x, float item_end_pos_y,
			                                        float item_padding_x,float item_padding_y,
...
//					float icon_pos_x = 0f;
	...
	private static void SnakeEffectMovingLeft2Right(ViewGroup3D cur_view, float degree,
										            float item_begin_pos_x, float item_begin_pos_y,
										            float item_end_pos_x, float item_end_pos_y,
										            float item_padding_x,float item_padding_y,
										            float icon_total_moving_distance)
