...
public class TrashIcon3D extends View3D implements DropTarget3D {
	...
	public TrashIcon3D(String name) {
		super(name);
	...
	public TrashIcon3D() {
		...
		if (DefaultLayout.hotseat_hide_title && DefaultLayout.trash_icon_pos!=TrashIcon3D.TRASH_POS_TOP)
		{
			...
			float V2 =normal.getV()
					+ (normal.getV2() - normal.getV()) * scale;
			normal.setV2(V2);
			
			float V3 = focus.getV()
						+ (focus.getV2() - focus.getV()) * scale;
			focus.setV2(V3);
		}
	...
	public TrashIcon3D(String name, Texture texture) {
		super(name, texture);

	}

	public TrashIcon3D(String name, TextureRegion region) {
		super(name, region);
//		setBackgroud(new NinePatch(new Texture(Gdx.files.internal("bgtest.png"))));
	...
	public void show() {
		...
		this.setUser(0f);
		this.stopTween();	
		showTween = this.startTween(View3DTweenAccessor.POS_XY, Cubic.OUT, 0.6f,
				0, Utils3D.getScreenHeight() - this.height, 0).setCallback(this);
...
//		SendMsgToAndroid.sendHideNoticeMsg();
	...
	public void hide() {
		...
		else
		{
			...
			hideTween = this.startTween(View3DTweenAccessor.POS_XY, Cubic.OUT, 0.6f,
					0, Utils3D.getScreenHeight(), 0).setCallback(this);
...
//			list.clear();
	...
	public void set(boolean dragOver){
		if(dragOver) this.region = focus;
		else this.region = normal;
	}
	@Override
	...
	public void draw(SpriteBatch batch, float parentAlpha) {
		...
		if (DefaultLayout.trash_icon_pos!=TrashIcon3D.TRASH_POS_TOP)
		{
			...
			if (region.getTexture() == null) return;
			...
			else
			{
				...
				batch.draw(region, x, y+R3D.hot_grid_bottom_margin, originX, originY, 
							this.region.getRegionWidth(), this.region.getRegionHeight(), scaleX,scaleY, rotation);
		}
		else
		{
