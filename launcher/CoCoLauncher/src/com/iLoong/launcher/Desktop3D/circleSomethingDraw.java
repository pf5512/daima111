...
//	private int mBitNoticeH;
	...
	public circleSomethingDraw(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	private Bitmap  getTextOnBmp(Bitmap oldBmp)
	{
	...
	private void drawImage(Canvas canvas)
	{
		...
		canvas.drawBitmap(mBitConnectPoint, touchDownX - mBitConnectPointW / 2,
				touchDownY - mBitConnectPointH / 2, null);
		canvas.drawBitmap(mBitFocus, mBitFocusX, mBitFocusY, null);
		canvas.drawBitmap(mBitConnectPoint, mBitConnectPointX,
				mBitConnectPointY, null);
//		mMatrix.reset();
...
//		canvas.translate(-sx, -sy);
		...
		//mPathLine.reset();
		float sx = (float) (touchDownX + mBitConnectPointW / 2
				* Math.cos(rotDegrees / 180 * Math.PI));
		float sy = (float) (touchDownY + mBitConnectPointW / 2
				* Math.sin(rotDegrees / 180 * Math.PI));
...
//    }  
	...
	public void onDraw(Canvas canvas)
	{
		...
		//Log.v("thread", "circle  onDraw thread="+Thread.currentThread());
	}
...
public  void libgdxAdaptAndroidViewEvent(int event, float x,float y,String toastString)
{
