...
public class Icon3D extends ViewCircled3D implements IconBase3D {
	...
	private static BitmapTexture selectedTexture;
	...
	static {
		...
		for (int i = 0; i < iconBgs.length; i++) {
			Bitmap bmp = ThemeManager.getInstance().getBitmap(
					"theme/iconbg/icon_" + i + ".png");
			Bitmap bmp2 = Bitmap.createScaledBitmap(bmp,
					DefaultLayout.app_icon_size, DefaultLayout.app_icon_size,
					true);
			iconBgs[i] = bmp2;
			if(bmp != bmp2)bmp.recycle();
		}
		Bitmap oriMask = ThemeManager.getInstance().getBitmap("theme/iconbg/mask.png");
		if(oriMask != null){
			mask = Bitmap.createScaledBitmap(oriMask,
					DefaultLayout.app_icon_size, DefaultLayout.app_icon_size,
					true);
			if(oriMask != mask)oriMask.recycle();
		}
		...
		if (DefaultLayout.show_font_bg && !DefaultLayout.font_double_line) {
			...
			float singleLineHeight = (float) Math.ceil(fontMetrics.bottom
					- fontMetrics.top);
			titleBg = Bitmap.createScaledBitmap(bmp, R3D.workspace_cell_width,
					(int) (singleLineHeight + singleLineHeight / 5), true);
			bmp.recycle();
		}
		...
		resumeTexture = new TextureRegion(new BitmapTexture(ThemeManager
				.getInstance().getBitmap("theme/pack_source/app-resume.png")));
		hideTexture = new TextureRegion(new BitmapTexture(ThemeManager
				.getInstance().getBitmap("theme/pack_source/app-hide.png")));
		uninstallTexture = new TextureRegion(
				new BitmapTexture(ThemeManager.getInstance().getBitmap(
						"theme/pack_source/app-uninstall.png")));

		paopaoRegion1 = new TextureRegion(new BitmapTexture(ThemeManager
				.getInstance().getBitmap("theme/pack_source/icon_dian.png")));
		paopaoRegion2 = new TextureRegion(new BitmapTexture(ThemeManager
				.getInstance().getBitmap("theme/pack_source/icon_dian_2.png")));
		numTextureRegion = new TextureRegion[10];
		for (int i = 0; i < 10; i++) {
			numTextureRegion[i] = new TextureRegion(new BitmapTexture(
					ThemeManager.getInstance().getBitmap(
							"theme/pack_source/icon_dian_num" + i + ".png")));
		}
		selectedTexture = new BitmapTexture(ThemeManager.getInstance()
				.getBitmap("theme/pack_source/shell-multi-choice.png"));
	
		// xiatian add start //DownloadIcon
		dynamicMenuDownloadTexture = new TextureRegion(
				new BitmapTexture(
						ThemeManager
						.getInstance()
						.getBitmap("theme/pack_source/dynamic_menu_download.png")
						));
		// xiatian add end
		
	}

	public Icon3D(String name) {
		super(name);

	}

	public Icon3D(String name, Texture texture) {
		super(name, texture);
		selectedImage = new ImageView3D("selectedicon", selectedTexture,
				R3D.workspace_multicon_width, R3D.workspace_multicon_height);
	}

	public Icon3D(String name, Bitmap bmp, String title) {
	...
	public Icon3D(String name, Bitmap bmp, String title, Bitmap IconBg) {
	...
	public Icon3D(String name, TextureRegion region) {
		super(name, region);
		selectedImage = new ImageView3D("selectedicon", selectedTexture,
				R3D.workspace_multicon_width, R3D.workspace_multicon_height);
	}
	...
	public void setting() {
		...
		if (info.intent.getComponent() == null) {
			if (info.intent.getAction() != null) {
				if (info.intent.getAction()
						.equals("android.intent.action.MAIN")) {
					if (info.intent.getType() != null
							&& info.intent.getType().equals(
									"vnd.android-dir/mms-sms")) {
						needShowPaoPao = true;
						isSmsFlag = true;
					}
				} else if (info.intent.getAction().equals(
			...
			if (packageName != null && className != null) {
				if (packageName.equals("com.android.mms")) {
					...
					isSmsFlag = true;
						...
						&& className.equals(DefaultLayout.call_component_name)) {
					needShowPaoPao = true;
					isSmsFlag = false;
				}
			}
	...
	public boolean onClick(float x, float y) {
		...
		if (isSelected()) {
			...
			if (
					(
							iLoongLauncher.getInstance().getD3dListener().getAppList().appList.mode 
							!= 
							iLoongLauncher.getInstance().getD3dListener().getAppList().appList.APPLIST_MODE_NORMAL
					)
					&&(isApplistVirtualIcon() == true)
			) {
				return true;
			}
	...
	/* 画数�?x,y是中心点 */
	public void drawNumber(SpriteBatch batch, float parentAlpha, float f,
			float g, int number) {
		int num = number;
		int i = 0, w = 0;
		if (num != 0) {
		...
		for (int j = 0; j <= i; j++) {
			int k = number % 10;
			number /= 10;
			batch.draw(numTextureRegion[k], f - w + (i - 1 - j)
					* paopaoNumStateIconHeight, g);
		}
	}

	/* 画数�?x,y是中心点 */
	public void drawNumber(SpriteBatch batch, float parentAlpha, float f,
			float g, int number, float orgx, float orgy, float scalex,
		...
		for (int j = 0; j < i; j++) {
			...
			batch.draw(numTextureRegion[k], x, g, this.x + orgx - x, orgy
					- this.height + paopaoNumStateIconHeight,
					paopaoNumStateIconWidth, paopaoNumStateIconHeight, scalex,
					scaley, rotate);
		}
	}
	...
	public void draw(SpriteBatch batch, float parentAlpha) {
		...
		if(region.getTexture() instanceof BitmapTexture){
			...
			if(needAntiAlias())texture.changeFilter(TextureFilter.Linear, TextureFilter.Linear);
			else texture.changeFilter(TextureFilter.Nearest, TextureFilter.Nearest);
		}
//		Log.d("icon", "AA:"+(region.getTexture().getMinFilter()==TextureFilter.Linear));
		...
		if (is3dRotation()) {
			...
			if (DefaultLayout.blend_func_dst_gl_one
					&& (srcBlendFunc != GL11.GL_SRC_ALPHA || dstBlendFunc != GL11.GL_ONE_MINUS_SRC_ALPHA))
				batch.setBlendFunction(GL11.GL_SRC_ALPHA,
						GL11.GL_ONE_MINUS_SRC_ALPHA);
			batch.setColor(color.r, color.g, color.b, color.a * parentAlpha);
			if (uninstall && canUninstall) {
				if(!isApplistVirtualIcon()){
					batch.draw(uninstallTexture, stateX, stateY, stateIconWidth,
							stateIconHeight);	
				}
			}
			if (hide && isHide) {
				batch.draw(resumeTexture, stateX, stateY, stateIconWidth,
						stateIconHeight);
			}
			if (hide && !isHide) {
				batch.draw(hideTexture, stateX, stateY, stateIconWidth,
						stateIconHeight);
			}
			if (!isSelected && needShowPaoPao && number > 0) {
				batch.draw(paopaoRegion, paopao_stateX, paopao_stateY,
						paopaoStateIconWidth, stateIconHeight);
				drawNumber(batch, parentAlpha, paopao_stateX
						+ paopaoStateIconWidth / 2, paopao_stateY
						+ stateIconHeight / 2, number);
			}
			if (DefaultLayout.blend_func_dst_gl_one
					&& (srcBlendFunc != GL11.GL_SRC_ALPHA || dstBlendFunc != GL11.GL_ONE_MINUS_SRC_ALPHA))
			...
			if(isApplistVirtualIcon() == true) {
				batch.draw(dynamicMenuDownloadTexture, dynamicMenuDownloadX,
						dynamicMenuDownloadY, dynamicMenuDownloadIconWidth,
			...
			if (uninstall && canUninstall) {
				if(!isApplistVirtualIcon()){
					batch.draw(uninstallTexture, stateX, stateY, stateOriginX,
							stateOriginY, stateIconWidth, stateIconHeight, scaleX,
							scaleY, iconRotation);	
				}
			}
			if (hide && isHide) {
				batch.draw(resumeTexture, stateX, stateY, stateOriginX,
						stateOriginY, stateIconWidth, stateIconHeight, scaleX,
						scaleY, iconRotation);
			}
			if (hide && !isHide) {
				batch.draw(hideTexture, stateX, stateY, stateOriginX,
						stateOriginY, stateIconWidth, stateIconHeight, scaleX,
						scaleY, iconRotation);
			}
			if (!isSelected && needShowPaoPao && number > 0) {
				...
				batch.draw(paopaoRegion, paopao_stateX, paopao_stateY,
						stateOriginX2, stateOriginY2, paopaoStateIconWidth,
						stateIconHeight, scaleX, scaleY, iconRotation/*this.rotation*/);
				drawNumber(batch, parentAlpha, paopao_stateX
						+ paopaoStateIconWidth / 2, paopao_stateY
						+ stateIconHeight / 2, number, this.originX,
						this.originY, scaleX, scaleY, iconRotation/*this.rotation*/);
			}
			if (DefaultLayout.blend_func_dst_gl_one
					&& (srcBlendFunc != GL11.GL_SRC_ALPHA || dstBlendFunc != GL11.GL_ONE_MINUS_SRC_ALPHA))
				batch.setBlendFunction(srcBlendFunc, dstBlendFunc);
			...
			{
				batch.draw(dynamicMenuDownloadTexture, dynamicMenuDownloadX,
						dynamicMenuDownloadY, dynamicMenuDownloadOriginX,
						dynamicMenuDownloadOriginY,
						dynamicMenuDownloadIconWidth,
						dynamicMenuDownloadIconHeight, scaleX, scaleY, iconRotation/*rotation*/);
			}
		...
		if (isSelected) {
			if (DefaultLayout.blend_func_dst_gl_one
				...
				batch.setBlendFunction(GL11.GL_SRC_ALPHA,
						GL11.GL_ONE_MINUS_SRC_ALPHA);
			selectedImage.x = this.x + this.width - selectedImage.width;
			selectedImage.y = this.y + this.height - selectedImage.height;
			selectedImage.setOrigin(originX - this.width + stateIconWidth,
					originY - this.height + stateIconHeight);
			selectedImage.setScale(scaleX, scaleY);
			selectedImage.setRotation(rotation);
			selectedImage.draw(batch, parentAlpha * this.color.a);
	...
	public void setInShowFolder(boolean canshow) {
	...
	public void setItemInfo(ItemInfo info) {
		...
		if (info instanceof ShortcutInfo) {
			...
			if (intent != null && intent.getComponent() != null) {
				isHide = PreferenceManager.getDefaultSharedPreferences(
						iLoongLauncher.getInstance()).getBoolean(
						"HIDE:" + intent.getComponent().toString(), false);
			}
		}
	}
	...
	public void onEvent(int type, BaseTween source) {
		...
		if (tmpView != null && source == tmpView.getTween()
...
//				this.setOrigin(point2.x, point2.y);
	...
	public void selected() {
		...
		selectedImage.setScale(0, 0);
		selectedImage.setOrigin(selectedImage.width / 2,
				selectedImage.height / 2);
		selectedImage.startTween(View3DTweenAccessor.SCALE_XY, Elastic.OUT,
				0.8f, 1f, 1f, 0);
		isSelected = true;
...
//			if(!(this instanceof Widget3DVirtual))
	...
	//teapotXu add start for Folder in Mainmenu
	public void setUninstallStatus(boolean is_uninstall)
	{
...
//			this.setOrigin(point1.x, point1.y);
			...
			shakeAnimation = this.startTween(View3DTweenAccessor.ROTATION,
					Linear.INOUT, shakeTime, SHAKE_ROTATE_ANGLE, 0, 0).setCallback(this);
		} else {
			// this.stopTween();
			if (shakeAnimation != null) {
				shakeAnimation.kill();
			...
			if (DefaultLayout.enable_icon_effect) {
				if (tmpView == null) {
					...
					if (tmpView != null) {
						...
						tmpView.x = point.x;
						tmpView.y = point.y;
						tmpView.startTween(View3DTweenAccessor.OPACITY,
								Cubic.OUT, 0.3f, 0, 0, 0);
						tmpView.startTween(View3DTweenAccessor.SCALE_XY,
								Cubic.OUT, 0.3f, 1.5f, 1.5f, 0).setCallback(
								this);
					}
				}
			} else {
		...
		if (DefaultLayout.enable_particle)
		{
			if (ParticleManager.particleManagerEnable)
			{
				if (!particleCanRender && !ParticleManager.disableClickIcon)
				{
					...
					startParticle(ParticleManager.PARTICLE_TYPE_NAME_CLICK_ICON, 
							positionX, positionY).setCallback(this);
				}
			}
		}
		/************************ added by zhenNan.ye end ***************************/
	...
	public void setShowPop(boolean show) {
	...
	public boolean pointerInParent(float x, float y) {
		...
		if (DefaultLayout.hotseat_title_disable_click) {
			if (this.info.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
				...
				return ((point.x >= 0 && point.x < width) && (point.y >= height
						- Utilities.sIconTextureHeight && point.y < height));
			}
		}
	...
	public boolean isApplistVirtualIcon() {
		...
		if (mShortcutInfo.appInfo != null
				&& mShortcutInfo.appInfo.intent.getBooleanExtra(
	...
	public float getDownloadIconX() 
	{
		...
		if(
				( this.getItemInfo().container == -101 )
				&& ( this.getItemInfo().angle == 0 )
		)
		{//mainGroup
...
class IconToTexture3D extends BitmapTexture {
	...
	public IconToTexture3D(Bitmap b, String title, Bitmap icn_bg,
			Bitmap title_bg) {
