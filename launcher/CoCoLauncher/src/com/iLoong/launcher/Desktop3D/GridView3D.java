...
public class GridView3D extends ViewGroup3D{
	...
	public GridView3D(String name,float width,float height,int countx, int county) {
		super(name);
	...
	private void layout(int index)
	{
		...
		for(int i = index;i<size;i++)
		{
			...
			if(cur_view!=mFocus)
			{
				...
				if(animation_flag)
				{
					animation_line.push(Tween.to(cur_view,View3DTweenAccessor.POS_XY, 0.5f)
							.target(mPosx,mPosy)
							.ease(Cubic.OUT).delay(i*animation_delay));
				}
				else
				{
					cur_view.setPosition(mPosx, mPosy);
...
//	}
	...
	public boolean onTouchUp(float x, float y, int pointer) {
		...
		if(mFocus!=null)
		{
			...
			if(handled == false)
			{
				...
				animation_focus = Tween.to(mFocus,View3DTweenAccessor.POS_XY, 0.5f)
				.target(mPosx,mPosy)
				.ease(Cubic.OUT);
	...
	public float getEffectiveWidth()
	{
		...
		if(getChildCount() <= mCountX) return getChildCount()* mCellWidth + mPaddingLeft + mPaddingRight;
