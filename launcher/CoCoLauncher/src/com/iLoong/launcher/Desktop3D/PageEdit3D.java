...
public class PageEdit3D extends ViewGroup3D implements DropTarget3D, DragSource3D{
	...
	public void setPageList(List<View3D> pageList,int pageSource){
	...
	private void addEditPage(View3D view,int index,int pageSource){
...
//						cell.y -= (1-tmpScaleY)*cell.y;
			...
			else view.setScale(pageWidth/view.width, pageHeight/view.height);
			view.setOrigin(0, 0);
			view.setRotation(0);
			...
			if(pageNum <= PAGE_COUNT_MIN)deleteIcon.hide();
			else deleteIcon.show();
			
			View3D homeIcon = null;
			if(index == homePage){
				homeIcon = new View3D("home_icon",R3D.getTextureRegion("page-edit2b"));
			}
			else homeIcon = new View3D("home_icon",R3D.getTextureRegion("page-edit2"));
	...
	private void setLayoutParams(int index,View3D page,boolean anim){
		page.width = pageWidth;
		page.height = pageHeight;
		page.setOrigin(pageWidth/2, pageHeight/2);
		float x = PAGE_PADDING_LEFT+index%3*(pageWidth+pagePaddingHorizontal);
		float y = Utils3D.getScreenHeight()-(PAGE_PADDING_TOP+index/3*(pageHeight+pagePaddingVertical)+pageHeight);
		if(anim){
			page.stopTween();
	...
	public void exeDeletePage(){
		...
		if(toDeleteIndex <= pageContainer.getSelectedIndex()){
			if(toDeleteIndex == pageContainer.getSelectedIndex()){
				...
				if(toDeleteIndex == 0)j = 1;
				View3D v = selectPageList.get(j);
				v.setBackgroud(PageContainer3D.pageSelectedBg);
				v.setColor(Color.WHITE);
			}
			if(pageContainer.getSelectedIndex() > 0)pageContainer.pageSelect.currentIndex--;
		}
		...
		if(toDeleteIndex <= homePage){
			if(homePage > 0)setHomePage(homePage-1);
			else setHomePage(homePage);
		}
		animatingEdit = true;
		deletedPage.startTween(View3DTweenAccessor.SCALE_XY, Cubic.OUT, 0.2f, 0, 0, 0).setCallback(this);
		for(int j = toDeleteIndex;j < getChildCount();j++){
	...
	private void sortPage(int newEmptyPosition){
	...
	private void enterPage(ViewGroup3D editPage){
		...
		if(selectPage != null){
			...
			if(homeIcon != null)editPage.removeView(homeIcon);
			View3D deleteIcon = editPage.findView("delete_icon");
			if(deleteIcon != null)editPage.removeView(deleteIcon);
			animatingEnter = true;
			timeline = Timeline.createParallel();
			...
			if(token != null)mWallpaperManager.setWallpaperOffsets(token,targetOffset, 0);
	...
	private int calculateEmptyPosition(float x,float y){
		x -= PAGE_PADDING_LEFT;
	...
	private boolean weakPointerInParent(View3D v,float x,float y){
	...
	public void goBack(){
		if(!isAnimating()){
			if(pageSource == PAGE_SOURCE_SELECT)pageContainer.changeMode();
			else {
				int index = pageContainer.getSelectedIndex();
				if(index < 0)index = 0;
				if(index >= this.getChildCount())index = this.getChildCount()-1;
	...
	public void enterHomePage(boolean anim){
		if(!anim){
			removeAllViews();
	...
	public boolean onTouchDragged(float x, float y, int pointer) {
		...
		if(tmp >= pageNum)tmp = pageNum-1;
		if(tmp == emptyPosition)return true;
		sortPage(tmp);
		return true;
	}
	...
	public boolean onTouchDown(float x, float y, int pointer) {
		...
		if(pointer == 0)downTime = System.currentTimeMillis();
		return true;
	}
...
//		}
	...
	public ArrayList<View3D> getDragList() {
		if(dragList == null)dragList = new ArrayList<View3D>();
		dragList.clear();
	...
	public boolean onDrop(ArrayList<View3D> list, float x, float y) {
		...
		if(selectPage != null){
			...
			if(cell != null)pageContainer.addCellLayout(emptyPosition, cell);
		}
	...
	public boolean hide(boolean anim){
		...
		for(int index = 0;index < selectPageList.size();index++){
			...
			if(deleteIcon != null)parent.removeView(deleteIcon);
			if(homeIcon != null)parent.removeView(homeIcon);
			float delay = 0f;
...
//			if(selected < 0)selected = 0;
		...
		if(pageNum < PAGE_COUNT_MAX)timeline.push(addIconPage.obtainTween(View3DTweenAccessor.OPACITY, Cubic.OUT, 0.3f, 0f, 0, 0));
		timeline.setCallback(this).start(View3DTweenAccessor.manager);
		return true;
	}
	...
	class EditPageView extends ViewGroup3D{
		...
		public EditPageView(String string) {
			super(string);
			transform = true;
		}
