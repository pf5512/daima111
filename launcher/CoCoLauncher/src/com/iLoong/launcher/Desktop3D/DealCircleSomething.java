...
public class DealCircleSomething extends View3D {
	...
	public DealCircleSomething(String name, Workspace3D workspace3D) {
		super(name);
	...
	public void addIconView(ViewGroup3D view3D) {
		actorBefore = mworkspace3D.getChildAt(mworkspace3D.getChildCount() - 1);
		mworkspace3D.addViewAfter(actorBefore, view3D);

	}

	public void addFolderView(FolderIcon3D folder) {
		if (mworkspace3D.addInCurrenScreen(folder, (int) folder.x,
				(int) folder.y)) {
	...
	private void startMergeAnimation() {
		...
		folderIcon.addFolderNode(mCircleSomething);
		mCircleSomething.clear();
	}

	private void dealEvent_CircleAutoMerge(float moveX, float moveY) {
	...
	private void dealEvent_CirclePopupWnd(float moveX, float moveY) {
		if (bPopWndExist == false && mCircleSomething.size() > 0) {
			// Log.d("launcher123",
			// "dealcircelsomething pop view mCircleSomething.size="+mCircleSomething.size());
			circlePopWnd3D circlePop = new circlePopWnd3D("circlePopWnd3D",
					moveX, moveY);
			actorBefore = mworkspace3D
					.getChildAt(mworkspace3D.getChildCount() - 1);
			mworkspace3D.addViewAfter(actorBefore, circlePop);
			// circlePop.requestFocus();
			circlePop.buildElements();
			bPopWndExist = true;
			circlePop.setScale(0, 0);
			circlePop.startTween(View3DTweenAccessor.SCALE_XY, Quad.IN, 0.2f,
					1, 1, 0);
		} else
			SendMsgToAndroid.sendShowWorkspaceMsg();
	}
	...
	public void DealCircleSomethingResult(float x, float y, int circle_State) {
	...
	public boolean dealEvent_circlePopWnd3D(int event_id) {
		boolean retVal = true;
		Log.v("ICONGROUP3D",
				" CellLayout3D dealEvent_circlePopWnd3D mCircleSomething.size="
						+ mCircleSomething.size());
			...
			for (int i = 0; i < Count; i++) {
				...
				if (childView instanceof ViewGroupCircled3D) {
					if (((ViewGroupCircled3D) childView).getCircled() == true) {
						...
						((ViewGroupCircled3D) childView)
								.setGroupColor(selectColor);
						childView.setColor(selectColor);
	...
	private void dealAutoSort() {
		...
		if (mCircleSomething.size() > 1) {
			...
			if (DefConfig.DEF_NEW_SIDEBAR == false) {
				IconGroup3D iconGroup = new IconGroup3D("PopIconGroupView",
						mCircleSomething);
				addIconView(iconGroup);
			} else {
	...
	public void Process_delALL(int result) {
		// resetToNothingCircle();
	...
	private void reserveIcon3DOnly() {
		...
		for (int i = Count; i >= 0; i--) {
			myActor = mCircleSomething.get(i);
			if (myActor instanceof Icon3D) {
				mworkspace3D.getCurrentCellLayout().removeView(myActor);
			} else {
	...
	private void dealCreateFolder() {
		...
		if (mCircleSomething.size() > 0) {
			...
			int FolderNum = (mCircleSomething.size() + R3D.folder_max_num - 1)
					/ R3D.folder_max_num;
			int offset = 0;
			int count = mCircleSomething.size();
			for (int i = 0; i < FolderNum; i++) {
				...
				pos.x = temp.x;
				pos.y = temp.y;
				if (pos.x < Utils3D.getScreenWidth() / 2
						&& pos.y > Utils3D.getScreenWidth() / 2) {
					pos.x = temp.x + R3D.folder_front_width / 2;
					pos.y = temp.y - R3D.Workspace_cell_each_height;
				}

				else if (pos.x < Utils3D.getScreenWidth() / 2
						&& pos.y < Utils3D.getScreenWidth() / 2) {
					pos.x = temp.x + R3D.folder_front_width / 2;
					pos.y = temp.y + R3D.Workspace_cell_each_height;
				}
