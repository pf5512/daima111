...
public class HotButton extends View3D {
	...
	public HotButton(String name, TextureRegion region) {
		super(name, region);
		normal = region;
	...
	public void show() {
		...
		this.setUser(0f);
		this.stopTween();	
		this.startTween(View3DTweenAccessor.POS_XY, Cubic.OUT, 0.8f,
				Utils3D.getScreenWidth() - getWidth(), 0, 0).setCallback(this);
		animating = true;
	}
	...
	public void hide() {
		this.stopTween();
		this.startTween(View3DTweenAccessor.POS_XY, Cubic.OUT, 0.8f,
				Utils3D.getScreenWidth(), 0, 0).setCallback(this);
		animating = true;
	}
	...
	public boolean onTouchUp(float x, float y, int pointer) {
		// TODO Auto-generated method stub
		if(((Desktop3D)getStage()).getFocus() != this) return false;
		releaseFocus();
		isFocus = false;
