...
public class Desktop3D extends Stage implements GestureListener3D
{
	...
	private static int DOUBLE_CLICK_TIME = 250;// 两次单击的时间间隔
	...
	public void draw()
	{
		...
		//Log.v("render", " max sprites:" + batch.maxSpritesInBatch + " totalRenderCalls:" + batch.totalRenderCalls+"continue="+Gdx.graphics.isContinuousRendering());
		camera.update();
		if( !root.visible )
			return;
			...
			int pointer )
	{
		...
		if( this.foucusView != null )
		{
			...
			else if( !doubleClick && !waitDouble )
			{ //直接发click
				waitDouble = true;
				doubleClick = false;
				singleClick( coords.x , coords.y );
