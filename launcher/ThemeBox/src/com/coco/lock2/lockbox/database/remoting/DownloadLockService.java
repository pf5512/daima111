...
public class DownloadLockService {
	...
	public static final Uri CONTENT_URI = Uri
			.parse("content://com.coco.lock2.lockbox/" + TABLE_NAME);
	private Context mContext;

	public static String getCreateSql() {
		String result = String
				.format("CREATE TABLE %s (%s TEXT PRIMARY KEY, %s INTEGER, %s INTEGER, %s TEXT, %s INTEGER, %s TEXT, %s INTEGER, %s TEXT, %s TEXT, %s TEXT);",
						TABLE_NAME, FIELD_PACKAGE_NAME, FIELD_DOWNLOAD_SIZE,
						FIELD_DOWNLOAD_STATUS, FIELD_APPLICATION_NAME,
						FIELD_VERSION_CODE, FIELD_VERSION_NAME,
						FIELD_APPLICATION_SIZE, FIELD_AUTHOR,
						FIELD_INTRODUCTION, FIELD_UPDATE_TIME);
		return result;
	}
	...
	public boolean insertItem(DownloadLockItem item) {
	...
	public DownloadLockItem queryByPackageName(String packageName) {
	...
	public boolean updateDownloadSizeAndStatus(String packageName, int size,
		...
		int updateResult = contentResolver.update(CONTENT_URI, cv,
				FIELD_PACKAGE_NAME + "=?", new String[] { packageName });
		if (updateResult <= 0) {
			return false;
		}
	...
	public boolean updateDownloadSizeAndStatus(String packageName,
			int downSize, int totalSize, DownloadStatus status) {
		ContentValues cv = new ContentValues();
	...
	public boolean deleteItem(String packageName) {
		...
		int delResult = contentResolver.delete(CONTENT_URI, FIELD_PACKAGE_NAME
	...
	private DownloadLockItem readLockInfo(Cursor cursor) {
		try {
			...
			item.setIntroduction_en(cursor.getString(cursor
					.getColumnIndexOrThrow(FIELD_INTRODUCTION_EN)));
