...
public class TabLockFactory implements TabHost.TabContentFactory,
	...
	public TabLockFactory(Context context, DownModule module, int Screen_w,
			int Screen_h) {
		PathTool.makeDirApp();
		mContext = context;
		downModule = module;
		...
		getThemeBtn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (file.exists()) {
					...
					downloadAPK(mContext, ThemesDB.LAUNCHER_PACKAGENAME,
							StaticClass.LOCKBOX_PACKAGE_NAME, fileName);
				}
	...
	public void setSelector(int id) {
		...
		for (int i = 0; i < title.length; i++) {
			if (id == i) {
				Bitmap bitmap = BitmapFactory.decodeResource(
						mContext.getResources(), R.drawable.fenleianxia);
				if (id > 0) {
					textViews.get(id - 1).setBackgroundDrawable(
				...
				if (i > 0) {
					textViews.get(i - 1).setBackgroundDrawable(
							new BitmapDrawable());
		...
		if (id == 3) {
			...
			horizontalScrollView.smoothScrollTo(
					(textViews.get(2).getWidth() * (2) - 180), 0);
	...
	public View createTabContent(String tag) {
		...
		horizontalScrollView = (HorizontalScrollView) result
		...
		fenleiLinearLayout = (LinearLayout) result
				.findViewById(R.id.fenleilinearLayout);
		initTile();

		{
					...
					handler.postDelayed(new Runnable() {
						...
						public void run() {
							...
							if (pagerAdapter != null
									&& pagerAdapter.viewDownloading != null) {
								pagerAdapter.viewDownloading
			...
			// 简约解锁
			simpleView = (GridView) (View.inflate(mContext, R.layout.lock_grid,
					null));
			...
			// 经典解锁
			classicView = (GridView) (View.inflate(mContext,
					R.layout.lock_grid, null));
			...
			pagerAdapter = new GridPagerAdapter(gridViewLocal, hotView,
					simpleView, classicView);
		...
		// 监听设置按钮
		ImageButton btnSetting = (ImageButton) result
	...
	private BroadcastReceiver packageReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_PACKAGE_REMOVED.equals(actionName)) {
				...
				hotAdapter.updateThumb(intent
						.getStringExtra(StaticClass.EXTRA_PACKAGE_NAME));
				localAdapter.updateThumb(intent
						.getStringExtra(StaticClass.EXTRA_PACKAGE_NAME));
				simpleAdapter.updateThumb(intent
						.getStringExtra(StaticClass.EXTRA_PACKAGE_NAME));
				classicAdapter.updateThumb(intent
						.getStringExtra(StaticClass.EXTRA_PACKAGE_NAME));
				...
				pagerAdapter.notifyDataSetChanged();
				Log.v("onHeaderRefresh",
						"ACTION_HOTLIST_CHANGED footerRefresh = "
								+ footerRefresh + " headerRefresh = "
								+ headerRefresh);
				if (listRefresh) {
					listRefresh = false;
				...
				Toast.makeText(mContext, R.string.list_isnew,
						Toast.LENGTH_SHORT).show();
			} else if (actionName
				...
				Log.v("********", "receive packName = " + packName);
					...
					.equals(StaticClass.ACTION_DEFAULT_LOCK_CHANGED)) {
				new Thread() {
					@Override
					public void run() {
						localAdapter.reloadPackage();
	...
	private static class GridPagerAdapter extends PagerAdapter {
		...
		public GridPagerAdapter(GridView local, View view, View simple,
				View classic) {
			gridLocal = local;
			hotView = view;
			SimpleView = simple;
	...
	public void setClickSelector(int id) {
		for (int i = 0; i < title.length; i++) {
			if (id == i) {
				Bitmap bitmap = BitmapFactory.decodeResource(
						mContext.getResources(), R.drawable.fenleianxia);
