...
public class ThemeReceiver extends BroadcastReceiver {
	...
	public void onReceive(final Context context, Intent intent) {
		if (intent.getAction().equals(Intent.ACTION_PACKAGE_ADDED)) {
			...
			Intent themeIntent = new Intent("com.coco.themes", null);
			themeIntent.setPackage(packageName);

			List<ResolveInfo> themesinfo = context.getPackageManager()
			...
			if (packageName == null || packageName.equals(ThemesDB.LAUNCHER_PACKAGENAME)) {
				...
				themesinfo = context.getPackageManager().queryIntentActivities(
						systemmain, 0);
			}
			if (themesinfo != null) {
