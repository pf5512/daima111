...
public class UrlAddressThemeService {
	...
	public static String getCreateSql() {
		String result = String
				.format("CREATE TABLE %s (%s INTEGER, %s INTEGER, %s TEXT);",
						TABLE_NAME, FIELD_APPLICATION_TYPE, FIELD_ADDRESS_TYPE,
						FIELD_ADDRESS, FIELD_APPLICATION_TYPE,
						FIELD_ADDRESS_TYPE);
		return result;
	}
	...
	public boolean batchInsertAddress(ApplicationType appType,
			AddressType addrType, String[] addrArr) {
	...
	public void deleteAddress(ApplicationType appType, AddressType addrType) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		try {
			db.delete(TABLE_NAME, String.format("%s=? and %s=?",
					FIELD_APPLICATION_TYPE, FIELD_ADDRESS_TYPE),
					new String[] { Integer.toString(appType.getValue()),
							Integer.toString(addrType.getValue()) });
		} finally {
			db.close();
			DbHelper.lock_db = false;
		}
	...
	public List<String> queryAddress(ApplicationType appType,
		...
		Cursor cursor = null;
		try {
			cursor = db.query(TABLE_NAME, new String[] { FIELD_ADDRESS },
					String.format("%s=? and %s=?", FIELD_APPLICATION_TYPE,
							FIELD_ADDRESS_TYPE),
					new String[] { Integer.toString(appType.getValue()),
							Integer.toString(addrType.getValue()) }, null,
					null, null);
			while (cursor.moveToNext()) {
				result.add(cursor.getString(cursor
						.getColumnIndexOrThrow(FIELD_ADDRESS)));
			}
		} finally {
			if (cursor != null) {
				cursor.close();
