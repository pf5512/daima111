 ...
 * events, animating actors and asking them to render themselves. A stage is 2D
 * scenegraph with hierarchies of Actors.
 * <p>
 * A stage fills the whole screen. It has a width and height given in device
 * independent pixels. It has a {@link Camera} that maps this viewport to the
 * given real screen resolution. If the stretched attribute is set to true then
 * the viewport is enforced no matter the difference in aspect ratio between the
 * stage object and the screen dimensions. In case stretch is disabled then the
 * viewport is extended in the bigger screen dimensions.
 * <p>
 * Actors have a z-order which is equal to the order they were inserted into the
 * stage. Actors inserted later will be drawn on top of actors added earlier.
 * Touch events that will get distributed to later actors first.
 * <p>
 * Actors can be focused. When your game pauses and resumes be sure to call the
 * {@link Stage#unfocusAll()} method so that the focus states get reset for each
 * pointer id. You also have to make sure that the actors that were focused
 * reset their state if the depend on being focused, e.g. wait for a touch up
 * event. An easier way to tackle this is to recreate the stage if possible.
 * 
 * @author mzechner
 */
public class Stage extends InputAdapter implements Disposable {
	 ...
	 *            whether to stretch the viewport to the real device resolution
	 * @param batch
	 *            the {@link SpriteBatch} this Stage should use. Will not be
	 *            disposed if {@link #dispose()} is called.
	 */
	...
	public void setViewport(float width, float height, boolean stretch) {
		if (!stretch) {
			if (width > height
					&& width / (float) Gdx.graphics.getWidth() <= height
	...
	/**
	 * Call this to distribute a mouse scroll event to the stage. This event
	 * will only ever appear on the desktop.
	 * 
	 * @param amount
	 *            the scroll amount.
	 * @return whether an {@link Actor} in the scene processed the event or not.
	...
	public boolean keyTyped(String characters) {
		if (keyboardFocus == null)
			return false;
		return keyboardFocus.keyTyped(characters);
	}

	/**
	 * Calls the {@link Actor#act(float)} method of all contained Actors. This
	 * will advance any {@link Action}s active for an Actor.
	 * 
	 * @param delta
	 *            the delta time in seconds since the last invocation
	 */
	 ...
	 * it up properly! The {@link Stage#draw()} will call the Camera's update()
	 * method and use it's combined matrix as the projection matrix for the
	 * SpriteBatch.
	 * 
	 * @param camera
	 *            the {@link Camera}
	...
	/**
	 * Returns the {@link Actor} intersecting with the point (x,y) in stage
	 * coordinates. Hit testing is performed in the order the Actors were
	 * inserted into the Stage, last inserted Actors being tested first. To get
	 * stage coordinates from screen coordinates use
	 * {@link #toStageCoordinates(int, int, Vector2)}.
	 * 
	 * @param x
	 *            the x-coordinate in stage coordinates
	 * @param y
