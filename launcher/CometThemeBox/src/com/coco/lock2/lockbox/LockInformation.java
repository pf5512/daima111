...
public class LockInformation
{
	...
	public boolean isComponent(
			...
			String clsName )
	{
		if( lockInfo.getPackageName().equals( pkgName ) && className.equals( clsName ) )
		{
			return true;
		}
		return false;
	...
	{
		if( lockInfo.getPackageName().equals( comName.getPackageName() ) && className.equals( comName.getClassName() ) )
		{
			return true;
		}
		return false;
			...
			Context cxt )
	{
		...
		if( installed )
		{
			...
			try
			{
				Context remoteContext = cxt.createPackageContext( lockInfo.getPackageName() , Context.CONTEXT_IGNORE_SECURITY );
	...
	public void loadUpdateInstallDetail(
			Context remoteContext ,
			ContentConfig cfg )
	{
		needLoadDetail = false;
	...
	public void reloadThumb()
	{
		if( installed )
		{
			thumbImage = BitmapFactory.decodeFile( PathTool.getThumbFile( lockInfo.getPackageName() ) );
		}
		else
		{
			String thumbPath = PathTool.getThumbFile( lockInfo.getPackageName() );
