...
public class ThemeService
{
	...
	public List<ThemeInformation> queryDownloadList()
	{
		...
		List<DownloadThemeItem> downlist = dSv.queryTable();
		List<ThemeInformation> result = new ArrayList<ThemeInformation>();
	...
	{
		HotThemeService hotSv = new HotThemeService( mContext );
	...
	public ComponentName queryCurrentTheme()
	{
		...
		{
			if( info.packageName.equals( cfg.theme ) && info.name.equals( cfg.className ) )
			{
	...
	{
		ThemesDB db = new ThemesDB( mContext );
		db.SaveThemes( apply.getPackageName() , apply.getClassName() );
	...
	private List<ActivityInfo> queryThemeActivityList()
	{
		...
		List<ResolveInfo> themesinfo = null;
		Intent defaultintent = new Intent( "com.cooeecomet.themes.default" , null );
		themesinfo = mContext.getPackageManager().queryIntentActivities( defaultintent , 0 );
		for( ResolveInfo info : themesinfo )
		{
			resultList.add( info.activityInfo );
		}
		Intent intent = new Intent( "com.cooeecomet.themes" , null );
		themesinfo = mContext.getPackageManager().queryIntentActivities( intent , 0 );
		...
		//
		//		themesinfo = mContext.getPackageManager().queryIntentActivities(
		//				systemmain, 0);
		//		if(themesinfo!=null)
		//		{
		//			for (ResolveInfo info : themesinfo) {
			...
			String pkgName )
	{
		...
		Intent intent1 = new Intent( "com.cooeecomet.themes.default" , null );
		intent1.setPackage( pkgName );
		themesinfo = mContext.getPackageManager().queryIntentActivities( intent1 , 0 );
		for( ResolveInfo info : themesinfo )
		{
			resultList.add( info.activityInfo );
		}
