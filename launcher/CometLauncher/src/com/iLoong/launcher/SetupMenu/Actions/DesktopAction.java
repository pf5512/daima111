...
public class DesktopAction extends Action
{
			...
			String key )
	{
		...
		//xiatian add end		
		else if( key.equals( DesktopSettingActivity.getKey( RR.string.dock_row_key ) ) )
		{
			mBundle.putInt( key , Integer.valueOf( PreferenceManager.getDefaultSharedPreferences( SetupMenu.getContext() ).getString( key , "2" ) ) );
		}
	}
	...
	public static class DesktopSettingActivity extends PreferenceActivity implements OnPreferenceChangeListener , Preference.OnPreferenceClickListener
	{
		...
		public static final int REQUEST_UPGRADE_VERIFICATION = 1;
		private ListPreference desktopEffectsListPreference , appEffectsListPreference , dockRowListPreference;
				...
				Bundle savedInstanceState )
		{
			...
			setTheme( RR.style.perference_set_activity );
			addPreferencesFromResource( RR.xml.customdesktopsetting );
			showitem();
			callback();
		}
		...
		private void callback()
		{
			...
			//xiatian add end
			if( findPreference( getKey( RR.string.dock_row_key ) ) != null )
			{
				findPreference( getKey( RR.string.dock_row_key ) ).setOnPreferenceClickListener( this );
			}
			...
			if( findPreference( getKey( RR.string.setting_key_upgrade ) ) != null )
			{
				findPreference( getKey( RR.string.setting_key_upgrade ) ).setOnPreferenceClickListener( this );
			}
		}
		...
		private void showitem()
		{
			...
			if( isUpgraded() )
			{
				...
				Preference preference = findPreference( res.getString( RR.string.setting_key_upgrade ) );
				if( preference != null )
				{
			...
			if( pf != null )
			{
				desktopEffectsListPreference = pf;
			...
			/************** added by zhenNan.ye end*******************/
			key = res.getString( RR.string.dock_row_key );
			pf = (ListPreference)findPreference( key );
			if( pf != null )
			{
				dockRowListPreference = pf;
				pf.setOnPreferenceChangeListener( this );
				summarys = pf.getEntries();
		...
		private void updateDesktop()
		{
			...
			//			}
			if( UpdateManager.isHaveInternet( SetupMenu.getContext() ) )
			{
				...
				Preference preference )
		{
			...
			else if( key.equals( getKey( RR.string.setting_key_feedback ) ) )
			{
				...
				SetupMenuActions.getInstance().Handle( ActionSetting.ACTION_FEEDBACK );
					...
					|| key.equals( getKey( RR.string.dock_row_key ) ) )
			{
				mKey.add( key );
			}
			...
			else if( key.equals( getKey( RR.string.setting_key_upgrade ) ) )
			{
				upgrade();
			}
			return false;
				...
				Object newValue )
		{
			...
			String dock_row_key = SetupMenu.getContext().getResources().getString( RR.string.dock_row_key );
			...
			//					|| preference.getKey().equals( dock_row_key ) )
			if( preference.getKey().equals( iconSizeKey ) || preference.getKey().equals( mainmenu_bg_key ) )
			{
		...
		private void rate()
		{
			if( UpdateManager.isHaveInternet( SetupMenu.getContext() ) )
			{
				...
				if( isPlayStoreInstalled() )
				{
					intent.setClassName( GOOGLE_PLAY_STORE_PACKAGE_NAME , "com.android.vending.AssetBrowserActivity" );
				}
				else
				{
			...
			{
				SendMsgToAndroid.sendToastMsg( SetupMenu.getKey( RR.string.internet_err ) );
			}
		}
		...
		private void upgrade()
		{
			...
			if( !preferences.getBoolean( Utils3D.UPGRADE_VERIFICATION , false ) )
			{
				...
				{
					MobclickAgent.onEvent( iLoongLauncher.getInstance() , UmengMobclickAgent.EVENT_ID_ENTRYPRIMEADS , "desktop_setting_upgrade" );
		...
		{
			SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences( iLoongLauncher.getInstance() );
			return preferences.getBoolean( Utils3D.UPGRADE_VERIFICATION , false );
		}
		...
		private boolean isPlayStoreInstalled()
		{
			try
			{
				this.getPackageManager().getPackageInfo( GOOGLE_PLAY_STORE_PACKAGE_NAME , PackageManager.GET_ACTIVITIES );
				return true;
			}
		...
		public void startActivityForResultSafely(
				Intent intent ,
				int requestCode )
		{
			try
			...
			if( resultCode == RESULT_OK )
			{
				switch( requestCode )
				{
					case REQUEST_UPGRADE_VERIFICATION:
						...
						( (MyListPreference)desktopEffectsListPreference ).update();
						( (MyListPreference)appEffectsListPreference ).update();
						( (MyListPreference)dockRowListPreference ).update();
						showitem();
						break;
				}
			}
