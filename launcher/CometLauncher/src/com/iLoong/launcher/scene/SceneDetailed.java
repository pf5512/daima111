...
public class SceneDetailed extends FrameLayout implements View.OnClickListener , PagedView.ViewSwitchListener
{
	...
	private boolean IsSystemByComponent(
			ComponentName componentName )
	{
		Intent intent = new Intent();
		intent.setComponent( componentName );
		List<ResolveInfo> mWidgetResolveInfoList = mContext.getPackageManager().queryIntentActivities( intent , PackageManager.GET_ACTIVITIES | PackageManager.GET_META_DATA );
	...
	private void LoadLayout()
	{
		...
		LinearLayout titleLayout = (LinearLayout)View.inflate( mContext , RR.layout.detil_scene_title , null );
		mMyTitle = (TextView)titleLayout.findViewById( RR.id.textSceneAppName );
		ImageButton btnReturn = (ImageButton)titleLayout.findViewById( RR.id.btnSceneReturn );
		ImageButton btnDelete = (ImageButton)titleLayout.findViewById( RR.id.btnSceneDelete );
		...
		{
			titleSize = (int)iLoongLauncher.getInstance().getResources().getDimension( RR.dimen.detile_scene_title_text_size );
		}
		else if( Utils3D.getScreenWidth() > 500 && Utils3D.getScreenWidth() < 800 )
		{
			titleSize = (int)iLoongLauncher.getInstance().getResources().getDimension( RR.dimen.detile_scene_title_text_size_seven );
		}
		else
		{
			titleSize = (int)iLoongLauncher.getInstance().getResources().getDimension( RR.dimen.detile_scene_title_text_size_big );
		}
		...
		lp.topMargin = (int)iLoongLauncher.getInstance().getResources().getDimension( RR.dimen.detile_scene_grid_margint_top );
		...
		lp = new LinearLayout.LayoutParams( LayoutParams.WRAP_CONTENT , mIndicatorLayout.getheight() );
		...
		mButtonApplyTheme.setBackgroundDrawable( iLoongLauncher.getInstance().getResources().getDrawable( RR.drawable.aplay_scene_selector ) );
	...
	public void LoadData(
			int index )
	{
		mData = SceneManager.getInstance().getThemeDescriptions().elementAt( index );
		LoadLayout();
		...
		if( Utils3D.getScreenWidth() < 500 )
		{
			nvlp.topMargin = (int)iLoongLauncher.getInstance().getResources().getDimension( RR.dimen.detile_scene_indi_margint_top );
		}
		else if( Utils3D.getScreenWidth() > 500 && Utils3D.getScreenWidth() < 800 )
		{
			nvlp.topMargin = (int)iLoongLauncher.getInstance().getResources().getDimension( RR.dimen.detile_scene_indi_margint_top_seven );
		}
		else
		{
			nvlp.topMargin = (int)iLoongLauncher.getInstance().getResources().getDimension( RR.dimen.detile_scene_indi_margint_top_big );
		}
		...
		{
			LinearLayout detilLinearLayout = (LinearLayout)View.inflate( mContext , RR.layout.scene_detil_item , null );
			ImageView detilImage = (ImageView)detilLinearLayout.findViewById( RR.id.imageSceneDetilThumb );
			ThemesGridView = new PageGridView( mContext );
			mglp = new LinearLayout.LayoutParams( cellwidth , cellheight );
			ThemesGridView.setCellDimensions( cellwidth , cellheight , 0 , 0 );
			...
			try
			{
				bmp = Tools.getImageFromInStream( mData.getContext().getAssets().open( SceneDescription.PREVIEW_DIR + bitmaps.get( i ) ) );
			}
			catch( IOException e )
			{
			}
			...
			switch( event.intValue() )
			{
				case BUTTON_EVENT_APPLYTHEME:
					...
					if( root != null )
					{
						...
						{
							sceneToast = iLoongLauncher.getInstance().getResources().getString( RR.string.sceneToast );
						}
						else
						{
							sceneToast = iLoongLauncher.getInstance().getResources().getString( RR.string.sceneChangeToast );
						}
						if( mData.mUse )
						{
							sceneToast = iLoongLauncher.getInstance().getResources().getString( RR.string.sceneToast );
						...
						Toast.makeText( mContext , sceneToast , Toast.LENGTH_SHORT ).show();
