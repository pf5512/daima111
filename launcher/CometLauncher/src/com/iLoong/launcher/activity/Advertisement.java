...
public class Advertisement extends PagerAdapter
{
			...
			Context context )
	{
		...
		if( Utils3D.getScreenWidth() >= specifiedScreenWidth )
		{
			for( int i = 0 ; i < imgList.size() ; i++ )
			{
				try
				{
					...
					layoutArray[i] = View.inflate( mContext , RR.layout.viewpager_item_preview , null );
					ImageView imgView = (ImageView)layoutArray[i].findViewById( RR.id.imageView );
		...
		else
		{
			for( int i = 0 ; i < imgList.size() ; i++ )
			{
				bmpArray[i] = Tools.getScaledBitmapBySpecifiedRes( 720 , Utils3D.getScreenWidth() , PREVIEW_DIR + imgList.get( i ) );
				layoutArray[i] = View.inflate( mContext , RR.layout.viewpager_item_preview , null );
				ImageView imgView = (ImageView)layoutArray[i].findViewById( RR.id.imageView );
				imgView.setImageBitmap( bmpArray[i] );
			}
	...
	public List<String> loadImageName()
	{
		...
		try
		{
			InputStream input = mContext.getAssets().open( PREVIEW_DIR + "preview.xml" );
			...
			int position )
	{
		...
		//		View view=layoutArray[position % 8];
		//		view.setOnClickListener( new OnClickListener() {
		//			@Override
		//			public void onClick(
		//					View v )
		//			{
		//				Intent  intent=new Intent( mContext , AdPreviewFullActivity.class );
		...
		try
		{
			( (ViewPager)collection ).addView( layoutArray[position % 8] , 0 );
		}
		catch( Exception e )
		{
		}
		return layoutArray[position % 8];
	}
