...
public class MediaList extends NPageBase
{
		...
		if( pageCount == 0 )
		{
			...
			iLoongLauncher.getInstance().mMainHandler.post( new Runnable() {
				...
				public void run()
				{
					...
					{
						Toast.makeText( iLoongLauncher.getInstance() , iLoongLauncher.getInstance().getResources().getString( RR.string.medialist_no_photo ) , Toast.LENGTH_SHORT ).show();
					}
					else if( adapter instanceof AudioAlbumAdapter )
					{
						Toast.makeText( iLoongLauncher.getInstance() , iLoongLauncher.getInstance().getResources().getString( RR.string.medialist_no_audio ) , Toast.LENGTH_SHORT ).show();
					}
					else if( adapter instanceof VideoAdapter )
					{
						Toast.makeText( iLoongLauncher.getInstance() , iLoongLauncher.getInstance().getResources().getString( RR.string.medialist_no_video ) , Toast.LENGTH_SHORT ).show();
					}
				}
			} );
		}
			...
			float parentAlpha )
	{
		...
		{
			batch.setColor( color.r , color.g , color.b , color.a * parentAlpha * ( height + y ) / height );
	...
	public void show()
	{
		...
		indicatorView.startTween( View3DTweenAccessor.OPACITY , Linear.INOUT , 0.5f , 1.0f , 0 , 0 );
	}
	...
	protected void updateEffect()
	{
		...
		if( super.getRandom() == false && mTypelist.get( this.mType ) == APageEase.COOLTOUCH_EFFECT_STANDARD )
		{
	...
	private void refreshBg()
	{
		...
		else if( ( mainmenu_bg_value.equals( DefaultLayout.lastMediaListMainmenuBgIndex + "" ) ) && ( translucentBg != null ) )
		{
			return;
		}
		else
