...
public class ThemeDescription
{
	...
	private String PREVIEW_FILENAME = "preview_config.xml";
	...
	private final String TAG_THEME = "app";
	private final String TAG_VERSION = "version";
	private final String TAG_INFO = "info";
	private final String TAG_ITEM = "item";
	private final String TAG_THEMEDIR = "theme";
	private final String TAG_DEFAULTDIR = "default";
	...
	public String defaultdir = "default";
	...
	{
		mContext = context;
		mClassName = ThemeManager.getInstance().getThemeClassName();
		...
		String file = PREVIEW_FILENAME;
		PreViewHandler handler = new PreViewHandler();
		LoadXml( file , handler );
		autoAdaptThemeDir = getAutoAdaptDir( context , themedir );
		file = autoAdaptThemeDir + CONFIG_FILENAME;
		if( !autoAdaptThemeDir.equals( themedir ) )
		{
			if( !mAndroidFiles.internal( file ).exists() )
			{
				file = themedir + CONFIG_FILENAME;
				if( !mAndroidFiles.internal( file ).exists() )
				{
		...
		// 加载手机分辨率精确适配的信息
		String specificFile = PREVIEW_FILENAME;
			...
			String fileName )
	{
		...
		if( strArray[0].equals( "theme" ) )
		{
			fileName = themedir + fileName.substring( fileName.indexOf( "/" ) );
			if( !mAndroidFiles.internal( fileName ).exists() )
			{
				fileName = "theme" + fileName.substring( fileName.indexOf( "/" ) );
			}
		}
	...
	class PreViewHandler extends DefaultHandler
	{
				...
				Attributes atts ) throws SAXException
		{
			...
			{
				themeclassname = atts.getValue( "className" );
			}
			if( mClassName.equals( themeclassname ) )
			{
				if( localName.equals( TAG_VERSION ) )
				{
				...
				else if( localName.equals( TAG_THEMEDIR ) )
				{
					themedir = atts.getValue( "path" );
				}
				else if( localName.equals( TAG_DEFAULTDIR ) )
				{
					defaultdir = atts.getValue( "path" );
				}
			}
		}
			...
			String filename )
	{
		...
		catch( IOException e )
		{
			try
			{
				stream = mContext.getAssets().open( themedir + "/" + filename );
			}
