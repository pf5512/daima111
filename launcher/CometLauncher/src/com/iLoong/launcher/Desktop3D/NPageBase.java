...
public class NPageBase extends ViewGroup3D
{
				...
				if( crystalGroup == null )
				{
					iLoongLauncher.getInstance().postRunnable( new Runnable() {
						...
						public void run()
						{
							...
							crystalGroup = new CrystalCore( "Crystal" , Utils3D.getScreenWidth() , Utils3D.getScreenHeight() - R3D.Workspace_celllayout_bottompadding );
						}
					} );
				}
			}
	...
	protected void changeEffect()
	{
		...
		{
			moving = true;
			mType = MathUtils.random( 0 , mTypelist.size() - 2 ); // 随机是数组最后一个特�?需要排�?
			if( mTypelist.get( mType ) == APageEase.COOLTOUCH_EFFECT_CRYSTAL )
			{
				mType = 3;
			}
	...
	protected void updateEffect()
	{
		...
		if( this.random == false && mTypelist.get( this.mType ) == APageEase.COOLTOUCH_EFFECT_STANDARD )
		{
			...
			float scroll_degree )
	{
		...
		if( getRandom() == false && mTypelist.get( this.mType ) == APageEase.COOLTOUCH_EFFECT_STANDARD )
		{
			APageEase.setStandard( true );
		}
		else
	...
	public void startAutoEffect()
	{
		...
		if( view_list.size() == 1 )
		{
			...
			if( bCrystalAnim )
			{
				crystalGroup.stop( duration , easeEquation );
			}
			return;
		}
			...
			float deltaY )
	{
		...
		if( crystalGroup != null )
		{
			if( NPageBase.this instanceof Workspace3D )
			{
				crystalGroup.start();
			}
		}
			...
			float parentAlpha )
	{
		...
		if( crystalGroup != null )
		{
			if( this instanceof Workspace3D && mTypelist.get( mType ) == APageEase.COOLTOUCH_EFFECT_CRYSTAL )
			{
				if( !AppHost3D.appList.isVisible() )
				{
