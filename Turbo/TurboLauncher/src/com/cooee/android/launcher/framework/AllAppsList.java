...
class AllAppsList
{
			...
			String packageName )
	{
		...
		if( matches.size() > 0 )
		{
			...
			{
				ApplicationInfo app=new ApplicationInfo( info , mIconCache , 0 );
				app.isNews=true;
			...
			String packageName )
	{
		...
		if( matches.size() > 0 )
		{
			...
			for( int i = 0 ; i < count ; i++ )
			{
				...
				{
					ApplicationInfo application=new ApplicationInfo( info , mIconCache );
					add(application);
				}
				else
				{
					...
					applicationInfo.flags = info.activityInfo.applicationInfo.flags;			
			...
			String packageName )
	{
		...
		if( packageName.equals( "com.iLoong.base.themebox" ) && DefaultLayout.personal_center_internal )
		{
			return new ArrayList<ResolveInfo>();
		}
		boolean includeCategoryLauncher = true;
		if( DefaultLayout.showAppList != null && DefaultLayout.showAppList.size() > 0 )
		{
			for( int i = 0 ; i < DefaultLayout.showAppList.size() ; i++ )
			{
				String showApp = DefaultLayout.showAppList.get( i );
				String showAppPackageName = "";
				String showAppClassName = "";
				if( showApp.contains( ";" ) )
				{
					showAppPackageName = showApp.split( ";" )[0];
					showAppClassName = showApp.split( ";" )[1];
				}
				else
				{
					showAppClassName = showApp;
				}
				if( showAppPackageName.equals( packageName ) )
				{
					includeCategoryLauncher = false;
					break;
				}
			}
		}
		final Intent mainIntent = new Intent( Intent.ACTION_MAIN , null );
		if( includeCategoryLauncher )
		{
