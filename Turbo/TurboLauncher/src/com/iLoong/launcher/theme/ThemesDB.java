...
public class ThemesDB
{
	...
	private void CreateTabScreenCount()
	{
		...
		sql = "create table IF NOT EXISTS tbscreen_count_big_icon ( ";
		sql += "screen_count INTEGER";
		sql += " );";
		try
		{
		...
		sql = "create table IF NOT EXISTS tbscreen_count_small_icon ( ";
		sql += "screen_count INTEGER";
		sql += " );";
		try
		{
	...
	public int getScreenCount()
	{
		...
		{
			sql = "select screen_count from tbscreen_count_big_icon;";
		}
		else if( icon_size_type == 2 && DefaultLayout.show_icon_size_different_layout )
		{
			sql = "select screen_count from tbscreen_count_small_icon;";
		}
		else
		{
	...
	public int getScreenCountByType(
			int type )
	{
		String sql;
		int screen_count = 0;
			...
			int screen_count )
	{
		...
		if( result.isAfterLast() )
		{
			if( icon_size_type == 0 && DefaultLayout.show_icon_size_different_layout )
			{
				sql = "insert into tbscreen_count_big_icon (screen_count) ";
			}
			else if( icon_size_type == 2 && DefaultLayout.show_icon_size_different_layout )
			{
				sql = "insert into tbscreen_count_small_icon (screen_count) ";
			}
			else
			{
		...
		else
		{
			if( icon_size_type == 0 && DefaultLayout.show_icon_size_different_layout )
			{
				sql = "update tbscreen_count_big_icon set ";
			}
			else if( icon_size_type == 2 && DefaultLayout.show_icon_size_different_layout )
			{
				sql = "update tbscreen_count_small_icon set ";
			}
			else
			{
	...
	public ThemeConfig getTheme()
	{
		...
		themeconf.theme = prefs.getString( "theme" , DefaultLayout.default_theme_package_name );
		return themeconf;
	}
