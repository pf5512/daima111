...
import com.coco.scene.scenebox.PathTool;
import com.coco.theme.themebox.util.DownModule;
import com.coco.theme.themebox.util.Tools;
import com.iLoong.base.themebox.R;


public class ScenePreviewHotAdapter extends BaseAdapter
{
		...
		if( strArray != null && strArray.length != 0 )
		{
			...
			for( String imgPath : strArray )
			{
				try
				{
					...
					Bitmap tmp = Tools.getPurgeableBitmap( imgPath , dis.widthPixels / 2 , dis.heightPixels / 2 );//BitmapFactory.decodeFile( imgPath );
					if( tmp == null )
					{
						needDownImage = true;
						break;
			...
			ViewGroup parent )
	{
		if( previewImages.size() == 0 )
		{
			if( needDownImage )
			{
				downModule.downloadPreview( packageName , DownloadList.Scene_Type );
			}
		}
		...
		if( viewType == VIEW_TYPE_DOWNLOADING )
		{
			...
			if( imgThumb == null )
				imgThumb = Tools.getPurgeableBitmap( PathTool.getThumbFile( packageName ) , -1 , -1 );
			if( imgThumb != null )
			{
