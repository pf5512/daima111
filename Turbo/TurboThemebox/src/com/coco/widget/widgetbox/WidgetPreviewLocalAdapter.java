...
public class WidgetPreviewLocalAdapter extends BaseAdapter
{
	...
	// 构造
	public WidgetPreviewLocalAdapter(
			Context c ,
			String pkg ,
			Context dstContext )
	{
		...
		if( paths != null && paths.length != 0 )
		{
			...
			for( int i = 0 ; i < paths.length ; i++ )
			{
				try
				{
					...
					previewImages[i] = Tools.getPurgeableBitmap( paths[i] , dis.widthPixels / 2 , dis.heightPixels / 2 );
				}
			...
			ViewGroup parent )
	{
		...
		if( isPreview )
			...
			convertView.setLayoutParams( new Gallery.LayoutParams( ViewGroup.LayoutParams.WRAP_CONTENT , ViewGroup.LayoutParams.WRAP_CONTENT ) );
