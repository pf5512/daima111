...
public class WidgetPreviewHotAdapter extends BaseAdapter
{
	...
	// 构造
	public WidgetPreviewHotAdapter(
			Context cxt ,
			String pkgName ,
			DownModule down )
	{
		...
		if( strArray != null && strArray.length != 0 )
		{
			...
			for( String imgPath : strArray )
			{
				try
				{
					...
					Bitmap tmp = Tools.getPurgeableBitmap( imgPath , dis.widthPixels / 2 , dis.heightPixels / 2 );
					if( tmp == null )
					{
						needDownImage = true;
						break;
			...
			ViewGroup parent )
	{
		if( previewImages.size() == 0 )
		{
			if( needDownImage )
			{
				downModule.downloadPreview( packageName , DownloadList.Widget_Type );
			}
		}
		...
		if( viewType == VIEW_TYPE_DOWNLOADING )
		{
			...
			if( imgThumb == null )
				imgThumb = Tools.getPurgeableBitmap( PathTool.getThumbFile( packageName ) , -1 , -1 );//BitmapFactory.decodeFile( PathTool.getThumbFile( packageName ) );
			if( imgThumb != null )
			{
