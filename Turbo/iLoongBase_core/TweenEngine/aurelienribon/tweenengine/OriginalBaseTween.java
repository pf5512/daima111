 ...
 */
public abstract class OriginalBaseTween<T>
{
	...
	public void kill()
	{
		if( this.isInitialized ) //防止kill已经free的tween,xupin&hupeng,20130926
			isKilled = true;
	}
	...
	protected abstract void computeOverride(
			int step ,
			int lastStep ,
			float delta );
	...
	protected abstract boolean containsTarget(
			Object target );
	...
	protected void forceToEnd(
			float time )
	{
		//Log.e("tween", "forceToEnd:"+this);
			...
			int type )
	{
		if( callback != null && ( callbackTriggers & type ) > 0 )
			callback.onEvent( type , holder );
	}
	
	protected boolean isYoyo(
			int step )
	{
	...
	private void testInnerTransition(
			int lastStep )
	{
		if( isIterationStep )
			return;
		if( step > lastStep )
			forceEndValues( step - 1 );
		else if( step < lastStep )
			forceStartValues( step + 1 );
	}
	
	private void testLimitTransition(
			int lastStep )
	{
		if( repeatCnt < 0 || step == lastStep )
			return;
		if( step > repeatCnt * 2 )
	...
	private void testCompletion()
	{
		...
		//if(isFinished)Log.e("tween", "finished:"+this);
	}
