...
public class ThemeReceiver extends BroadcastReceiver
{
	...
	private class addRunnable implements Runnable
	{
		...
		{
			String packageName = intent.getData().getSchemeSpecificPart();
			Intent themeIntent = new Intent( ThemeManager.ACTION_INTENT_THEME , null );
			themeIntent.setPackage( packageName );
			List<ResolveInfo> themesinfo = mContext.getPackageManager().queryIntentActivities( themeIntent , 0 );
			...
			if( themesinfo != null )
			{
				if( themesinfo.size() > 0 )
				{
					...
					if( imgThumb != null )
					{
						StaticClass.saveMyBitmap( mContext , infor.getPackageName() , infor.getClassName() , imgThumb );
					}
				}
			}
		}
