...
public class ThemePreviewHotActivity extends Activity
{
			...
			Bundle savedInstanceState )
	{
		...
		btnApply.setOnClickListener( new View.OnClickListener() {
			...
			public void onClick(
					View v )
			{
				//MobclickAgent.onEvent( mContext , "BoxApplyTheme" , packageName );
					...
					View v )
			{
				...
				if( apt != null && apt instanceof ThemePreviewHotAdapter ){
					...
					if(pha.needDownImage){
						Toast.makeText( mContext , R.string.init_waitting , Toast.LENGTH_SHORT ).show();
						return ;
					}
				}
				//MobclickAgent.onEvent( mContext , "BoxDownloadTheme" , packageName );
				...
				if( MainActivity.sbGoogleVersion == true && !channelString.equals( "amazon" ) )
				{
					...
					{
						Uri playUri = Uri.parse( "https://play.google.com/store/apps/details?id=" + packageName );
						Intent browserIntent = new Intent( Intent.ACTION_VIEW , playUri );
						if( isPlayStoreInstalled() )
						{
							browserIntent.setClassName( GOOGLE_PLAY_STORE_PACKAGE_NAME , GOOGLE_PLAY_STORE_CLASS_NAME );
						}
		...
		btnUninstall.setOnClickListener( new View.OnClickListener() {
			...
			public void onClick(
					View v )
			{
				//MobclickAgent.onEvent( mContext , "BoxUninstallTheme" , packageName );
				removePackage();
			}
		} );
	}
	...
	private void updateShowInfo()
	{
		...
		else
		{
			...
			String author = getString( R.string.previewThemeSize , themeInformation.getApplicationSize() / 1024 , themeInformation.getAuthor( mContext ) );
