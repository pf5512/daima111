...
public class WidgetGridHotAdapter extends BaseAdapter {
	...
	public WidgetGridHotAdapter(Context cxt, DownModule down) {
		context = cxt;
		downThumb = down;
	...
	public void reloadPackage(Set<String> pkgNameSet) {
		if (pageTask != null
				&& pageTask.getStatus() != PageTask.Status.FINISHED) {
			pageTask.cancel(true);
		}
	...
	public List<WidgetInformation> queryPackage(Set<String> pkgNameSet) {
		// appList.clear();
		...
		if (FunctionConfig.isPriceVisible()) {
			...
			if (info.getPrice() > 0) {
				...
				boolean ispay = Tools.isContentPurchased(context,
						DownloadList.Widget_Type, info.getPackageName());
				if (ispay) {
					viewHolder.pricetxt
	...
	public class PageTask extends
			AsyncTask<String, Integer, List<WidgetInformation>> {

		public PageTask() {
		}

		@Override
		protected void onPostExecute(List<WidgetInformation> result) {
		...
		@Override
		protected List<WidgetInformation> doInBackground(String... params) {
			// TODO Auto-generated method stub
