...
public class WidgetInformation extends ThemeInformation {
	...
	public boolean isDownloaded(Context context) {
		...
		if (!file.exists()
			...
			dSv.updateDownloadSizeAndStatus(getPackageName(), 0,
					DownloadStatus.StatusInit, DownloadList.Widget_Type);
			return false;
		}
		return downloaded;
	}
	...
	public void setActivity(Context cxt, AppWidgetProviderInfo provider) {
		...
		mSystem = provider.provider.getPackageName().equals(
		...
		if (installed) {
			...
			try {
				...
				try {
					if (stream == null) {
						try {
							DisplayMetrics displayMetrics = cxt.getResources()
									.getDisplayMetrics();
							iconPath = "iLoong/image/"
						...
						try {
							thumbImage = Tools.getPurgeableBitmap(stream, -1,
									-1);// BitmapFactory.decodeStream( stream );
