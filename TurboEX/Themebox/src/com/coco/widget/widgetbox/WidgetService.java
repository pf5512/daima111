...
public class WidgetService {
	...
	public List<WidgetInformation> queryInstallList() {
		...
		for (AppWidgetProviderInfo widget : widgets) {
			...
			if (pkg.contains("com.iLoong") || pkg.contains("com.CoCo")
					|| pkg.contains("com.cooee") || pkg.contains("com.coco")) {
	...
	public List<WidgetInformation> queryDownloadList() {
		...
		List<DownloadThemeItem> downlist = dSv
				.queryTable(DownloadList.Widget_Type);
		List<WidgetInformation> result = new ArrayList<WidgetInformation>();
	...
	public List<WidgetInformation> queryHotList() {
		...
		List<ThemeInfoItem> hotlist = hotSv
				.queryTable(DownloadList.Widget_Type);
		List<WidgetInformation> result = new ArrayList<WidgetInformation>();
	...
	public WidgetInformation queryWidget(String packageName, String className) {
		...
		DownloadThemeService dSv = new DownloadThemeService(mContext);
		DownloadThemeItem downItem = dSv.queryByPackageName(packageName,
	...
	public ComponentName queryComponent(String packageName) {
		List<ActivityInfo> installList = queryWidgetListByPackageName(packageName);
	...
	private List<ActivityInfo> queryWidgetActivityList() {
		...
		List<ResolveInfo> themesinfo = pm.queryIntentActivities(intent,
	...
	private List<ActivityInfo> queryWidgetListByPackageName(String pkgName) {
		List<ActivityInfo> resultList = new ArrayList<ActivityInfo>();
		Intent intent = new Intent("com.iLoong.widget", null);
	...
	private WidgetInformation activityToWidget(ActivityInfo activityInfo) {
		WidgetInformation item = new WidgetInformation();
		item.setActivity(mContext, activityInfo);
		return item;
	}

	private WidgetInformation activityToWidget(AppWidgetProviderInfo provider) {
		WidgetInformation item = new WidgetInformation();
