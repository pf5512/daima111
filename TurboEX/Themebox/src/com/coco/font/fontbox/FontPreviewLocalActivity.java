...
public class FontPreviewLocalActivity extends Activity {
			...
			public void onClick(View v) {
				if (FunctionConfig.isEnable_topwise_style()) {
					...
					SharedPreferences pref = PreferenceManager
							.getDefaultSharedPreferences(FontPreviewLocalActivity.this);
					if (fontType == 1) {
						intent.putExtra("action", "settings.default.font");
						pref.edit()
								.putString("currentFont",
										"settings.default.font").commit();
					} else {
	...
	public class FontPreviewLocalAdapter extends BaseAdapter {
		...
		// 构造
		public FontPreviewLocalAdapter(Context cxt, int font_type) {
			mContext = cxt;
			fontType = font_type;
			loadImage();
		}
		...
		private void loadImage() {
			...
			try {
				InputStream in = mContext.getAssets().open(
						"fonts/font_preview_" + fontType + ".png");
				DisplayMetrics dis = mContext.getResources()
						.getDisplayMetrics();
				Bitmap tmp = Tools.getPurgeableBitmap(in, dis.widthPixels / 2,
						dis.heightPixels / 2);
