...
public class FontPreviewActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
					...
					.setOnItemSelectedListener(new OnItemSelectedListener() {
						...
						public void onItemSelected(AdapterView<?> parent,
							...
							if (parent.getAdapter().getCount() <= 1) {
								...
								scrollGallery.setMax(parent.getAdapter()
										.getCount() - 1);
							}
						}
		...
		btnApply.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (FunctionConfig.isEnable_topwise_style()) {
					String path = copyFile(PathTool.getAppFile(packageName),
							packageName);
					Intent intent = new Intent("com.topwise.fontpath");
					intent.putExtra("action", path);
					SharedPreferences pref = PreferenceManager
							.getDefaultSharedPreferences(FontPreviewActivity.this);
			...
			public void onClick(View v) {
				if (com.coco.theme.themebox.StaticClass.canDownToInternal) {
					...
					if (num >= 5) {
						...
						.isAllowDownloadWithToast(FontPreviewActivity.this)) {
					return;
				}
		...
		butBuy.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// FontPreviewActivity.this , paymentInfo );
			}
		});
		// 删除
	...
	private void updateShowStatus() {
		...
		if (!Information.isDownloaded(this)) {
			boolean ispay = Tools.isContentPurchased(mContext,
					DownloadList.Font_Type, packageName);
	...
	private void loadFontInformation(boolean reloadGallery) {
		FontService service = new FontService(this);
		Information = service.queryFont(packageName);
		if (reloadGallery) {
			galleryPreview.setAdapter(new FontPreviewHotAdapter(this,
	...
	public String copyFile(String oldPath, String pkgname) {
		String path = null;
		try {
			int byteread = 0;
			...
			if (oldfile.exists()) { // 文件存在时
				...
				String driPath = getFilesDir().getParent() + File.separator
						+ "TTF";
	...
	// Toast.makeText( FontPreviewActivity.this , "计费成功" , Toast.LENGTH_SHORT
	// ).show();
	...
	// Toast.makeText( FontPreviewActivity.this , "计费失败" +
	// paymentInfo.getVersionName() , Toast.LENGTH_SHORT ).show();
	// break;
	// case CooeePaymentResultNotify.COOEE_PAYMENT_RESULT_CANCEL_BY_USER:
	// Toast.makeText( FontPreviewActivity.this , "用户取消付费" , Toast.LENGTH_SHORT
	// ).show();
	// break;
	// case 1:
	// Toast.makeText( FontPreviewActivity.this , "配置免费" , Toast.LENGTH_SHORT
	// ).show();
	// break;
	// case 2:
	// Toast.makeText( FontPreviewActivity.this , "不需要重复计费" , Toast.LENGTH_SHORT
	// ).show();
	// break;
	// case 3:
	// Toast.makeText( FontPreviewActivity.this , "无可用指令" , Toast.LENGTH_SHORT
	// ).show();
	// break;
	// }
	// }
