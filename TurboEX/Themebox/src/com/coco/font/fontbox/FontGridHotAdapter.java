...
public class FontGridHotAdapter extends BaseAdapter {
	...
	public FontGridHotAdapter(Context cxt, DownModule down) {
		context = cxt;
		downThumb = down;
	...
	public List<FontInformation> queryPackage(Set<String> pkgNameSet) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (FunctionConfig.isPriceVisible()) {
			...
			if (info.getPrice() > 0) {
				...
				boolean ispay = Tools.isContentPurchased(context,
						DownloadList.Font_Type, info.getPackageName());
				if (ispay) {
					viewHolder.pricetxt
		...
		if (info.getDownloadStatus() == DownloadStatus.StatusInit
			...
			if (info.getDownloadStatus() == DownloadStatus.StatusDownloading) {
				...
				viewHolder.barPause.setVisibility(View.INVISIBLE);
				viewHolder.barDownloading
						.setProgress(info.getDownloadPercent());
			} else {
				viewHolder.barDownloading.setVisibility(View.INVISIBLE);
	...
	public class PageTask extends
			AsyncTask<String, Integer, List<FontInformation>> {

		public PageTask() {
		}

		@Override
		protected void onPostExecute(List<FontInformation> result) {
		...
		@Override
		protected List<FontInformation> doInBackground(String... params) {
			// TODO Auto-generated method stub
			DownloadThemeService dSv = new DownloadThemeService(context);
			List<DownloadThemeItem> downlist = dSv
					.queryTable(DownloadList.Font_Type);
