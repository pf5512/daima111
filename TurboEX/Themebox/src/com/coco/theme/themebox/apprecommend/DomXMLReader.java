...
public class DomXMLReader {
	...
	public DomXMLReader(Context context) {
		mcontext = context;
	...
	public void readXML(InputStream inStream) {
		...
		try {
			...
			db.execSQL("CREATE TABLE IF NOT EXISTS " + Profile.TABLE_NAME + "("
					+ Profile.COLUMN_ITEMTYPE + " TEXT,"
					+ Profile.COLUMN_PACKAGE + " TEXT,"
					+ Profile.COLUMN_VERSION + " TEXT," + Profile.COLUMN_ID
					+ " INTEGER PRIMARY KEY AUTOINCREMENT,"
					+ Profile.COLUMN_NAME_CH + " TEXT,"
					+ Profile.COLUMN_NAME_EN + " TEXT," + Profile.COLUMN_APK
					+ " TEXT," + Profile.COLUMN_ICON + " TEXT,"
					+ Profile.COLUMN_URL_APK + " TEXT,"
					+ Profile.COLUMN_URL_ICON + " TEXT);");
			...
			for (int i = 0; i < items4.getLength(); i++) {
				...
				for (int j = 0; j < items5.getLength(); j++) {
					...
					values.put(Profile.COLUMN_ITEMTYPE,
							personNode1.getTextContent());
				}
				...
				for (int j = 0; j < items6.getLength(); j++) {
					...
					values.put(Profile.COLUMN_NAME_CH,
							personNode1.getTextContent());
				}
				...
				for (int j = 0; j < items7.getLength(); j++) {
					...
					values.put(Profile.COLUMN_PACKAGE,
							personNode1.getTextContent());
				}
				...
				for (int j = 0; j < items8.getLength(); j++) {
					...
					values.put(Profile.COLUMN_NAME_EN,
							personNode1.getTextContent());
				}
				...
				for (int j = 0; j < items9.getLength(); j++) {
					...
					values.put(Profile.COLUMN_ICON,
							personNode1.getTextContent());
				}
			...
			Intent intent = new Intent(
					StaticClass.ACTION_THEME_UPDATE_RECOMMEND);
