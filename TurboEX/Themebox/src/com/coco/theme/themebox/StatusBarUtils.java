...
public final class StatusBarUtils {// 状态栏工具
	...
	private static Method loadSetStatusBarBgTransparentMethod() {// 加载设置状态栏透明度的方法
		Method method = null;
		Class<?> cls = null;
		try {
			cls = Class.forName(STATUS_BAR_CLASS_NAME);
			method = cls.getMethod(SET_STATUS_BAR_TRANSPARENT_METHOD,
					boolean.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/** 设置状态栏透明度 **/
	public static void setStatusBarBackgroundTransparent(final Context context,
			boolean transparent) {
		...
		try {
			method.invoke(context.getSystemService(STATUS_BAR_SERVICE),
					transparent);
		} catch (Exception e) {
			e.printStackTrace();
		}
