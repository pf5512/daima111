...
public class ThemeService {
	...
	public List<ThemeInformation> queryDownloadList() {
		...
		List<DownloadThemeItem> downlist = dSv
				.queryTable(DownloadList.Theme_Type);
		List<ThemeInformation> result = new ArrayList<ThemeInformation>();
	...
	public ComponentName queryCurrentTheme() {
		...
		return new ComponentName(activityList.get(0).packageName,
				activityList.get(0).name);
	}

	public boolean applyTheme(ComponentName apply) {
		ThemesDB db = new ThemesDB(mContext);
		db.SaveThemes(apply.getPackageName());
		if (ThemesDB.ACTION_LAUNCHER_RESTART == null
				|| ThemesDB.ACTION_LAUNCHER_RESTART.equals("")) {
	...
	public ThemeInformation queryTheme(String packageName, String className) {
	...
	private List<ActivityInfo> queryThemeListByPackageName(String pkgName) {
		List<ActivityInfo> resultList = new ArrayList<ActivityInfo>();
		Intent intent = new Intent(ThemeManager.ACTION_INTENT_THEME, null);
