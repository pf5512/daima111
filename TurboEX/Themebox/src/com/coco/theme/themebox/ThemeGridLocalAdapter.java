...
public class ThemeGridLocalAdapter extends BaseAdapter {
	...
	public ThemeGridLocalAdapter(Context cxt, DownModule down) {
		context = cxt;
		downThumb = down;
	...
	private List<ThemeInformation> queryPackage() {
		...
		for (ThemeInformation info : installList) {
			info.setThumbImage(context, info.getPackageName(),
					info.getClassName());
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView != null) {
			...
			int itemWidth = (int) (context.getResources().getDisplayMetrics().widthPixels / 3 - 6 * 2 * context
					.getResources().getDisplayMetrics().density);
			viewHolder.viewThumb
					.setLayoutParams(new RelativeLayout.LayoutParams(
							(int) (itemWidth + 6 * 2 * context.getResources()
									.getDisplayMetrics().density),
							(int) (itemWidth / 0.6f + 6 * 2 * context
									.getResources().getDisplayMetrics().density)));
		}
		...
		// Tools.Recyclebitmap( imgDefaultThumb , imgThumb ,
		// viewHolder.viewThumb , recycle );
		if (imgThumb == null) {
			...
			// {
			// StaticClass.saveMyBitmap( context , themeInfo.getPackageName() ,
			// themeInfo.getClassName() , themeInfo.getThumbImage() );
			// }
			// addBitmapToMemoryCache( themeInfo.getPackageName() ,
		...
		if (currentTheme != null
				&& currentTheme.getPackageName().equals(
		...
		if (themeInfo.isInstalled(context)
			...
			if (themeInfo.getDownloadStatus() == DownloadStatus.StatusDownloading) {
				...
				viewHolder.barPause.setVisibility(View.INVISIBLE);
				viewHolder.barDownloading.setProgress(themeInfo
						.getDownloadPercent());
			} else {
				viewHolder.barDownloading.setVisibility(View.INVISIBLE);
				viewHolder.barPause.setVisibility(View.VISIBLE);
	...
	public class PageItemTask extends
			AsyncTask<ThemeInformation, Integer, ThemeInformation> {
		...
		@Override
		protected void onPostExecute(ThemeInformation themeInfo) {
			// TODO Auto-generated method stub
			notifyDataSetChanged();
		}
		...
		@Override
		protected ThemeInformation doInBackground(ThemeInformation... params) {
			// TODO Auto-generated method stub
	...
	public class PageTask extends
			AsyncTask<String, Integer, List<ThemeInformation>> {

		public PageTask() {
		}

		@Override
		protected void onPostExecute(List<ThemeInformation> result) {
			// TODO Auto-generated method stub
			if (result == null) {
				return;
			}
			...
			notifyDataSetChanged();
			pageTask = null;
			// if( itemTask != null && itemTask.getStatus() !=
			// PageItemTask.Status.FINISHED )
			// {
		...
		@Override
		protected List<ThemeInformation> doInBackground(String... params) {
			// TODO Auto-generated method stub
