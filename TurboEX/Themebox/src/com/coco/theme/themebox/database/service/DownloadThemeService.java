...
public class DownloadThemeService {
	...
	public static String getCreateSql() {
		String result = String
				.format(
				// Jone
				"CREATE TABLE %s (%s TEXT , %s INTEGER, %s INTEGER, %s TEXT,%s TEXT, %s INTEGER, %s TEXT, %s INTEGER,"
						+ " %s TEXT, %s TEXT, %s TEXT, %s TEXT, %s INTEGER , %s TEXT , "
						+ "%s TEXT, %s TEXT, %s TEXT, %s TEXT, CONSTRAINT PK_%s PRIMARY KEY (%s,%s));",
						TABLE_NAME,
						FIELD_PACKAGE_NAME,
						FIELD_DOWNLOAD_SIZE,
						...
						FIELD_INTRODUCTION, FIELD_UPDATE_TIME, FIELD_TYPE,
						FIELD_THEME_PRICE, FIELD_PRICE_POINT,
						FIELD_ENGINE_PACKAGENAME, FIELD_ENGINE_URL,
						FIELD_ENGINE_SIZE, FIELD_ENGINE_DESC, TABLE_NAME,
						FIELD_PACKAGE_NAME, FIELD_TYPE);
		return result;
	}
	...
	public synchronized List<DownloadThemeItem> queryTable(String type) {
	...
	public boolean insertItem(DownloadThemeItem item) {
	...
	public DownloadThemeItem queryByPackageName(String packageName, String type) {
	...
	public boolean updateDownloadSizeAndStatus(String packageName, int size,
			DownloadStatus status, String type) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		ContentValues cv = new ContentValues();
		cv.put(FIELD_DOWNLOAD_SIZE, size);
		cv.put(FIELD_DOWNLOAD_STATUS, status.getValue());
		try {
			int updateResult = db.update(TABLE_NAME, cv, FIELD_PACKAGE_NAME
					+ "=? and " + FIELD_TYPE + "=?", new String[] {
					packageName, type });
			db.close();
			DbHelper.lock_db = false;
			if (updateResult <= 0) {
	...
	public boolean updateDownloadSizeAndStatus(String packageName,
			int downSize, int totalSize, DownloadStatus status, String type) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		ContentValues cv = new ContentValues();
	...
	public boolean updateDownloadStatus(String packageName,
			DownloadStatus status, String type) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		ContentValues cv = new ContentValues();
		cv.put(FIELD_DOWNLOAD_STATUS, status.getValue());
	...
	public boolean pauseAllDownloadStatus() {
		...
		int updateResult = db
				.update(TABLE_NAME,
						cv,
						FIELD_DOWNLOAD_STATUS + "=?",
						new String[] { DownloadStatus.StatusDownloading
								.getValue() + "" });
		db.close();
		DbHelper.lock_db = false;
		if (updateResult <= 0) {
	...
	public boolean deleteItem(String packageName, String type) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		int delResult = db.delete(TABLE_NAME, FIELD_PACKAGE_NAME + "=? and "
				+ FIELD_TYPE + "=?", new String[] { packageName, type });
		db.close();
		DbHelper.lock_db = false;
		if (delResult <= 0) {
	...
	private DownloadThemeItem readLockInfo(Cursor cursor) {
		try {
			...
			item.setDownloadSize(cursor.getInt(cursor
					.getColumnIndexOrThrow(FIELD_DOWNLOAD_SIZE)));
			item.setDownloadStatus(DownloadStatus.fromValue(cursor
					.getInt(cursor.getColumnIndexOrThrow(FIELD_DOWNLOAD_STATUS))));
			...
			item.setPrice(cursor.getInt(cursor
					.getColumnIndexOrThrow(FIELD_THEME_PRICE)));
