...
public class ThemeInformation {
	...
	private boolean isAppInstalled(Context cxt, String uri) {
	...
	public boolean isDownloaded(Context context) {
		File file = new File(PathTool.getAppDir() + "/" + getPackageName()
				+ ".apk");
		if (!file.exists()
				&& (getDownloadStatus() == DownloadStatus.StatusInit || getDownloadStatus() == DownloadStatus.StatusFinish)) {
			File f1 = new File(PathTool.getDownloadingDir() + getPackageName()
					+ "_app.tmp");
			f1.delete();
			DownloadThemeService dSv = new DownloadThemeService(context);
			dSv.updateDownloadSizeAndStatus(getPackageName(), 0,
					DownloadStatus.StatusInit, DownloadList.Theme_Type);
			return false;
		}
		return downloaded;
	}
	...
	public void loadDetail(Context cxt) {
		...
		if (installed) {
			...
			if (new File(thumbPath).exists()) {
				try {
					thumbImage = Tools.getPurgeableBitmap(thumbPath,
							Tools.dip2px(cxt, FunctionConfig.getGridWidth()),
							Tools.dip2px(cxt, FunctionConfig.getGridHeight()));// BitmapFactory.decodeFile(
																				// thumbPath
																				// );
	...
	public Bitmap getThumbImage(Context remoteContext, ContentConfig cfg) {
	...
	public void reloadThumb() {
		...
		if (new File(thumbPath).exists()) {
			try {
				thumbImage = Tools.getPurgeableBitmap(thumbPath, -1, -1);// BitmapFactory.decodeFile(
																			// thumbPath
																			// );
			} catch (OutOfMemoryError error) {
	...
	public void disposeThumb() {
		if (thumbImage != null && !thumbImage.isRecycled()) {
			Log.v("meminfo",
					"meminfo recycle " + "(w,h)=" + thumbImage.getWidth() + " "
							+ thumbImage.getHeight() + " " + thumbImage);
			thumbImage.recycle();
		}
		thumbImage = null;
	}
