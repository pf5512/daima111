...
public class ThemeGridHotAdapter extends BaseAdapter {
	...
	public ThemeGridHotAdapter(Context cxt, DownModule down) {
		context = cxt;
		downThumb = down;
	...
	public List<ThemeInformation> queryPackage(Set<String> pkgNameSet) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (FunctionConfig.isPriceVisible()) {
			...
			if (lockInfo.getPrice() > 0) {
				...
				boolean ispay = Tools.isContentPurchased(context,
						DownloadList.Theme_Type, lockInfo.getPackageName());
				if (ispay) {
					viewHolder.pricetxt
		...
		if (lockInfo.getDownloadStatus() == DownloadStatus.StatusInit
			...
			if (lockInfo.getDownloadStatus() == DownloadStatus.StatusDownloading) {
				...
				viewHolder.barPause.setVisibility(View.INVISIBLE);
				viewHolder.barDownloading.setProgress(lockInfo
						.getDownloadPercent());
			} else {
				viewHolder.barDownloading.setVisibility(View.INVISIBLE);
