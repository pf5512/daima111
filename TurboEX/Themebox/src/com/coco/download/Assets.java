...
public class Assets {
	...
	public static String getSerialNo(Context context) {
		...
		if (config != null) {
			try {
				String serialno = config
						.getString(PREFERENCE_KEY_CONFIG_SERIALNO);
				return serialno;
	...
	private static void initAssets(Context context) {
		...
		if (com.coco.theme.themebox.util.FunctionConfig.isNetVersion())
			...
			uri = Uri.parse("content://" + "com.iLoong.launcher.pub.provider"
					+ "/" + "config");
		...
		try {
			...
			if (cursor != null) {
				if (cursor.moveToFirst()) {
					serialno = cursor.getString(cursor
							.getColumnIndex("propertyValue"));
				}
				cursor.close();
			} else {
		...
		try {
			...
			if (cursor != null) {
				if (cursor.moveToFirst()) {
					app_id = cursor.getString(cursor
							.getColumnIndex("propertyValue"));
				}
				cursor.close();
			} else {
		...
		try {
			...
			if (cursor != null) {
				if (cursor.moveToFirst()) {
					template_id = cursor.getString(cursor
							.getColumnIndex("propertyValue"));
				}
				cursor.close();
			} else {
		...
		try {
			...
			if (cursor != null) {
				if (cursor.moveToFirst()) {
					channel_id = cursor.getString(cursor
							.getColumnIndex("propertyValue"));
				}
				cursor.close();
			} else {
		...
		try {
			Log.v("theme", "assets " + PREFERENCE_KEY_CONFIG_DOMAIN + ":"
					+ config.getString(PREFERENCE_KEY_CONFIG_DOMAIN));
			Log.v("theme", "assets " + PREFERENCE_KEY_CONFIG_SERIALNO + ":"
					+ config.getString(PREFERENCE_KEY_CONFIG_SERIALNO));
			Log.v("theme", "assets " + PREFERENCE_KEY_CONFIG_APPID + ":"
					+ config.getString(PREFERENCE_KEY_CONFIG_APPID));
			Log.v("theme", "assets " + PREFERENCE_KEY_CONFIG_TEMPLATEID + ":"
					+ config.getString(PREFERENCE_KEY_CONFIG_TEMPLATEID));
			Log.v("theme", "assets " + PREFERENCE_KEY_CONFIG_CHANNELID + ":"
	...
	public static String getEffect(Context context, String name) {
		ContentResolver resolver = context.getContentResolver();
		Uri uri;
		if (com.coco.theme.themebox.util.FunctionConfig.isNetVersion())
			uri = Uri.parse("content://" + TURBO_LAUNCHER_PROVIDER_AUTHOR + "/"
		...
		try {
			...
			if (cursor != null) {
				if (cursor.moveToFirst()) {
					effect = cursor.getString(cursor
							.getColumnIndex("propertyValue"));
				}
				cursor.close();
			} else {
	...
	public static String getTheme(Context context, String name) {
		ContentResolver resolver = context.getContentResolver();
		Uri uri;
		if (com.coco.theme.themebox.util.FunctionConfig.isNetVersion()) {
			uri = Uri.parse("content://" + TURBO_LAUNCHER_PROVIDER_AUTHOR + "/"
	...
	public static String getScene(Context context, String name) {
		ContentResolver resolver = context.getContentResolver();
		Uri uri;
		if (com.coco.theme.themebox.util.FunctionConfig.isNetVersion())
			uri = Uri.parse("content://" + TURBO_LAUNCHER_PROVIDER_AUTHOR + "/"
