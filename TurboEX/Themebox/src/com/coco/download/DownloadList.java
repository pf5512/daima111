...
public class DownloadList {
	...
	public static String types[] = new String[] { Theme_Type, Lock_Type,
			Scene_Type, Wallpaper_Type, Widget_Type, Font_Type,
			LiveWallpaper_Type };
	...
	private void downloadListFinish() {
		...
		for (String item : types) {
			if (parserXml.parseList(DownloadList.getInstance(mContext)
					.getListInfo(), item, mContext)) {
					...
					try {
						...
						if (retCode == 0) {
							...
							for (Iterator iter = json.keys(); iter.hasNext();) {
								...
								for (Iterator mIterator = tmJson.keys(); mIterator
										.hasNext();) {
									String mKey = (String) mIterator.next();
									if (mKey.equals("tabid")) {
										listInfo.setTabid(tmJson.get(mKey)
												.toString());
									} else if (mKey.equals("enname")) {
										listInfo.setEnname(tmJson.get(mKey)
												.toString());
									} else if (mKey.equals("cnname")) {
										listInfo.setCnname(tmJson.get(mKey)
												.toString());
									} else if (mKey.equals("twname")) {
										listInfo.setTwname(tmJson.get(mKey)
												.toString());
									} else if (mKey.equals("typeid")) {
										listInfo.setTypeid(tmJson.get(mKey)
												.toString());
									} else {
										JSONObject jsObj = (JSONObject) tmJson
												.get(mKey);
										ItemInfo itemInfo = new ItemInfo();
										itemInfo.setIndex(mKey);
										System.out.println("mKey = " + mKey
												+ " jsObj.getString() = "
												+ jsObj.getString("cnname"));
										itemInfo.setResid(jsObj
												.getString("resid"));
										itemInfo.setEnname(jsObj
												.getString("enname"));
										itemInfo.setCnname(jsObj
												.getString("cnname"));
										// itemInfo.setTwname(jsObj.getString("twname"));
										itemInfo.setResurl(jsObj
												.getString("resurl"));
										itemInfo.setPackname(jsObj
												.getString("packname"));
										itemInfo.setSize(jsObj
												.getString("size"));
										itemInfo.setAuthor(jsObj
												.getString("author"));
										itemInfo.setAboutchinese(jsObj
												.getString("aboutchinese"));
										itemInfo.setVersion(jsObj
												.getString("version"));
										itemInfo.setVersionname(jsObj
												.getString("versionname"));
										itemInfo.setAboutenglish(jsObj
												.getString("aboutenglish"));
										itemInfo.setPrice(jsObj
												.getString("price"));
										itemInfo.setPricedetail(jsObj
												.getString("pricedetail"));
										itemInfo.setPricePoint(jsObj
												.getString("pricepoint"));
										itemInfo.setIcon(jsObj
												.getString("icon"));
										itemInfo.setThumbimg(jsObj
												.getString("thumbimg"));
										try {
											itemInfo.setEnginepackname(jsObj
													.getString("enginepackname"));
											itemInfo.setEngineurl(jsObj
													.getString("engineurl"));
											itemInfo.setEnginesize(jsObj
													.getString("enginesize"));
										...
										try {
											itemInfo.setEnginedesc(jsObj
													.getString("enginedesc"));
										} catch (JSONException e) {
											itemInfo.setEnginedesc(null);
										}
										try {
											itemInfo.setThirdparty(jsObj
													.getString("thirdparty"));
										} catch (JSONException e) {
											itemInfo.setThirdparty(null);
										}
										try {
											JSONArray preview = jsObj
													.getJSONArray("previewlist");
											String[] pre = new String[preview
													.length()];
											for (int k = 0; k < preview
													.length(); k++) {
												pre[k] = preview.getString(k);
											}
											itemInfo.setPreviewlist(pre);
										} catch (JSONException e) {
											itemInfo.setPreviewlist(new String[] { itemInfo
													.getThumbimg() });
										}
	...
	private boolean ErrTimeTransfinite(TYPE type) {
		if (errtime.equals(DEFAULT_ERRTIME)) {
			if (enable_Statistics_LOG)
				Log.v("UICenter - StatisticsNew", LOG_TAG
						+ "\n=ErrTimeTransfinite" + "\n--DEFAULT_ERRTIME ");
	...
	private void recordErrCount(int count) {
		if (mContext != null) {
			ConfigDB config = new ConfigDB(mContext);
			config.open();
	...
	private String getParams0017WithMd5(String logtext, String listItems) {
	...
	private String getParams0017List(String logtext, String listItem) {
		JSONArray array = null;
		if (logtext == null) {
			array = new JSONArray();
		} else {
	...
	private String getParams0017NoMd5() {
		...
		try {
			...
			res.put("imsi", mTelephonyMgr.getSubscriberId() == null ? ""
	...
	private String getParams(String logText, boolean isAddMd5) {
		...
		try {
			...
			if (action.equals(ACTION_USE_LOG)) {
				...
				res.put("count", 0);
			} else if (action.equals(ACTION_DOWNLOAD_LOG)
					|| action.equals(ACTION_INSTALL_LOG)) {
				res.put("param1", resid);
				res.put("param2", packageName);
				res.put("count", 0);
			}
	 ...
	 * 判断是否联网
	 */
	private boolean IsHaveInternet(final Context context) {
		try {
