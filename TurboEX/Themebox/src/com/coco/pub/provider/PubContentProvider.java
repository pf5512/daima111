...
public class PubContentProvider extends ContentProvider {
	...
	// "com.cooeeui.brand.turbolauncher.pub.provider";
	...
	static {
		...
		route.type = "item";
		matchSet.put(route.key, route);
		Iterator<Entry<String, RouteMatch>> iter = matchSet.entrySet()
				.iterator();
		while (iter.hasNext()) {
			Entry<String, RouteMatch> entry = (Entry<String, RouteMatch>) iter
					.next();
			RouteMatch val = (RouteMatch) entry.getValue();
			MATCHER.addURI(PERSONAL_CENTER_AUTHORITY, val.table, val.code);
		}
	}
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		if (routeMatch != null) {
			if (routeMatch.type.equals("dir")) {
				...
				throw new IllegalArgumentException("Unkwon type:"
						+ routeMatch.type + "for Uri:" + uri.toString());
			}
		} else {
	...
	public String getType(Uri uri) {
		...
		if (routeMatch != null) {
			if (routeMatch.type.equals("dir")) {
				...
				throw new IllegalArgumentException("Unkwon type:"
						+ routeMatch.type + " Uri:" + uri.toString());
			}
		} else {
			throw new IllegalArgumentException("Unkwon Uri:" + uri.toString());
		}
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		if (routeMatch != null) {
			if (routeMatch.type.equals("dir")) {
				return db.query(routeMatch.table, projection, selection,
				...
				return db.query(routeMatch.table, projection, where,
						selectionArgs, null, null, sortOrder);
			} else {
				throw new IllegalArgumentException("Unkwon Uri:"
						+ uri.toString());
			}
		} else {
			throw new IllegalArgumentException("Unkwon Uri:" + uri.toString());
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		if (routeMatch != null) {
			if (routeMatch.type.equals("dir")) {
				count = db.update(routeMatch.table, values, selection,
						selectionArgs);
				return count;
			} else if (routeMatch.type.equals("item")) {
				long id = ContentUris.parseId(uri);
				...
				count = db.update(routeMatch.table, values, where,
						selectionArgs);
				return count;
			} else {
				throw new IllegalArgumentException("Unkwon Uri:"
