...
import com.cooee.launcher.InstallWidgetReceiver;
import com.cooee.launcher.InstallWidgetReceiver.WidgetMimeTypeHandlerData;
...
public class LauncherModel extends BroadcastReceiver {
	...
	// 添加新的widget liangxiaoling@2014/12/30 ADD START
	static final ArrayList<CooeeWidgetInfo> sBgCooeeWidgets = new ArrayList<CooeeWidgetInfo>();
	// 添加新的widget liangxiaoling@2014/12/30 ADD END
	...
	public interface Callbacks {
		...
		// 添加新的widget liangxiaoling@2014/12/30 ADD START
		public void bindCooeeWidget(CooeeWidgetInfo info);
	 ...
	 */
	public static int getCellLayoutChildId(long container, int screen,
	 ...
	 */
	public void updateWorkspaceScreenOrder(Context context,
			...
			public void run() {
				...
				synchronized (sBgLock) {
					...
					case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
						...
						break;
					// 添加新的widget liangxiaoling@2014/12/30 ADD START
					case LauncherSettings.Favorites.ITEM_TYPE_COOEEWIDGET:
						sBgCooeeWidgets.remove((CooeeWidgetInfo) item);
						break;
					// 添加新的widget liangxiaoling@2014/12/30 ADD END
					}
	...
	private class LoaderTask implements Runnable {
		...
		LoaderTask(Context context, boolean isLaunching) {
			...
			// not a good place to inject smart layout into desktop.
			// here is just for test.
			if (SharePreferenceUtil.getSLstate() == false) {
				SLInjection smartLayout = new SLInjection(mContext);

				SharePreferenceUtil.setSLState(true);
			}
			...
			synchronized (sBgLock) {
				...
				try {
					...
					final int spanYIndex = c
							.getColumnIndexOrThrow(LauncherSettings.Favorites.SPANY);
					// 添加新的widget liangxiaoling@2014/12/30 ADD START
					final int desktopIndex = c
							.getColumnIndexOrThrow(LauncherSettings.Favorites.DESKTOP);
					final int loadbyinsternelIndex = c
							.getColumnIndexOrThrow(LauncherSettings.Favorites.LOADBYINTERNAL);
					final int pkgnameIndex = c
							.getColumnIndexOrThrow(LauncherSettings.Favorites.PKGNAME);
					final int classnameIndex = c
							.getColumnIndexOrThrow(LauncherSettings.Favorites.CLSNAME);
					// 添加新的widget liangxiaoling@2014/12/30 ADD END
					// final int uriIndex =
					...
					// 添加新的widget liangxiaoling@2014/12/30 ADD START
					CooeeWidgetInfo cooeeWidgetInfo;
					// 添加新的widget liangxiaoling@2014/12/30 ADD END
					int container;
					long id;
					Intent intent = null;
						...
						try {
							...
							case LauncherSettings.Favorites.ITEM_TYPE_VIRTUALITY:
								...
								if (itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
									...
									// God loves Zhongqihong@2014/12/23 ADD
									// START
									if (itemType == LauncherSettings.Favorites.ITEM_TYPE_VIRTUALITY) {
										intentDescription = c
												.getString(iconResourceIndex);
									}
									...
									// God loves Zhongqihong@2014/12/22 ADD
									// START
									// virtual icon is a special shortcut
									// without intent
									if (itemType != LauncherSettings.Favorites.ITEM_TYPE_VIRTUALITY)
							...
							case LauncherSettings.Favorites.ITEM_TYPE_COOEEWIDGET:
								ComponentName cn = new ComponentName(
										c.getString(pkgnameIndex),
										c.getString(classnameIndex));
								cooeeWidgetInfo = new CooeeWidgetInfo(cn);
								cooeeWidgetInfo.id = c.getLong(idIndex);
								cooeeWidgetInfo.screen = c.getInt(screenIndex);
								cooeeWidgetInfo.cellX = c.getInt(cellXIndex);
								cooeeWidgetInfo.cellY = c.getInt(cellYIndex);
								cooeeWidgetInfo.spanX = c.getInt(spanXIndex);
								cooeeWidgetInfo.spanY = c.getInt(spanYIndex);
								if (c.getString(desktopIndex).equals("true")) {
									cooeeWidgetInfo.desktop = true;
								} else {
									cooeeWidgetInfo.desktop = false;
								}
								if (c.getString(loadbyinsternelIndex).equals(
										"true")) {
									cooeeWidgetInfo.loadbyInternel = true;
								} else {
									cooeeWidgetInfo.loadbyInternel = false;
								}

								sBgItemsIdMap.put(cooeeWidgetInfo.id,
										cooeeWidgetInfo);
								sBgCooeeWidgets.add(cooeeWidgetInfo);
				...
				if (loadedOldDb) {
					...
					mApp.getLauncherProvider().updateMaxItemId(maxItemId);
				} else {
		...
		// 添加新的widget liangxiaoling@2014/12/30 ADD START
		private void filterCurrentCooeeWidgets(int currentScreen,
				ArrayList<CooeeWidgetInfo> appWidgets,
				ArrayList<CooeeWidgetInfo> currentScreenWidgets,
				ArrayList<CooeeWidgetInfo> otherScreenWidgets) {
			// If we aren't filtering on a screen, then the set of items to load
			// is the full set of
			// widgets given.
			if (currentScreen < 0) {
			...
			for (CooeeWidgetInfo widget : appWidgets) {
				if (widget == null)
					continue;
				if (widget.container == LauncherSettings.Favorites.CONTAINER_DESKTOP
						&& widget.screen == currentScreen) {
		...
		// 添加新的widget liangxiaoling@2014/12/30 UPD START
		private void bindWorkspaceItems(final Callbacks oldCallbacks,
				final ArrayList<ItemInfo> workspaceItems,
				final ArrayList<LauncherAppWidgetInfo> appWidgets,
				final ArrayList<CooeeWidgetInfo> cooeeWidgets,
				final HashMap<Long, FolderInfo> folders,
				ArrayList<Runnable> deferredBindRunnables) {
			// 添加新的widget liangxiaoling@2014/12/30 UPD END
			...
			N = cooeeWidgets.size();
			for (int i = 0; i < N; i++) {
				final CooeeWidgetInfo widget = cooeeWidgets.get(i);
				final Runnable r = new Runnable() {
					public void run() {
						Callbacks callbacks = tryGetCallbacks(oldCallbacks);
						if (callbacks != null) {
							callbacks.bindCooeeWidget(widget);
						}
					}
				};
				if (postOnMainThread) {
		...
		private void bindWorkspace(int synchronizeBindPage) {
			...
			// 添加新的widget liangxiaoling@2014/12/30 UPD START
			ArrayList<CooeeWidgetInfo> CooeeWidgets = new ArrayList<CooeeWidgetInfo>();
			// 添加新的widget liangxiaoling@2014/12/30 UPD END
			...
			synchronized (sBgLock) {
				...
				// 添加新的widget liangxiaoling@2014/12/30 UPD START
				CooeeWidgets.addAll(sBgCooeeWidgets);
				// 添加新的widget liangxiaoling@2014/12/30 UPD END
			...
			// 添加新的widget liangxiaoling@2014/12/30 ADD START
			ArrayList<CooeeWidgetInfo> currentCooeeWidgets = new ArrayList<CooeeWidgetInfo>();
			ArrayList<CooeeWidgetInfo> otherCooeeWidgets = new ArrayList<CooeeWidgetInfo>();
			// 添加新的widget liangxiaoling@2014/12/30 ADD END
			...
			// 添加新的widget liangxiaoling@2014/12/30 ADD START
			filterCurrentCooeeWidgets(currentScreen, CooeeWidgets,
					currentCooeeWidgets, otherCooeeWidgets);
			// 添加新的widget liangxiaoling@2014/12/30 ADD END
			...
			bindWorkspaceItems(oldCallbacks, currentWorkspaceItems,
					currentAppWidgets, currentCooeeWidgets, currentFolders,
					null);
			// 添加新的widget liangxiaoling@2014/12/30 UPD END
			...
			bindWorkspaceItems(oldCallbacks, otherWorkspaceItems,
					otherAppWidgets, otherCooeeWidgets, otherFolders,
	...
	private ShortcutInfo getShortcutInfo(Cursor c, Context context,
			...
			int iconIndex, int titleIndex, String intentDescription,
			int itemType) {
		...
		// provides an other way to get custom bitmap.
		case LauncherSettings.Favorites.ICON_TYPE_CUSTOM:

			icon = ThemeManager.getInstance().imageInterpreter(
					intentDescription);
			info.customIcon = false;
			info.usingFallbackIcon = true;
			break;
