...
import com.android.launcher.framework.CooeeWidgetInfo;
...
import com.android.launcher.framework.LauncherAppWidgetInfo;
...
import com.android.launcher.framework.PendingAddWidgetInfo;
import com.android.launcher.framework.ShortcutInfo;
import com.cooee.launcher.common.ReflectionMethodsInvoke;
import com.cooee.launcher.desktopedit.DesktopEditModeTabHost;
import com.cooee.launcher.desktopsettings.DesktopSettingsActivity;
import com.cooee.launcher.dockbar.Hotseat;
import com.cooee.launcher.drawer.AppsCustomizePagedView;
import com.cooee.launcher.drawer.AppsCustomizeTabHost;
import com.cooee.launcher.folder.Folder;
import com.cooee.launcher.folder.FolderIcon;
...
import com.cooee.launcher.gesture.LauncherGesture;
import com.cooee.launcher.gesture.LauncherSimpleGesture;
import com.cooee.launcher.layout.virtualicon.VirtualInvoker;
import com.cooee.launcher.pub.provider.Assets;
import com.cooee.launcher.pub.provider.PubProviderHelper;
import com.cooee.launcher.theme.ThemeManager;
import com.cooee.launcher.trashcan.Trashcan;
...
public final class Launcher extends Activity implements View.OnClickListener,
	...
	// 修改菜单项 liangxiaoling@2014/12/27 UPD START
	private static final int MENU_LANDSCAOING_CENTER = 1;
	private static final int MENU_PRESS_EDITOR = MENU_LANDSCAOING_CENTER + 1;
	private static final int MENU_DESKTOP_SETTINGS = MENU_PRESS_EDITOR + 1;
	private static final int MENU_SYSTEM_SETTINGS = MENU_DESKTOP_SETTINGS + 1;
	// 修改菜单项 liangxiaoling@2014/12/27 UPD END
	...
	private FrameLayout mDesktopEditBottomLayout;
	private DesktopEditModeTabHost mDesktopEditModeTabHost;
	private LinearLayout mDesktopEditBottomSecondaryLayout;
	...
	// leexingwang leexingwang@2014/12/18 ADD START
	private View mFolderBg;

	// leexingwang leexingwang@2014/12/18 ADD END

	// Hugo.ye@2014/12/23 ADD START
	private static Launcher instance;

	// Hugo.ye@2014/12/23 ADD END

	// God loves Zhongqihong@2014/12/26 ADD START
	private static LauncherGesture launcherGesture;

	// God loves Zhongqihong@2014/12/26 ADD END
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// zhongqihong--2014/10/8
		initTheme();
		// zhongqihong end

		// God loves Zhongqihong@2014/12/26 ADD START
		launcherGesture = new LauncherGesture(this, new LauncherSimpleGesture());
	...
	protected void onResume() {
		...
		// zhq: transmit system apps(if there is one) to our server
		// AppColloctor appCollector = new AppColloctor(this, 10000);
	}
	...
	private void setupViews() {
		...
		if (Build.VERSION.SDK_INT >= 16) { // Added by cooee Hugo.ye for API
											// level
			mLauncherView
		...
		// Hugo.ye@2014/12/20 ADD START
		// get the Trashcan
		Trashcan trashcan = (Trashcan) findViewById(R.id.trashcan);
		// Hugo.ye@2014/12/20 ADD END
		...
		// 桌面编辑模块代码 LinYu@2014/12/19 UPD START
		mDesktopEditBottomLayout = (FrameLayout) findViewById(R.id.fl_desktop_edit);
		mDesktopEditModeTabHost = (DesktopEditModeTabHost) findViewById(R.id.tab_host);
		mDesktopEditBottomSecondaryLayout = (LinearLayout) findViewById(R.id.ll_secondary);
		// 桌面编辑模块代码 LinYu@2014/12/19 UPD END
		...
		if (trashcan != null) {
			trashcan.setup(this, dragController);
		}
	...
	public View createShortcut(int layoutResId, ViewGroup parent,
		...
		// Zhongqihong@2015/01/04 DEL START
		// favorite.setOnClickListener(this);
		// Zhongqihong@2015/01/04 DEL END

		// God loves Zhongqihong@2015/01/04 ADD START
		favorite.setOnTouchListener(this);
		// God loves Zhongqihong@2015/01/04 ADD END
	...
	static int[] getSpanForWidget(Context context, ComponentName component,
		...
		Rect padding;
		if (Build.VERSION.SDK_INT >= 15) { // Modified by cooee Hugo.ye for API
											// level
			padding = AppWidgetHostView.getDefaultPaddingForWidget(context,
					component, null);
		} else {
			...
			padding.right = r
					.getDimensionPixelSize(com.android.internal.R.dimen.default_app_widget_padding_right);
			padding.top = r
					.getDimensionPixelSize(com.android.internal.R.dimen.default_app_widget_padding_top);
			padding.bottom = r
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		settings.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
				| Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
		menu.add(0, MENU_LANDSCAOING_CENTER, 0, R.string.menu_landcenter)
				.setIcon(R.drawable.menu_iconbeautify)
				.setAlphabeticShortcut('C');
		menu.add(0, MENU_PRESS_EDITOR, 0, R.string.menu_editor)
				.setIcon(R.drawable.menu_iconedit).setAlphabeticShortcut('E');
		menu.add(0, MENU_DESKTOP_SETTINGS, 0, R.string.menu_desktopsetting)
				.setIcon(R.drawable.menu_icondesktopset)
				.setAlphabeticShortcut('D');
		menu.add(0, MENU_SYSTEM_SETTINGS, 0, R.string.menu_settings)
				.setIcon(R.drawable.menu_iconsystemset).setIntent(settings)
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		// 修改菜单项 liangxiaoling@2014/12/27 UPD START
		case MENU_LANDSCAOING_CENTER:
			Intent landcenter = new Intent();
			landcenter.setComponent(new ComponentName(Launcher.this,
					"com.coco.theme.themebox.MainActivity"));
			startActivity(landcenter);
			return true;
		case MENU_PRESS_EDITOR:
			if (mWorkspace.isInEdit1Mode()) {
			} else {
				mWorkspace.enterEdit1Mode();
			}
			return true;
		case MENU_DESKTOP_SETTINGS:
			Intent dstings = new Intent(Launcher.this,
					DesktopSettingsActivity.class);
			startActivity(dstings);
			return true;
		}
		// 修改菜单项 liangxiaoling@2014/12/27 UPD END
	...
	// 添加菜单显示图标功能 liangxiaoling@2014/12/27 ADD START
	...
	// 添加菜单显示图标功能 liangxiaoling@2014/12/27 ADD END
	...
	void addAppWidgetFromDrop(PendingAddWidgetInfo info, long container,
		...
		if (hostView != null) {
			...
			if (options != null) {
				// Modified by cooee Hugo.ye for API level
				if (Build.VERSION.SDK_INT >= 17) {
					...
					success = ReflectionMethodsInvoke.bindAppWidgetId(
							mAppWidgetManager, appWidgetId, info.componentName);
				}
			} else {
				// Modified by cooee Hugo.ye for API level
			...
			if (success) {
				...
				Intent intent = new Intent();
				if (Build.VERSION.SDK_INT >= 16) { // Modified by cooee Hugo.ye
													// for API level
					...
					intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_PROVIDER,
							info.componentName);
					// TODO: we need to make sure that this accounts for the
					// options
					// bundle.
	...
	public void onBackPressed() {
		if (isAllAppsVisible()) {
			showWorkspace(true);
		} else if (mWorkspace.isInEdit2Mode()) {
		} else if (mWorkspace.isInEdit1Mode()) {
			// 桌面编辑模块代码 LinYu@2014/12/19 UPD START
			if (mDesktopEditBottomSecondaryLayout.getVisibility() == View.VISIBLE) {
				mDesktopEditBottomSecondaryLayout.setVisibility(View.GONE);
				mDesktopEditModeTabHost.setVisibility(View.VISIBLE);
			} else {
				mWorkspace.exitEdit1Mode(true);
			}
			// 桌面编辑模块代码 LinYu@2014/12/19 UPD END
	...
	public void onClick(View v) {
		...
		if (tag instanceof ShortcutInfo) {
			...
			// God loves Zhongqihong@2014/12/23 ADD START
			if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_VIRTUALITY) {
				VirtualInvoker.init(this);
				new VirtualInvoker().invoke(info);

				return;
			}
			// God loves Zhongqihong@2014/12/23 ADD END
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		// slide down
		showWorkspace(true);
		// God loves Zhongqihong@2015/01/04 UPD START
		launcherGesture.onTouchEvent(v, event);
		// God loves Zhongqihong@2015/01/04 UPD END
		return false;
	}
	...
	boolean startActivity(View v, Intent intent, Object tag) {
		...
		try {
			...
			// Added by cooee Hugo.ye for API level
			if (Build.VERSION.SDK_INT < 16) {
	...
	public void openFolder(FolderIcon folderIcon) {
		...
		// leexingwang 打开时添加文件夹背景并设置folder mappingtop参数 leexingwang@2014/12/22
		// ADD START
		if (folder.getParent() == null) {
			if (mFolderBg == null) {// 初始化mFolderBg
				mFolderBg = getLayoutInflater().inflate(R.layout.folder_bg,
						null);
				mFolderBg.setAlpha(1.0f);
			}
			if (mFolderBg.getParent() == null) {
				mDragLayer.addView(mFolderBg);
			}
			DragLayer.LayoutParams lp = new DragLayer.LayoutParams(0, 0);
			lp.gravity = Gravity.CENTER_HORIZONTAL;
			lp.topMargin = (int) getResources().getDimension(
					R.dimen.folder_name_margintop);
			folder.setOpenBg(mFolderBg);
			mDragLayer.addView(folder, lp);
			// leexingwang 打开时添加文件夹背景并设置folder mappingtop参数
		...
		frameLayout = (FrameLayout) getLayoutInflater().inflate(
				R.layout.folder_openanimation_helplayout, null);
		folder.animateOpen(frameLayout);
		FolderIconToFolderOpenAnimation(folderIcon, folder);
	}

	// leexingwang leexingwang@2014/12/29 ADD START
	/**
	 * 打开文件夹（FolderIcon时外面的放大动画）
	 * 
	 * @param folderIcon
	 * @param folder
	 */
	private void FolderIconToFolderOpenAnimation(FolderIcon folderIcon,
			Folder folder) {
		Resources res = getResources();
		DragLayer parent = (DragLayer) findViewById(R.id.drag_layer);
		Rect mTempRect = new Rect();
		float scale = parent.getDescendantRectRelativeToSelf(folderIcon,
				mTempRect);
		...
		PropertyValuesHolder scaleY = PropertyValuesHolder.ofFloat("scaleY",
				7.0f);
		ObjectAnimator oa = LauncherAnimUtils.ofPropertyValuesHolder(
				mWorkspace, alpha, scaleX, scaleY);
		mWorkspace.setPivotX(centerX);
		mWorkspace.setPivotY(centerY);
		mHotseat.setPivotX(centerX);
		mHotseat.setPivotY(-centerY);
		oa.setDuration(res
				.getInteger(R.integer.config_openfoldericonAnimDuration));
		oa.addListener(new AnimatorListener() {
			...
			@Override
			public void onAnimationEnd(Animator animation) {
				mWorkspace.setScaleX(1);
				mWorkspace.setScaleY(1);
			}
		...
		oa.start();
		ObjectAnimator oa1 = LauncherAnimUtils.ofPropertyValuesHolder(mHotseat,
				alpha, scaleX, scaleY);
		oa1.setDuration(res
				.getInteger(R.integer.config_openfoldericonAnimDuration));
		oa1.addListener(new AnimatorListener() {
			...
			public void onAnimationEnd(Animator animation) {
				// TODO Auto-generated method stub
				hideCurPageViews();
				mHotseat.setScaleX(1);
				mHotseat.setScaleY(1);
			}
	...
	// leexingwang leexingwang@2014/12/29 ADD START
	/**
	 * 隐藏workspace、hotseat、Indicator上的icon的函数 当打开文件夹后做放大动画后执行此函数
	 */
	private void hideCurPageViews() {
		this.hidePageIndicator(true);// 隐藏页面指示器
		this.hideHotseat(true);// 隐藏底边栏
		this.hideCurPageAllView();// 隐藏桌面当前页所有图标
	}
	...
	// leexingwang leexingwang@2014/12/29 ADD START
	/**
	 * 显示workspace、hotseat、Indicator上的icon函数 当关闭文件夹做动画前执行此函数
	 */
	public void showCurPageViews() {
		this.hidePageIndicator(false);// 显示页面指示器
		// this.showHotseat(true);// 显示底边栏
		this.showCurPageAllView(getCurrentWorkspaceScreen());// 显示桌面当前页所有图标
	}

	// leexingwang leexingwang@2014/12/29 ADD END

	// leexingwang 关闭文件夹动画 leexingwang@2014/12/29 ADD START
	private void FolderToFolderIconCloseAnimation(FolderIcon folderIcon) {
		Resources res = getResources();
		...
		ObjectAnimator oa = LauncherAnimUtils.ofPropertyValuesHolder(
				mWorkspace, alpha, scaleX, scaleY);
		oa.setDuration(res
				.getInteger(R.integer.config_closefoldericonAnimDuration));
		oa.start();
		ObjectAnimator oa11 = LauncherAnimUtils.ofPropertyValuesHolder(
				mHotseat, alpha, scaleX, scaleY);
		oa11.setDuration(res
				.getInteger(R.integer.config_closefoldericonAnimDuration));
		oa11.start();
	}

	// leexingwang 关闭文件夹动画 leexingwang@2014/12/29 ADD END
	...
	void closeFolder(Folder folder) {
		...
		if (parent != null) {
			...
			FolderToFolderIconCloseAnimation(fi);
		}
	...
	// 桌面编辑模块代码 LinYu@2014/12/19 UPD START
	public FrameLayout getDesktopEditBottomLayout() {
		return mDesktopEditBottomLayout;
	}

	public LinearLayout getDesktopEditBottomSecondaryLayout() {
		return mDesktopEditBottomSecondaryLayout;
	}

	public DesktopEditModeTabHost getDesktopEditModeTabHost() {
		return mDesktopEditModeTabHost;
	}
	...
	private void setWorkspaceBackground(boolean workspace) {
		if (Build.VERSION.SDK_INT >= 16) { // Added by cooee Hugo.ye for API
			// level
			mLauncherView.setBackground(workspace ? null
					: mBlackBackgroundDrawable);
		} else {
			mLauncherView.setBackgroundDrawable(workspace ? null
					: mBlackBackgroundDrawable);
		}
	}
			...
			if (delayAnim) {
				final OnGlobalLayoutListener delayedStart = new OnGlobalLayoutListener() {
					...
					public void onGlobalLayout() {
						...
						if (Build.VERSION.SDK_INT >= 16) { // Modified by cooee
															// Hugo.ye for API
															// level
	...
	// leexingwang 隐藏指示器函数 leexingwang@2014/12/29 ADD START
	public void hidePageIndicator(boolean isHide) {
		findViewById(R.id.page_indicator).setVisibility(
				isHide ? View.GONE : View.VISIBLE);
	}

	// leexingwang 隐藏指示器函数 leexingwang@2014/12/29 ADD END

	// leexingwang 隐藏workspace上的所有icon（当前页） leexingwang@2014/12/29 ADD START
	/**
	 * @author 隐藏桌面当前页所有图标(cellLayout中的所有图标)
	 */
	public void hideCurPageAllView() {
		int currentPage = getWorkspace().getNextPage();
		CellLayout currentLayout = (CellLayout) getWorkspace().getChildAt(
				currentPage);
	...
	// leexingwang 隐藏workspace上的所有icon（当前页） leexingwang@2014/12/29 ADD END

	// leexingwang 显示桌面当前页所有图标(cellLayout中的所有图标) leexingwang@2014/12/29 ADD
	// START
	/**
	 * @param index
	 *            要显示的页的下标 显示桌面当前页所有图标(cellLayout中的所有图标)
	 */
	public void showCurPageAllView(int index) {
		if (index < 0) {
			index = getWorkspace().getNextPage();
		}
		CellLayout currentLayout = (CellLayout) getWorkspace()
				.getChildAt(index);
		int childCount = currentLayout.getChildCount();
		for (int i = 0; i < childCount; i++) {
			View view = currentLayout.getChildAt(i);
	...
	// leexingwang 显示桌面当前页所有图标(cellLayout中的所有图标) leexingwang@2014/12/29 ADD END
	...
	// 页面动态配置功能 Hugo.ye@2014/12/31 ADD START
	@Override
	...
	// 页面动态配置功能 Hugo.ye@2014/12/31 ADD END
	...
	public void bindItems(ArrayList<ItemInfo> shortcuts, int start, int end) {
		...
		for (int i = start; i < end; i++) {
			...
			case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
				...
				// God loves Zhongqihong@2014/12/23 ADD START
			case LauncherSettings.Favorites.ITEM_TYPE_VIRTUALITY:
				// God loves Zhongqihong@2014/12/23 ADD END
				...
				if (info.itemType != LauncherSettings.Favorites.ITEM_TYPE_VIRTUALITY)
					...
					uri = info.intent.toUri(0).toString();
				...
				// if(item.container==LauncherSettings.Favorites.CONTAINER_HOTSEAT){
				//
				// if(item.getPackageName(info.intent)!=null){
				// boolean flag =false;
				// String pkgName =info.intent.getComponent().getPackageName();
				// String clsName =info.intent.getComponent().getClassName();
				// for(int m=0;m<ThemeData.hotseatIcons.size();m++){
				// if(ThemeData.hotseatIcons.get(m).className.equals(clsName)&&
				// ThemeData.hotseatIcons.get(m).pkgName.equals(pkgName)
				// ){
				// flag =true;
				// break;
				// }
				// }
				// if(flag==false){
				// FavoriteInfo favorite =new FavoriteInfo(item);
				// favorite.className=clsName;
				// favorite.pkgName=pkgName;
				// Log.v(TAG, "favorite: "+favorite);
				// ThemeData.hotseatIcons.add(favorite);
				// }
				//
				// }
				//
	...
	// 绑定 添加新的widget类型liangxiaoling@2014/12/30 ADD START
	public void bindCooeeWidget(CooeeWidgetInfo item) {
		setLoadOnResume();
		final Workspace workspace = mWorkspace;
		ViewGroup cooeeview = item.createView(this, item.providerName);
		if (cooeeview != null && item.desktop) {
			cooeeview.setTag(item);
			workspace.addInScreen(cooeeview, item.container, item.screen,
					item.cellX, item.cellY, item.spanX, item.spanY, false);
			workspace.requestLayout();
		}
	}

	// 绑定 添加新的widget类型liangxiaoling@2014/12/30 ADD END
