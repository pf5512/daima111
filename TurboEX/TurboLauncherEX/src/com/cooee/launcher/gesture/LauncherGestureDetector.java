 ...
 * @author zhongqihong
 * 
 *         这个类基本抄了源码 {@link GestureDetector} ，改了一些算法，使他适合我们的需求。
 */
public class LauncherGestureDetector {
	...
	public interface OnDoubleTapListener {
		 ...
		 * @return true if the event is consumed, else false
		 */
		boolean onSingleTapConfirmed(Context context, View view, MotionEvent e);
		 ...
		 * @return true if the event is consumed, else false
		 */
		boolean onDoubleTapEvent(Context context, View view,MotionEvent e);
	}
	...
	public static class SimpleOnGestureListener implements OnGestureListener,
		...
		public boolean onSingleTapConfirmed(Context context, View v,
				MotionEvent e) {
			return false;
	...
	private static final int DOUBLE_TAP_TIMEOUT = ViewConfiguration
			.getDoubleTapTimeout();
	// Zhongqihong@2014/12/26 DEL START
	// private static final int DOUBLE_TAP_MIN_TIME = ViewConfiguration.getDoubleTapMinTime();

	private static final int DOUBLE_TAP_MIN_TIME = 40;

	// Zhongqihong@2014/12/26 DEL END
	 ...
	 */
	private final InputEventConsistencyVerifier mInputEventConsistencyVerifier = InputEventConsistencyVerifier
			.isInstrumentationEnabled() ? new InputEventConsistencyVerifier(
			this, 0) : null;
		...
		public void handleMessage(Message msg) {
			...
			case TAP:
				...
				if (mDoubleTapListener != null) {
					if (!mStillDown) {
						mDoubleTapListener.onSingleTapConfirmed(mContext,
								touchedView, mCurrentDownEvent);
					} else {
	 ...
	 */
	@Deprecated
	public LauncherGestureDetector(OnGestureListener listener, Handler handler) {
	 ...
	 */
	@Deprecated
	public LauncherGestureDetector(OnGestureListener listener) {
	 ...
	 *             if {@code listener} is null.
	 */
	public LauncherGestureDetector(Context context, OnGestureListener listener) {
		...
		case MotionEvent.ACTION_DOWN:
			if (mDoubleTapListener != null) {
				...
				if ((mCurrentDownEvent != null)
					...
					handled |= mDoubleTapListener.onDoubleTapEvent(mContext, view,ev);
				} else {
					// This is a first tap
		...
		case MotionEvent.ACTION_UP:
			...
			if (mIsDoubleTapping) {
				...
				if (mDeferConfirmSingleTap && mDoubleTapListener != null) {
					mDoubleTapListener.onSingleTapConfirmed(mContext, view, ev);
				}
			} else {
	...
	private boolean isConsideredDoubleTap(MotionEvent firstDown,
		...
		final long deltaTime = secondDown.getEventTime()
		...
		return ( result< mDoubleTapSlopSquare);
	}
