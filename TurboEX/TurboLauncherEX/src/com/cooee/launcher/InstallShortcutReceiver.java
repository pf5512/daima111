...
public class InstallShortcutReceiver extends BroadcastReceiver {
	...
	private static void processInstallShortcut(Context context,
		...
		synchronized (app) {
			...
			final boolean exists = LauncherModel.shortcutExists(context, name,
					intent);
			...
			for (int i = 0; i < (2 * Launcher.SCREEN_COUNT) + 1 && !found; ++i) {
				...
				if (0 <= si && si < Launcher.SCREEN_COUNT) {
					found = installShortcut(context, data, items, name, intent,
							si, exists, sp, result);
				}
			}
		}
	...
	private static boolean installShortcut(Context context, Intent data,
			ArrayList<ItemInfo> items, String name, Intent intent,
			final int screen, boolean shortcutExists,
		...
		if (findEmptyCell(context, items, tmpCoordinates, screen)) {
			if (intent != null) {
				...
				if (duplicate || !shortcutExists) {
					...
					int newAppsScreen = sharedPrefs.getInt(NEW_APPS_PAGE_KEY,
							screen);
					Set<String> newApps = new HashSet<String>();
					if (newAppsScreen == screen) {
						newApps = sharedPrefs.getStringSet(NEW_APPS_LIST_KEY,
								newApps);
					}
						...
						public void run() {
							synchronized (savedNewApps) {
								sharedPrefs
										...
										.putStringSet(NEW_APPS_LIST_KEY,
												savedNewApps).commit();
							}
						}
					}.start();
					...
					ShortcutInfo info = app.getModel().addShortcut(context,
							...
							screen, tmpCoordinates[0], tmpCoordinates[1], true);
					if (info == null) {
						return false;
					}
				} else {
	...
	private static boolean findEmptyCell(Context context,
			ArrayList<ItemInfo> items, int[] xy, int screen) {
