...
public class UninstallShortcutReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent data) {
		...
		PendingUninstallShortcutInfo info = new PendingUninstallShortcutInfo(
				data);
	...
	static void disableAndFlushUninstallQueue(Context context) {
		...
		Iterator<PendingUninstallShortcutInfo> iter = mUninstallQueue
				.iterator();
		while (iter.hasNext()) {
	...
	private static void processUninstallShortcut(Context context,
		...
		SharedPreferences sharedPrefs = context.getSharedPreferences(spKey,
					...
					public void run() {
						synchronized (savedNewApps) {
							...
							if (savedNewApps.isEmpty()) {
								// Reset the page index if there are no more
								// items
								editor.putInt(
										InstallShortcutReceiver.NEW_APPS_PAGE_KEY,
										-1);
							}
							editor.commit();
						}
