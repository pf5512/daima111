...
import com.cooee.launcher.LauncherAppWidgetHost;
...
public class DeleteDropTarget extends ButtonDropTarget {
	...
	protected void onFinishInflate() {
		...
		mUninstallDrawable = (TransitionDrawable) r
				.getDrawable(R.drawable.uninstall_target_selector);
		mRemoveDrawable = (TransitionDrawable) r
				.getDrawable(R.drawable.remove_target_selector);
	...
	// 添加新的widget类型 删除时加入的判断条件 liangxiaoling@2014/12/30 ADD START
	private boolean isCooeeWidget(DragObject d) {
		return isDragSourceWorkspaceOrFolder(d)
				&& (d.dragInfo instanceof CooeeWidgetInfo);
	}

	// 添加新的widget类型 删除时加入的判断条件 liangxiaoling@2014/12/30 ADD END
	private boolean isWorkspaceFolder(DragObject d) {
		return (d.dragSource instanceof Workspace)
				&& (d.dragInfo instanceof FolderInfo);
	}
	...
	public void onDragStart(DragSource source, Object info, int dragAction) {
		...
		// If we are dragging a widget from AppsCustomize, hide the delete
		// target
		...
		if (isAllAppsApplication(source, info)) {
			...
			if ((appInfo.flags & ApplicationInfo.DOWNLOADED_FLAG) != 0) {
				// Hugo.ye@2014/12/20 DEL START
				// isUninstall = true;
				// Hugo.ye@2014/12/20 DEL END
		...
		if (isUninstall) {
			setCompoundDrawablesWithIntrinsicBounds(mUninstallDrawable, null,
					null, null);
		} else {
			setCompoundDrawablesWithIntrinsicBounds(mRemoveDrawable, null,
					null, null);
		}
		mCurrentDrawable = (TransitionDrawable) getCurrentDrawable();
	...
	private void completeDrop(DragObject d) {
		...
		// 添加新的widget类型，删除时在数据库中删除liangxiaoling@2014/12/30 ADD START
		else if (isCooeeWidget(d)) {
			LauncherModel.deleteItemFromDatabase(mLauncher, item);
			// 添加新的widget类型，删除时在数据库中删除 liangxiaoling@2014/12/30 ADD END
		}
	}
	...
	private AnimatorUpdateListener createFlingToTrashAnimatorListener(
			final DragLayer dragLayer, DragObject d, PointF vel,
		...
		float vp = Math.min(1f,
				velocity / (config.getScaledMaximumFlingVelocity() / 2f));
		...
		return new AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				final DragView dragView = (DragView) dragLayer
						.getAnimatedView();
				...
				float x = (1f - t) * (1f - t) * (x1 - x1o) + 2 * (1f - t) * t
						* (x2 - x1o) + (t * t) * x3;
				float y = (1f - t) * (1f - t) * (y1 - y1o) + 2 * (1f - t) * t
						* (y2 - x1o) + (t * t) * y3;
	...
	private static class FlingAlongVectorAnimatorUpdateListener implements
		...
		public FlingAlongVectorAnimatorUpdateListener(DragLayer dragLayer,
	...
	private AnimatorUpdateListener createFlingAlongVectorAnimatorListener(
			final DragLayer dragLayer, DragObject d, PointF vel,
			final long startTime, final int duration, ViewConfiguration config) {
		final Rect from = new Rect();
		dragLayer.getViewRectRelativeToSelf(d.dragView, from);

		return new FlingAlongVectorAnimatorUpdateListener(dragLayer, vel, from,
	...
	public void onFlingToDelete(final DragObject d, int x, int y, PointF vel) {
				...
				if (mCount < 0) {
					...
					mOffset = Math.min(
							...
							(float) (AnimationUtils
									.currentAnimationTimeMillis() - startTime)
									/ duration);
					mCount++;
				}
		...
		if (mFlingDeleteMode == MODE_FLING_DELETE_TO_TRASH) {
			updateCb = createFlingToTrashAnimatorListener(dragLayer, d, vel,
					config);
		} else if (mFlingDeleteMode == MODE_FLING_DELETE_ALONG_VECTOR) {
			updateCb = createFlingAlongVectorAnimatorListener(dragLayer, d,
					vel, startTime, duration, config);
		}
		Runnable onAnimationEndRunnable = new Runnable() {
			@Override
			public void run() {
		...
		dragLayer
				.animateView(d.dragView, updateCb, duration, tInterpolator,
