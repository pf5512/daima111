...
import com.cooee.launcher.drawer.AccessibleTabView;
import com.cooee.launcher.drawer.FocusOnlyTabWidget;
import com.cooee.launcher.drawer.PagedViewCellLayout;
import com.cooee.launcher.drawer.PagedViewCellLayoutChildren;
import com.cooee.launcher.drawer.PagedViewGridLayout;
import com.cooee.launcher.drawer.PagedViewWidget;
...
public class FocusHelper {
	 ...
	 * the shop button.
	 */
	static boolean handleAppsCustomizeTabKeyEvent(View v, int keyCode,
			KeyEvent e) {
	 ...
	 * PagedViewWidgets.
	 */
	static boolean handlePagedViewGridLayoutWidgetKeyEvent(PagedViewWidget w,
			int keyCode, KeyEvent e) {
		...
		final int pageIndex = ((PagedView) container).indexToPage(container
				.indexOfChild(parent));
				...
				if (widgetIndex > 0) {
					...
					if (pageIndex > 0) {
						newParent = getAppsCustomizePage(container,
								pageIndex - 1);
						if (newParent != null) {
							child = newParent.getChildAt(newParent
									.getChildCount() - 1);
							if (child != null)
								child.requestFocus();
						}
		...
		case KeyEvent.KEYCODE_DPAD_DOWN:
			if (handleKeyEvent) {
				...
				// nothing
				if (y < (cellCountY - 1)) {
					int newWidgetIndex = Math.min(widgetCount - 1,
							((y + 1) * cellCountX) + x);
					child = parent.getChildAt(newWidgetIndex);
					if (child != null)
						child.requestFocus();
	 ...
	 */
	public static boolean handleAppsCustomizeKeyEvent(View v, int keyCode,
			KeyEvent e) {
		...
		case KeyEvent.KEYCODE_DPAD_DOWN:
			if (handleKeyEvent) {
				...
				// nothing
				if (y < (countY - 1)) {
					int newiconIndex = Math.min(itemCount - 1,
							((y + 1) * countX) + x);
					itemContainer.getChildAt(newiconIndex).requestFocus();
				}
			}
	 ...
	 */
	public static boolean handleTabKeyEvent(AccessibleTabView v, int keyCode,
			KeyEvent e) {
			...
			if (handleKeyEvent) {
				...
				if (tabIndex < (tabCount - 1)) {
					...
					if (v.getNextFocusRightId() != View.NO_ID) {
						tabHost.findViewById(v.getNextFocusRightId())
								.requestFocus();
					}
				}
			}
	...
	static boolean handleHotseatButtonKeyEvent(View v, int keyCode, KeyEvent e,
		...
		final Workspace workspace = (Workspace) launcher
		...
		case KeyEvent.KEYCODE_DPAD_UP:
			if (handleKeyEvent) {
				...
				// workspace
				final CellLayout layout = (CellLayout) workspace
	...
	private static ArrayList<View> getCellLayoutChildrenSortedSpatially(
		...
		Collections.sort(views, new Comparator<View>() {
			...
			public int compare(View lhs, View rhs) {
				CellLayout.LayoutParams llp = (CellLayout.LayoutParams) lhs
						.getLayoutParams();
				CellLayout.LayoutParams rlp = (CellLayout.LayoutParams) rhs
						.getLayoutParams();
	...
	private static View getIconInDirection(CellLayout layout, ViewGroup parent,
			int i, int delta) {
		final ArrayList<View> views = getCellLayoutChildrenSortedSpatially(
				layout, parent);
	 ...
	 *            search
	 */
	private static View getClosestIconOnLine(CellLayout layout,
			ViewGroup parent, View v, int lineDelta) {
		final ArrayList<View> views = getCellLayoutChildrenSortedSpatially(
				layout, parent);
		final CellLayout.LayoutParams lp = (CellLayout.LayoutParams) v
				.getLayoutParams();
		...
		if (0 <= newRow && newRow < cellCountY) {
			...
			while (index != endIndex) {
				...
				CellLayout.LayoutParams tmpLp = (CellLayout.LayoutParams) newV
						.getLayoutParams();
				boolean satisfiesRow = (lineDelta < 0) ? (tmpLp.cellY < row)
						: (tmpLp.cellY > row);
				if (satisfiesRow
						&& (newV instanceof BubbleTextView || newV instanceof FolderIcon)) {
					float tmpDistance = (float) Math.sqrt(Math.pow(tmpLp.cellX
							- lp.cellX, 2)
