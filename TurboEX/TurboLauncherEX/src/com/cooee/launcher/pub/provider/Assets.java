package com.cooee.launcher.pub.provider;
...
public class Assets {
	...
	public static void initAssets(Context context) {
		...
		if (config != null) {
			SharedPreferences prefs = mContext.getSharedPreferences(
					PREFERENCE_KEY_CONFIG, Activity.MODE_WORLD_READABLE);
			try {
				...
				try {
					...
					if (config != null) {
						PubProviderHelper
								.addOrUpdateValue(
										"config",
										Assets.PREFERENCE_KEY_CONFIG_DOMAIN,
										config.getString(Assets.PREFERENCE_KEY_CONFIG_DOMAIN));
						PubProviderHelper
								.addOrUpdateValue(
										"config",
										Assets.PREFERENCE_KEY_CONFIG_SERIALNO,
										config.getString(Assets.PREFERENCE_KEY_CONFIG_SERIALNO));
						PubProviderHelper
								.addOrUpdateValue(
										"config",
										Assets.PREFERENCE_KEY_CONFIG_APPID,
										config.getString(Assets.PREFERENCE_KEY_CONFIG_APPID));
						PubProviderHelper
								.addOrUpdateValue(
										"config",
										Assets.PREFERENCE_KEY_CONFIG_TEMPLATEID,
										config.getString(Assets.PREFERENCE_KEY_CONFIG_TEMPLATEID));
						PubProviderHelper
								.addOrUpdateValue(
										"config",
										Assets.PREFERENCE_KEY_CONFIG_CHANNELID,
										config.getString(Assets.PREFERENCE_KEY_CONFIG_CHANNELID));
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	public static String getWallpaperCooeeChange(Context context, String name) {
		ContentResolver resolver = context.getContentResolver();
		// Uri uri = Uri.parse( "content://" +
		// "com.iLoong.launcher.pub.provider" + "/" + "wallpaper" );
		Uri uri = Uri.parse("content://"
				+ TurboPubContentProvider.LAUNCHER_AUTHORITY + "/"
				+ "wallpaper");
		...
		try {
			...
			if (cursor != null) {
				if (cursor.moveToFirst()) {
					isCooeeChange = cursor.getString(cursor
							.getColumnIndex("propertyValue"));
				}
				cursor.close();
			} else {
				isCooeeChange = "false";
