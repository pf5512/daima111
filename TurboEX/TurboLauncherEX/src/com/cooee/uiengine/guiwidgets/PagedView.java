 ...
 * ************************ 补充解释：Modified by Hugo.ye 20141217 ******************
 * 此类是被抽象出来，用于管理一系列页面的切换，使用者只需要继承这个类，然后将实例化出来的页面直接通过addView方法添加进来， PagedView
 * 已经处理好了所有的切页管理。使用此类有两种方式，我将其概括为“静态”和“动态”。-------------------------------- * *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * 1、“静态” * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * --------关键点：初始化的时候，必须将父类变量mContentIsRefreshable置为false，并调用方法setDataIsReady()
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * （可参考工程中workspace的实现方式）：例如public class PagedViewTest extends PagedView {
 * }，PagedViewTest继承了PagedView，可在xml布局文件中对其及其子布局直接进行布局，或者也可以直接在构建的时候通过代码对需要
 * 管理的字页面进行布局。 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * 2、“动态” * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * --------关键点：重写两个抽象方法public abstract void syncPages(); public abstract void
 * syncPageItems(int page, boolean
 * immediate);（可参考工程中AppsCustomizePagedView的实现方式） * * * * * * * * * * * * * * *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 */
public abstract class PagedView extends ViewGroup implements
	...
	// 指示器模块代码 LinYu@2014/12/17 UPD START
	private int mPageIndicatorViewId;
	private PageIndicator mPageIndicator;
	private boolean mAllowPagedViewAnimations = false;

	// 指示器模块代码 LinYu@2014/12/17 UPD END

	// 用于控制页面是否可以循环滚动，默认为true，循环滚动 Hugo.ye@2014/12/25 ADD START
	protected boolean mLoopScrollEnable = true;
	protected boolean mLoopingToPre;
	protected boolean mLoopingToNext;

	// Hugo.ye@2014/12/25 ADD END
	...
	// Hugo.ye@2014/12/29 ADD START
	/**
	 * 设置页面是否循环滑动，默认为true
	 * 
	 * @param loop
	 */
	public void setPageLoopScroll(boolean loop) {
		mLoopScrollEnable = loop;
	}

	// Hugo.ye@2014/12/29 ADD END
	...
	public void getVisiblePages(int[] range) {
		...
		if (pageCount > 0) {
			...
			// 页面循环滚动 Hugo.ye@2014/12/29 ADD START
			if (mLoopScrollEnable && getScrollX() < 0) {
			...
			// 页面循环滚动 Hugo.ye@2014/12/29 ADD START
			if (mLoopScrollEnable
					&& ((getScrollX() < 0) || (getScrollX() > mMaxScrollX))) {
				rightScreen = 0;
			}
			// 页面循环滚动 Hugo.ye@2014/12/29 ADD END
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (pageCount > 0) {
			...
			if (leftScreen != -1 && rightScreen != -1) {
				...
				// 页面循环滚动 Hugo.ye@2014/12/29 UPD START
				if (mLoopScrollEnable) {
					int width = getWidth();
					int offset = pageCount * width;
					if (rightScreen < leftScreen) {
						if (getScrollX() >= mMaxScrollX) {
							if (shouldDrawChild(getPageAt(leftScreen))) {
								drawChild(canvas, getPageAt(leftScreen),
										drawingTime);
							}
							canvas.translate(+offset, 0);
							if (shouldDrawChild(getPageAt(rightScreen))) {
								drawChild(canvas, getPageAt(rightScreen),
										drawingTime);
							}
							canvas.translate(-offset, 0);
						} else {
				...
				// 页面循环滚动 Hugo.ye@2014/12/29 UPD END
				canvas.restore();
			}
		}
	}
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if (((isSignificantMove && deltaX > 0 && !isFling) || (isFling && velocityX > 0))) {
					if (mLoopScrollEnable && mCurrentPage <= 0) {
						mLoopingToPre = true;
