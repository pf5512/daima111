	...
	void update() {
		synchronized (mLock) {
			...
			for (int i = 0; i < dinfos.length; i++) {
				...
				if (i > mPids.size()) {
					Log.e(TAG, "update: unknown process info received: "
							+ dinfo);
					break;
				}
				...
				info.uss[info.head] = info.currentUss = dinfo
				...
				if (info.currentPss == 0) {
					Log.v(TAG, "update: pid " + pid
							+ " has pss=0, it probably died");
					mData.remove(pid);
				}
			}
	...
	public void onCreate() {
		...
		for (ActivityManager.RunningServiceInfo svc : svcs) {
			if (svc.service.getPackageName().equals(getPackageName())) {
				Log.v(TAG, "discovered running service: " + svc.process + " ("
						+ svc.pid + ")");
		...
		List<ActivityManager.RunningAppProcessInfo> procs = mAm
				.getRunningAppProcesses();
		for (ActivityManager.RunningAppProcessInfo proc : procs) {
			final String pname = proc.processName;
			if (pname.startsWith(getPackageName())) {
				Log.v(TAG, "discovered other running process: " + pname + " ("
						+ proc.pid + ")");
				startTrackingProcess(proc.pid, pname,
						System.currentTimeMillis());
			}
		}
	}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (intent != null) {
			if (ACTION_START_TRACKING.equals(intent.getAction())) {
				...
				final long start = intent.getLongExtra("start",
						System.currentTimeMillis());
