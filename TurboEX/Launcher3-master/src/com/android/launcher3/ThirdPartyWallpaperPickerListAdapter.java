...
public class ThirdPartyWallpaperPickerListAdapter extends BaseAdapter implements
		ListAdapter {
	...
	public static class ThirdPartyWallpaperTile extends
	...
	public ThirdPartyWallpaperPickerListAdapter(Context context) {
		...
		final Intent pickWallpaperIntent = new Intent(
				Intent.ACTION_SET_WALLPAPER);
		final List<ResolveInfo> apps = pm.queryIntentActivities(
				pickWallpaperIntent, 0);

		// Get list of image picker intents
		Intent pickImageIntent = new Intent(Intent.ACTION_GET_CONTENT);
		pickImageIntent.setType("image/*");
		final List<ResolveInfo> imagePickerActivities = pm
				.queryIntentActivities(pickImageIntent, 0);
		final ComponentName[] imageActivities = new ComponentName[imagePickerActivities
				.size()];
		for (int i = 0; i < imagePickerActivities.size(); i++) {
			ActivityInfo activityInfo = imagePickerActivities.get(i).activityInfo;
			imageActivities[i] = new ComponentName(activityInfo.packageName,
					activityInfo.name);
		}
		...
		outerLoop: for (ResolveInfo info : apps) {
			...
			if (itemPackageName.equals(context.getPackageName())
					...
					|| itemPackageName
							.equals("com.android.wallpaper.livepicker")) {
				continue;
			}
			...
			for (ResolveInfo imagePickerActivityInfo : imagePickerActivities) {
				if (itemPackageName
						.equals(imagePickerActivityInfo.activityInfo.packageName)) {
					continue outerLoop;
				}
			}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			view = mInflater.inflate(
					R.layout.wallpaper_picker_third_party_item, parent, false);
		} else {
			view = convertView;
		}
		...
		TextView label = (TextView) view
				.findViewById(R.id.wallpaper_item_label);
