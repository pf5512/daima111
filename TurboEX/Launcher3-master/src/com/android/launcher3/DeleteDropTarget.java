...
public class DeleteDropTarget extends ButtonDropTarget {
	...
	public void onDragStart(DragSource source, Object info, int dragAction) {
		...
		if (useUninstallLabel) {
			if (Build.VERSION.SDK_INT >= 17) { // Modified by cooee Hugo.ye for
												// API level
				setCompoundDrawablesRelativeWithIntrinsicBounds(
						mUninstallDrawable, null, null, null);
			} else {
				setCompoundDrawablesWithIntrinsicBounds(mUninstallDrawable,
						null, null, null);
			}
		} else {
			if (Build.VERSION.SDK_INT >= 17) { // Modified by cooee Hugo.ye for
												// API level
				setCompoundDrawablesRelativeWithIntrinsicBounds(
						mRemoveDrawable, null, null, null);
			} else {
	...
	private void deferCompleteDropIfUninstalling(DragObject d) {
		...
		if (isUninstallFromWorkspace(d)) {
			if (d.dragSource instanceof Folder) {
				((Folder) d.dragSource)
						.deferCompleteDropAfterUninstallActivity();
	...
	private void completeDrop(DragObject d) {
		...
		if (isAllAppsApplication(d.dragSource, item)) {
			...
			if (shortcut.intent != null
					&& shortcut.intent.getComponent() != null) {
				final ComponentName componentName = shortcut.intent
						.getComponent();
				final DragSource dragSource = d.dragSource;
				int flags = AppInfo.initFlags(ShortcutInfo.getPackageInfo(
						getContext(), componentName.getPackageName()));
				mWaitingForUninstall = mLauncher
						.startApplicationUninstallActivity(componentName, flags);
				if (mWaitingForUninstall) {
					final Runnable checkIfUninstallWasSuccess = new Runnable() {
						...
						public void run() {
							...
							List<ResolveInfo> activities = AllAppsList
									.findActivitiesForPackage(getContext(),
											packageName);
