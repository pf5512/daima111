...
public class Cling extends FrameLayout implements Insettable,
	...
	void setFocusedHotseatApp(int drawableId, int appRank, ComponentName cn,
			String title, String description) {
		// Get the app to draw
		Resources r = getResources();
		int appIconId = drawableId;
		Hotseat hotseat = mLauncher.getHotseat();
		if (hotseat != null && appIconId > -1 && appRank > -1
			...
			mFocusedHotseatAppBounds = new Rect(pos.left, pos.top, pos.left
					+ Utilities.sIconTextureWidth, pos.top
	...
	void show(boolean animate, int duration) {
		...
		if (mDrawIdentifier.equals(WORKSPACE_PORTRAIT)
			...
			content.setAlpha(0f);
			content.animate().alpha(1f).setDuration(duration).setListener(null)
					.start();
			setAlpha(1f);
		} else {
			if (animate) {
		...
		if (mScrimView != null) {
			...
			mScrimView.animate().alpha(1f).setDuration(duration)
	...
	void hide(final int duration, final Runnable postCb) {
			...
			content.animate().alpha(0f).setDuration(duration)
					.setListener(new AnimatorListenerAdapter() {
						public void onAnimationEnd(Animator animation) {
							// We are about to trigger the workspace cling, so
		...
		// Show the scrim if necessary
		if (mScrimView != null) {
			mScrimView.animate().alpha(0f).setDuration(duration)
					.setListener(new AnimatorListenerAdapter() {
						public void onAnimationEnd(Animator animation) {
	...
	public boolean onHoverEvent(MotionEvent event) {
		return (mDrawIdentifier.equals(WORKSPACE_PORTRAIT)
				|| mDrawIdentifier.equals(WORKSPACE_LANDSCAPE)
				|| mDrawIdentifier.equals(WORKSPACE_LARGE) || mDrawIdentifier
					.equals(WORKSPACE_CUSTOM));
	}
	...
	public void onClick(View v) {
		if (mDrawIdentifier.equals(WORKSPACE_PORTRAIT)
			...
			if (mFocusedHotseatAppBounds != null
					&& mFocusedHotseatAppBounds.contains(mTouchDownPt[0],
							mTouchDownPt[1])) {
	...
	protected void dispatchDraw(Canvas canvas) {
		if (mIsInitialized) {
			...
			if (mScrimView != null) {
				...
				eraseBg = Bitmap.createBitmap(getMeasuredWidth(),
			...
			if (mDrawIdentifier.equals(FIRST_RUN_PORTRAIT)
				...
				float buffer = DynamicGrid.pxFromDp(
						FIRST_RUN_CIRCLE_BUFFER_DPS, metrics);
				canvas.drawCircle(metrics.widthPixels / 2,
					...
					|| mDrawIdentifier.equals(WORKSPACE_LANDSCAPE)
					|| mDrawIdentifier.equals(WORKSPACE_LARGE)) {
				int offset = DynamicGrid.pxFromDp(
						WORKSPACE_CIRCLE_Y_OFFSET_DPS, metrics);
				mErasePaint.setAlpha((int) (128));
				eraseCanvas.drawCircle(metrics.widthPixels / 2,
						metrics.heightPixels / 2 - offset, DynamicGrid
								.pxFromDp(WORKSPACE_OUTER_CIRCLE_RADIUS_DPS,
										metrics), mErasePaint);
				mErasePaint.setAlpha(0);
				eraseCanvas.drawCircle(metrics.widthPixels / 2,
						metrics.heightPixels / 2 - offset, DynamicGrid
								.pxFromDp(WORKSPACE_INNER_CIRCLE_RADIUS_DPS,
										metrics), mErasePaint);
				...
				if (mFocusedHotseatAppBounds != null
						&& mFocusedHotseatApp != null) {
					mFocusedHotseatApp.setBounds(mFocusedHotseatAppBounds.left,
							mFocusedHotseatAppBounds.top,
							mFocusedHotseatAppBounds.right,
