...
public class WeightWatcher extends LinearLayout {
	...
	public WeightWatcher(Context context, AttributeSet attrs) {
		...
		ServiceConnection connection = new ServiceConnection() {
			public void onServiceConnected(ComponentName className,
					IBinder service) {
				mMemoryService = ((MemoryTracker.MemoryTrackerInterface) service)
						.getService();
				initViews();
			}
	...
	public class ProcessWatcher extends LinearLayout {
		...
		public void setPid(int pid) {
			...
			if (mMemInfo == null) {
				Log.v("WeightWatcher", "Missing info for pid " + mPid
						+ ", removing view: " + this);
				initViews();
			}
		}
		...
		public void update() {
			...
			mText.setText("("
					+ mPid
					+ (mPid == android.os.Process.myPid() ? "/A" // app
							: "/S") // service
					+ ") up " + getUptimeString() + " P=" + mMemInfo.currentPss
		...
		public class GraphView extends View {
			...
			public void onDraw(Canvas c) {
				...
				for (i = 0; i < N; i++) {
					x = i * barStep;
					c.drawRect(x, h - scale * mMemInfo.pss[i], x + barWidth, h,
							pssPaint);
					c.drawRect(x, h - scale * mMemInfo.uss[i], x + barWidth, h,
							ussPaint);
				}
