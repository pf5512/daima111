...
public class SavedWallpaperImages extends BaseAdapter implements ListAdapter {
	...
	public static class SavedWallpaperTile extends
			WallpaperPickerActivity.WallpaperTileInfo {
		private int mDbId;
		...
		public void onClick(WallpaperPickerActivity a) {
			...
			int rotation = WallpaperCropActivity.getRotationFromExif(file
					.getAbsolutePath());
			v.setTileSource(
					new BitmapRegionTileSource(a, file.getAbsolutePath(), 1024,
							rotation), null);
			v.moveToLeft();
	...
	public void loadThumbnailsAndImageIdList() {
		...
		Cursor result = db.query(ImageDb.TABLE_NAME, new String[] {
				ImageDb.COLUMN_ID, ImageDb.COLUMN_IMAGE_THUMBNAIL_FILENAME }, // cols
																				// to
																				// return
				null, // select query
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		return WallpaperPickerActivity.createImageTileView(mLayoutInflater,
				position, convertView, parent, thumbDrawable);
	}
	...
	public void writeImage(Bitmap thumbnail, byte[] imageBytes) {
		try {
			File imageFile = File.createTempFile("wallpaper", "",
					mContext.getFilesDir());
			FileOutputStream imageFileStream = mContext.openFileOutput(
					imageFile.getName(), Context.MODE_PRIVATE);
			imageFileStream.write(imageBytes);
			imageFileStream.close();

			File thumbFile = File.createTempFile("wallpaperthumb", "",
					mContext.getFilesDir());
			FileOutputStream thumbFileStream = mContext.openFileOutput(
					thumbFile.getName(), Context.MODE_PRIVATE);
			thumbnail.compress(Bitmap.CompressFormat.JPEG, 95, thumbFileStream);
			thumbFileStream.close();

			SQLiteDatabase db = mDb.getWritableDatabase();
			ContentValues values = new ContentValues();
			values.put(ImageDb.COLUMN_IMAGE_THUMBNAIL_FILENAME,
					thumbFile.getName());
