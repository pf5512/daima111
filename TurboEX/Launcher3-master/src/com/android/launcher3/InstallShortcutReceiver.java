...
public class InstallShortcutReceiver extends BroadcastReceiver {
	...
	private static void addToInstallQueue(SharedPreferences sharedPrefs,
		...
		synchronized (sLock) {
			try {
				JSONStringer json = new JSONStringer().object()
						...
						.value(info.launchIntent.toUri(0)).key(NAME_KEY)
						.value(info.name);
				if (info.icon != null) {
					byte[] iconByteArray = ItemInfo.flattenBitmap(info.icon);
					json = json.key(ICON_KEY).value(
							Base64.encodeToString(iconByteArray, 0,
									iconByteArray.length, Base64.DEFAULT));
				}
				if (info.iconResource != null) {
					json = json.key(ICON_RESOURCE_NAME_KEY).value(
							info.iconResource.resourceName);
					json = json.key(ICON_RESOURCE_PACKAGE_NAME_KEY).value(
							info.iconResource.packageName);
				}
				json = json.endObject();
				SharedPreferences.Editor editor = sharedPrefs.edit();
				if (DBG)
					Log.d(TAG, "Adding to APPS_PENDING_INSTALL: " + json);
				addToStringSet(sharedPrefs, editor, APPS_PENDING_INSTALL,
						json.toString());
				editor.commit();
	...
	public static void removeFromInstallQueue(SharedPreferences sharedPrefs,
		...
		synchronized (sLock) {
			Set<String> strings = sharedPrefs.getStringSet(
					APPS_PENDING_INSTALL, null);
			if (DBG) {
			...
			if (strings != null) {
				...
				while (newStringsIter.hasNext()) {
					...
					try {
						JSONObject object = (JSONObject) new JSONTokener(json)
								.nextValue();
						Intent launchIntent = Intent.parseUri(
								object.getString(LAUNCH_INTENT_KEY), 0);
				...
				sharedPrefs
						.edit()
						.putStringSet(APPS_PENDING_INSTALL,
	...
	private static ArrayList<PendingInstallShortcutInfo> getAndClearInstallQueue(
		...
		synchronized (sLock) {
			Set<String> strings = sharedPrefs.getStringSet(
					APPS_PENDING_INSTALL, null);
			if (DBG)
				Log.d(TAG, "Getting and clearing APPS_PENDING_INSTALL: "
						+ strings);
			if (strings == null) {
			...
			for (String json : strings) {
				try {
					JSONObject object = (JSONObject) new JSONTokener(json)
							.nextValue();
					Intent data = Intent.parseUri(
							object.getString(DATA_INTENT_KEY), 0);
					Intent launchIntent = Intent.parseUri(
							object.getString(LAUNCH_INTENT_KEY), 0);
					String name = object.getString(NAME_KEY);
					String iconBase64 = object.optString(ICON_KEY);
					String iconResourceName = object
							.optString(ICON_RESOURCE_NAME_KEY);
					String iconResourcePackageName = object
							.optString(ICON_RESOURCE_PACKAGE_NAME_KEY);
					if (iconBase64 != null && !iconBase64.isEmpty()) {
						byte[] iconArray = Base64.decode(iconBase64,
								Base64.DEFAULT);
						Bitmap b = BitmapFactory.decodeByteArray(iconArray, 0,
								iconArray.length);
						data.putExtra(Intent.EXTRA_SHORTCUT_ICON, b);
					} else if (iconResourceName != null
							&& !iconResourceName.isEmpty()) {
			...
			sharedPrefs.edit()
					.putStringSet(APPS_PENDING_INSTALL, new HashSet<String>())
					.commit();
			return infos;
		}
	}
	...
	public void onReceive(Context context, Intent data) {
		...
		if (name == null) {
			try {
				...
				ActivityInfo info = pm
						.getActivityInfo(intent.getComponent(), 0);
		...
		PendingInstallShortcutInfo info = new PendingInstallShortcutInfo(data,
				name, intent);
		info.icon = icon;
		...
		if (!installQueue.isEmpty()) {
			...
			while (iter.hasNext()) {
				...
				if (!exists /* && allowDuplicate */) {
					...
					ShortcutInfo info = getShortcutInfo(context,
							pendingInfo.data, pendingInfo.launchIntent);
