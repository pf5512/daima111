...
public abstract class PagedView extends ViewGroup implements
	...
	public PagedView(Context context, AttributeSet attrs, int defStyle) {
		...
		mPageIndicatorViewId = a.getResourceId(
				R.styleable.PagedView_pageIndicator, -1);
		a.recycle();
	...
	protected void onAttachedToWindow() {
		...
		if (mPageIndicator == null && mPageIndicatorViewId > -1) {
			mPageIndicator = (PageIndicator) parent
					.findViewById(mPageIndicatorViewId);
	...
	public boolean isLayoutRtl() {
		if (Build.VERSION.SDK_INT >= 17) { // Modified by cooee Hugo.ye for API
											// level
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int maxSize = Math.max(dm.widthPixels, dm.heightPixels + mInsets.top
				+ mInsets.bottom);
		...
		if (DEBUG)
			Log.d(TAG, "PagedView.onMeasure(): " + widthSize + ", "
					+ heightSize);
		if (DEBUG)
			Log.d(TAG, "PagedView.scaledSize: " + scaledWidthSize + ", "
					+ scaledHeightSize);
		if (DEBUG)
			Log.d(TAG, "PagedView.parentSize: " + parentWidthSize + ", "
					+ parentHeightSize);
		if (DEBUG)
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (child.getVisibility() != GONE) {
				...
				if (!lp.isFullScreenPage) {
					...
					childHeight = heightSize - verticalPadding - mInsets.top
							- mInsets.bottom;
					...
					if (mUseMinScale) {
						...
						childWidth = widthSize - getPaddingLeft()
								- getPaddingRight();
						childHeight = heightSize - getPaddingTop()
								- getPaddingBottom();
					}
				}
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		int childLeft = mFirstChildLeft = offsetX
				+ (screenWidth - getChildWidth(startIndex)) / 2;
		...
		for (int i = startIndex; i != endIndex; i += delta) {
			...
			if (child.getVisibility() != View.GONE) {
				...
				if (lp.isFullScreenPage) {
					...
					if (mCenterPagesVertically) {
						childTop += (getViewportHeight() - mInsets.top
								- mInsets.bottom - verticalPadding - child
								.getMeasuredHeight()) / 2;
					}
				}
				...
				if (i != endIndex - delta) {
					...
					int nextScrollOffset = (getViewportWidth() - getChildWidth(i
							+ delta)) / 2;
	...
	protected void getVisiblePages(int[] range) {
		...
		if (pageCount > 0) {
			...
			for (int i = 0; i < count; i++) {
				...
				mTmpIntPoint[0] = 0;
				Utilities.getDescendantCoordRelativeToParent(currPage, this,
						mTmpIntPoint, false);
	...
	int getNearestHoverOverPageIndex() {
		if (mDragView != null) {
			int dragX = (int) (mDragView.getLeft()
					+ (mDragView.getMeasuredWidth() / 2) + mDragView
					.getTranslationX());
			getOverviewModePages(mTempVisiblePagesRange);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			mDownScrollX = getScrollX();
			float[] p = mapPointFromViewToParent(this, mLastMotionX,
					mLastMotionY);
			mParentDownMotionX = p[0];
			mParentDownMotionY = p[1];
			mLastMotionXRemainder = 0;
		...
		case MotionEvent.ACTION_MOVE:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				// new movement into
				// account
				float[] pt = mapPointFromViewToParent(this, mLastMotionX,
						mLastMotionY);
				...
				setPageHoveringOverDeleteDropTarget(dragViewIndex,
				...
				if (pageUnderPointIndex > -1
						&& pageUnderPointIndex != indexOfChild(mDragView)
						&& !isHoveringOverDelete) {
					...
					if (mTempVisiblePagesRange[0] <= pageUnderPointIndex
						...
						mSidePageHoverRunnable = new Runnable() {
							...
							public void run() {
								...
								int shiftDelta = (dragViewIndex < pageUnderPointIndex) ? -1
										: 1;
								...
								for (int i = lowerIndex; i <= upperIndex; ++i) {
									...
									// position to its new
									// position
									AnimatorSet anim = (AnimatorSet) v
											.getTag(ANIM_TAG_KEY);
									if (anim != null) {
										anim.cancel();
									}
									...
									anim.playTogether(ObjectAnimator.ofFloat(v,
											"translationX", 0f));
									anim.start();
									v.setTag(anim);
								}
						...
						postDelayed(mSidePageHoverRunnable,
								REORDERING_SIDE_PAGE_HOVER_TIMEOUT);
					}
				} else {
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				final int pageWidth = getPageAt(mCurrentPage)
						.getMeasuredWidth();
				...
				if (!mFreeScroll) {
					...
					boolean isVelocityXLeft = isRtl ? velocityX > 0
							: velocityX < 0;
	...
	int getPageNearestToCenterOfScreen() {
		...
		int screenCenter = getViewportOffsetX() + getScrollX()
				+ (getViewportWidth() / 2);
		final int childCount = getChildCount();
		for (int i = 0; i < childCount; ++i) {
			...
			int childCenter = getViewportOffsetX() + getChildOffset(i)
					+ halfChildWidth;
	...
	public boolean startReordering(View v) {
		...
		if (mTempVisiblePagesRange[0] <= dragViewIndex
			...
			mDragView.animate().scaleX(1.15f).scaleY(1.15f).setDuration(100)
					.start();
			mDragViewBaselineLeft = mDragView.getLeft();
			disableFreeScroll(-1);
			onStartReordering();
	 ...
	 */
	private static class FlingAlongVectorAnimatorUpdateListener implements
		...
		private final TimeInterpolator mAlphaInterpolator = new DecelerateInterpolator(
				0.75f);

		public FlingAlongVectorAnimatorUpdateListener(View dragView,
				PointF vel, Rect from, long startTime, float friction) {
			mDragView = dragView;
			mVelocity = vel;
			mFrom = from;
			mPrevTime = startTime;
	...
	private Runnable createPostDeleteAnimationRunnable(final View dragView) {
		return new Runnable() {
			...
			public void run() {
				...
				int firstIndex = (isLastWidgetPage ? 0
						: mTempVisiblePagesRange[0]);
				int lastIndex = Math.min(mTempVisiblePagesRange[1],
						getPageCount() - 1);
				int lowerIndex = (slideFromLeft ? firstIndex
						: dragViewIndex + 1);
	...
	public void onFlingToDelete(PointF vel) {
		...
		AnimatorUpdateListener updateCb = new FlingAlongVectorAnimatorUpdateListener(
				dragView, vel, from, startTime, FLING_TO_DELETE_FRICTION);
	...
	private void onDropToDelete() {
		...
		alphaAnim.playTogether(ObjectAnimator.ofFloat(dragView, "alpha",
				toAlpha));
