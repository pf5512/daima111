...
class BitmapFactoryOptionsCache extends
...
public class WidgetPreviewLoader {
	...
	public Bitmap getPreview(final Object o) {
		...
		if (packageValid) {
			synchronized (mLoadedPreviews) {
				...
				if (mLoadedPreviews.containsKey(name)
						&& mLoadedPreviews.get(name).get() != null) {
		...
		synchronized (mUnusedBitmaps) {
			...
			while ((unusedBitmap == null || !unusedBitmap.isMutable()
					|| unusedBitmap.getWidth() != mPreviewBitmapWidth || unusedBitmap
					.getHeight() != mPreviewBitmapHeight)
		...
		if (unusedBitmap == null) {
			unusedBitmap = Bitmap.createBitmap(mPreviewBitmapWidth,
					mPreviewBitmapHeight, Bitmap.Config.ARGB_8888);
		}
	...
	static class CacheDb extends SQLiteOpenHelper {
		...
		public CacheDb(Context context) {
			super(context, new File(context.getCacheDir(), DB_NAME).getPath(),
					null, DB_VERSION);
		...
		public void onCreate(SQLiteDatabase database) {
			database.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " ("
					+ COLUMN_NAME + " TEXT NOT NULL, " + COLUMN_SIZE
					+ " TEXT NOT NULL, " + COLUMN_PREVIEW_BITMAP
					+ " BLOB NOT NULL, " + "PRIMARY KEY (" + COLUMN_NAME + ", "
					+ COLUMN_SIZE + ") " + ");");
		}
	...
	public static void removePackageFromDb(final CacheDb cacheDb,
		...
		new AsyncTask<Void, Void, Void>() {
			public Void doInBackground(Void... args) {
				...
				db.delete(CacheDb.TABLE_NAME, CacheDb.COLUMN_NAME
						+ " LIKE ? OR " + CacheDb.COLUMN_NAME + " LIKE ?", // SELECT
																			// query
	...
	private Bitmap readFromDb(String name, Bitmap b) {
		...
		Cursor result = db.query(CacheDb.TABLE_NAME,
				new String[] { CacheDb.COLUMN_PREVIEW_BITMAP }, // cols to
																// return
		...
		if (result.getCount() > 0) {
			...
			result.close();
			final BitmapFactory.Options opts = mCachedBitmapFactoryOptions
					.get();
			opts.inBitmap = b;
			opts.inSampleSize = 1;
			try {
				return BitmapFactory
						.decodeByteArray(blob, 0, blob.length, opts);
	...
	public Bitmap generatePreview(Object info, Bitmap preview) {
		if (preview != null
				&& (preview.getWidth() != mPreviewBitmapWidth || preview
						.getHeight() != mPreviewBitmapHeight)) {
			throw new RuntimeException(
		...
		if (info instanceof AppWidgetProviderInfo) {
			...
			return generateShortcutPreview((ResolveInfo) info,
					mPreviewBitmapWidth, mPreviewBitmapHeight, preview);
		}
	}
	...
	public Bitmap generateWidgetPreview(AppWidgetProviderInfo info,
		...
		return generateWidgetPreview(info.provider, info.previewImage,
				info.icon, cellSpans[0], cellSpans[1], maxWidth, maxHeight,
				preview, null);
	}
	...
	public Bitmap generateWidgetPreview(ComponentName provider,
		...
		if (widgetPreviewExists) {
			...
			BitmapDrawable previewDrawable = (BitmapDrawable) mContext
					.getResources().getDrawable(R.drawable.widget_tile);
		...
		// the right size
		if (preview == null) {
			preview = Bitmap.createBitmap(previewWidth, previewHeight,
					Config.ARGB_8888);
		}
	...
	private Bitmap generateShortcutPreview(ResolveInfo info, int maxWidth,
			int maxHeight, Bitmap preview) {
		...
		if (preview != null
			...
			preview = Bitmap
					.createBitmap(maxWidth, maxHeight, Config.ARGB_8888);
		}
	...
	private static void renderDrawableToBitmap(Drawable d, Bitmap bitmap,
