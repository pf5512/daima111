...
class AllAppsList {
	...
	public void addPackage(Context context, String packageName) {
		...
		if (matches.size() > 0) {
			for (ResolveInfo info : matches) {
				add(new AppInfo(context.getPackageManager(), info, mIconCache,
						null));
			}
		}
	}
		...
		if (matches.size() > 0) {
			...
			for (int i = 0; i < count; i++) {
				...
				if (applicationInfo == null) {
					add(new AppInfo(context.getPackageManager(), info,
							mIconCache, null));
				} else {
	 ...
	 * Returns whether <em>apps</em> contains <em>component</em>.
	 */
	private static boolean findActivity(ArrayList<AppInfo> apps,
			ComponentName component) {
	 ...
	 */
	private AppInfo findApplicationInfoLocked(String packageName,
			String className) {
