...
public class LauncherModel extends BroadcastReceiver {
	...
	public interface Callbacks {
		...
		public void bindComponentsRemoved(ArrayList<String> packageNames,
				ArrayList<AppInfo> appInfos, boolean matchPackageNamesOnly);
	...
	public void addAndBindAddedApps(final Context context,
	...
	public void addAndBindAddedApps(final Context context,
			final ArrayList<ItemInfo> workspaceApps, final Callbacks callbacks,
			final ArrayList<AppInfo> allAppsApps) {
			...
			while (sc.moveToNext()) {
				try {
					...
					Launcher.addDumpLog(TAG,
							"Desktop items loading interrupted - invalid screens: "
									+ e, true);
				}
			}
		} finally {
	...
	private class LoaderTask implements Runnable {
		...
		private boolean checkItemPlacement(
			...
			if (item.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
				...
				// button
				if (mCallbacks == null
						|| mCallbacks.get().isAllAppsButtonRank(
								(int) item.screenId)) {
				...
				if (occupied
						.containsKey(LauncherSettings.Favorites.CONTAINER_HOTSEAT)) {
					if (occupied
							.get(LauncherSettings.Favorites.CONTAINER_HOTSEAT)[(int) item.screenId][0] != null) {
						Log.e(TAG,
								"Error loading shortcut into hotseat "
										...
										+ ") occupied by "
										+ occupied
												.get(LauncherSettings.Favorites.CONTAINER_HOTSEAT)[(int) item.screenId][0]);
						return false;
					}
				} else {
			...
			synchronized (sBgLock) {
				...
				try {
					...
					while (!mStopped && c.moveToNext()) {
						AtomicBoolean deleteOnItemOverlap = new AtomicBoolean(
								false);
						try {
							...
							case LauncherSettings.Favorites.ITEM_TYPE_FOLDER:
								...
								if (container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
									if (checkItemDimensions(folderInfo)) {
										Log.d(TAG,
												"Skipped loading out of bounds folder");
										continue;
									}
								}
								// check & update map of what's occupied
								...
								if (!isSafeMode
									...
									if (container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
										if (checkItemDimensions(appWidgetInfo)) {
											Log.d(TAG,
													"Skipped loading out of bounds app widget");
											continue;
										}
									}
									// check & update map of what's occupied
									...
									if (!checkItemPlacement(occupied,
											appWidgetInfo, deleteOnItemOverlap)) {
										if (deleteOnItemOverlap.get()) {
											itemsToRemove.add(id);
										}
										break;
		...
		private void loadAllApps() {
			final long loadTime = DEBUG_LOADERS ? SystemClock.uptimeMillis()
					: 0;
					...
					if (callbacks != null) {
						...
						if (DEBUG_LOADERS) {
							Log.d(TAG, "bound " + added.size() + " apps in "
									+ (SystemClock.uptimeMillis() - bindTime)
									+ "ms");
						}
					} else {
	...
	private class PackageUpdatedTask implements Runnable {
		...
		public void run() {
			...
			if (added != null) {
				...
				Callbacks cb = mCallbacks != null ? mCallbacks.get() : null;
				if (!AppsCustomizePagedView.DISABLE_ALL_APPS) {
					addAndBindAddedApps(context, new ArrayList<ItemInfo>(), cb,
							added);
				} else {
	...
	public void dumpState() {
		...
		AppInfo.dumpApplicationInfoList(TAG, "mAllAppsList.data",
				mBgAllAppsList.data);
		AppInfo.dumpApplicationInfoList(TAG, "mAllAppsList.added",
				mBgAllAppsList.added);
		AppInfo.dumpApplicationInfoList(TAG, "mAllAppsList.removed",
				mBgAllAppsList.removed);
		AppInfo.dumpApplicationInfoList(TAG, "mAllAppsList.modified",
