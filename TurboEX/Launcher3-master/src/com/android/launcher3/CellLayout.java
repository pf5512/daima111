...
public class CellLayout extends ViewGroup {
	...
	public CellLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		mShortcutsAndWidgets.setCellDimensions(mCellWidth, mCellHeight,
				mWidthGap, mHeightGap, mCountX, mCountY);

		addView(mShortcutsAndWidgets);
	}

	public void enableHardwareLayer(boolean hasLayer) {
		mShortcutsAndWidgets.setLayerType(hasLayer ? LAYER_TYPE_HARDWARE
				: LAYER_TYPE_NONE, sPaint);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mPressedOrFocusedIcon != null) {
			...
			if (b != null) {
				int offset = getMeasuredWidth() - getPaddingLeft()
						- getPaddingRight() - (mCountX * mCellWidth);
				int left = getPaddingLeft() + (int) Math.ceil(offset / 2f);
				int top = getPaddingTop();
				canvas.drawBitmap(b, mPressedOrFocusedIcon.getLeft() + left
						- padding, mPressedOrFocusedIcon.getTop() + top
						- padding, null);
			}
		}
		...
		if (mFolderLeaveBehindCell[0] >= 0 && mFolderLeaveBehindCell[1] >= 0) {
			...
			cellToPoint(mFolderLeaveBehindCell[0], mFolderLeaveBehindCell[1],
					mTempLocation);
			View child = getChildAt(mFolderLeaveBehindCell[0],
					mFolderLeaveBehindCell[1]);
			if (child != null) {
				int centerX = mTempLocation[0] + mCellWidth / 2;
				int centerY = mTempLocation[1] + previewOffset / 2
						+ child.getPaddingTop() + grid.folderBackgroundOffset;
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mBackgroundRect.set(-padding.left, -padding.top, w + padding.right, h
				+ padding.bottom);
				...
				if (dragOffset != null && dragRegion != null) {
					...
					left += dragOffset.x
							+ ((mCellWidth * spanX) + ((spanX - 1) * mWidthGap) - dragRegion
									.width()) / 2;
					int cHeight = getShortcutsAndWidgets()
							.getCellContentHeight();
					int cellPaddingY = (int) Math.max(0,
							((mCellHeight - cHeight) / 2f));
	...
	private class ViewCluster {
		...
		boolean leftEdgeDirty, rightEdgeDirty, topEdgeDirty, bottomEdgeDirty,
				boundingRectDirty;
		...
		public Rect getBoundingRect() {
			if (boundingRectDirty) {
				...
				for (View v : views) {
					...
					if (first) {
						boundingRect
								.set(c.x, c.y, c.x + c.spanX, c.y + c.spanY);
						first = false;
					} else {
	...
	private boolean pushViewsToTempLocation(ArrayList<View> views,
			Rect rectOccupiedByPotentialDrop, int[] direction, View dragView,
			ItemConfiguration currentState) {
			...
			for (View v : currentState.sortedViews) {
				...
				if (!cluster.views.contains(v) && v != dragView) {
					if (cluster.isViewTouchingEdge(v, whichEdge)) {
						...
						markCellsForView(c.x, c.y, c.spanX, c.spanY,
								mTmpOccupied, false);
					}
				}
			}
	...
	public static int[] rectToCell(int width, int height, int[] result) {
		...
		int parentWidth = grid.calculateCellWidth(grid.widthPx - padding.left
				- padding.right, (int) grid.numColumns);
		int parentHeight = grid.calculateCellHeight(grid.heightPx - padding.top
				- padding.bottom, (int) grid.numRows);
