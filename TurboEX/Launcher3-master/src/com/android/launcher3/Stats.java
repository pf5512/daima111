...
public class Stats {
		...
		if (LOCAL_LAUNCH_LOG) {
			try {
				mLog = new DataOutputStream(mLauncher.openFileOutput(
						LOG_FILE_NAME, Context.MODE_APPEND));
	...
	public void recordLaunch(Intent intent, ShortcutInfo shortcut) {
		...
		Intent broadcastIntent = new Intent(ACTION_LAUNCH).putExtra(
				EXTRA_INTENT, flat);
		if (shortcut != null) {
	...
	private void saveStats() {
		...
		try {
			stats = new DataOutputStream(mLauncher.openFileOutput(
					STATS_FILE_NAME + ".tmp", Context.MODE_PRIVATE));
	...
	private void loadStats() {
		...
		try {
			stats = new DataInputStream(
					mLauncher.openFileInput(STATS_FILE_NAME));
