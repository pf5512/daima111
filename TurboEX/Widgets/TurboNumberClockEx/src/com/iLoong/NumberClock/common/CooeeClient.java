		...
		if (NumberClockHelper.isHaveInternet(context)) {
			...
			try {
				...
				if (status == 200) {
					...
					if (what == 1) {
						...
						if (dataentity != null
							...
							Bundle bundle = new Bundle();
							bundle.putSerializable(
									Parameter.SerializableWidgetFlushName,
									dataentity);
							if (NumberClockView.mHandler != null) {
								NumberClockView.mHandler.obtainMessage(
										NumberClockView.MSG_SUCCESS, bundle)
										.sendToTarget();
							}
						} else {
							if (NumberClockView.mHandler != null) {
								NumberClockView.mHandler.obtainMessage(
										NumberClockView.MSG_FAILURE)
										.sendToTarget();
							}
						}
					} else if (what == 4) {
						if (dataentity != null
							...
							if (NumberClockView.mHandler != null) {
								NumberClockView.mHandler
										.obtainMessage(
												NumberClockView.MSG_AUTOUPDATE_INLANDSUCCESS,
												bundle).sendToTarget();
							}
						} else {
							if (NumberClockView.mHandler != null) {
								NumberClockView.mHandler
										.obtainMessage(
												NumberClockView.MSG_AUTOUPDATE_INLANDFAILURE)
										.sendToTarget();
							}
						}
					} else {
				...
				if (what == 1) {
					...
					if (NumberClockView.mHandler != null) {
						NumberClockView.mHandler.obtainMessage(
								NumberClockView.MSG_FAILURE).sendToTarget();
					}
			...
			if (what == 1) {
				...
				if (NumberClockView.mHandler != null) {
					NumberClockView.mHandler.obtainMessage(
							NumberClockView.MSG_NETWORK_FAILURE).sendToTarget();
				}

			} else if (what == 4) {
				if (NumberClockView.mHandler != null) {
					NumberClockView.mHandler.obtainMessage(
							NumberClockView.MSG_AUTOUPDATE_NETWORK_FAILURE)
							.sendToTarget();
				}
